...
kernel_flash_kmeans_assign_cleanroom_tcgen05_v10(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ x_sq, float* __restrict__ c_sq, int* __restrict__ out, int B, int N, int D, int K, int num_n_tiles, int K_tiles)
{
    ...
    __nv_bfloat16* smem_c = reinterpret_cast<__nv_bfloat16*>(smem_raw + 33792);
    const int smem_c_addr = smem + 33792;
    float* smem_csq = reinterpret_cast<float*>(smem_raw + 99328);
    const int smem_csq_addr = smem + 99328;
        ...
        { // mma_main
            ...
            for (unsigned int tile_idx_1 = bid; tile_idx_1 < num_tiles_1; tile_idx_1 += num_bids) {
                ...
                for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                    ...
                    int _mma_ss_a_addr_2 = smem_x_addr;
