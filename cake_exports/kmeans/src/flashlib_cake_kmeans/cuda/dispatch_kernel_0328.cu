...
kernel_flash_kmeans_assign_highd_splitk_partial_blockn64_g2r4_b5a6_v1(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ c_sq, float* __restrict__ partial_scores, int* __restrict__ partial_indices, int B, int N, int D, int K, int num_n_tiles, int K_tiles, int K_slices)
{
    ...
    float* smem_csq = reinterpret_cast<float*>(smem_raw + 41984);
    const int smem_csq_addr = smem + 41984;
        ...
        { // load_main
            ...
            if (elect_sync()) {
                ...
                for (unsigned int work_idx_1 = bid; work_idx_1 < total_work_1; work_idx_1 += num_bids) {
                    ...
                    #pragma unroll 1
                    for (int local_k_1 = 0; local_k_1 < 2; local_k_1++) {
    ...
    // ---- Role: mma ----
        ...
        { // mma_main
            ...
            for (unsigned int work_idx_2 = bid; work_idx_2 < total_work_2; work_idx_2 += num_bids) {
                ...
                int slice_k_start_2 = iter_slice_2 * 2;
                #pragma unroll 1
                for (int local_k_2 = 0; local_k_2 < 2; local_k_2++) {
                    ...
                    for (int feat_tile_1 = 0; feat_tile_1 < feature_tiles_1; feat_tile_1++) {
                        ...
                        int _mma_ss_a_addr_0 = smem_x_addr;
                        int _mma_ss_a_lo_0 = make_warp_uniform((_mma_ss_a_addr_0 >> 4) & 0x3FFF);
                        int _mma_ss_b_addr_0 = smem_c_addr;
