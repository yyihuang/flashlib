...
kernel_flash_kmeans_assign_cleanroom_tcgen05_v15(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ x_sq, float* __restrict__ c_sq, int* __restrict__ out, int B, int N, int D, int K, int num_n_tiles, int K_tiles)
{
    ...
    __nv_bfloat16* smem_x1 = reinterpret_cast<__nv_bfloat16*>(smem_raw + 33792);
    const int smem_x1_addr = smem + 33792;
    __nv_bfloat16* smem_c = reinterpret_cast<__nv_bfloat16*>(smem_raw + 66560);
    const int smem_c_addr = smem + 66560;
    float* smem_csq = reinterpret_cast<float*>(smem_raw + 132096);
    const int smem_csq_addr = smem + 132096;
    ...
    const int tmem_score_tmem = tmem_addr_storage[0];
    ...
    // ---- Role: compute ----
        ...
        { // compute_main
            ...
            for (unsigned int tile_idx_2 = bid; tile_idx_2 < num_tiles_2; tile_idx_2 += num_bids) {
                ...
                int global_n0 = off_n0_1 + thread_row_idx;
                int global_n1 = off_n1_1 + thread_row_idx;
                ...
                for (int iter_k_2 = 0; iter_k_2 < K_tiles; iter_k_2++) {
                    ...
                    if ((float)thread_row_idx < 64.0f) {
                        int csq_base = thread_row_idx * 4;
                        float _vec_load_0[4];
                        {
                    ...
                    asm volatile(
                        ...
                        : "r"(taddr + (unsigned int)tmem_row_addr_offset + 256 + (unsigned int)score_base)
                        : "memory");
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    #pragma unroll
                    ...
                    asm volatile(
                        ...
                        : "r"(taddr + (unsigned int)tmem_row_addr_offset + (unsigned int)score_base)
                        : "memory");
                    asm volatile("tcgen05.wait::ld.sync.aligned;");
                    #pragma unroll
