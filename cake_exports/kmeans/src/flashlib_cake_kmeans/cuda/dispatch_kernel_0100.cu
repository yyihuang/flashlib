...
kernel_flash_kmeans_assign_cleanroom_tcgen05_d192_single_v1(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ x_sq, float* __restrict__ c_sq, int* __restrict__ out, int B, int N, int D, int K, int num_n_tiles, int K_tiles)
{
    ...
    __nv_bfloat16* smem_c = reinterpret_cast<__nv_bfloat16*>(smem_raw + 50176);
    const int smem_c_addr = smem + 50176;
    float* smem_csq = reinterpret_cast<float*>(smem_raw + 148480);
        ...
        { // mma_main
            ...
            for (unsigned int tile_idx_1 = bid; tile_idx_1 < num_tiles_1; tile_idx_1 += num_bids) {
                ...
                for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                    ...
                    _phase_c_full_0 ^= 1;
                    asm volatile("tcgen05.fence::after_thread_sync;");
                    int _desc_addr_0 = smem_x_addr + 16384;
    ...
    // ---- Role: compute ----
        ...
        { // compute_main
            ...
            for (unsigned int tile_idx_2 = bid; tile_idx_2 < num_tiles_2; tile_idx_2 += num_bids) {
                ...
                int global_n = off_n_1 + thread_row_idx;
