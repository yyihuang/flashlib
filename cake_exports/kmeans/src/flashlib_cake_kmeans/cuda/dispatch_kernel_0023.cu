...
kernel_flash_kmeans_assign_microdim_raw_tma_08f9_v1(const void* __restrict__ x_tmap, const void* __restrict__ c_tmap, float* __restrict__ x_sq, float* __restrict__ c_sq, int* __restrict__ out, int B, int N, int D, int K, int num_n_tiles, int K_tiles)
{
            ...
            if (warp == 0) {
                if (elect_sync()) {
                    ...
                    for (unsigned int tile_idx = bid; tile_idx < num_tiles; tile_idx += num_bids) {
                        ...
                        _phase_x_empty_0 ^= 1;
                        tma_2d_gmem2smem(smem_x_addr, x_tmap, 0, x_row, x_full_addr);
                        ...
                        for (int iter_k = 0; iter_k < K_tiles; iter_k++) {
                            ...
                            _phase_c_empty_0 ^= 1;
                            tma_2d_gmem2smem(smem_c_addr, c_tmap, 0, c_row, c_full_addr);
                ...
                for (int iter_k_1 = 0; iter_k_1 < K_tiles; iter_k_1++) {
                    ...
                    for (int kk = 0; kk < 128; kk += 4) {
                        ...
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            ...
                            : "r"(csq_smem_addr + (off_k_1 + score_base + kk) * 4));
                    ...
                    for (int kk_1 = 0; kk_1 < 128; kk_1 += 4) {
                        ...
                        asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                            ...
                            : "r"(csq_smem_addr + (off_k_1 + score_base + kk_1) * 4));
