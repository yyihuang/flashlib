...
(``lm.mma_ss``) for exact D=64 rows. It reuses the single 64-wide feature tile
...
flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1:flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1', 256)
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1:ir"}'))
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0342"}, "kernel_flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1", 51200, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(f'flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1 requires bfloat16 input, got {dtype}')
    if dim not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_d64_direct_1p2gap_9f2a_v1 requires D=64, got {dim}')
