"""Flash-KMeans Euclidean assignment D160/D192/D256 repeated-MMA bucket seed.
...
from . import flash_kmeans_assign_cleanroom_tcgen05_d160_padded_b23d_v1 as _d160
...
SUPPORTED_DIMS = (160, 192, 256)
D160_BUCKET_SHAPES: list[dict[str, Any]] = [{'label': 'd160_paired_b1_n2048_k2048_d160', 'params': {'B': 1, 'N': 2048, 'D': 160, 'K': 2048, 'dtype': 'bfloat16', 'seed': 16001}}, {'label': 'd160_fallback_b2_n2432_k1024_d160', 'params': {'B': 2, 'N': 2432, 'D': 160, 'K': 1024, 'dtype': 'bfloat16', 'seed': 16002}}]
BUCKET_SHAPES: list[dict[str, Any]] = [*D160_BUCKET_SHAPES, {'label': 'd192_small_b1_n1024_k512_d192', 'params': {'B': 1, 'N': 1024, 'D': 192, 'K': 512, 'dtype': 'bfloat16', 'seed': 19201}}, {'label': 'd192_paired_b1_n2048_k2048_d192', 'params': {'B': 1, 'N': 2048, 'D': 192, 'K': 2048, 'dtype': 'bfloat16', 'seed': 19202}}, {'label': 'd192_random_b2_n4096_k768_d192', 'params': {'B': 2, 'N': 4096, 'D': 192, 'K': 768, 'dtype': 'bfloat16', 'seed': 19204}}, {'label': 'd256_small_b1_n1024_k512_d256', 'params': {'B': 1, 'N': 1024, 'D': 256, 'K': 512, 'dtype': 'bfloat16', 'seed': 25601}}, {'label': 'd256_paired_b1_n2048_k2048_d256', 'params': {'B': 1, 'N': 2048, 'D': 256, 'K': 2048, 'dtype': 'bfloat16', 'seed': 25602}}, {'label': 'd256_fallback_b2_n2432_k1024_d256', 'params': {'B': 2, 'N': 2432, 'D': 256, 'K': 1024, 'dtype': 'bfloat16', 'seed': 25603}}, {'label': 'd256_large_b4_n16384_k1024_d256', 'params': {'B': 4, 'N': 16384, 'D': 256, 'K': 1024, 'dtype': 'bfloat16', 'seed': 25604}}, {'label': 'd256_hugek_b1_n1024_k8192_d256', 'params': {'B': 1, 'N': 1024, 'D': 256, 'K': 8192, 'dtype': 'bfloat16', 'seed': 25605}}]
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    selected_route = f'd{dim}_{route_kind}_repeated_mma_v2'
    normalized['selected_route'] = selected_route
    normalized['route_trace'] = {'shape_key': _shape_key(inputs), 'selected_route': selected_route, 'selected_entrypoint': f'{module.__name__}:launch_for_eval', 'selected_seed': f'd{dim}-{route_kind}-repeated-mma-b23d-v2', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'guard_d{dim}_{route_kind}_repeated_mma_b23d_v2', 'guard_condition': f'dtype == bfloat16 and D == {dim} and N % {BLOCK_N} == 0 and K % {BLOCK_K} == 0', 'classification': 'seed-probe', 'dispatcher_kernel_ms': None, 'reason': 'non-128D bucket seed wrapper routes to D-specific Weave tcgen05 candidate'}
    return normalized
...
def _module_for_dim(dim: int):
    ...
    if dim == 256:
        return _d256
    raise ValueError(f'non128d split-D seed supports D in {SUPPORTED_DIMS}, got {dim}')

def _route_kind(inputs: dict[str, Any]) -> str:
    dim = int(inputs['D'])
    if dim == 160:
        return 'padded_single'
    if dim == 256:
        return 'single'
...
def compile_and_launch_flash_kmeans_assign_cleanroom(B: int=1, N: int=2048, K: int=2048, D: int=160, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 16001}}])
    return result
