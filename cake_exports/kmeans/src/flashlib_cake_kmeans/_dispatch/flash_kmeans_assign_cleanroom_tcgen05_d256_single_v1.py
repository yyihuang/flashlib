...
(``lm.mma_ss``) for the D-specific repeated-MMA score tile producer and is not intended for
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0345"}, "kernel_flash_kmeans_assign_cleanroom_tcgen05_d256_single_v1", 198656, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    n_clusters = int(inputs['K'])
    if dim != FEAT_D:
        raise ValueError(f'flash_kmeans_assign_cleanroom_tcgen05_d256_single_v1 requires D={FEAT_D}, got {dim}')
