...
between-bucket BF16 dimensions to the next existing Weave seed bucket, then
...
SUPPORTED_PAD: dict[int, int] = {48: 64, 112: 128, 224: 256, 288: 320, 352: 384, 416: 448, 480: 512}
...
flash_kmeans_assign_gap_pad_pack_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_gap_pad_v1:flash_kmeans_assign_gap_pad_pack_v1', 256)
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_gap_pad_v1:ir"}'))
pack_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_gap_pad_v1:pack_ir"}'))
...
def _compiled_pack_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0341"}, "kernel_flash_kmeans_assign_gap_pad_pack_v1", 0, 256]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(f'flash_kmeans_assign_gap_pad_v1 requires bfloat16 input, got {dtype}')
    if dim not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_gap_pad_v1 requires D in {sorted(SUPPORTED_D)}, got {dim}')
    ...
    normalized['selected_route'] = f'd{dim}_pad{d_pad}_{route_id}'
    normalized['route_trace'] = {'shape_key': _shape_key(inputs), 'selected_route': normalized['selected_route'], 'selected_entrypoint': f'{__name__}:launch_for_eval', 'selected_seed': f'd{dim}-pad{d_pad}-{route_seed}', 'route_kind': 'specialized', 'route_source': 'generated-variant', 'guard_id': f'guard_d{dim}_pad{d_pad}_gap_pad_v1', 'guard_condition': f'dtype == bfloat16 and D == {dim} and N % {BLOCK_N} == 0 and K % {BLOCK_K} == 0', 'classification': 'route-ok', 'dispatcher_kernel_ms': None, 'residual_contract_regions': [f'zero_pad_to_d{d_pad}_pack'], 'reason': f'D{dim} gap route pads to D{d_pad} and delegates to {route_entrypoint}'}
    return normalized
...
def _scratch_buffers(inputs: dict[str, Any], d_pad: int) -> tuple[Any, Any]:
    ...
    n_clusters = int(inputs['K'])
    dim = int(inputs['D'])
    key = (device_index, int(inputs['x'].data_ptr()), int(inputs['centroids'].data_ptr()), bsz, n_points, n_clusters, dim, d_pad)
...
def _route_inputs(inputs: dict[str, Any], x_pad: Any, c_pad: Any, d_pad: int) -> dict[str, Any]:
    key = (int(x_pad.data_ptr()), int(c_pad.data_ptr()), int(inputs['x_sq'].data_ptr()), int(inputs['c_sq'].data_ptr()), int(inputs['out'].data_ptr()), int(inputs['B']), int(inputs['N']), int(inputs['K']), d_pad)
...
def compile_and_launch_flash_kmeans_assign_gap_pad(B: int=2, N: int=2048, K: int=512, D: int=112, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 11205}}])
    return result
