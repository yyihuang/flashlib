"""Clean-room Flash-KMeans Euclidean assignment candidate.

Minimum architecture: sm_100a. This candidate uses Blackwell tcgen05/TMEM
(``lm.mma``) for the D=128 score tile producer and is not intended for
...
flash_kmeans_assign_cleanroom_tcgen05_v10 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_cleanroom_tcgen05_v10", "arg_keys": ["x_tmap", "c_tmap", "x_sq", "c_sq", "out", "B", "N", "D", "K", "num_n_tiles", "K_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 100352, "constants": [], "cta_group": 1, "threads": 192}'))
...
def _make_tmaps(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_cleanroom_v10_tmaps')
    ...
    inputs['_flash_kmeans_assign_cleanroom_v10_tmaps'] = (cache_key, tmap_x, tmap_c)
    return (tmap_x, tmap_c)

def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0002"}, "kernel_flash_kmeans_assign_cleanroom_tcgen05_v10", 100352, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    n_clusters = int(inputs['K'])
    if dim != FEAT_D:
        raise ValueError(''.join(['flash_kmeans_assign_cleanroom_tcgen05_v10 requires D=', format(FEAT_D, ''), ', got ', format(dim, '')]))
    ...
    block = (threads, 1, 1)
    args = pack_kernel_args(flash_kmeans_assign_cleanroom_tcgen05_v10, x_tmap=tmap_x, c_tmap=tmap_c, x_sq=inputs['x_sq'], c_sq=inputs['c_sq'], out=inputs['out'], B=bsz, N=n_points, D=dim, K=n_clusters, num_n_tiles=num_n_tiles, K_tiles=k_tiles)
