...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_microdim_hybrid_9c0d_v1:ir"}'))
...
def _validate_supported_shape(*, N: int, D: int, K: int, dtype: Any) -> None:
    ...
    if dtype_name not in BF16_DTYPE_NAMES:
        raise ValueError(f'flash_kmeans_assign_microdim_hybrid_9c0d_v1 requires bfloat16 input, got {dtype}')
    if D not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_microdim_hybrid_9c0d_v1 requires D in {sorted(SUPPORTED_D)}, got {D}')
    if N % BLOCK_N != 0:
