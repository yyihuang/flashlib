"""Clean-room Flash-KMeans Euclidean assignment 16-wide argmin candidate.

Minimum architecture: sm_100a. This candidate dispatches between the v10
single point-tile path for small grids and a 16-wide argmin variant of the
paired point-tile tcgen05/TMEM path for large grids. It is not intended for
...
flash_kmeans_assign_cleanroom_tcgen05_v15 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_cleanroom_tcgen05_v15:flash_kmeans_assign_cleanroom_tcgen05_v15", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 133120, "cta_group": 1, "threads": 192}'))
...
def _make_tmaps(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_cleanroom_v15_tmaps')
    ...
    inputs['_flash_kmeans_assign_cleanroom_v15_tmaps'] = (cache_key, tmap_x, tmap_c)
    return (tmap_x, tmap_c)

def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0343"}, "kernel_flash_kmeans_assign_cleanroom_tcgen05_v15", 133120, 192]}'))
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_cleanroom_tcgen05_v15:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 133120, "cta_group": 1, "threads": 192}'))

def _use_single_tile_path(*, n_points: int, n_clusters: int) -> bool:
    num_n_tiles = n_points // BLOCK_N
    k_tiles = n_clusters // BLOCK_K
    return num_n_tiles <= 8 and k_tiles <= 2
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    if dim != FEAT_D:
        raise ValueError(f'flash_kmeans_assign_cleanroom_tcgen05_v15 requires D={FEAT_D}, got {dim}')
    ...
    num_n_tiles = n_points // BLOCK_N
    if num_n_tiles % 2 != 0:
        raise ValueError('flash_kmeans_assign_cleanroom_tcgen05_v15 requires an even number of BLOCK_N point tiles')
...
def compile_and_launch_flash_kmeans_assign_cleanroom(B: int=2, N: int=1024, K: int=512, D: int=128, *, benchmark: bool=False) -> dict[str, Any]:
    import torch
