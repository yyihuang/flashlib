...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    selected_route = f'd{dim}_{route_kind}_repeated_mma_v1'
    selected_entrypoint = _selected_entrypoint(dim, route_kind)
    normalized['selected_route'] = selected_route
    normalized['route_trace'] = {'shape_key': _shape_key(inputs), 'selected_route': selected_route, 'selected_entrypoint': selected_entrypoint, 'selected_seed': f'd{dim}-{route_kind}-repeated-mma-v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'guard_d{dim}_{route_kind}_repeated_mma_v1', 'guard_condition': f'dtype == bfloat16 and D == {dim} and N % {BLOCK_N} == 0 and K % {BLOCK_K} == 0', 'classification': 'seed-probe', 'dispatcher_kernel_ms': None, 'reason': 'non-128D bucket seed wrapper routes to D-specific Weave tcgen05 candidate'}
    return normalized
...
def _selected_entrypoint(dim: int, route_kind: str) -> str:
    if dim == 192 and route_kind == 'single':
        return f'{_d192_single.__name__}:launch_for_eval'
    if dim == 192:
        return f'{_d192.__name__}:launch_for_eval'
    if dim == 256:
        return f'{_d256.__name__}:launch_for_eval'
    raise ValueError(f'non128d split-D seed supports D in {SUPPORTED_DIMS}, got {dim}')
