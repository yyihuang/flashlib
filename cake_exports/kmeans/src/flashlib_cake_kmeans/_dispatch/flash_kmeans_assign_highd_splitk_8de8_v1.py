...
(``lm.mma_ss``) for D in {320, 384, 448, 512}. High-K rows split the cluster
...
flash_kmeans_assign_highd_splitk_partial_8de8_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_highd_splitk_8de8_v1:flash_kmeans_assign_highd_splitk_partial_8de8_v1', 256)
flash_kmeans_assign_highd_splitk_reduce_8de8_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_highd_splitk_8de8_v1:flash_kmeans_assign_highd_splitk_reduce_8de8_v1', 256)
partial_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_8de8_v1:partial_ir"}'))
reduce_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_8de8_v1:reduce_ir"}'))

def _verify_export_ir() -> Any:
    if os.environ.get('LOOM_FLASH_KMEANS_HIGHD_SPLITK_VERIFY_KERNEL') == 'reduce':
        return reduce_ir
    return partial_ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_8de8_v1:ir"}'))

def _cuda_include_dirs() -> list[str]:
    return _splitd._cuda_include_dirs()

def _compiled_partial_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0331"}, "kernel_flash_kmeans_assign_highd_splitk_partial_8de8_v1", 51200, 192]}'))

def _compiled_reduce_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0332"}, "kernel_flash_kmeans_assign_highd_splitk_reduce_8de8_v1", 0, 128]}'))
...
def _validate_shape(inputs: dict[str, Any], *, bsz: int, n_points: int, dim: int, n_clusters: int) -> None:
    ...
    if dtype_name not in BF16_DTYPE_NAMES:
        raise ValueError(f'flash_kmeans_assign_highd_splitk_8de8_v1 requires bfloat16 input, got {dtype_name}')
    if dim not in SUPPORTED_DIMS:
        raise ValueError(f'flash_kmeans_assign_highd_splitk_8de8_v1 supports D={sorted(SUPPORTED_DIMS)}, got {dim}')
...
def _partial_buffers(inputs: dict[str, Any], total_work: int) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_highd_splitk_8de8_v1_partials')
    ...
    inputs['_flash_kmeans_assign_highd_splitk_8de8_v1_partials'] = (cache_key, scores, indices)
