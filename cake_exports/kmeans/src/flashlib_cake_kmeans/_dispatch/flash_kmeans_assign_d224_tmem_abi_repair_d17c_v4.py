"""Flash-KMeans exact-D224 high-K ownership-repair candidate.

Minimum architecture: sm_100a. This candidate uses Blackwell tcgen05/TMEM
(``lm.mma``) for an exact D=224 score producer by accumulating seven D=32
...
flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4", "arg_keys": ["x_tmap", "c_tmap", "x_sq", "c_sq", "out", "B", "N", "D", "K", "num_n_tiles", "K_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 22528, "constants": [], "cta_group": 1, "threads": 192}'))
...
def _make_tmaps(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4_tmaps')
    ...
    inputs['_flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4_tmaps'] = (cache_key, tmap_x, tmap_c)
    return (tmap_x, tmap_c)

def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0025"}, "kernel_flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4", 22528, 192]}'))
ir = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4", "arg_keys": ["x_tmap", "c_tmap", "x_sq", "c_sq", "out", "B", "N", "D", "K", "num_n_tiles", "K_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 22528, "constants": [], "cta_group": 1, "threads": 192}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    n_clusters = int(inputs['K'])
    if dim != FEAT_D:
        raise ValueError(''.join(['flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4 requires exact D=', format(FEAT_D, ''), ', got ', format(dim, '')]))
    ...
    return {'cluster_ids': inputs['out'], 'selected_route': 'd224_tmem_abi_repair_d17c_v4', 'route_trace': {'selected_route': 'd224_tmem_abi_repair_d17c_v4', 'selected_seed': 'flash_kmeans_assign_d224_tmem_abi_repair_d17c_v4', 'route_kind': 'shape_specific_seed', 'guard_condition': 'dtype == bfloat16 and D == 224 and N % 64 == 0 and K % 256 == 0'}}

def compile_and_launch_flash_kmeans_assign_splitd_d224(B: int=1, N: int=1024, K: int=512, D: int=224, *, benchmark: bool=False) -> dict[str, Any]:
    import torch
    if not torch.cuda.is_available():
        raise RuntimeError('CUDA GPU required')
    torch.manual_seed(22401)
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': ''.join(['manual_b', format(B, ''), '_n', format(N, ''), '_k', format(K, ''), '_d', format(D, '')]), 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 22401}}])
    return result
