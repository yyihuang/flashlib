...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d288_parent_splitk_hybrid_20260629_v1:ir"}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if n_clusters <= 2048:
        ...
        raise ValueError(f'flash_kmeans_assign_d288_parent_splitk_hybrid_20260629_v1 supports K <= 2048 through the parent or K >= 4096 through Split-K, got K={n_clusters}')
...
def _validate_supported_shape(*, N: int, D: int, K: int, dtype: Any) -> None:
    ...
    if dtype_name not in BF16_DTYPE_NAMES:
        raise ValueError(f'flash_kmeans_assign_d288_parent_splitk_hybrid_20260629_v1 requires bfloat16 input, got {dtype}')
    if D not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_d288_parent_splitk_hybrid_20260629_v1 requires D == 288, got {D}')
    if N % BLOCK_N != 0:
