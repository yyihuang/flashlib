...
flash_kmeans_assign_cleanroom_tcgen05_d160_pack_padded_b23d_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_cleanroom_tcgen05_d160_pack_padded_b23d_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
pack_ir = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_cleanroom_tcgen05_d160_pack_padded_b23d_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
...
def _scratch_buffers(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_cleanroom_d160_padded_b23d_v1_scratch')
    if cached is not None and cached[0] == cache_key:
        return (cached[1], cached[2])
    ...
    inputs['_flash_kmeans_assign_cleanroom_d160_padded_b23d_v1_scratch'] = (cache_key, x_pad, c_pad)
...
def _compiled_pack_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0355"}, "kernel_flash_kmeans_assign_cleanroom_tcgen05_d160_pack_padded_b23d_v1", 0, 256]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(f'flash_kmeans_assign_cleanroom_tcgen05_d160_padded_b23d_v1 requires bfloat16, got {dtype}')
    if dim != FEAT_D:
        raise ValueError(f'flash_kmeans_assign_cleanroom_tcgen05_d160_padded_b23d_v1 requires D={FEAT_D}, got {dim}')
