...
flash_kmeans_assign_d160_pad192_pack_f9b2_v1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d160_pad192_tail_repair_f9b2_v1:flash_kmeans_assign_d160_pad192_pack_f9b2_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d160_pad192_tail_repair_f9b2_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
pack_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d160_pad192_tail_repair_f9b2_v1:pack_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
...
def _compiled_pack_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0354"}, "kernel_flash_kmeans_assign_d160_pad192_pack_f9b2_v1", 0, 256]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(f'flash_kmeans_assign_d160_pad192_tail_repair_f9b2_v1 requires bfloat16 input, got {dtype}')
    if dim not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_d160_pad192_tail_repair_f9b2_v1 requires D in {sorted(SUPPORTED_D)}, got {dim}')
    ...
    selected_route = f'd{dim}_pad192_{route_kind}_repeated_mma_f9b2_v1'
    normalized['selected_route'] = selected_route
    normalized['route_trace'] = {'shape_key': _shape_key(inputs), 'selected_route': selected_route, 'selected_entrypoint': f'{__name__}:launch_for_eval', 'selected_seed': f'd{dim}-pad192-{route_kind}-repeated-mma-f9b2-v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'guard_d{dim}_pad192_tail_repair_f9b2_v1', 'guard_condition': f'dtype == bfloat16 and D == {dim} and N % {BLOCK_N} == 0 and K % {BLOCK_K} == 0', 'classification': 'seed-probe', 'dispatcher_kernel_ms': None, 'residual_contract_regions': ['zero_pad_to_d192_pack'], 'reason': 'D144/D160/D176 tail-safe route pads to D192 before the tcgen05 score producer'}
    return normalized
...
def _scratch_buffers(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    n_clusters = int(inputs['K'])
    dim = int(inputs['D'])
    key = (device_index, int(inputs['x'].data_ptr()), int(inputs['centroids'].data_ptr()), bsz, n_points, n_clusters, dim, FEAT_D_PAD)
...
def _route_inputs(inputs: dict[str, Any], x_pad: Any, c_pad: Any) -> dict[str, Any]:
    key = (int(x_pad.data_ptr()), int(c_pad.data_ptr()), int(inputs['x_sq'].data_ptr()), int(inputs['c_sq'].data_ptr()), int(inputs['out'].data_ptr()), int(inputs['B']), int(inputs['N']), int(inputs['K']), FEAT_D_PAD)
...
def compile_and_launch_flash_kmeans_assign_pad192(B: int=1, N: int=2048, K: int=2048, D: int=160, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 16005}}])
    return result
