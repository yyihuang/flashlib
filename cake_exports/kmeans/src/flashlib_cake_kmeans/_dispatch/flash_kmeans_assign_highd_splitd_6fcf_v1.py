...
(``lm.mma_ss``) for D in {320, 384, 448, 512}. It slices the feature dimension
...
flash_kmeans_assign_highd_splitd_6fcf_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_highd_splitd_6fcf_v1:flash_kmeans_assign_highd_splitd_6fcf_v1', 256)
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitd_6fcf_v1:ir"}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if dtype_name not in BF16_DTYPE_NAMES:
        raise ValueError(f'flash_kmeans_assign_highd_splitd_6fcf_v1 requires bfloat16 input, got {dtype_name}')
    if dim not in SUPPORTED_DIMS:
        raise ValueError(f'flash_kmeans_assign_highd_splitd_6fcf_v1 supports D={sorted(SUPPORTED_DIMS)}, got {dim}')
...
def compile_and_launch_flash_kmeans_assign_highd_splitd(B: int=1, N: int=1024, K: int=512, D: int=512, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 51201}}])
    return result
