...
(``lm.mma_ss``) with a Weave BF16 pack/zero-fill producer from logical
...
flash_kmeans_assign_lowdim_pack_e50c_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_lowdim_e50c_v1:flash_kmeans_assign_lowdim_pack_e50c_v1', 256)
flash_kmeans_assign_lowdim_e50c_v1 = _ir_proxy('loom.examples.weave.flash_kmeans_assign_lowdim_e50c_v1:flash_kmeans_assign_lowdim_e50c_v1', 256)
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_lowdim_e50c_v1:ir"}'))
pack_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_lowdim_e50c_v1:pack_ir"}'))
...
def _scratch_buffers(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    n_points = int(inputs['N'])
    n_clusters = int(inputs['K'])
    cache_key = (int(inputs['x'].data_ptr()), int(inputs['centroids'].data_ptr()), bsz, n_points, n_clusters, int(inputs['D']), FEAT_D_PAD)
    cached = inputs.get('_flash_kmeans_assign_lowdim_e50c_v1_scratch')
    if cached is not None and cached[0] == cache_key:
        return (cached[1], cached[2])
    ...
    inputs['_flash_kmeans_assign_lowdim_e50c_v1_scratch'] = (cache_key, x_pad, c_pad)
    inputs.pop('_flash_kmeans_assign_lowdim_e50c_v1_tmaps', None)
    return (x_pad, c_pad)

def _compiled_pack_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0338"}, "kernel_flash_kmeans_assign_lowdim_pack_e50c_v1", 0, 256]}'))

def _compiled_d64_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0339"}, "kernel_flash_kmeans_assign_microdim_6cd2_v1", 51200, 192]}'))
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0340"}, "kernel_flash_kmeans_assign_lowdim_e50c_v1", 100352, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(f'flash_kmeans_assign_lowdim_e50c_v1 requires bfloat16 input, got {dtype}')
    if dim not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_lowdim_e50c_v1 requires D in {sorted(SUPPORTED_D)}, got {dim}')
...
def compile_and_launch_flash_kmeans_assign_lowdim(B: int=4, N: int=1024, K: int=512, D: int=64, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 6401}}])
    return result
