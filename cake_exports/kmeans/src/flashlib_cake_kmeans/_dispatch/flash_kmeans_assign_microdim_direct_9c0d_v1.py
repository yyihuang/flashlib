...
(``lm.mma_ss``) for D=16/32 by staging logical BF16 rows directly into a
...
flash_kmeans_assign_microdim_direct_9c0d_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_microdim_direct_9c0d_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 51200, "cta_group": 1, "threads": 192}'))
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0335"}, "kernel_flash_kmeans_assign_microdim_direct_9c0d_v1", 51200, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if dtype_name not in BF16_DTYPE_NAMES:
        raise ValueError(f'flash_kmeans_assign_microdim_direct_9c0d_v1 requires bfloat16 input, got {dtype_name}')
    if dim not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_microdim_direct_9c0d_v1 requires D in {sorted(SUPPORTED_D)}, got {dim}')
