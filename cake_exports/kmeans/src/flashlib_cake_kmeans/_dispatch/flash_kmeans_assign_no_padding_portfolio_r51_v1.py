...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_no_padding_portfolio_r51_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 75776, "cta_group": 1, "threads": 192}'))
...
def _validate_shape(inputs: dict[str, Any], *, bsz: int, n_points: int, dim: int, n_clusters: int) -> None:
    ...
    if (bsz, n_points, n_clusters, dim) not in NO_PADDING_HIGHD_SHAPES:
        raise ValueError(f'{ROUTE_ID} is exact-shape only for the no-padding high-D bucket, got B={bsz}, N={n_points}, K={n_clusters}, D={dim}')
