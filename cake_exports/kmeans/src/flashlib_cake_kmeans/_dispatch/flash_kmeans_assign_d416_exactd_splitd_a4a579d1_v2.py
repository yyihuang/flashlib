"""Flash-KMeans Euclidean assignment exact-D416 split-D candidate.

Minimum architecture: sm_100a. This candidate uses Blackwell tcgen05/TMEM
(``lm.mma_ss``) for an exact D=416 score producer by accumulating thirteen D=32
...
flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2:flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 26624, "cta_group": 1, "threads": 192}'))
...
def _make_tmaps(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2_tmaps')
    ...
    inputs['_flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2_tmaps'] = (cache_key, tmap_x, tmap_c)
    return (tmap_x, tmap_c)

def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0350"}, "kernel_flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2", 26624, 192]}'))
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 26624, "cta_group": 1, "threads": 192}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    n_clusters = int(inputs['K'])
    if dim % CHUNK_D:
        raise ValueError(f'D must be divisible by CHUNK_D={CHUNK_D}, got {dim}')
    ...
    return {'cluster_ids': inputs['out'], 'selected_route': 'd416_exactd_splitd_a4a579d1_v2', 'route_trace': {'selected_route': 'd416_exactd_splitd_a4a579d1_v2', 'selected_seed': 'flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2', 'route_kind': 'shape_specific_seed', 'guard_condition': 'dtype == bfloat16 and D == 416 and N % 128 == 0 and K % 256 == 0'}}

def compile_and_launch_flash_kmeans_assign_splitd_d416(B: int=1, N: int=1024, K: int=512, D: int=416, *, benchmark: bool=False) -> dict[str, Any]:
    import torch
    if not torch.cuda.is_available():
        raise RuntimeError('CUDA GPU required')
    torch.manual_seed(41601)
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 41601}}])
    return result
