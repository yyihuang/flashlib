...
(``lm.mma``) for an exact D=416 score producer by accumulating thirteen D=32
...
flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2", "arg_keys": ["x_tmap", "c_tmap", "x_sq", "c_sq", "out", "B", "N", "D", "K", "num_n_tiles", "K_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 26624, "constants": [], "cta_group": 1, "threads": 192}'))
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0034"}, "kernel_flash_kmeans_assign_d416_exactd_splitd_a4a579d1_v2", 26624, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    n_clusters = int(inputs['K'])
    if dim % CHUNK_D:
        raise ValueError(''.join(['D must be divisible by CHUNK_D=', format(CHUNK_D, ''), ', got ', format(dim, '')]))
...
def compile_and_launch_flash_kmeans_assign_splitd_d416(B: int=1, N: int=1024, K: int=512, D: int=416, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': ''.join(['manual_b', format(B, ''), '_n', format(N, ''), '_k', format(K, ''), '_d', format(D, '')]), 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 41601}}])
    return result
