"""Flash-KMeans Euclidean assignment micro-D seed for D=16/32.

Minimum architecture: sm_100a. This candidate uses Blackwell tcgen05/TMEM
(``lm.mma_ss``) with a Weave BF16 pack/zero-fill producer from logical D=16/32
to a single 64-wide MMA K slice. It is not intended for sm_120a/sm_121a where
...
flash_kmeans_assign_microdim_pack_6cd2_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_microdim_pack_6cd2_v1", "arg_keys": ["x", "centroids", "x_pad", "c_pad", "B", "N", "D", "K", "total_x_pad", "total_c_pad"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [], "cta_group": 1, "threads": 256}'))
flash_kmeans_assign_microdim_6cd2_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_microdim_6cd2_v1", "arg_keys": ["x_tmap", "c_tmap", "x_sq", "c_sq", "out", "B", "N", "D", "K", "num_n_tiles", "K_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 51200, "constants": [], "cta_group": 1, "threads": 192}'))
ir = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_microdim_6cd2_v1", "arg_keys": ["x_tmap", "c_tmap", "x_sq", "c_sq", "out", "B", "N", "D", "K", "num_n_tiles", "K_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 51200, "constants": [], "cta_group": 1, "threads": 192}'))
pack_ir = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_microdim_pack_6cd2_v1", "arg_keys": ["x", "centroids", "x_pad", "c_pad", "B", "N", "D", "K", "total_x_pad", "total_c_pad"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [], "cta_group": 1, "threads": 256}'))
...
def _make_tmaps(inputs: dict[str, Any], x_pad: Any, c_pad: Any) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_microdim_6cd2_v1_tmaps')
    ...
    inputs['_flash_kmeans_assign_microdim_6cd2_v1_tmaps'] = (cache_key, tmap_x, tmap_c)
...
def _scratch_buffers(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    n_points = int(inputs['N'])
    n_clusters = int(inputs['K'])
    cache_key = (int(inputs['x'].data_ptr()), int(inputs['centroids'].data_ptr()), bsz, n_points, n_clusters, int(inputs['D']))
    cached = inputs.get('_flash_kmeans_assign_microdim_6cd2_v1_scratch')
    if cached is not None and cached[0] == cache_key:
        return (cached[1], cached[2])
    ...
    inputs['_flash_kmeans_assign_microdim_6cd2_v1_scratch'] = (cache_key, x_pad, c_pad)
    inputs.pop('_flash_kmeans_assign_microdim_6cd2_v1_tmaps', None)
    return (x_pad, c_pad)

def _compiled_pack_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0336"}, "kernel_flash_kmeans_assign_microdim_pack_6cd2_v1", 0, 256]}'))
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0337"}, "kernel_flash_kmeans_assign_microdim_6cd2_v1", 51200, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(f'flash_kmeans_assign_microdim_6cd2_v1 requires bfloat16 input, got {dtype}')
    if dim not in SUPPORTED_D:
        raise ValueError(f'flash_kmeans_assign_microdim_6cd2_v1 requires D in {sorted(SUPPORTED_D)}, got {dim}')
...
def compile_and_launch_flash_kmeans_assign_microdim(B: int=4, N: int=1024, K: int=512, D: int=16, *, benchmark: bool=False) -> dict[str, Any]:
    import torch
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 1601}}])
    return result
