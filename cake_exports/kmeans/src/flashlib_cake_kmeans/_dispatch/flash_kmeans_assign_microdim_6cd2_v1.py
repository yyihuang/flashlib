...
(``lm.mma``) with a Weave BF16 pack/zero-fill producer from logical D=16/32
...
def _scratch_buffers(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cache = inputs.setdefault('_flash_kmeans_assign_microdim_6cd2_v1_scratch', {})
...
def _compiled_pack_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0009"}, "kernel_flash_kmeans_assign_microdim_pack_6cd2_v1", 0, 256]}'))
...
def _compiled_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0010"}, "kernel_flash_kmeans_assign_microdim_6cd2_v1", 51200, 192]}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    if dtype not in {'bfloat16', 'bf16'}:
        raise ValueError(''.join(['flash_kmeans_assign_microdim_6cd2_v1 requires bfloat16 input, got ', format(dtype, '')]))
    if dim not in SUPPORTED_D:
        raise ValueError(''.join(['flash_kmeans_assign_microdim_6cd2_v1 requires D in ', format(sorted(SUPPORTED_D), ''), ', got ', format(dim, '')]))
