...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_d128_splitk_priority_575c_v1:ir"}'))

def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    """Launch only the exact D128, no-padding priority Split-K schedule."""
    if int(inputs['B']) != 1:
        raise ValueError(f'{ROUTE_ID} requires B=1')
    if int(inputs['D']) != TARGET_D:
        raise ValueError(f'{ROUTE_ID} requires D={TARGET_D}, got {inputs['D']}')
    if int(inputs['N']) not in (512, 1024, 2048):
        raise ValueError(f'{ROUTE_ID} requires N in {{512, 1024, 2048}}')
    if int(inputs['K']) not in (4096, 8192):
        raise ValueError(f'{ROUTE_ID} requires K in {{4096, 8192}}')
