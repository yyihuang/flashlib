...
def _compiled_reduce_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0349"}, "kernel_flash_kmeans_assign_d480_splitk_reduce_d32k256_v1", 0, 256]}'))
...
def _partial_buffers(inputs: dict[str, Any], total_work: int) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_d480_splitk_producer_reducer_d32k256_v1_partials')
    if cached is not None and cached[0] == cache_key:
        return (cached[1], cached[2])
    ...
    inputs['_flash_kmeans_assign_d480_splitk_producer_reducer_d32k256_v1_partials'] = (cache_key, scores, indices)
