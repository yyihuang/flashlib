"""Flash-KMeans exact-D480 D32/K256 Split-K producer/reducer candidate.

Minimum architecture: sm_100a.  This D480-only candidate preserves the
tcgen05/TMEM producer and exposes each K=256 centroid tile as a coarse CTA
partial; a second Weave reducer consumes the global partial-score/index ABI
and writes contract-visible ``cluster_ids``.  D=480 is fifteen exact D32
chunks, so no feature padding or fallback is used.
"""
...
from . import flash_kmeans_assign_d480_exactd_splitd_d480_exact_seed_v1 as _direct
...
flash_kmeans_assign_d480_splitk_partial_d32k256_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d480_splitk_partial_d32k256_v1", "arg_keys": ["x_tmap", "c_tmap", "c_sq", "partial_scores", "partial_indices", "B", "N", "D", "K", "num_n_tiles", "K_tiles", "K_slices"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 22528, "constants": [], "cta_group": 1, "threads": 192}'))
flash_kmeans_assign_d480_splitk_reduce_d32k256_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d480_splitk_reduce_d32k256_v1", "arg_keys": ["partial_scores", "partial_indices", "out", "B", "N", "K", "num_n_tiles", "K_slices"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [], "cta_group": 1, "threads": 256}'))
partial_ir = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d480_splitk_partial_d32k256_v1", "arg_keys": ["x_tmap", "c_tmap", "c_sq", "partial_scores", "partial_indices", "B", "N", "D", "K", "num_n_tiles", "K_tiles", "K_slices"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 22528, "constants": [], "cta_group": 1, "threads": 192}'))
reduce_ir = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d480_splitk_reduce_d32k256_v1", "arg_keys": ["partial_scores", "partial_indices", "out", "B", "N", "K", "num_n_tiles", "K_slices"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [], "cta_group": 1, "threads": 256}'))
...
def _compiled_partial_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0029"}, "kernel_flash_kmeans_assign_d480_splitk_partial_d32k256_v1", 22528, 192]}'))

def _compiled_reduce_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0030"}, "kernel_flash_kmeans_assign_d480_splitk_reduce_d32k256_v1", 0, 256]}'))
...
def _launch_blockn64_splitk(inputs: dict[str, Any], *, bsz: int, n_points: int, dim: int, n_clusters: int, num_n_tiles: int, k_tiles: int, k_slices: int) -> None:
    ...
    partial_args = pack_kernel_args(flash_kmeans_assign_d480_splitk_partial_d32k256_v1, x_tmap=tmap_x, c_tmap=tmap_c, c_sq=inputs['c_sq'], partial_scores=partial_scores, partial_indices=partial_indices, B=bsz, N=n_points, D=dim, K=n_clusters, num_n_tiles=num_n_tiles, K_tiles=k_tiles, K_slices=k_slices)
    ...
    reduce_args = pack_kernel_args(reduce_ir, partial_scores=partial_scores, partial_indices=partial_indices, out=inputs['out'], B=bsz, N=n_points, K=n_clusters, num_n_tiles=num_n_tiles, K_slices=k_slices)
...
def _make_tmaps(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_d480_splitk_producer_reducer_d32k256_v1_tmaps')
    ...
    n_clusters = int(inputs['K'])
    dim = int(inputs['D'])
    tmap_x = create_tensor_map_3d_64b(inputs['x'].data_ptr(), bsz * n_points, BLOCK_N, dim, FEAT_CHUNK)
    tmap_c = create_tensor_map_3d_64b(inputs['centroids'].data_ptr(), bsz * n_clusters, MMA_BLOCK_K, dim, FEAT_CHUNK)
    inputs['_flash_kmeans_assign_d480_splitk_producer_reducer_d32k256_v1_tmaps'] = (cache_key, tmap_x, tmap_c)
...
def _partial_buffers(inputs: dict[str, Any], total_work: int) -> tuple[Any, Any]:
    ...
    cache = inputs.setdefault('_flash_kmeans_assign_d480_splitk_producer_reducer_d32k256_v1_partials', {})
...
def compile_and_launch_flash_kmeans_assign_d480_splitk_producer_reducer(B: int=1, N: int=512, K: int=8192, D: int=512, *, benchmark: bool=False) -> dict[str, Any]:
    import torch
