...
(``lm.mma_ss``) for an exact D=288 score producer by accumulating nine D=32
...
flash_kmeans_assign_d288_exactd_a532_v1 = _decode_capture(_json_loads('{"__ir__": "flash_kmeans_assign_d288_exactd_a532_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 26624, "cta_group": 1, "threads": 192}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> Any:
    ...
    n_clusters = int(inputs['K'])
    if dim != FEAT_D:
        raise ValueError(f'flash_kmeans_assign_d288_exactd_a532_v1 requires exact D={FEAT_D}, got {dim}')
...
def compile_and_launch_flash_kmeans_assign_splitd_d288(B: int=1, N: int=1024, K: int=512, D: int=256, *, benchmark: bool=False) -> dict[str, Any]:
    ...
    if benchmark:
        ...
        result['contract_eval'] = evaluate(launch_for_eval, shapes=[{'label': f'manual_b{B}_n{N}_k{K}_d{D}', 'params': {'B': B, 'N': N, 'D': D, 'K': K, 'dtype': 'bfloat16', 'seed': 25601}}])
    return result
