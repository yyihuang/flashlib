"""Flash-KMeans high-D Split-K BLOCK_N=64 G2/R4 tile-search candidate.

Minimum architecture: sm_100a. This candidate keeps the high-D Split-K
tcgen05/TMEM score producer on the contract-visible path, keeps the M=64
ROW_16x256B partial geometry, and tests a two-MMA grouped producer with the
four-lane Split-K reducer. The goal is to expose twice as many producer CTAs
as the G4/R4 parent without the K256 candidate's fourfold partial traffic.
It is not intended for sm_120a/sm_121a where ptxas rejects tcgen05 instructions.
"""
...
SPLITK_GROUP_K_TILES = 2
SPLITK_TILE_K = MMA_BLOCK_K * SPLITK_GROUP_K_TILES
MAX_POINT_TILES = 32
NUM_COMPUTE_WARPS = 4
REDUCE_LANES_PER_ROW = 4
REDUCE_THREADS = BLOCK_N * REDUCE_LANES_PER_ROW
ROUTE_ID = 'highd_splitk_blockn64_g2r4_b5a6_v1'
SEED_ID = 'highd-splitk-blockn64-g2r4-b5a6-v1'
flash_kmeans_assign_highd_splitk_partial_blockn64_g2r4_b5a6_v1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1:flash_kmeans_assign_highd_splitk_partial_blockn64_g2r4_b5a6_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 43008, "cta_group": 1, "threads": 192}'))
flash_kmeans_assign_highd_splitk_reduce_blockn64_g2r4_b5a6_v1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1:flash_kmeans_assign_highd_splitk_reduce_blockn64_g2r4_b5a6_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
partial_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1:partial_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 43008, "cta_group": 1, "threads": 192}'))
reduce_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1:reduce_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))

def _verify_export_ir() -> Any:
    if os.environ.get('LOOM_FLASH_KMEANS_HIGHD_SPLITK_BLOCKN64_G2R4_B5A6_VERIFY_KERNEL') == 'reduce':
        return reduce_ir
    return partial_ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 43008, "cta_group": 1, "threads": 192}'))

def _compiled_partial_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0328"}, "kernel_flash_kmeans_assign_highd_splitk_partial_blockn64_g2r4_b5a6_v1", 43008, 192]}'))

def _compiled_reduce_kernel() -> tuple[bytes, str, int, int]:
    return _decode_capture(_json_loads('{"__tuple__": [{"__kernel_source__": "dispatch_kernel_0329"}, "kernel_flash_kmeans_assign_highd_splitk_reduce_blockn64_g2r4_b5a6_v1", 0, 256]}'))
...
def _launch_blockn64_splitk(inputs: dict[str, Any], *, bsz: int, n_points: int, dim: int, n_clusters: int, num_n_tiles: int, k_tiles: int, k_slices: int) -> None:
    ...
    partial_args = [inputs['c_sq'], partial_scores, partial_indices, tmap_x, tmap_c, bsz, n_points, dim, n_clusters, num_n_tiles, k_tiles, k_slices]
...
def _make_tmaps(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1_tmaps')
    ...
    inputs['_flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1_tmaps'] = (cache_key, tmap_x, tmap_c)
...
def _partial_buffers(inputs: dict[str, Any], total_work: int) -> tuple[Any, Any]:
    ...
    cached = inputs.get('_flash_kmeans_assign_highd_splitk_blockn64_g2r4_b5a6_v1_partials')
    if cached is not None and cached[0] == cache_key:
        return (cached[1], cached[2])
