...
kernel_knn_build_common_d_5e7f_rag_d64_repair_stage1(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    const int smem_smem_database = smem + 9216;
    const int smem_smem_database_sq = smem + 17408;
    ...
    __nv_bfloat16* smem_database = (__nv_bfloat16*)(smem_raw + 9216);
    #define smem_database_addr (smem + 9216)
    float* smem_database_sq = (float*)(smem_raw + 17408);
    #define smem_database_sq_addr (smem + 17408)
            ...
            for (unsigned int work_idx = bid; work_idx < total_work; work_idx += num_bids) {
                ...
                if (valid_q != 0) {
                    ...
                    for (int out_k = 0; out_k < TOP_K_MAX; out_k++) {
                        ...
                        #pragma unroll
                        for (int scan_pos = 1; scan_pos < TOP_K_MAX; scan_pos++) {
                            if (best_d[scan_pos] < selected_d) {
                                selected_d = best_d[scan_pos];
                                selected_i = best_i[scan_pos];
                                selected_pos = scan_pos;
                            }
                        }
                        if (out_k < K) {
