...
#define SMEM_SMEM_QUERY_STAGE_BYTES 24576
#define SMEM_SMEM_QUERY_STRIDE 24576
#define SMEM_SMEM_DATABASE_OFF 25600
#define SMEM_SMEM_DATABASE_STAGE_BYTES 12288
#define SMEM_SMEM_DATABASE_STRIDE 12288
#define SMEM_SMEM_DATABASE_SQ_OFF 37888
...
kernel_knn_build_non128_frontier_4be7_d96exact_stage1(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    const int smem_smem_database = smem + 25600;
    const int smem_smem_database_sq = smem + 37888;
    ...
    __nv_bfloat16* smem_database = (__nv_bfloat16*)(smem_raw + 25600);
    #define smem_database_addr (smem + 25600)
    float* smem_database_sq = (float*)(smem_raw + 37888);
    #define smem_database_sq_addr (smem + 37888)
            ...
            if (warp_id == 0) {
                if (elect_sync()) {
                    ...
                    for (unsigned int work_idx = bid; work_idx < total_work; work_idx += num_bids) {
                        ...
                        mbarrier_arrive_expect_tx(query_full_addr, 24576);
                        ...
                        for (int local_db_tile = 0; local_db_tile < db_tiles_per_split; local_db_tile++) {
                            ...
                            mbarrier_arrive_expect_tx(database_full_addr, 12288);
