...
kernel_knn_build_common_d_generic_direct_v1(__nv_bfloat16* __restrict__ query, __nv_bfloat16* __restrict__ database, float* __restrict__ out_dists, int32_t* __restrict__ out_indices, int B, int Q, int M, int K, int D)
{
    ...
    if (batch_id < B) {
        ...
        if (tid == 0) {
            ...
            for (int kk = 0; kk < K_MAX_; kk++) {
                if (kk < K) {
                    out_dists[out_base + kk] = final_d[kk];
