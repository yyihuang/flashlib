...
kernel_knn_build_evolve_7bfc_k32_merge_s4_unordered_warp_select(float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, float* __restrict__ out_dists, int32_t* __restrict__ out_indices, int total_queries)
{
    ...
    if (row < total_queries) {
        ...
        float d2 = (float)partial_dists[base2 + cand_k];
        int i2 = partial_indices[base2 + cand_k];
        int base3 = base2 + split_stride;
        float d3 = (float)partial_dists[base3 + cand_k];
