...
kernel_knn_build_evolve_7bfc_split_cg2_stage1_batch8_cond4_vmin_threshold_k5(const void* __restrict__ tmap_query, const void* __restrict__ tmap_database, float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int* __restrict__ partial_indices, int B, int Q, int M, int K, int num_q_tile_pairs, int db_tiles_per_split, int split_count, int total_work)
{
            ...
            for (unsigned int work_idx_2 = cluster_id; work_idx_2 < total_work; work_idx_2 += num_clusters) {
                ...
                if (valid_q != 0) {
                    ...
                    for (int out_k = 0; out_k < TOP_K_SMALL; out_k++) {
                        ...
                        int selected_pos = 0;
                        #pragma unroll
                        for (int scan = 1; scan < TOP_K_SMALL; scan++) {
                            if (selected_d > best_d[scan]) {
