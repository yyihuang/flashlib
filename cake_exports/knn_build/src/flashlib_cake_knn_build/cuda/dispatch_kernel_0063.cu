...
kernel_knn_build_k20_mergeown_08ec_warp8_select_s2warp8(float* __restrict__ partial_dists, int* __restrict__ partial_indices, float* __restrict__ out_dists, int* __restrict__ out_indices, int total_queries)
{
    ...
    if (row < total_queries) {
        ...
        for (int out_k = 0; out_k < TOP_K_MAX; out_k++) {
            ...
            if (lane == winner_lane) {
                #pragma unroll
                for (int split_idx_2 = 0; split_idx_2 < SPLIT_COUNT; split_idx_2++) {
