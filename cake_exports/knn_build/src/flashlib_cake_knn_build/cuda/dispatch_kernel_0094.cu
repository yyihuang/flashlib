...
kernel_knn_build_evolve_7bfc_k10_merge_s7(float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, float* __restrict__ out_dists, int32_t* __restrict__ out_indices, int B, int Q, int total_queries)
{
    ...
    for (int row = start_row; row < total_queries; row += stride) {
        ...
        for (int out_k = 0; out_k < TOP_K_MAX; out_k++) {
            ...
            #pragma unroll
            for (int split_idx = 0; split_idx < SPLIT_COUNT; split_idx++) {
                int partial_base = ((split_idx * B + batch_idx) * Q + q_idx) * TOP_K_MAX;
                int cand_pos = split_pos[split_idx];
                float cand_d = (float)partial_dists[partial_base + cand_pos];
