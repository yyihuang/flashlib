...
kernel_knn_build_rag_microbucket_k32_q16irreg2warp_a444_v2_stage1_q16_rowld1_2warp_q16irreg2warp_a444_v2(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    const int smem_smem_local_i = smem + 44288;
    ...
    int* smem_local_i = (int*)(smem_raw + 44288);
    #define smem_local_i_addr (smem + 44288)
            ...
            for (unsigned int work_idx = bid; work_idx < total_work; work_idx += num_bids) {
                ...
                for (int local_db_tile = 0; local_db_tile < db_tiles_per_split; local_db_tile++) {
                    ...
                    for (int repeat = 0; repeat < 8; repeat++) {
                        ...
                        if (((take1 != 0) ? cand1_d : cand0_d) < best_d[31]) {
                            ...
                            if (((take1 != 0) ? cand0_d : cand1_d) < best_d[31]) {
