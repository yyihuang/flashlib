...
kernel_knn_build_q4096_k8_fd9b_stage1_unordered_exact_prefill(const void* __restrict__ tmap_query, const void* __restrict__ tmap_database, float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int* __restrict__ partial_indices, int B, int Q, int M, int K, int num_q_tile_pairs, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    // ---- Role: mma ----
        ...
        { // mma_main
            ...
            if (cta_rank == 0) {
                #pragma unroll 1
                for (unsigned int _work_idx = cluster_id; _work_idx < total_work; _work_idx += num_clusters) {
    ...
    // ---- Role: compute ----
        ...
        { // compute_main
            ...
            for (unsigned int work_idx_1 = cluster_id; work_idx_1 < total_work; work_idx_1 += num_clusters) {
                ...
                float q_sq_val = query_sq[batch_idx_1 * Q + q_idx];
                ...
                for (int local_db_tile_1 = 0; local_db_tile_1 < db_tiles_per_split; local_db_tile_1++) {
                    ...
                    if (local_db_tile_1 == 0) {
                        #pragma unroll
                        for (int col_base = 0; col_base < TOP_K_MAX; col_base += 4) {
                            float dist_vec[4];
                        ...
                        #pragma unroll 2
                        for (int col_base_1 = TOP_K_MAX; col_base_1 < 64; col_base_1 += 4) {
                            float dist_vec_1[4];
                            dist_vec_1[0] = _tmem_load_0[col_base_1];
                            dist_vec_1[1] = _tmem_load_0[col_base_1 + 1];
                            dist_vec_1[2] = _tmem_load_0[col_base_1 + 2];
                            dist_vec_1[3] = _tmem_load_0[col_base_1 + 3];
                            const float2 _fma_b2_2 = {-2.0f, -2.0f};
                            const float2 _fma_c2_3 = {q_sq_val, q_sq_val};
                            #pragma unroll
                            for (int _lf = 0; _lf < 2; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_vec_1)[_lf], _fma_b2_2, _fma_c2_3);
                            float db_sq_vec_1[4];
                            db_sq_vec_1[0] = smem_database_sq[col_base_1];
                            db_sq_vec_1[1] = smem_database_sq[col_base_1 + 1];
                            db_sq_vec_1[2] = smem_database_sq[col_base_1 + 2];
                            db_sq_vec_1[3] = smem_database_sq[col_base_1 + 3];
                            float _t1[4];
                            #pragma unroll
                            for (int _la = 0; _la < 2; _la++)
                                reinterpret_cast<float2*>(_t1)[_la] = add_f32x2(reinterpret_cast<float2*>(dist_vec_1)[_la], reinterpret_cast<const float2*>(db_sq_vec_1)[_la]);
                            float group_min = _t1[0];
                            if (_t1[1] < group_min) {
                                group_min = _t1[1];
                            }
                            if (_t1[2] < group_min) {
                            ...
                            if (group_min < worst_d) {
                                ...
                                for (int vec_col_1 = 0; vec_col_1 < 4; vec_col_1++) {
                                    int db_idx = db_start + col_base_1 + vec_col_1;
                                    float dist = _t1[vec_col_1];
                                    if (dist < worst_d) {
                                        ...
                                        for (int scan_pos_1 = 1; scan_pos_1 < TOP_K_MAX; scan_pos_1++) {
                                            if (worst_d < best_d[scan_pos_1]) {
                                                worst_d = best_d[scan_pos_1];
                                                worst_pos = scan_pos_1;
                                            }
                                        }
                                    }
                                }
                        ...
                        #pragma unroll 2
                        for (int col_base_2 = 0; col_base_2 < 64; col_base_2 += 4) {
                            float dist_vec_2[4];
                            dist_vec_2[0] = _tmem_load_0[col_base_2];
                            dist_vec_2[1] = _tmem_load_0[col_base_2 + 1];
                            dist_vec_2[2] = _tmem_load_0[col_base_2 + 2];
                            dist_vec_2[3] = _tmem_load_0[col_base_2 + 3];
                            const float2 _fma_b2_4 = {-2.0f, -2.0f};
                            const float2 _fma_c2_5 = {q_sq_val, q_sq_val};
                            #pragma unroll
                            for (int _lf = 0; _lf < 2; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_vec_2)[_lf], _fma_b2_4, _fma_c2_5);
                            float db_sq_vec_2[4];
                            db_sq_vec_2[0] = smem_database_sq[col_base_2];
                            db_sq_vec_2[1] = smem_database_sq[col_base_2 + 1];
                            db_sq_vec_2[2] = smem_database_sq[col_base_2 + 2];
                            db_sq_vec_2[3] = smem_database_sq[col_base_2 + 3];
                            float _t2[4];
                            #pragma unroll
                            for (int _la = 0; _la < 2; _la++)
                                reinterpret_cast<float2*>(_t2)[_la] = add_f32x2(reinterpret_cast<float2*>(dist_vec_2)[_la], reinterpret_cast<const float2*>(db_sq_vec_2)[_la]);
                            float group_min_1 = _t2[0];
                            if (_t2[1] < group_min_1) {
                                group_min_1 = _t2[1];
                            }
                            if (_t2[2] < group_min_1) {
                                group_min_1 = _t2[2];
                            }
                            if (_t2[3] < group_min_1) {
                                group_min_1 = _t2[3];
                            }
                            if (group_min_1 < worst_d) {
                                #pragma unroll
                                for (int vec_col_2 = 0; vec_col_2 < 4; vec_col_2++) {
                                    int db_idx_1 = db_start + col_base_2 + vec_col_2;
                                    float dist_1 = _t2[vec_col_2];
                                    if (dist_1 < worst_d) {
                                        ...
                                        worst_pos = 0;
                                        #pragma unroll
                                        for (int scan_pos_2 = 1; scan_pos_2 < TOP_K_MAX; scan_pos_2++) {
                                            if (worst_d < best_d[scan_pos_2]) {
                                                worst_d = best_d[scan_pos_2];
                                                worst_pos = scan_pos_2;
                                            }
                                        }
                                    }
                                }
