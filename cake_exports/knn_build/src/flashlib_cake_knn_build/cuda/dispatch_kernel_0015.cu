...
kernel_knn_build_q4096_k8_fd9b_stage1_unordered_exact_prefill(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tile_pairs, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    // ---- Role: compute ----
        ...
        { // compute_main
            ...
            for (unsigned int work_idx = cluster_id; work_idx < total_work; work_idx += num_clusters) {
                ...
                float q_sq_val = (float)query_sq[batch_idx * Q + q_idx];
                ...
                for (int local_db_tile = 0; local_db_tile < db_tiles_per_split; local_db_tile++) {
                    ...
                    if (local_db_tile == 0) {
                        ...
                        #pragma unroll 2
                        for (int col_base = TOP_K_MAX; col_base < 64; col_base += 4) {
                            float dist_vec[4];
                            dist_vec[0] = dots[col_base];
                            dist_vec[1] = dots[col_base + 1];
                            dist_vec[2] = dots[col_base + 2];
                            ...
                            #pragma unroll
                            for (int _lf = 0; _lf < 2; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_vec)[_lf], _fma_b2_2, _fma_c2_3);
                            float db_sq_vec[4];
                            db_sq_vec[0] = smem_database_sq[col_base];
                            db_sq_vec[1] = smem_database_sq[col_base + 1];
                            db_sq_vec[2] = smem_database_sq[col_base + 2];
                            ...
                            #pragma unroll
                            for (int _la = 0; _la < 2; _la++)
                                reinterpret_cast<float2*>(_t1)[_la] = add_f32x2(reinterpret_cast<float2*>(dist_vec)[_la], reinterpret_cast<const float2*>(db_sq_vec)[_la]);
                        ...
                        for (int col_base = 0; col_base < 64; col_base += 4) {
                            ...
                            #pragma unroll
                            for (int _lf = 0; _lf < 2; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_vec)[_lf], _fma_b2_4, _fma_c2_5);
                            float db_sq_vec[4];
                            db_sq_vec[0] = smem_database_sq[col_base];
                            db_sq_vec[1] = smem_database_sq[col_base + 1];
                            db_sq_vec[2] = smem_database_sq[col_base + 2];
                            ...
                            #pragma unroll
                            for (int _la = 0; _la < 2; _la++)
                                reinterpret_cast<float2*>(_t2)[_la] = add_f32x2(reinterpret_cast<float2*>(dist_vec)[_la], reinterpret_cast<const float2*>(db_sq_vec)[_la]);
                            float group_min = _t2[0];
                            if (_t2[1] < group_min) {
                                group_min = _t2[1];
                            }
                            if (_t2[2] < group_min) {
