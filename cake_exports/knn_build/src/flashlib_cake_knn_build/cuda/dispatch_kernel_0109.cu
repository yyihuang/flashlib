...
kernel_knn_build_rag_microbucket_k32_q32rowld2exact_s141_72d1_v1_stage1_q32rowld2exact_f653_v1(const void* __restrict__ tmap_query, const void* __restrict__ tmap_database, float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int* __restrict__ partial_indices, int B, int Q, int M, int K, int num_q_tiles, int num_db_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    if (warp <= 1) {
        { // compute_main
            ...
            for (unsigned int work_idx = bid; work_idx < SPLIT_COUNT_CONST; work_idx += num_bids) {
                int split_idx = work_idx % (unsigned int)SPLIT_COUNT_CONST;
                ...
                int lane_col = lane % 4;
                int slot = lane_col;
                float q_sq_top = query_sq[row_top];
                float q_sq_bot = query_sq[row_bot];
    ...
    // ---- Role: load ----
        ...
        { // load_main
            ...
            if (warp == 2) {
                if (elect_sync()) {
                    #pragma unroll 1
                    for (unsigned int work_idx_1 = bid; work_idx_1 < SPLIT_COUNT_CONST; work_idx_1 += num_bids) {
                        int split_idx_1 = work_idx_1 % (unsigned int)SPLIT_COUNT_CONST;
                        int extra_before_1 = split_idx_1;
                        if (extra_before_1 > EXTRA_SPLITS_CONST) {
                            extra_before_1 = EXTRA_SPLITS_CONST;
                        }
                        int split_tile_count_1 = TILES_FLOOR_CONST;
                        if (split_idx_1 < EXTRA_SPLITS_CONST) {
                            split_tile_count_1 = TILES_FLOOR_CONST + 1;
                        }
                        int db_tile_start_1 = split_idx_1 * TILES_FLOOR_CONST + extra_before_1;
                        ...
                        #pragma unroll 1
                        for (int local_db_tile_1 = 0; local_db_tile_1 < DB_TILES_PER_SPLIT_CONST; local_db_tile_1++) {
    ...
    // ---- Role: mma ----
        ...
        { // mma_main
            ...
            #pragma unroll 1
            for (unsigned int work_idx_2 = bid; work_idx_2 < SPLIT_COUNT_CONST; work_idx_2 += num_bids) {
                int split_idx_2 = work_idx_2 % (unsigned int)SPLIT_COUNT_CONST;
                int split_tile_count_2 = TILES_FLOOR_CONST;
                if (split_idx_2 < EXTRA_SPLITS_CONST) {
                    split_tile_count_2 = TILES_FLOOR_CONST + 1;
                }
                mbarrier_wait(query_full_addr, _phase_query_full_0);
                _phase_query_full_0 ^= 1;
                #pragma unroll 1
                for (int local_db_tile_2 = 0; local_db_tile_2 < DB_TILES_PER_SPLIT_CONST; local_db_tile_2++) {
