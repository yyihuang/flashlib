...
kernel_knn_build_rag_microbucket_q32rowld_e5db_v1_stage1_q32_k32_m64_q128rowld_60fb_v1(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    const int smem_smem_local_i = smem + 66816;
    ...
    int* smem_local_i = (int*)(smem_raw + 66816);
    #define smem_local_i_addr (smem + 66816)
