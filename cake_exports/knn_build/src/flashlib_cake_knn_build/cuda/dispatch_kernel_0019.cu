...
kernel_knn_build_d64_q4096_c271_stage1_unordered_syncdrop(const void* __restrict__ tmap_query, const void* __restrict__ tmap_database, float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int* __restrict__ partial_indices, int B, int Q, int M, int K, int num_q_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    float* smem_database_sq = reinterpret_cast<float*>(smem_raw + 25600);
    const int smem_database_sq_addr = smem + 25600;
