...
kernel_knn_build_evolve_7bfc_d256_twomma_base(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ out_dists, int32_t* __restrict__ out_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int num_db_tiles, int total_tiles)
{
            ...
            if (warp_id == 0) {
                if (elect_sync()) {
                    ...
                    for (unsigned int tile_idx = bid; tile_idx < total_tiles; tile_idx += num_bids) {
                        int batch_idx = tile_idx / num_q_tiles;
                        int q_tile = tile_idx % num_q_tiles;
    ...
    // ---- Role: compute ----
        ...
        { // compute_main
            ...
            for (unsigned int tile_idx = bid; tile_idx < total_tiles; tile_idx += num_bids) {
                ...
                for (int db_tile = 0; db_tile < num_db_tiles; db_tile++) {
                    ...
                    for (int col = 0; col < 64; col++) {
                        ...
                        if (valid_q != 0 & db_idx < M) {
                            float db_sq_val = (float)database_sq[batch_idx * M + db_idx];
                            float dist = q_sq_val + db_sq_val - 2.0f * dots[col];
                ...
                if (valid_q != 0) {
                    int out_base = (batch_idx * Q + q_idx) * K;
                    #pragma unroll
