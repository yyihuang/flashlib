...
kernel_knn_build_evolve_7bfc_k5_merge_s4_tree(float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, float* __restrict__ out_dists, int32_t* __restrict__ out_indices, int B, int Q, int total_queries)
{
    ...
    for (int row = start_row; row < total_queries; row += stride) {
        ...
        int base_row = (batch_idx * Q + q_idx) * TOP_K_SMALL;
        int split_stride = B * Q * TOP_K_SMALL;
