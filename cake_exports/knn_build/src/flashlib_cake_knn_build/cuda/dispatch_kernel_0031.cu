...
#define SMEM_SMEM_QUERY_STAGE_BYTES 81920
#define SMEM_SMEM_QUERY_STRIDE 81920
...
kernel_knn_build_non128_frontier_8227_d320tail_stage1(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    const int smem_smem_database = smem + 82944;
    const int smem_smem_database_sq = smem + 123904;
    ...
    __nv_bfloat16* smem_database = (__nv_bfloat16*)(smem_raw + 82944);
    #define smem_database_addr (smem + 82944)
    float* smem_database_sq = (float*)(smem_raw + 123904);
    #define smem_database_sq_addr (smem + 123904)
            ...
            if (warp_id == 0) {
                if (elect_sync()) {
                    ...
                    for (unsigned int work_idx = bid; work_idx < total_work; work_idx += num_bids) {
                        ...
                        mbarrier_arrive_expect_tx(query_full_addr, 81920);
                        ...
                        for (int local_db_tile = 0; local_db_tile < db_tiles_per_split; local_db_tile++) {
                            ...
                            mbarrier_arrive_expect_tx(database_full_addr, 40960);
