...
#define SPLIT_COUNT_CONST 141
#define NUM_DB_TILES_CONST 1563
#define TILES_FLOOR_CONST 11
#define EXTRA_SPLITS_CONST 12
#define DB_TILES_PER_SPLIT_CONST 12
...
kernel_knn_build_rag_microbucket_k32_q32rowld2exact_s141_72d1_v1_stage1_q32rowld2exact_f653_v1(float* __restrict__ query_sq, float* __restrict__ database_sq, float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, const void* tmap_query, const void* tmap_database, int B, int Q, int M, int K, int num_q_tiles, int num_db_tiles, int db_tiles_per_split, int split_count, int total_work)
{
    ...
    const int smem_smem_local_d = smem + 34048;
    const int smem_smem_local_i = smem + 50432;
    ...
    float* smem_local_d = (float*)(smem_raw + 34048);
    #define smem_local_d_addr (smem + 34048)
    int* smem_local_i = (int*)(smem_raw + 50432);
    #define smem_local_i_addr (smem + 50432)
    ...
    if (warp <= 1) {
        ...
        { // compute_main
            int warp_id_in_role = warp_id - 0;
            uint32_t _phase_score_full_0 = 0;
            #pragma unroll 1
            for (unsigned int work_idx = bid; work_idx < SPLIT_COUNT_CONST; work_idx += num_bids) {
                int split_idx = work_idx % SPLIT_COUNT_CONST;
                ...
                int lane_col = lane % 4;
                int slot = lane_col;
                float q_sq_top = (float)query_sq[row_top];
                float q_sq_bot = (float)query_sq[row_bot];
                ...
                if (extra_before > EXTRA_SPLITS_CONST) {
                    extra_before = EXTRA_SPLITS_CONST;
                }
                int split_tile_count = TILES_FLOOR_CONST;
                if (split_idx < EXTRA_SPLITS_CONST) {
                    split_tile_count = TILES_FLOOR_CONST + 1;
                }
                int db_tile_start = split_idx * TILES_FLOOR_CONST + extra_before;
                #pragma unroll 1
                for (int local_db_tile = 0; local_db_tile < DB_TILES_PER_SPLIT_CONST; local_db_tile++) {
                    if (local_db_tile < split_tile_count) {
                        ...
                        int cross_addr = taddr + (tmem_row_origin << 16);
                        float dots[32];
                        asm volatile(
                        ...
                        for (int repeat = 0; repeat < 8; repeat++) {
                            ...
                            if (db_idx0 < M_LIMIT) {
                                top_d0 = max_noftz(q_sq_top + (float)database_sq[db_idx0] - 2.0f * dots[reg_base], 0.0f);
                            }
                            if (db_idx1 < M_LIMIT) {
                                top_d1 = max_noftz(q_sq_top + (float)database_sq[db_idx1] - 2.0f * dots[reg_base + 1], 0.0f);
                            }
                            int top_take1 = ((top_d1 < top_d0) ? 1 : 0);
                            if (((top_take1 != 0) ? top_d1 : top_d0) < best_top_d[31]) {
                                ...
                                if (((top_take1 != 0) ? top_d0 : top_d1) < best_top_d[31]) {
                            ...
                            if (db_idx0 < M_LIMIT) {
                                bot_d0 = max_noftz(q_sq_bot + (float)database_sq[db_idx0] - 2.0f * dots[reg_base + 2], 0.0f);
                            }
                            if (db_idx1 < M_LIMIT) {
                                bot_d1 = max_noftz(q_sq_bot + (float)database_sq[db_idx1] - 2.0f * dots[reg_base + 3], 0.0f);
                            }
                            int bot_take1 = ((bot_d1 < bot_d0) ? 1 : 0);
                            if (((bot_take1 != 0) ? bot_d1 : bot_d0) < best_bot_d[31]) {
                                ...
                                if (((bot_take1 != 0) ? bot_d0 : bot_d1) < best_bot_d[31]) {
