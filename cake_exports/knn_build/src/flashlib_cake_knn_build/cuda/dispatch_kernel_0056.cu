...
kernel_knn_build_evolve_7bfc_k32_merge_s4_unordered_warp_select(float* __restrict__ partial_dists, int* __restrict__ partial_indices, float* __restrict__ out_dists, int* __restrict__ out_indices, int total_queries)
{
    ...
    if (row < total_queries) {
        float d0 = partial_dists[base_row + cand_k];
        int i0 = partial_indices[base_row + cand_k];
        int base1 = base_row + split_stride;
        float d1 = partial_dists[base1 + cand_k];
        int i1 = partial_indices[base1 + cand_k];
        int base2 = base1 + split_stride;
        float d2 = partial_dists[base2 + cand_k];
        int i2 = partial_indices[base2 + cand_k];
        int base3 = base2 + split_stride;
        float d3 = partial_dists[base3 + cand_k];
        int i3 = partial_indices[base3 + cand_k];
        #pragma unroll
