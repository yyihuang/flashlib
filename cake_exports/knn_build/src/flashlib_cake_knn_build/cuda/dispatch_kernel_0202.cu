                        ...
                        for (int col_base = 0; col_base < 64; col_base += 4) {
                            ...
                            if (group_min < worst_d) {
                                ...
                                for (int visit = 0; visit < 4; visit++) {
                                    ...
                                    if (fill_count < TOP_K_MAX) {
                                        if (db_idx < M) {
                                            best_d[fill_count] = dist;
                                            best_i[fill_count] = db_idx;
                                            fill_count += 1;
                                            if (fill_count == TOP_K_MAX) {
                                                #pragma unroll
                                                for (int chunk_1 = 0; chunk_1 < 24; chunk_1++) {
                                                    int chunk_base_1 = chunk_1 * 4;
                                                    chunk_worst_d[chunk_1] = best_d[chunk_base_1];
                                                    chunk_worst_pos[chunk_1] = chunk_base_1;
                                                    #pragma unroll
                                                    for (int offset = 1; offset < 4; offset++) {
                                                        int scan_pos = chunk_base_1 + offset;
                                                        if (best_d[scan_pos] > chunk_worst_d[chunk_1]) {
                                                            chunk_worst_d[chunk_1] = best_d[scan_pos];
                                                            chunk_worst_pos[chunk_1] = scan_pos;
                                                        }
                                                    }
                                                }
                                        ...
                                        if (db_idx < M) {
                                            ...
                                            worst_chunk = 0;
                                            #pragma unroll
                                            for (int chunk_3 = 1; chunk_3 < 24; chunk_3++) {
                                                if (worst_d < chunk_worst_d[chunk_3]) {
                                                    worst_d = chunk_worst_d[chunk_3];
                                                    worst_pos = chunk_worst_pos[chunk_3];
                                                    worst_chunk = chunk_3;
                                                }
                                            }
                                        }
