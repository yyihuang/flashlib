...
kernel_knn_build_common_d_56f3_d256_q1024_k10_merge_rowbase_cache_s16(float* __restrict__ partial_dists, int32_t* __restrict__ partial_indices, float* __restrict__ out_dists, int32_t* __restrict__ out_indices, int total_queries)
{
    ...
    for (int row = start_row; row < total_queries; row += stride) {
        ...
        for (int out_k = 0; out_k < TOP_K_MAX; out_k++) {
            ...
            float out_d = ((best_i == row) ? 0.0f : best_d);
            *((float*)(out_dists + base_row + out_k)) = out_d;
