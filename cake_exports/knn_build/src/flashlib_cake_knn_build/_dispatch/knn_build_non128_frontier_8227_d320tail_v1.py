...
knn_build_non128_frontier_8227_d320tail_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_non128_frontier_8227_d320tail_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 124160, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
stage1_d320tail_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_non128_frontier_8227_d320tail_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 124160, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _verify_export_ir() -> Any:
    ...
    if verify_kernel == 'merge':
        return merge_ir
    return stage1_d320tail_ir
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_non128_frontier_8227_d320tail_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 124160, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _split_count_for_label(label: str) -> int:
    if _uses_d320tail(label):
        env_key = f'LOOM_KNN_NON128_FRONTIER_8227_D320TAIL_SPLITS_{label.upper().replace('-', '_')}'
...
def _launch_d320tail(inputs: dict[str, Any], label: str) -> None:
    ...
    if dim != D320_FEAT_D:
        raise ValueError(f'D320 tail route expected D={D320_FEAT_D}, got {dim}')
    ...
    _compiled_d320tail_stage1().launch(grid=(stage1_grid, 1, 1), block=(THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_d320tail_ir.computed_smem_bytes)
...
def route_trace_for_contract_shapes(shape_labels=None, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        if label is None or force_fallback:
            rows.append({'shape_key': inputs['label'], 'selected_route': wide_m64.route_for_contract_inputs(inputs, force_fallback=force_fallback), 'selected_entrypoint': f'{wide_m64.MODULE}:launch_from_contract_inputs', 'selected_seed': None, 'expected_seed': 'non128_frontier_8227_d320tail_v1' if inputs['label'] in TARGET_SHAPE_SET else None, 'route_kind': 'fallback', 'route_source': '8227-wide-m64-parent-dispatcher', 'guard_id': 'forced_fallback' if force_fallback else 'wide_m64_parent_guard', 'classification': 'forced_fallback' if force_fallback else 'delegated'})
            continue
        spec = SHAPE_SPECS[label]
        uses_d320tail = _uses_d320tail(label)
        feature_dim = _feature_dim_for_label(label)
        rows.append({'shape_key': label, 'selected_route': route_for_contract_inputs(inputs), 'selected_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'selected_seed': 'non128_frontier_8227_d320tail_v1', 'expected_seed': 'non128_frontier_8227_d320tail_v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': '8227_d320tail_non128_exact_shape_guard', 'guard_condition': f'exact BF16 B={spec['B']} Q={spec['Q']} M={spec['M']} D={spec['D']} K={spec['K']} build={spec['build']}', 'feature_dim': feature_dim, 'split_count': _split_count_for_label(label), 'producer': _producer_for_label(label), 'preprocess_stage': None if uses_d320tail else wide_m64.route_trace_for_contract_shapes((label,))[0].get('preprocess_stage'), 'source_route': 'd320_exact_tail64' if uses_d320tail else wide_m64.route_for_contract_inputs(inputs), 'classification': 'd320-tail64' if uses_d320tail else 'wide-m64-parent'})
    return rows
