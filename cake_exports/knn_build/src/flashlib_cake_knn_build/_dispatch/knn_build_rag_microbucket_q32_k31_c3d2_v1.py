...
knn_build_rag_microbucket_q32_k31_c3d2_v1_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_k31_c3d2_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 97536, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 31], ["ROWS_COVERED", 32]], "cta_group": 1, "threads": 128}'))
stage1_q32_k31_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_k31_c3d2_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 97536, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 31], ["ROWS_COVERED", 32]], "cta_group": 1, "threads": 128}'))
...
def _warp_merge_ir(split_count: int) -> Any:
    return _ir_with_constants(q32exact.rows4.base.k32_warp_row_merge_ir, suffix=f'q32k31s{split_count}r{Q32_K31_ROWS_PER_MERGE_CTA}_c3d2_v1', TOP_K_MAX=Q32_K31_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=q32exact.rows4.base._splits_per_lane(split_count), ROWS_PER_CTA=Q32_K31_ROWS_PER_MERGE_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_k31_c3d2_v1_stage1_q32k31_c3d2_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 97536, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 31], ["ROWS_COVERED", 32]], "cta_group": 1, "threads": 128}'))
...
def _route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    return f'rag_microbucket_q32_k31_c3d2_v1_q{int(inputs.get('Q', -1))}_m{int(inputs.get('M', -1))}_k31_row16x256b2cw_s{split_count}_r{Q32_K31_ROWS_PER_MERGE_CTA}'
...
def _launch_q32_k31(inputs: dict[str, Any], *, split_count: int=Q32_K31_SPLIT_COUNT) -> None:
    ...
    if top_k != Q32_K31_TOP_K_MAX:
        raise ValueError(f'q32_k31 seed only supports K={Q32_K31_TOP_K_MAX}, got K={top_k}')
    ...
    _compiled_stage1_q32_k31().launch(grid=(stage1_grid, 1, 1), block=(Q32_K31_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
...
def benchmark_knn_build_rag_microbucket_q32_k31_c3d2_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, split_count: int=Q32_K31_SPLIT_COUNT, run_dispatch_baseline: bool=True) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': BENCHMARK_ENTRYPOINT, 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'candidate': f'Q32 exact two-compute-warp ROW_16x256B tcgen05/TMA producer, K31 capacity, split{split_count}', 'merge': f'four-row split-list merge with TOP_K_MAX={Q32_K31_TOP_K_MAX}', 'guard_misses': 'delegate to current v11 common-D dispatcher'}, 'route_trace': _annotate_route_trace(route_trace_for_contract_shapes(shape_labels, split_count=split_count), candidate_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report}
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, split_count: int=Q32_K31_SPLIT_COUNT, run_dispatch_baseline: bool=True) -> dict[str, str]:
    ...
    path = out_dir / f'q32_k31_c3d2_{len(tuple(shape_labels))}row_s{split_count}_{suffix}.json'
