...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_rag_stream_k32_q128_dualm_a162_v1'
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_stream_k32_q128_dualm_a162_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "cta_group": 1, "threads": 192}'))
...
def _route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    return f'rag_stream_k32_q128_dualm_a162_v1_q128_m{int(inputs.get('M', -1))}_k32_row16x256b_s{split_count}_r{K32_MERGE_ROWS_PER_CTA}_warpmerge'
...
def _select_contract_shapes(shape_labels) -> list[dict[str, Any]]:
    ...
    if len(selected) != len(wanted):
        ...
        raise KeyError(f'unknown knn_build contract shape labels: {sorted(wanted - found)}')
    return selected
