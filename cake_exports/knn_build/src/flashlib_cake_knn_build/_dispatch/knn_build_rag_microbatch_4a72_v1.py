...
knn_build_rag_microbatch_4a72_k10_fused_group_final_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbatch_4a72_k10_fused_group_final_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "constants": [["TOP_K_MAX", 10], ["GROUP_COUNT", 8], ["GROUP_SPLITS", 9]], "cta_group": 1, "threads": 32}'))
fused_merge_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbatch_4a72_k10_fused_group_final_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "constants": [["TOP_K_MAX", 10], ["GROUP_COUNT", 8], ["GROUP_SPLITS", 9]], "cta_group": 1, "threads": 32}'))
...
def _fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(fused_merge_ir, suffix=f's{split_count}g{group_count}_4a72_v1', GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbatch_4a72_k10_fused_group_final_merge_s72g8_4a72_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "constants": [["TOP_K_MAX", 10], ["GROUP_COUNT", 8], ["GROUP_SPLITS", 9]], "cta_group": 1, "threads": 32}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, split_count: int=K10_SPLIT_COUNT, group_count: int=K10_GROUP_COUNT) -> str:
    if _eligible_rag_microbatch(inputs):
        return f'rag_microbatch_4a72_k10_s{split_count}_g{group_count}_fusedmerge'
...
def benchmark_split_sweep(*, use_cupti: bool=True, split_counts=K10_CANDIDATE_SPLITS, group_count: int=K10_GROUP_COUNT) -> dict[str, Any]:
    ...
    for split_count in split_counts:
        rows[f's{split_count}_g{group_count}'] = benchmark_knn_build_rag_microbatch_4a72_v1(use_cupti=use_cupti, split_count=int(split_count), group_count=group_count)
