...
ROUTE_Q32_ROWLD2_ROWS4 = f'{MODULE}:launch_from_contract_inputs'
...
def _q32_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    return f'rag_microbucket_k32_f590_q32rowld2rows4_v1_q{int(inputs.get('Q', -1))}_m{int(inputs.get('M', -1))}_k32_row16x256b2cw_s{split_count}_r4_warpmerge'
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, str]:
    ...
    path = out_dir / f'f590_q32rowld2rows4_{len(tuple(shape_labels))}row_{suffix}.json'
