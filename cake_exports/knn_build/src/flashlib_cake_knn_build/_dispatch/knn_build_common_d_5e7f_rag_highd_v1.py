"""High-D common-D RAG microbatch seed for v11 round 5e7f.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes the exact BF16 non-build D1024 and D4096 v11 RAG microbatch rows through
the existing M64/N64 tcgen05/TMA producer with a D-parametric feature chunk
loop, followed by the existing fused split merge. Guard misses intentionally
fall back to the current v11 common-D dispatcher; this module does not edit
production dispatch.
"""
...
MODULE = 'loom.examples.weave.knn_build_common_d_5e7f_rag_highd_v1'
ROUTE_PREFIX = 'knn_build_common_d_5e7f_rag_highd_v1'
ROUTE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_common_d_5e7f_rag_highd_v1'
RAG_D1024 = 'rag_microbatch_common_d1024_b1_q8_m50000_k10'
RAG_D4096 = 'rag_microbatch_common_d4096_b1_q4_m32768_k10'
TARGET_SHAPES = (RAG_D1024, RAG_D4096)
...
DEFAULT_SPLIT_COUNT = _decode_capture(_json_loads('144'))
DEFAULT_GROUP_COUNT = _decode_capture(_json_loads('12'))
SHAPE_SPECS = _decode_capture(_json_loads('{"rag_microbatch_common_d1024_b1_q8_m50000_k10": {"B": 1, "D": 1024, "K": 10, "M": 50000, "Q": 8, "build": false, "feature_chunks": 8, "group_count": 12, "split_count": 144}, "rag_microbatch_common_d4096_b1_q4_m32768_k10": {"B": 1, "D": 4096, "K": 10, "M": 32768, "Q": 4, "build": false, "feature_chunks": 32, "group_count": 8, "split_count": 128}}'))
...
def _stage1_ir(feature_chunks: int) -> Any:
    return _ir_with_constants(m64_parent.stage1_m64_ir, suffix=f'd{int(feature_chunks) * K_TILE}_5e7f_highd_v1', FEATURE_CHUNKS=int(feature_chunks))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_COMMON_D_5E7F_RAG_HIGHD_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_COMMON_D_5E7F_RAG_HIGHD_VERIFY_SPLIT', DEFAULT_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_COMMON_D_5E7F_RAG_HIGHD_VERIFY_GROUPS', DEFAULT_GROUP_COUNT))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_5e7f_rag_highd_v1:ir"}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return f'{ROUTE_PREFIX}:{label}:d{int(spec['D'])}:q{int(spec['Q'])}:m{int(spec['M'])}:m64n64:s{_split_count_for_label(label)}:g{_group_count_for_label(label)}:chunks{_feature_chunks_for_label(label)}'

def _launch_highd_rag(inputs: dict[str, Any], label: str) -> None:
    query = inputs['query']
    database = inputs['database']
    if str(query.dtype) != 'torch.bfloat16' or str(database.dtype) != 'torch.bfloat16':
        raise ValueError('knn_build_common_d_5e7f_rag_highd_v1 supports bfloat16 inputs only')
    bsz = int(inputs['B'])
    ...
    if dim != tma_dim:
        raise ValueError(f'{label} expected D={tma_dim}, got {dim}')
    ...
    tmap_query = m64_parent.non128_base.base_v1._create_tensor_map_3d_oob_zero(query.data_ptr(), total_queries, M64_BLOCK_Q, tma_dim, K_TILE)
    tmap_database = m64_parent.non128_base.base_v1._create_tensor_map_3d_oob_zero(database.data_ptr(), bsz * n_database, M64_BLOCK_M, tma_dim, K_TILE)
    stage_ir = _stage1_ir(feature_chunks)
    _compiled_stage1(feature_chunks).launch(grid=(stage1_grid, 1, 1), block=(M64_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage_ir.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> None:
    ...
    if not force_fallback and label is not None:
        _launch_highd_rag(inputs, label)
        return
...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in _select_contract_shapes(shape_labels):
        ...
        label = _target_label_for_inputs(inputs)
        if force_fallback or label is None:
            rows.append({'shape_key': params['label'], 'selected_route': default_dispatcher.ROUTE_ENTRYPOINT, 'selected_entrypoint': default_dispatcher.ROUTE_ENTRYPOINT, 'selected_seed': None, 'expected_seed': 'common_d_5e7f_rag_highd_v1' if params['label'] in TARGET_SHAPE_SET else None, 'route_kind': 'forced_fallback' if force_fallback else 'delegated', 'route_source': 'default-v11-common-d-dispatcher', 'guard_id': 'forced_fallback' if force_fallback else 'guard_miss'})
            continue
        spec = SHAPE_SPECS[label]
        rows.append({'shape_key': label, 'selected_route': route_for_contract_inputs(inputs), 'selected_entrypoint': ROUTE_ENTRYPOINT, 'selected_seed': 'common_d_5e7f_rag_highd_v1', 'expected_seed': 'common_d_5e7f_rag_highd_v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': '5e7f_common_d_highd_rag_exact_guard', 'guard_condition': f'exact BF16 non-build B={spec['B']} Q={spec['Q']} M={spec['M']} D={spec['D']} K={spec['K']}', 'feature_chunks': spec['feature_chunks'], 'split_count': _split_count_for_label(label), 'group_count': _group_count_for_label(label), 'producer_topology': 'M64_N64_tcgen05_tma', 'merge_topology': 'fused_group_split_merge', 'classification': 'highd-rag-seed'})
    return rows

def benchmark_knn_build_common_d_5e7f_rag_highd_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
    report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels)
    return {'contract': report['contract'], 'contract_version': report['contract_version'], 'measured_entrypoint': BENCHMARK_ENTRYPOINT, 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {label: f'M64N64/S{_split_count_for_label(label)}/G{_group_count_for_label(label)}/chunks{_feature_chunks_for_label(label)}' for label in TARGET_SHAPES}, 'route_trace': route_trace_for_contract_shapes(shape_labels), 'summary': report['summary'], 'performance': report['performance'], 'correctness': report['correctness'], 'report': report}
