...
knn_build_ragonline_mbucket_aa88_q1m_s72_k10_coop_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_ragonline_mbucket_aa88_q1m_s72_k10_coop_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 512, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 72], ["MERGE_GROUPS", 4], ["SPLITS_PER_GROUP", 18]], "cta_group": 1, "threads": 128}'))

def _ir_with_constants(ir_obj: Any, *, suffix: str, **updates: int) -> Any:
    constants = tuple(((name, updates.get(name, value)) for name, value in ir_obj.constants))
    return replace(ir_obj, name=f'{ir_obj.name}_{suffix}', constants=constants)
coop_merge_s72_k10_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_ragonline_mbucket_aa88_q1m_s72_k10_coop_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 512, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 72], ["MERGE_GROUPS", 4], ["SPLITS_PER_GROUP", 18]], "cta_group": 1, "threads": 128}'))
coop_merge_s74_k10_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_ragonline_mbucket_aa88_q1m_s72_k10_coop_merge_s74_m250", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 512, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 74], ["MERGE_GROUPS", 4], ["SPLITS_PER_GROUP", 19]], "cta_group": 1, "threads": 128}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible_rag_online_mbucket(inputs):
        ...
        return f'rag_online_mbucket_aa88_q1m_{suffix}_coopmerge'
