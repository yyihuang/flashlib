...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    candidate_path = out_dir / ''.join([format(denom, ''), '_dispatch_4247_non128_8199_widecombine_full82_v1.json'])
    baseline_path = out_dir / ''.join([format(denom, ''), '_same_session_baseline_splitretune_for_non128_8199_widecombine_v1.json'])
    route_trace_path = out_dir / ''.join([format(denom, ''), '_route_trace_4247_non128_8199_widecombine_full82_v1.json'])
    forced_trace_path = out_dir / ''.join([format(denom, ''), '_forced_fallback_trace_4247_non128_8199_widecombine_full82_v1.json'])
    seed_matrix_path = out_dir / ''.join([format(denom, ''), '_seed_delta_matrix_4247_non128_8199_widecombine_full82_v1.json'])
