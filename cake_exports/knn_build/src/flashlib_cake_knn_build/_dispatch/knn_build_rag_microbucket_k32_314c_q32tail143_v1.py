"""Exact Q32/M100001 RAG K32 split143 tail seed.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
targets only the expanded BF16 non-build ``B=1,Q=32,M=100001,D=128,K=32`` row
from generalize-auto-tuning round 180. It keeps the accepted f590 exact-Q32
ROW_16x256B tcgen05/TMA producer and rows4 merge, but lowers the high-tail row
with split143 to reduce padded M-tile work. Guard misses delegate to the
accepted 0cb5 q31tail/q32tail seed.
"""
...
MODULE = 'loom.examples.weave.knn_build_rag_microbucket_k32_314c_q32tail143_v1'
EXPANDED_Q32_TAIL_HIGH_SHAPE = parent.EXPANDED_Q32_TAIL_HIGH_SHAPE
TARGET_SHAPES = (EXPANDED_Q32_TAIL_HIGH_SHAPE,)
EXPANDED_SHAPES = (parent.EXPANDED_SHAPES_BY_LABEL[EXPANDED_Q32_TAIL_HIGH_SHAPE],)
EXPANDED_SHAPES_BY_LABEL = _decode_capture(_json_loads('{"expanded_tail_q32_m100001_d128_k32": {"label": "expanded_tail_q32_m100001_d128_k32", "params": {"B": 1, "D": 128, "K": 32, "M": 100001, "Q": 32, "benchmark": true, "build": false, "check_correctness": true, "correctness_query_sample": 32, "dtype": "bfloat16", "recall_min": 0.999, "seed": 627001, "time_flashlib": true}}}'))
K32_Q32TAIL143_SPLIT_COUNT = 143
ROUTE_Q32TAIL143_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
ROUTE_PARENT_0CB5_ENTRYPOINT = f'{parent.MODULE}:launch_from_contract_inputs'
ROUTE_DISPATCH_V11_ENTRYPOINT = f'{dispatch_v11.MODULE}:launch_from_contract_inputs'
SEED_K32_314C_Q32TAIL143_ID = 'rag_microbucket_k32_314c_q32tail143_v1'

def _verify_export_ir() -> Any:
    import os
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_K32_314C_Q32TAIL143_V1_VERIFY_KERNEL')
    if verify_kernel == 'q32tail143_merge':
        return q32exact.rows4._warp_merge_ir(K32_Q32TAIL143_SPLIT_COUNT)
    return q32exact._stage1_q32_exact_ir()
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32_314c_q32tail143_v1:ir"}'))

def _eligible_q32tail143(inputs: dict[str, Any]) -> bool:
    return parent.uneven.base._is_bf16_d128_nonbuild(inputs) and int(inputs.get('Q', -1)) == 32 and (int(inputs.get('M', -1)) == 100001) and (int(inputs.get('K', -1)) == q32exact.K32_TOP_K_MAX)

def _q32tail143_route_name(inputs: dict[str, Any]) -> str:
    return f'rag_microbucket_k32_314c_q32tail143_v1_q{int(inputs.get('Q', -1))}_m{int(inputs.get('M', -1))}_k32_row16x256b2cw_exact_s{K32_Q32TAIL143_SPLIT_COUNT}_r{q32exact.rows4.K32_ROWS4_ROWS_PER_CTA}_warpmerge'

def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    if _eligible_q32tail143(inputs):
        return _q32tail143_route_name(inputs)
    return parent.route_for_contract_inputs(inputs)

def launch_from_contract_inputs(inputs: dict[str, Any]) -> None:
    if _eligible_q32tail143(inputs):
        q32exact._launch_q32_exact_rows4(inputs, split_count=K32_Q32TAIL143_SPLIT_COUNT)
        return
...
def benchmark_knn_build_rag_microbucket_k32_314c_q32tail143_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
...
def write_benchmark_artifact(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, str]:
    ...
    payload = benchmark_knn_build_rag_microbucket_k32_314c_q32tail143_v1(use_cupti=use_cupti, shape_labels=shape_labels)
    path = out_dir / f'314c_q32tail143_{len(tuple(shape_labels))}row_{suffix}.json'
