...
ROUTE_Q32TAIL143_ENTRYPOINT = ''.join([format(MODULE, ''), ':launch_from_contract_inputs'])
...
def _q32tail143_route_name(inputs: dict[str, Any]) -> str:
    return ''.join(['rag_microbucket_k32_314c_q32tail143_v1_q', format(int(inputs.get('Q', -1)), ''), '_m', format(int(inputs.get('M', -1)), ''), '_k32_row16x256b2cw_exact_s', format(K32_Q32TAIL143_SPLIT_COUNT, ''), '_r', format(q32exact.rows4.K32_ROWS4_ROWS_PER_CTA, ''), '_warpmerge'])
...
def write_benchmark_artifact(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, str]:
    ...
    path = out_dir / ''.join(['314c_q32tail143_', format(len(tuple(shape_labels)), ''), 'row_', format(suffix, ''), '.json'])
