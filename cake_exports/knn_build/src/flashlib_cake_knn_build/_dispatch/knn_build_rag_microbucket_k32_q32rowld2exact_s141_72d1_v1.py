...
knn_build_rag_microbucket_k32_q32rowld2exact_s141_72d1_v1_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32_q32rowld2exact_s141_72d1_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 66816, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32], ["ROWS_COVERED", 32], ["SPLIT_COUNT_CONST", 141], ["NUM_DB_TILES_CONST", 1563], ["TILES_FLOOR_CONST", 11], ["EXTRA_SPLITS_CONST", 12], ["DB_TILES_PER_SPLIT_CONST", 12], ["M_LIMIT", 100000]], "cta_group": 1, "threads": 128}'))
...
def _warp_merge_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=f'k32q32exact_s{split_count}r{K32_ROWS4_ROWS_PER_CTA}_f653_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge_k32q32exact_s141r4_f653_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 141], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))
...
def _q32_rowld2exact_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return f'rag_microbucket_k32_q32rowld2exact_s141_72d1_v1_q{n_query}_m{n_database}_k32_row16x256b2warp_exact_s{split_count}_r{K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def _launch_q32_rowld2exact_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(f'k32 q32 rowld2exact only supports K={K32_TOP_K_MAX}, got K={top_k}')
    if split_count != K32_Q32_SPLIT_COUNT:
        raise ValueError(f'q32 rowld2exact stage1 is compile-time specialized for split_count={K32_Q32_SPLIT_COUNT}, got runtime split_count={split_count}')
...
def benchmark_knn_build_rag_microbucket_k32_q32rowld2exact_s141_72d1_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> dict[str, Any]:
