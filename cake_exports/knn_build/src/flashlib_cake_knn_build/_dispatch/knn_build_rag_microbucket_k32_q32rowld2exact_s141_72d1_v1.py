...
def _ir_with_constants(ir_obj: Any, *, suffix: str, **updates: int) -> Any:
    ...
    return replace(ir_obj, symbol=''.join([format(ir_obj.symbol, ''), '_', format(suffix, '')]), constants=constants)
...
def _q32_rowld2exact_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return ''.join(['rag_microbucket_k32_q32rowld2exact_s141_72d1_v1_q', format(n_query, ''), '_m', format(n_database, ''), '_k32_row16x256b2warp_exact_s', format(split_count, ''), '_r', format(K32_ROWS4_ROWS_PER_CTA, ''), '_warpmerge'])
