"""RAG microbucket Q8/K32 M64 producer plus inherited v7 routes.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only ``rag_microbatch_largek_b1_q8_m100000_d128_k32`` through a
smaller-row M64/N64 tcgen05/TMA producer and the existing K32 fused split
merge. All other target rows inherit the validated v7 microbucket routes, and
guard misses delegate to the current 4247 dispatcher.
"""
...
Q8_M64_STAGE1_THREADS = _decode_capture(_json_loads('96'))
Q8_M64_BLOCK_Q = 64
Q8_M64_BLOCK_M = 64
Q8_M64_FEAT_D = 128
Q8_M64_TOP_K_MAX = 32
...
knn_build_rag_microbucket_3505_v9_stage1_q8_k32_m64 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_3505_v9_stage1_q8_k32_m64", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 34048, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 96}'))
stage1_q8_k32_m64_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_3505_v9_stage1_q8_k32_m64", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 34048, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 96}'))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V9_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V9_VERIFY_K32_SPLIT', K32_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V9_VERIFY_K32_GROUPS', K32_GROUP_COUNT))
    ...
    if verify_kernel == 'k32_q8_m64_stage1':
    ...
    return stage1_q8_k32_m64_ir
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_3505_v9_stage1_q8_k32_m64", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 34048, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 96}'))
...
def _q8_k32_m64_route_name(*, split_count: int, group_count: int) -> str:
    return f'rag_microbucket_3505_v9_q8_m100000_k32_m64n64_s{split_count}_g{group_count}'

def _compact_route_name(*, split_count: int, group_count: int, inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_3505_v9_inherit_v7_q{n_query}_m{n_database}_k32_tailinf_cta1_cw1_s{split_count}_g{group_count}'
...
def _launch_q8_k32_m64(inputs: dict[str, Any], *, split_count: int=K32_SPLIT_COUNT, group_count: int=K32_GROUP_COUNT) -> None:
    ...
    _compiled_stage1_q8_k32_m64().launch(grid=(stage1_grid, 1, 1), block=(Q8_M64_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_q8_k32_m64_ir.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> None:
    if _eligible_q8_k32_m64(inputs):
        _launch_q8_k32_m64(inputs, split_count=k32_split_count, group_count=k32_group_count)
        return
    if _eligible_q4_k10(inputs):
...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        specialized = str(route).startswith('rag_microbucket_3505_v9')
        rows.append({'shape_key': shape['label'], 'selected_route': route, 'route_kind': 'specialized' if specialized else 'general', 'guard_condition': 'exact BF16 non-build B1 D128 Q<=64 K10, Q8 M64/N64 K32, or inherited v7 K32 microbucket' if specialized else 'guard miss to 4247 dispatcher', 'fallback': ROUTE_BASE_4247})
    return rows
...
def benchmark_knn_build_rag_microbucket_3505_v9(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_3505_v9:benchmark_knn_build_rag_microbucket_3505_v9', 'candidate_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_3505_v9:launch_from_contract_inputs', 'prior_v7_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_3505_v7:launch_from_contract_inputs', 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'K10': f'inherited-v7/M64/S{faeb.M64_SPLIT_COUNT}/G{faeb.M64_GROUP_COUNT}', 'K32': f'Q8-M64N64/S{k32_split_count}/G{k32_group_count}/fused; other K32 inherited-v7'}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, prior_v7_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'prior_v7_summary': prior_v7_report['summary'], 'prior_v7_performance': prior_v7_report['performance'], 'prior_v7_report': prior_v7_report}
