...
def _q8_k32_m64_route_name(*, split_count: int, group_count: int) -> str:
    return ''.join(['rag_microbucket_3505_v9_q8_m100000_k32_m64n64_s', format(split_count, ''), '_g', format(group_count, '')])

def _compact_route_name(*, split_count: int, group_count: int, inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return ''.join(['rag_microbucket_3505_v9_inherit_v7_q', format(n_query, ''), '_m', format(n_database, ''), '_k32_tailinf_cta1_cw1_s', format(split_count, ''), '_g', format(group_count, '')])

def _m64_route_name(inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    return ''.join(['rag_microbucket_3505_v9_inherit_v7_q', format(n_query, ''), '_k10_m64_s', format(faeb.M64_SPLIT_COUNT, ''), '_g', format(faeb.M64_GROUP_COUNT, '')])
