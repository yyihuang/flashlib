...
ROUTE_Q16_DUAL_2WARP_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
def _warp_merge_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=f'k32s{split_count}r{K32_ROWS4_ROWS_PER_CTA}_56ed_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32_q16dual2warp_56ed_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 128}'))
...
def _dual2warp_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return f'rag_microbucket_k32_q16dual2warp_56ed_v1_q16_m{n_database}_k32_row16x256b_2cw_s{split_count}_r{K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def _launch_rowld1_2warp_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(f'k32 q16 dual2warp only supports K={K32_TOP_K_MAX}, got K={top_k}')
