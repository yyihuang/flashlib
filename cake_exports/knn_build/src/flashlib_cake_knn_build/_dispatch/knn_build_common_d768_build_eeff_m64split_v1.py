...
ROUTE_PREFIX = f'{MODULE}:d768_build'
ROUTE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_common_d768_build_eeff_m64split_v1'
...
knn_build_common_d768_build_eeff_m64split_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_common_d768_build_eeff_m64split_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["FEATURE_CHUNKS", 6]], "cta_group": 1, "threads": 192}'))
stage1_m64_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_common_d768_build_eeff_m64split_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["FEATURE_CHUNKS", 6]], "cta_group": 1, "threads": 192}'))

def _check_split_count(split_count: int) -> int:
    split_count = int(split_count)
    if split_count not in SPLIT_CHOICES:
        raise ValueError(f'unsupported D768 build split count: {split_count}; choices={SPLIT_CHOICES}')
    return split_count
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_common_d768_build_eeff_m64split_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["FEATURE_CHUNKS", 6]], "cta_group": 1, "threads": 192}'))
...
def _launch_d768_build(inputs: dict[str, Any], *, split_count: int=DEFAULT_SPLIT_COUNT, group_count: int | None=None) -> None:
    ...
    _compiled_stage1_m64().launch(grid=(stage1_grid, 1, 1), block=(STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_m64_ir.computed_smem_bytes)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, split_count: int=DEFAULT_SPLIT_COUNT, group_count: int | None=None) -> str:
    if _eligible_d768_build(inputs):
        split_count = _check_split_count(split_count)
        group_count = _group_count_for_split(split_count, group_count)
        return f'{ROUTE_PREFIX}:s{split_count}:g{group_count}:m64n64'
