...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_rag_stream_k32_q128m100000_staticn128_664a_v1'
...
TOP_K_MAX = S128_K
knn_build_rag_stream_k32_q128m100000_staticn128_664a_stage1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_stream_k32_q128m100000_staticn128_664a_v1:knn_build_rag_stream_k32_q128m100000_staticn128_664a_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 181504, "cta_group": 1, "threads": 256}'))
stage1_staticn128_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_stream_k32_q128m100000_staticn128_664a_v1:stage1_staticn128_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 181504, "cta_group": 1, "threads": 256}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_stream_k32_q128m100000_staticn128_664a_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 181504, "cta_group": 1, "threads": 256}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return f'{ROUTE_PREFIX}:{label}:d128:q128:m100000:n128:s{_split_count()}:g{_group_count()}'
...
def _launch_staticn128(inputs: dict[str, Any]) -> None:
    ...
    top_k = int(inputs['K'])
    if dim != S128_D:
        raise ValueError(f'{TARGET_SHAPE} expected D={S128_D}, got {dim}')
...
def benchmark_knn_build_rag_stream_k32_q128m100000_staticn128_664a_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
    ...
    return {'contract': report['contract'], 'contract_version': report['contract_version'], 'measured_entrypoint': BENCHMARK_ENTRYPOINT, 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': f'D128Q64M128/S{_split_count()}/G{_group_count()}', 'route_trace': route_trace_for_contract_shapes(shape_labels), 'summary': report['summary'], 'performance': report['performance'], 'correctness': report['correctness'], 'report': report}
