...
BENCHMARK_ENTRYPOINT = ''.join([format(MODULE, ''), ':benchmark_knn_build_v12_d256_q128_k10_longm_59fe_v1'])
...
def _stage1_ir() -> Any:
    ...
    return dc.replace(q128_parent.stage1_m64_ir, symbol=''.join([format(q128_parent.stage1_m64_ir.symbol, ''), '_d256_q128_k10_59fe_v1']), constants=constants)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if not force_fallback and label is not None:
        ...
        return ''.join(['v12_d256_q128_k10_59fe:', format(label, ''), ':q', format(int(spec['Q']), ''), ':m', format(int(spec['M']), ''), ':d256:m128n64:s', format(_split_count_for_label(label), ''), ':g', format(_group_count_for_label(label), '')])
...
def _launch_q128_d256_k10_longm(inputs: dict[str, Any], label: str) -> None:
    ...
    if top_k != q128_parent.TOP_K_MAX:
        raise ValueError(''.join([format(label, ''), ' expected K=', format(q128_parent.TOP_K_MAX, ''), ', got ', format(top_k, '')]))
