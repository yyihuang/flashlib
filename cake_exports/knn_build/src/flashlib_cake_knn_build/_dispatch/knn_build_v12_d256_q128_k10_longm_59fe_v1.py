"""v12 D256 Q128 long-M K10 RAG seed for kNN build/search.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only the BF16 non-build
``rag_stream_common_d256_b1_q128_m100000_k10`` row through a 128-row
M64/N64 tcgen05/TMA producer adapted from the source-policy-clean D768 build
seed, followed by the inherited fused split merge. The existing Q4 D256/K10
seed remains delegated to e2df; all other guard misses delegate through that
same Weave-only parent path.
"""
...
from . import knn_build_v12_d256_k10_longm_e2df_v1 as q4_parent
MODULE = 'loom.examples.weave.knn_build_v12_d256_q128_k10_longm_59fe_v1'
ROUTE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_v12_d256_q128_k10_longm_59fe_v1'
CANDIDATE_ID = 'knn_build_v12_d256_q128_k10_longm_59fe_v1'
RAG_MICRO_D256_Q4_M100 = q4_parent.RAG_MICRO_D256_Q4_M100
RAG_STREAM_D256_Q128_M100 = 'rag_stream_common_d256_b1_q128_m100000_k10'
TARGET_SHAPES = (RAG_STREAM_D256_Q128_M100,)
INHERITED_SHAPES = (RAG_MICRO_D256_Q4_M100,)
BUCKET_SHAPES = (*INHERITED_SHAPES, *TARGET_SHAPES)
TARGET_SHAPE_SET = set(TARGET_SHAPES)
D256_FEATURE_CHUNKS = 2
D256_TMA_DIM = D256_FEATURE_CHUNKS * q128_parent.K_TILE
SPLIT_COUNT = _decode_capture(_json_loads('144'))
GROUP_COUNT = _decode_capture(_json_loads('12'))
SHAPE_SPECS: dict[str, dict[str, Any]] = {RAG_STREAM_D256_Q128_M100: {'B': 1, 'Q': 128, 'M': 100000, 'D': 256, 'K': 10, 'build': False, 'feature_chunks': D256_FEATURE_CHUNKS, 'split_count': SPLIT_COUNT, 'group_count': GROUP_COUNT}}

def _stage1_ir() -> Any:
    constants = tuple(((name, D256_FEATURE_CHUNKS if name == 'FEATURE_CHUNKS' else value) for name, value in q128_parent.stage1_m64_ir.constants))
    return replace(q128_parent.stage1_m64_ir, name=f'{q128_parent.stage1_m64_ir.name}_d256_q128_k10_59fe_v1', constants=constants)

def _merge_ir(split_count: int=SPLIT_COUNT, group_count: int=GROUP_COUNT) -> Any:
    fused_merge_parent._validate_group_shape(int(split_count), int(group_count))
    return fused_merge_parent._fused_merge_ir(int(split_count), int(group_count))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_V12_D256_Q128_K10_59FE_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_V12_D256_Q128_K10_59FE_VERIFY_SPLIT', SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_V12_D256_Q128_K10_59FE_VERIFY_GROUPS', GROUP_COUNT))
    if verify_kernel == 'merge':
        return _merge_ir(split_count, group_count)
    return _stage1_ir()
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_common_d768_build_eeff_m64split_stage1_d256_q128_k10_59fe_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["FEATURE_CHUNKS", 2]], "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if not force_fallback and label is not None:
        ...
        return f'v12_d256_q128_k10_59fe:{label}:q{int(spec['Q'])}:m{int(spec['M'])}:d256:m128n64:s{_split_count_for_label(label)}:g{_group_count_for_label(label)}'
    return q4_parent.route_for_contract_inputs(inputs, force_fallback=force_fallback)

def _launch_q128_d256_k10_longm(inputs: dict[str, Any], label: str) -> None:
    query = inputs['query']
    database = inputs['database']
    if str(query.dtype) != 'torch.bfloat16' or str(database.dtype) != 'torch.bfloat16':
        raise ValueError('knn_build_v12_d256_q128_k10_longm_59fe_v1 supports bfloat16 inputs only')
    bsz = int(inputs['B'])
    ...
    if top_k != q128_parent.TOP_K_MAX:
        raise ValueError(f'{label} expected K={q128_parent.TOP_K_MAX}, got {top_k}')
    fused_merge_parent._validate_group_shape(split_count, group_count)
    num_q_tiles = (n_query + q128_parent.M64_BLOCK_Q - 1) // q128_parent.M64_BLOCK_Q
    num_db_tiles = (n_database + q128_parent.M64_BLOCK_M - 1) // q128_parent.M64_BLOCK_M
    db_tiles_per_split = (num_db_tiles + split_count - 1) // split_count
    total_work = bsz * num_q_tiles * split_count
    total_queries = bsz * n_query
    stage1_grid = min(total_work, q128_parent.GRID_DIM_DEFAULT)
    merge_grid = min(total_queries, q128_parent.GRID_DIM_DEFAULT)
    ...
    tmap_query = m64_parent.non128_base.base_v1._create_tensor_map_3d_oob_zero(query.data_ptr(), total_queries, q128_parent.M64_BLOCK_Q, D256_TMA_DIM, q128_parent.K_TILE)
    tmap_database = m64_parent.non128_base.base_v1._create_tensor_map_3d_oob_zero(database.data_ptr(), bsz * n_database, q128_parent.M64_BLOCK_M, D256_TMA_DIM, q128_parent.K_TILE)
    stage_ir = _stage1_ir()
    _compiled_stage1().launch(grid=(stage1_grid, 1, 1), block=(q128_parent.STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage_ir.computed_smem_bytes)
    merge_ir = _merge_ir(split_count, group_count)
    _compiled_fused_merge(split_count, group_count).launch(grid=(merge_grid, 1, 1), block=(fused_merge_parent.D768_FUSED_MERGE_THREADS, 1, 1), args=[partial_dists, partial_indices, inputs['out_dists'], inputs['out_indices'], total_queries], shared_mem=merge_ir.computed_smem_bytes)

def launch_from_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> None:
    label = _target_label_for_inputs(inputs)
    if not force_fallback and label is not None:
        _launch_q128_d256_k10_longm(inputs, label)
        return
    q4_parent.launch_from_contract_inputs(inputs, force_fallback=force_fallback)
...
def benchmark_knn_build_v12_d256_q128_k10_longm_59fe_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
