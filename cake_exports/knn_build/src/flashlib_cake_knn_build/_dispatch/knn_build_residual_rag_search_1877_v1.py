...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for label in tuple(shape_labels):
        ...
        rows.append(fallback_full90._normalize_route_row({'shape_key': label, 'selected_route': route, 'selected_entrypoint': selected_entrypoints[selected_seed], 'selected_seed': selected_seed, 'expected_seed': selected_seed, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'residual_rag_search_1877_{selected_seed}', 'guard_condition': guard_conditions[selected_seed], 'matched_label': matched_label, 'fallback_full90_route': fallback_route, 'baseline_dispatcher_route': fallback_row.get('selected_route'), 'classification': 'seed-consumed'}))
    return rows
