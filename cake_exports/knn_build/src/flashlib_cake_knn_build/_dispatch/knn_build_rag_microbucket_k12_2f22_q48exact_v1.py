...
knn_build_rag_microbucket_k12_2f22_q48exact_v1_stage1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k12_2f22_q48exact_v1:knn_build_rag_microbucket_k12_2f22_q48exact_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 58624, "cta_group": 1, "threads": 192}'))
stage1_q48_k12_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k12_2f22_q48exact_v1:stage1_q48_k12_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 58624, "cta_group": 1, "threads": 192}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k12_2f22_q48exact_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 58624, "cta_group": 1, "threads": 192}'))
...
def _route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    return f'rag_microbucket_k12_2f22_q48exact_v1_q{int(inputs.get('Q', -1))}_m{int(inputs.get('M', -1))}_k12_m64n64_row16x256b_s{split_count}_r{Q48_K12_ROWS_PER_MERGE_CTA}'
...
def _launch_q48_k12(inputs: dict[str, Any], *, split_count: int=Q48_K12_SPLIT_COUNT) -> None:
    ...
    _compiled_stage1_q48_k12().launch(grid=(stage1_grid, 1, 1), block=(Q48_K12_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_q48_k12_ir.computed_smem_bytes)
...
def benchmark_knn_build_rag_microbucket_k12_2f22_q48exact_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, split_count: int=Q48_K12_SPLIT_COUNT, run_v9_probe: bool=True, run_dispatch_baseline: bool=True) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': BENCHMARK_ENTRYPOINT, 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'candidate': f'64-row M64/N64 ROW_16x256B tcgen05/TMA producer specialized to K12, split{split_count}', 'merge': f'four-row split-list merge, rows_per_cta={Q48_K12_ROWS_PER_MERGE_CTA}', 'guard_misses': 'delegate to current v11 common-D dispatcher'}, 'route_trace': _annotate_route_trace(route_trace_for_contract_shapes(shape_labels, split_count=split_count), candidate_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report}
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, split_count: int=Q48_K12_SPLIT_COUNT, run_v9_probe: bool=True, run_dispatch_baseline: bool=True) -> dict[str, str]:
    ...
    path = out_dir / f'2f22_q48exact_{len(tuple(shape_labels))}row_s{split_count}_{suffix}.json'
