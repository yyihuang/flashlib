...
ROUTE_PARENT_Q16IRREG = f'{parent_q16irreg.MODULE}:launch_from_contract_inputs'
ROUTE_BUCKET_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32bucket_0077_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "cta_group": 1, "threads": 192}'))
...
def _q16_exact_rowld_route_name(*, split_count: int, group_count: int) -> str:
    return f'rag_microbucket_k32bucket_0077_v1_q16_m100000_k32_row16x256b_s{split_count}_g{group_count}'
...
def benchmark_knn_build_rag_microbucket_k32bucket_0077_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_k32bucket_0077_v1', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'parent_entrypoint': ROUTE_PARENT_Q16IRREG, 'accelerated_shape_labels': list(Q16_EXACT_ROWLD_TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q16_K32_exact': f'ROW_16x256B/M64N64/S{k32_split_count}/G{k32_group_count}/fused', 'inherited': 'q16irreg 2691 for Q8 ROW_16x256B, Q32 ROW_16x256B, Q16 irregular M64, and fallback routes'}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
