"""Exact RAG microbatch K10 M64 seed for d555 residual blockers.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only the exact BF16 contract rows
``rag_microbatch_b1_q4_m100000_d128_k10`` and
``rag_microbatch_b1_q64_m100000_d128_k10`` through the existing 3505/FAEB
M64 K10 tcgen05 stage-1 producer and S128/G8 fused merge. Guard misses
delegate to the d555 full82 Weave dispatcher; no external runtime fallback is
used.
"""
...
from . import knn_build_rag_microbucket_3505_v2 as seed_3505
MODULE = 'loom.examples.weave.knn_build_rag_microbatch_k10_q4q64_d555_v1'
Q4_SHAPE = 'rag_microbatch_b1_q4_m100000_d128_k10'
Q64_SHAPE = 'rag_microbatch_b1_q64_m100000_d128_k10'
TARGET_SHAPES = (Q4_SHAPE, Q64_SHAPE)
TARGET_SHAPE_SET = set(TARGET_SHAPES)
SEED_ID = 'rag_microbatch_k10_q4q64_m64_3505_d555_v1'
ROUTE_NAME = f'{MODULE}:m64_s128_g8_fused_merge'
...
SOURCE_TASKS = {SEED_ID: 'generalize-auto-tuning-knn-build-d555 / design_doc/active/generalize_auto_tuning_knn_build_round_115_d555.md', 'm64_3505_parent_seed': 'weave-evolve-knn-build-3505 / design_doc/active/weave_evolve_knn_build_round_33_3505.md'}
eval_mod = base_d555.eval_mod

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_D555_RAGMICRO_Q4Q64_VERIFY_KERNEL')
...
def _eligible_rag_microbatch_k10_q4q64(inputs: dict[str, Any]) -> bool:
    label = inputs.get('label')
    return not bool(inputs.get('build', False)) and _dtype_name(inputs, 'query') == 'bfloat16' and (_dtype_name(inputs, 'database') == 'bfloat16') and (int(inputs.get('B', -1)) == 1) and (int(inputs.get('Q', -1)) in {4, 64}) and (int(inputs.get('M', -1)) == 100000) and (int(inputs.get('D', -1)) == 128) and (int(inputs.get('K', -1)) == 10) and (label is None or str(label) in TARGET_SHAPE_SET)

def _select_contract_shapes(shape_labels):
    return base_d555._select_contract_shapes(shape_labels)

def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible_rag_microbatch_k10_q4q64(inputs):
        return seed_3505.route_for_contract_inputs(inputs)
    return base_d555.route_for_contract_inputs(inputs, force_fallback=force_fallback)

def launch_from_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> None:
    if not force_fallback and _eligible_rag_microbatch_k10_q4q64(inputs):
        seed_3505.launch_from_contract_inputs(inputs)
        return
    base_d555.launch_from_contract_inputs(inputs, force_fallback=force_fallback)

def candidate_rag_microbatch_k10_q4q64_m64_3505_d555_v1(inputs: dict[str, Any]) -> None:
...
def _trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if force_fallback and _eligible_rag_microbatch_k10_q4q64(inputs):
        row['expected_seed'] = SEED_ID
        row['guard_id'] = 'forced_fallback_d555_ragmicro_q4q64_disabled'
        row['guard_condition'] = 'forced fallback to d555; Q4/Q64 M64 microbatch seed disabled'
...
def benchmark_candidate_rag_microbatch_k10_q4q64_m64_3505_d555_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, baseline_report: dict[str, Any] | None=None, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, Any]:
    ...
    candidate_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_rag_microbatch_k10_q4q64_m64_3505_d555_v1, correctness=benchmark_correctness, time_flashlib=time_flashlib)
