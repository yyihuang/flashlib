...
SPLIT_COUNT = 8
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_d192_tile_search_b10e_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible(inputs):
        return f'{MODULE}:exact_d192_wide256_s{SPLIT_COUNT}'
...
def _select_contract_shapes(shape_labels=TARGET_SHAPES):
    ...
    if wanted - {str(shape['label']) for shape in selected}:
        raise ValueError(f'unknown contract shapes: {sorted(wanted)}')
    return selected
