...
ROUTE_PARENT_Q8HALF = f'{q8half.MODULE}:launch_from_contract_inputs'
ROUTE_B3E0_MIX_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
def _mix_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    if n_query == 8:
        return f'rag_microbucket_k32_b3e0_mix_v1_q8_m{n_database}_k32_halfrow_s{split_count}_warpmerge'
    if n_query == 16:
        return f'rag_microbucket_k32_b3e0_mix_v1_q16_m{n_database}_k32_rowld1_s{split_count}_warpmerge'
    return f'rag_microbucket_k32_b3e0_mix_v1_q{n_query}_m{n_database}_k32_rows4_s{split_count}_warpmerge'
...
def benchmark_knn_build_rag_microbucket_k32_b3e0_mix_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_k32_b3e0_mix_v1', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'parent_entrypoint': ROUTE_PARENT_Q8HALF, 'accelerated_shape_labels': [Q8_K32_SHAPE, Q16_K32_SHAPE, Q32_K32_SHAPE], 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q8_exact': '0077 half-row ROW_16x256B one-compute-warp stage1', 'Q16_exact': '0077 rowld1 ROW_16x256B one-compute-warp stage1', 'Q32_exact': '0077 ROW_16x256B stage1 plus rows4 warp-row merge', 'Q16_irregular': 'inherited 0077 rowld1 one-compute-warp stage1', 'guard_misses': 'delegate to 0077 q8half/rowld1 stack'}, 'merge_topology': {'Q8_Q16_exact_Q16_irregular': '0077 warp-row split-list merge/1 row per CTA', 'Q32_exact': f'0077 warp-row split-list merge/{rows4.K32_ROWS4_ROWS_PER_CTA} rows per CTA', 'split_count': k32_split_count, 'splits_per_lane': rows4.base._splits_per_lane(k32_split_count)}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
