"""RAG microbatch K32 Q32 rowld2 uneven-split exact bucket wrapper.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
targets the exact v11 `rag_microbatch_largek_b1_q32_m100000_d128_k32` floor
row. It keeps the f653 rowld2 tcgen05/TMA producer and rows4 merge, but changes
stage1 split ownership so each split processes either floor(M_tiles/splits) or
one extra database tile. The final merge still sees one partial list per split.
Guard misses delegate to the current f653 rowld2 parent route. The production
...
MODULE = 'loom.examples.weave.knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1'
Q32_K32_SHAPE = parent.Q32_K32_SHAPE
Q32_ROWLD2UNEVEN_TARGET_SHAPES = (Q32_K32_SHAPE,)
K32_BUCKET_SHAPES = Q32_ROWLD2UNEVEN_TARGET_SHAPES
TARGET_SHAPES = Q32_ROWLD2UNEVEN_TARGET_SHAPES
...
base = parent.base
Q32_ROWLD2UNEVEN_STAGE1_THREADS = _decode_capture(_json_loads('128'))
Q32_ROWLD2UNEVEN_ACTIVE_ROWS = 32
Q32_ROWLD2UNEVEN_LOCAL_LISTS_PER_ROW = rowld2_seed.Q16_ROWLD1_LOCAL_LISTS_PER_ROW
Q32_ROWLD2UNEVEN_SMEM_BASE_BYTES = rowld2_seed.Q16_ROWLD1_SMEM_BASE_BYTES
Q32_ROWLD2UNEVEN_LOCAL_ELEMS = Q32_ROWLD2UNEVEN_ACTIVE_ROWS * Q32_ROWLD2UNEVEN_LOCAL_LISTS_PER_ROW * K32_TOP_K_MAX
Q32_ROWLD2UNEVEN_LOCAL_D_OFFSET = Q32_ROWLD2UNEVEN_SMEM_BASE_BYTES
Q32_ROWLD2UNEVEN_LOCAL_I_OFFSET = Q32_ROWLD2UNEVEN_LOCAL_D_OFFSET + Q32_ROWLD2UNEVEN_LOCAL_ELEMS * 4
Q32_ROWLD2UNEVEN_SMEM_POOL_BYTES = Q32_ROWLD2UNEVEN_LOCAL_I_OFFSET + Q32_ROWLD2UNEVEN_LOCAL_ELEMS * 4
ROUTE_PARENT_F653 = f'{parent.MODULE}:launch_from_contract_inputs'
ROUTE_Q32_ROWLD2UNEVEN_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
SEED_K32_Q32_ROWLD2UNEVEN_F653_V1_ID = 'rag_microbucket_k32_q32rowld2uneven_f653_v1'
_insert_sorted_pair = _ir_proxy('loom.examples.weave.knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1:_insert_sorted_pair', 256)
knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 66816, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32], ["ROWS_COVERED", 32]], "cta_group": 1, "threads": 128}'))

def _ir_with_constants(ir_obj: Any, *, suffix: str, **updates: int) -> Any:
    constants = tuple(((name, updates.get(name, value)) for name, value in ir_obj.constants))
    return replace(ir_obj, name=f'{ir_obj.name}_{suffix}', constants=constants)

def _stage1_q32_rowld2uneven_ir() -> Any:
    return _ir_with_constants(knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1_stage1, suffix='q32rowld2uneven_f653_v1', BLOCK_Q=rowld2_seed.Q16_ROWLD1_BLOCK_Q, BLOCK_M=rowld2_seed.Q16_ROWLD1_BLOCK_M, FEAT_D=rowld2_seed.Q16_ROWLD1_FEAT_D, TOP_K_MAX=K32_TOP_K_MAX, ROWS_COVERED=Q32_ROWLD2UNEVEN_ACTIVE_ROWS)

def _warp_merge_ir(split_count: int) -> Any:
    if K32_ROWS4_ROWS_PER_CTA <= 0 or K32_ROWS4_ROWS_PER_CTA > K32_ROWS4_WARPS:
        raise ValueError(f'rows_per_cta={K32_ROWS4_ROWS_PER_CTA} exceeds merge warps={K32_ROWS4_WARPS}')
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=f'k32q32uneven_s{split_count}r{K32_ROWS4_ROWS_PER_CTA}_f653_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_K32_Q32ROWLD2UNEVEN_F653_V1_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_K32_Q32ROWLD2UNEVEN_F653_V1_VERIFY_K32_SPLIT', K32_Q32_SPLIT_COUNT))
    if verify_kernel == 'rowld2_uneven_stage1':
        return _stage1_q32_rowld2uneven_ir()
    return _warp_merge_ir(split_count)
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge_k32q32uneven_s141r4_f653_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 141], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))
...
def _eligible_q32_rowld2uneven(inputs: dict[str, Any]) -> bool:
    return parent._eligible_q32_rowld2(inputs)

def _q32_rowld2uneven_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_k32_q32rowld2uneven_f653_v1_q{n_query}_m{n_database}_k32_row16x256b2warp_uneven_s{split_count}_r{K32_ROWS4_ROWS_PER_CTA}_warpmerge'

def route_for_contract_inputs(inputs: dict[str, Any], *, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> str:
    if _eligible_q32_rowld2uneven(inputs):
        return _q32_rowld2uneven_route_name(inputs, split_count=k32_q32_split_count)
    return parent.route_for_contract_inputs(inputs)

def _launch_q32_rowld2uneven_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    query = inputs['query']
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(f'k32 q32 rowld2uneven only supports K={K32_TOP_K_MAX}, got K={top_k}')
    ...
    stage1_ir = _stage1_q32_rowld2uneven_ir()
    _compiled_stage1_q32_rowld2uneven().launch(grid=(stage1_grid, 1, 1), block=(Q32_ROWLD2UNEVEN_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, num_db_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> None:
    if _eligible_q32_rowld2uneven(inputs):
        _launch_q32_rowld2uneven_rows4_merge(inputs, split_count=k32_q32_split_count)
        return
...
def route_trace_for_contract_shapes(shape_labels=K32_BUCKET_SHAPES, *, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> list[dict[str, Any]]:
    ...
    for shape in _select_contract_shapes(shape_labels):
        ...
        selected = _eligible_q32_rowld2uneven(inputs)
...
def benchmark_knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_k32_q32rowld2uneven_f653_v1', 'candidate_entrypoint': ROUTE_Q32_ROWLD2UNEVEN_ENTRYPOINT, 'parent_entrypoint': ROUTE_PARENT_F653, 'accelerated_shape_labels': list(Q32_ROWLD2UNEVEN_TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q32_M100000': f'f653 ROW_16x256B rowld2 stage1 with uneven database-tile ownership; {num_db_tiles} tiles distributed over {k32_q32_split_count} splits', 'guard_misses': 'delegate to current f653 rowld2 parent'}, 'merge_topology': {'Q32': f'f653 rows4 warp-row merge/{parent.K32_ROWS4_ROWS_PER_CTA} rows per CTA', 'q32_split_count': k32_q32_split_count, 'q32_splits_per_lane': parent.base._splits_per_lane(k32_q32_split_count)}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_q32_split_count=k32_q32_split_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
