...
ROUTE_Q16_SPLIT148_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32_2e8e_q16split148_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 128}'))
...
def benchmark_knn_build_rag_microbucket_k32_2e8e_q16split148_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_q16_split_count: int=K32_Q16_SPLIT_COUNT, k32_default_split_count: int=K32_DEFAULT_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_k32_2e8e_q16split148_v1', 'candidate_entrypoint': ROUTE_Q16_SPLIT148_ENTRYPOINT, 'parent_entrypoint': ROUTE_PARENT_B3E0, 'accelerated_shape_labels': list(Q16_SPLIT_TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q8_exact': 'b3e0 half-row ROW_16x256B one-compute-warp stage1 at split144', 'Q16_exact': 'b3e0 rowld1 ROW_16x256B one-compute-warp stage1 at split148', 'Q16_irregular': 'b3e0 rowld1 ROW_16x256B one-compute-warp stage1 at split148', 'Q32_exact': 'b3e0 ROW_16x256B stage1 plus rows4 warp-row merge at split144', 'guard_misses': 'delegate to b3e0 parent routes at split144'}, 'merge_topology': {'Q16_exact_Q16_irregular': '0077 warp-row split-list merge/1 row per CTA at split148', 'Q8_exact': '0077 warp-row split-list merge/1 row per CTA at split144', 'Q32_exact': f'0077 warp-row split-list merge/{parent.rows4.K32_ROWS4_ROWS_PER_CTA} rows per CTA at split144', 'q16_split_count': k32_q16_split_count, 'default_split_count': k32_default_split_count, 'q16_splits_per_lane': parent.rows4.base._splits_per_lane(k32_q16_split_count), 'default_splits_per_lane': parent.rows4.base._splits_per_lane(k32_default_split_count)}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_q16_split_count=k32_q16_split_count, k32_default_split_count=k32_default_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
