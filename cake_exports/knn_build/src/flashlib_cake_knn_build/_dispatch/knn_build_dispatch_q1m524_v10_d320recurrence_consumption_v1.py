...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_dispatch_q1m524_v10_d320recurrence_consumption_v1'
BASELINE_ENTRYPOINT = base.ROUTE_ENTRYPOINT
SEED_ID = 'd320_producer_recurrence_4287'
SEED_ENTRYPOINT = f'{seed.MODULE}:launch_from_contract_inputs'
...
def write_checkpointed_full_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, max_new_rows: int | None=None) -> dict[str, Any]:
    ...
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = out_dir / f'full{len(labels)}_v10_d320recurrence_paired_progress.json'
    audit = ResumableRowAudit(checkpoint_path, audit_id=f'{CANDIDATE_ID}:paired-v1', labels=labels, metadata={'contract_version': eval_mod.CONTRACT.contract_version, 'timing_backend': 'cupti' if use_cupti else 'cuda_event', 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'baseline_entrypoint': BASELINE_ENTRYPOINT})
