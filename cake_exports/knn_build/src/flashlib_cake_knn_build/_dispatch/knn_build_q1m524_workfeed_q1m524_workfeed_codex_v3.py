"""Q1 online RAG K10 M524 S147/G21 register-resident merge evolution.
...
knn_build_q1m524_workfeed_s147_g21_register_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_q1m524_workfeed_s147_g21_register_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10], ["GROUP_COUNT", 21], ["GROUP_SPLITS", 7]], "cta_group": 1, "threads": 32}'))
...
def _fused_merge_ir() -> Any:
    ...
    return replace(knn_build_q1m524_workfeed_s147_g21_register_merge, name='knn_build_q1m524_workfeed_s147_g21_register_merge', constants=constants)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and ea43._eligible_q1_m524_n128(inputs):
        return 'rag_online_mbucket_workfeed_q1m524_n128_s147_g21_register_merge'
    return ea43.route_for_contract_inputs(inputs, force_fallback=force_fallback)

def launch_from_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> None:
    if not force_fallback and ea43._eligible_q1_m524_n128(inputs):
        _launch_q1_m524_s147_g21(inputs)
        return
...
def _launch_q1_m524_s147_g21(inputs: dict[str, Any]) -> None:
    ...
    ea43._compiled_stage1_q1_k10_m64n128().launch(grid=(stage1_grid, 1, 1), block=(ea43.Q1_N128_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, Q1_S147_SPLIT, total_work], shared_mem=ea43.stage1_q1_k10_m64n128_ir.computed_smem_bytes)
    merge_ir = _fused_merge_ir()
    _compiled_fused_merge().launch(grid=(merge_grid, 1, 1), block=(Q1_S147_MERGE_THREADS, 1, 1), args=[partial_dists, partial_indices, inputs['out_dists'], inputs['out_indices'], total_queries], shared_mem=merge_ir.computed_smem_bytes)
...
def route_trace_for_contract_shapes(shape_labels=None, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        if specialized:
            ...
            row['group_count'] = Q1_S147_GROUPS
        rows.append(row)
    return rows
...
def _benchmark_payload(candidate_report: dict[str, Any], parent_report: dict[str, Any], *, use_cupti: bool, shape_labels) -> dict[str, Any]:
    ...
    for label in TARGET_SHAPES:
        ...
        target_rows[label] = {'candidate_s147_g21': cand, 'parent_s147_g7': parent, 'candidate_ms': cand_ms, 'parent_ms': parent_ms, 'speedup_vs_parent_s147_g7': parent_ms / cand_ms if cand_ms and parent_ms else None, 'ratio_vs_flashlib': cand.get('ratio_vs_flashlib')}
