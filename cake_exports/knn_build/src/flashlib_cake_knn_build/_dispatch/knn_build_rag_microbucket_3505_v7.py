...
def _compact_route_name(*, split_count: int, group_count: int, inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_3505_v7_q{n_query}_m{n_database}_k32_tailinf_cta1_cw1_s{split_count}_g{group_count}'

def _m64_route_name(inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    return f'rag_microbucket_3505_v7_q{n_query}_k10_m64_s{faeb.M64_SPLIT_COUNT}_g{faeb.M64_GROUP_COUNT}'
