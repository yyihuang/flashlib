...
ROUTE_PARENT_Q32ROWLD = f'{parent_q32rowld.__name__}:launch_from_contract_inputs'
ROUTE_Q16_M64_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
def _q16_k32_m64_route_name(*, split_count: int, group_count: int) -> str:
    return f'rag_microbucket_q16m64_19b3_v1_q16_m100000_k32_m64n64_s{split_count}_g{group_count}'
...
def benchmark_knn_build_rag_microbucket_q16m64_19b3_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_q16m64_19b3_v1', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'parent_entrypoint': ROUTE_PARENT_Q32ROWLD, 'accelerated_shape_labels': list(Q16_M64_TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q16_K32': f'M64N64/S{k32_split_count}/G{k32_group_count}/fused', 'inherited': 'q32rowld routes for Q8/Q32/K10/irregular-Q16 and parent fallback'}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
