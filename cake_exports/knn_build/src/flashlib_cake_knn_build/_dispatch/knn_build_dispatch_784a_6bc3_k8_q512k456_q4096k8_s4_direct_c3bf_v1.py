"""c3bf split4 Q4096/K8 dispatcher over the d5f8 full90 baseline.

Minimum target architecture: sm_100a. This additive dispatcher-consumption
wrapper starts from the round-131/d5f8 direct-Q512/Q4096 wrapper and changes
only the exact BF16 build ``B=1,Q=M=4096,D=128,K=8`` row. The new row routes
through the existing v20 K8 stage1 producer with four database splits and the
existing four-way K8 merge, writing contract-visible distances and indices. All
non-matching rows delegate to the d5f8 parent wrapper.
"""
...
from . import knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_direct_fcf2_v1 as parent_direct
...
MODULE = 'loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_s4_direct_c3bf_v1'
eval_mod = parent_direct.eval_mod
BASE_784A_KEY = parent_direct.BASE_784A_KEY
PARENT_DIRECT_KEY = 'parent_d5f8_q4096_k8_s8'
CANDIDATE_Q4096K8_DIRECT = '784a_plus_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8'
...
SEED_Q4096_K8_DIRECT_ID = 'c3bf_direct_v20_q4096_k8_s4'
ROUTE_Q4096_K8_S4 = f'{MODULE}:q4096_k8_v20_s4'
ROUTE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
PARENT_DIRECT_ENTRYPOINT = parent_direct.ROUTE_ENTRYPOINT
CANDIDATE_ENTRYPOINT = f'{MODULE}:benchmark_candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1'
BASE_784A_ID = parent_direct.BASE_784A_ID
PARENT_DIRECT_ID = parent_direct.CANDIDATE_ID
CANDIDATE_ID = 'candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1'
PRODUCTION_ROUTE_MODULES = {**parent_direct.PRODUCTION_ROUTE_MODULES, SEED_Q4096_K8_DIRECT_ID: ROUTE_Q4096_K8_S4, PARENT_DIRECT_ID: PARENT_DIRECT_ENTRYPOINT, CANDIDATE_ID: ROUTE_ENTRYPOINT}
SOURCE_TASKS = {**parent_direct.SOURCE_TASKS, SEED_Q4096_K8_DIRECT_ID: 'c3bf Q4096/K8 performance repair / v20 K8 split4 stage1+merge from loom.examples.weave.knn_build_evolve_7bfc_split_cg2_u2_smallmedfan_rag7_k10merge_stage1batch_cond4_k5merge4tree_vmin_maxtree_k5tree_mintree_k10s4s7cache_t32r32_k10mintree_fixedbuild_dispatch_v2_k32split_v20'}

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_C3BF_Q4096K8_S4_VERIFY_KERNEL')
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_s4_direct_c3bf_v1:ir"}'))
...
def _candidate_config(candidate_key: str) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f'unknown fcf2 Q4096/K8 direct candidate {candidate_key!r}') from exc
...
def candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1(inputs: dict[str, Any]) -> None:
    launch_from_contract_inputs(inputs, candidate_key=CANDIDATE_Q4096K8_DIRECT)

def candidate(inputs: dict[str, Any]) -> None:
    candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1(inputs)
...
def _candidate_kernel_fn(candidate_key: str) -> Callable[[dict[str, Any]], None]:
    ...
    if candidate_key == CANDIDATE_Q4096K8_DIRECT:
        return candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1
    raise ValueError(f'unknown fcf2 Q4096/K8 direct candidate {candidate_key!r}')
...
_PARENT_S8_SELECTED_SEEDS = _decode_capture(_json_loads('{"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25", "6bc3_v20_q512_k8_static_s8", "6bc3_v20_q2048_k8_static_s8", "fcf2_direct_lowk_q512_k4_k5_k6_s4", "fcf2_direct_v20_q4096_k8_s8"]}'))
CANDIDATE_CONFIGS = _decode_capture(_json_loads('{"784a_plus_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8": {"benchmark_entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_s4_direct_c3bf_v1:benchmark_candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1", "candidate_id": "candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1", "entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_s4_direct_c3bf_v1:candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1", "expected_shape_wins": {"__tuple__": ["build_k_sweep_qm512_k4", "build_k_sweep_qm512_k5", "build_k_sweep_qm512_k6", "build_k_sweep_qm512_k8", "build_qm2048_d128_k8", "build_qm4096_d128_k8"]}, "fallback": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_direct_fcf2_v1:launch_from_contract_inputs", "guard_plan": {"__tuple__": ["direct exact BF16 build Q4096 K8 v20 split4 guard", "then direct exact BF16 build Q512 K4/K5/K6 low-K split4 guard", "then fcf2 selective 6bc3 exact Q512/Q2048 K8 guards", "then 784a 005f build-lowfloor portfolio", "then inherited d5f8/a444/9db7 full82 Weave fallback"]}, "rejected_reason": null, "selected_seeds": {"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25", "6bc3_v20_q512_k8_static_s8", "6bc3_v20_q2048_k8_static_s8", "fcf2_direct_lowk_q512_k4_k5_k6_s4", "c3bf_direct_v20_q4096_k8_s4"]}}, "base_784a_005f": {"benchmark_entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_selective_full82_v1:benchmark_baseline_784a_005f", "candidate_id": "candidate_dbd7_005f_buildbucket_full82_v1", "entrypoint": "loom.examples.weave.knn_build_dispatch_dbd7_lowfloor_005f_8a78_full82_v1:launch_from_contract_inputs", "expected_shape_wins": {"__tuple__": ["build_k_sweep_qm1024_k12", "build_k_sweep_qm1024_k20", "build_qm2048_d128_k10", "build_k_sweep_qm2048_k12", "build_k_sweep_qm2048_k20", "build_tail_b1_q1536_m1536_d128_k10", "build_k_sweep_qm4096_k12", "build_k_sweep_qm4096_k20", "build_over32_stress_qm2048_k48", "build_over32_stress_qm4096_k48"]}, "fallback": "loom.examples.weave.knn_build_dispatch_dbd7_lowfloor_005f_8a78_full82_v1:launch_from_contract_inputs", "guard_plan": {"__tuple__": ["005f exact BF16 build low-floor portfolio for K10/K12/K20/K48 rows", "a444/9db7 full82 Weave fallback for guard misses and Q1536/K10 tail"]}, "rejected_reason": "same-session 784a baseline", "selected_seeds": {"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25"]}}, "parent_d5f8_q4096_k8_s8": {"benchmark_entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_direct_fcf2_v1:benchmark_candidate_784a_direct_q512_k456_q4096_k8_plus_6bc3_k8_v1", "candidate_id": "candidate_784a_direct_q512_k456_q4096_k8_plus_6bc3_k8_v1", "entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_direct_fcf2_v1:launch_from_contract_inputs", "expected_shape_wins": {"__tuple__": ["build_k_sweep_qm512_k4", "build_k_sweep_qm512_k5", "build_k_sweep_qm512_k6", "build_k_sweep_qm512_k8", "build_qm2048_d128_k8", "build_qm4096_d128_k8"]}, "fallback": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_direct_fcf2_v1:launch_from_contract_inputs", "guard_plan": {"__tuple__": ["direct exact BF16 build Q4096 K8 v20 split8 guard", "then direct exact BF16 build Q512 K4/K5/K6 low-K split4 guard", "then fcf2 selective 6bc3 exact Q512/Q2048 K8 guards", "then 784a 005f build-lowfloor portfolio", "then inherited a444/9db7 full82 Weave fallback"]}, "rejected_reason": "same-session d5f8 split8 baseline", "selected_seeds": {"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25", "6bc3_v20_q512_k8_static_s8", "6bc3_v20_q2048_k8_static_s8", "fcf2_direct_lowk_q512_k4_k5_k6_s4", "fcf2_direct_v20_q4096_k8_s8"]}}}'))
CANDIDATE_DISPATCHERS = _decode_capture(_json_loads('{"__tuple__": [{"consumed_seeds": {"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25"]}, "entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_selective_full82_v1:benchmark_baseline_784a_005f", "expected_shape_wins": {"__tuple__": ["build_k_sweep_qm1024_k12", "build_k_sweep_qm1024_k20", "build_qm2048_d128_k10", "build_k_sweep_qm2048_k12", "build_k_sweep_qm2048_k20", "build_tail_b1_q1536_m1536_d128_k10", "build_k_sweep_qm4096_k12", "build_k_sweep_qm4096_k20", "build_over32_stress_qm2048_k48", "build_over32_stress_qm4096_k48"]}, "fallback": "loom.examples.weave.knn_build_dispatch_dbd7_lowfloor_005f_8a78_full82_v1:launch_from_contract_inputs", "guard_plan": {"__tuple__": ["005f exact BF16 build low-floor portfolio for K10/K12/K20/K48 rows", "a444/9db7 full82 Weave fallback for guard misses and Q1536/K10 tail"]}, "id": "candidate_dbd7_005f_buildbucket_full82_v1", "rejected_reason": "same-session 784a baseline"}, {"consumed_seeds": {"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25", "6bc3_v20_q512_k8_static_s8", "6bc3_v20_q2048_k8_static_s8", "fcf2_direct_lowk_q512_k4_k5_k6_s4", "fcf2_direct_v20_q4096_k8_s8"]}, "entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_direct_fcf2_v1:benchmark_candidate_784a_direct_q512_k456_q4096_k8_plus_6bc3_k8_v1", "expected_shape_wins": {"__tuple__": ["build_k_sweep_qm512_k4", "build_k_sweep_qm512_k5", "build_k_sweep_qm512_k6", "build_k_sweep_qm512_k8", "build_qm2048_d128_k8", "build_qm4096_d128_k8"]}, "fallback": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_direct_fcf2_v1:launch_from_contract_inputs", "guard_plan": {"__tuple__": ["direct exact BF16 build Q4096 K8 v20 split8 guard", "then direct exact BF16 build Q512 K4/K5/K6 low-K split4 guard", "then fcf2 selective 6bc3 exact Q512/Q2048 K8 guards", "then 784a 005f build-lowfloor portfolio", "then inherited a444/9db7 full82 Weave fallback"]}, "id": "candidate_784a_direct_q512_k456_q4096_k8_plus_6bc3_k8_v1", "rejected_reason": "same-session d5f8 split8 baseline"}, {"consumed_seeds": {"__tuple__": ["v20_k12_q1024_q4096_exact", "q2048_k12_4f30_v1", "v12_k20_q2048k10_mixedfanout", "over32_k48_v25", "6bc3_v20_q512_k8_static_s8", "6bc3_v20_q2048_k8_static_s8", "fcf2_direct_lowk_q512_k4_k5_k6_s4", "c3bf_direct_v20_q4096_k8_s4"]}, "entrypoint": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_s4_direct_c3bf_v1:benchmark_candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1", "expected_shape_wins": {"__tuple__": ["build_k_sweep_qm512_k4", "build_k_sweep_qm512_k5", "build_k_sweep_qm512_k6", "build_k_sweep_qm512_k8", "build_qm2048_d128_k8", "build_qm4096_d128_k8"]}, "fallback": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_q512k456_q4096k8_direct_fcf2_v1:launch_from_contract_inputs", "guard_plan": {"__tuple__": ["direct exact BF16 build Q4096 K8 v20 split4 guard", "then direct exact BF16 build Q512 K4/K5/K6 low-K split4 guard", "then fcf2 selective 6bc3 exact Q512/Q2048 K8 guards", "then 784a 005f build-lowfloor portfolio", "then inherited d5f8/a444/9db7 full82 Weave fallback"]}, "id": "candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1", "rejected_reason": null}]}'))
...
def _denominator_name(shape_labels) -> str:
    if shape_labels is None:
        return f'full{len(eval_mod.CANONICAL_SHAPES)}_v10'
    labels = tuple((str(label) for label in shape_labels))
    if labels == TARGET_SHAPES:
        return 'q512k456_q4096k8_s4_plus_6bc3_k8_target_rows'
...
def _guard_condition(seed_id: str | None) -> str:
    if seed_id == SEED_Q4096_K8_DIRECT_ID:
        return 'exact BF16 build B=1 Q=M=4096 D=128 K=8 v20 split4 route'
...
def benchmark_candidate_portfolio(candidate_key: str, *, use_cupti: bool=True, shape_labels=None, parent_report: dict[str, Any] | None=None, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, Any]:
    if candidate_key == BASE_784A_KEY:
        return parent_direct.benchmark_candidate_portfolio(parent_direct.BASE_784A_KEY, use_cupti=use_cupti, shape_labels=shape_labels, benchmark_correctness=benchmark_correctness, time_flashlib=time_flashlib)
    if candidate_key == PARENT_DIRECT_KEY:
...
def benchmark_candidate_784a_direct_q512_k456_q4096_k8_s4_plus_6bc3_k8_v1(**kwargs) -> dict[str, Any]:
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, str]:
    ...
    parent_path = out_dir / f'{denom_label}_same_session_parent_q512k456_direct_for_q4096k8_v1.json'
    payload_path = out_dir / f'{denom_label}_dispatch_784a_direct_q512k456_q4096k8_s4_plus_6bc3_k8_v1.json'
    trace_path = out_dir / f'{denom_label}_route_trace_784a_direct_q512k456_q4096k8_s4_plus_6bc3_k8_v1.json'
    forced_trace_path = out_dir / f'{denom_label}_forced_fallback_trace_784a_direct_q512k456_q4096k8_s4_plus_6bc3_k8_v1.json'
    seed_matrix_path = out_dir / f'{denom_label}_seed_delta_matrix_784a_direct_q512k456_q4096k8_s4_plus_6bc3_k8_v1.json'
