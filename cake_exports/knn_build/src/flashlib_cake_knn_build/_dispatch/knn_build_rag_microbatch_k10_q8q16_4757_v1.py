...
ROUTE_Q8 = f'{MODULE}:q8_m64_s128_g8'
ROUTE_Q16 = f'{MODULE}:q16_m64_s136_g8'
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_k10_q8q16_4757_v1:ir"}'))
...
def _route_for_split(split_count: int) -> str:
    ...
    if split_count == Q16_SPLIT_COUNT:
        return ROUTE_Q16
    raise ValueError(f'unsupported q8/q16 split count: {split_count}')
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    return parent._normalize_route_row({'shape_key': label, 'selected_route': _selected_route_name(split_count), 'selected_entrypoint': ROUTE_ENTRYPOINT, 'selected_seed': SEED_ID, 'expected_seed': SEED_ID, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'4757_rag_microbatch_k10_q{q_value}_m64_s{split_count}_g8_exact_guard', 'guard_condition': f'exact BF16 non-build B=1 Q={q_value} M=100000 D=128 K=10', 'coverage': 'q8/q16 split-selected M64 Weave seed before Q24/Q128 full90 parent', 'consumed_seed': SEED_ID, 'replaced_route': parent_route, 'parent_portfolio_route': parent_route, 'split_count': split_count, 'group_count': GROUP_COUNT, 'classification': 'seed-consumed'})
