"""Exact large-square build K32 split2 probe for the 8a83 follow-up.
...
existing v20 tcgen05/TMA unordered K32 stage-1 producer but reduces the large
square row from four database splits to two, then merges the two split-local
K32 unordered lists with a K32 warp-select reducer. Guard misses delegate to
the a989 large-square K20/K32 seed.
"""
...
K32_MERGE_THREADS = parent_v20.K32_COOP_MERGE_THREADS
...
knn_build_large_square_k32_s2_warp_select = _decode_capture(_json_loads('{"__ir__": "knn_build_large_square_k32_s2_warp_select", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32]], "cta_group": 1, "threads": 128}'))
stage1_k32_split2_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_cg2_u2_stage1_k32_unordered", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
merge_k32_s2_warp_select_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_large_square_k32_s2_warp_select", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32]], "cta_group": 1, "threads": 128}'))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_LARGE_SQUARE_K32_8A83_VERIFY_KERNEL')
    if verify_kernel == 'merge_k32_s2_warp_select':
        return merge_k32_s2_warp_select_ir
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_cg2_u2_stage1_k32_unordered", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def _launch_q8192_k32_split2(inputs: dict[str, Any]) -> None:
    query = inputs['query']
    ...
    stage1_kernel.launch_cluster(grid=(stage1_grid, 1, 1), block=(STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tile_pairs, db_tiles_per_split, SPLIT_COUNT, total_work], cluster_dims=(CTA_GROUP, 1, 1), shared_mem=stage1_k32_split2_ir.computed_smem_bytes)
    merge_kernel = _compiled_merge_k32_s2_warp_select()
    merge_kernel.launch(grid=(merge_grid, 1, 1), block=(K32_MERGE_THREADS, 1, 1), args=[partial_dists, partial_indices, inputs['out_dists'], inputs['out_indices'], bsz * n_query], shared_mem=merge_k32_s2_warp_select_ir.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any]) -> None:
    if _eligible_q8192_k32(inputs):
        _launch_q8192_k32_split2(inputs)
        return
...
def route_trace_for_shapes(shape_labels=None) -> list[dict[str, Any]]:
    ...
    for shape in _select_contract_shapes(shape_labels):
        ...
        trace.append({'shape_key': shape['label'], 'selected_route': route, 'route_kind': 'specialized' if route == ROUTE_Q8192_K32_SPLIT2 else 'parent_delegate', 'guard_condition': _guard_description(route)})
    return trace

def _guard_description(route: str) -> str:
    if route == ROUTE_Q8192_K32_SPLIT2:
        return 'exact BF16 build B1 Q=M=8192 D128 K32 split2 warp-select route'
...
def _per_shape_delta(candidate_report: dict[str, Any], baseline_report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'candidate_route': ROUTE_Q8192_K32_SPLIT2, 'baseline_route': ROUTE_PARENT_A989, 'candidate_ms': cand_ms, 'baseline_ms': base_ms, 'flashlib_ms': cand.get('flashlib_ms'), 'candidate_tflops': cand.get('tflops'), 'baseline_tflops': base.get('tflops'), 'speedup_vs_a989': base_ms / cand_ms if cand_ms and base_ms else None, 'ratio_vs_flashlib': cand.get('ratio_vs_flashlib'), 'passed': cand.get('passed'), 'timing_backend': cand.get('timing_backend')}

def benchmark_knn_build_large_square_k32_8a83_v1(*, use_cupti: bool=True, run_baseline: bool=True) -> dict[str, Any]:
    """Benchmark the exact q8192 K32 split2 sidecar against a989."""
