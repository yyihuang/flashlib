...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_v12_d64_tail_017a_v1'
...
knn_build_v12_d64_tail_017a_stage1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_v12_d64_tail_017a_v1:knn_build_v12_d64_tail_017a_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 20224, "cta_group": 1, "threads": 96}'))
stage1_v12_d64_tail_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_v12_d64_tail_017a_v1:stage1_v12_d64_tail_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 20224, "cta_group": 1, "threads": 96}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_v12_d64_tail_017a_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 20224, "cta_group": 1, "threads": 96}'))
...
def _launch_d64_tail(inputs: dict[str, Any], label: str) -> None:
    ...
    if dim != D64_FEAT_D:
        raise ValueError(f'{label} expected D={D64_FEAT_D}, got {dim}')
    ...
    _compiled_stage1_v12_d64_tail().launch(grid=(stage1_grid, 1, 1), block=(D64_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_v12_d64_tail_ir.computed_smem_bytes)
    merge_ir = d64_parent.fused_merge_parent._fused_merge_ir(split_count, group_count)
    _compiled_fused_merge(split_count, group_count).launch(grid=(merge_grid, 1, 1), block=(d64_parent.fused_merge_parent.D768_FUSED_MERGE_THREADS, 1, 1), args=[partial_dists, partial_indices, inputs['out_dists'], inputs['out_indices'], total_queries], shared_mem=merge_ir.computed_smem_bytes)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return f'v12_d64_tail_017a:{label}:q{int(spec['Q'])}:m{int(spec['M'])}:m64n64k64:s{_split_count_for_label(label)}:g{_group_count_for_label(label)}'
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
    ...
    path = out_dir / f'v12_d64_tail_017a_{len(tuple(shape_labels))}row_{suffix}.json'
