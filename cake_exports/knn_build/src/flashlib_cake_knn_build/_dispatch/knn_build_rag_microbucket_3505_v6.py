"""RAG microbucket widened compact-warp K32 seed.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
keeps the validated 3505 Q4/Q64 K10 M64 routes and reuses the compact-warp
tcgen05/TMA K32 producer for the v7 RAG microbatch large-K rows with Q <= 32.
...
ROUTE_Q4_K10 = 'rag_microbucket_3505_v6_q4_k10_m64_s128_g8'
ROUTE_Q64_K10 = 'rag_microbucket_3505_v6_q64_k10_m64_s128_g8'
ROUTE_BASE_4247 = parent_3505.ROUTE_BASE_4247

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V6_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V6_VERIFY_K32_SPLIT', K32_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V6_VERIFY_K32_GROUPS', K32_GROUP_COUNT))
...
def _compact_route_name(*, split_count: int, group_count: int, inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_3505_v6_q{n_query}_m{n_database}_k32_tailinf_cta1_cw1_s{split_count}_g{group_count}'
...
def candidate_compact_seed(inputs: dict[str, Any]):
    compact_seed.launch_from_contract_inputs(inputs)
    return None
...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        specialized = str(route).startswith('rag_microbucket_3505_v6')
        rows.append({'shape_key': shape['label'], 'selected_route': route, 'route_kind': 'specialized' if specialized else 'general', 'guard_condition': 'exact BF16 non-build B1 D128 Q4/Q64 K10 or Q8/Q16/Q32 K32 microbucket' if specialized else 'guard miss to 4247 dispatcher', 'fallback': ROUTE_BASE_4247})
    return rows

def _per_shape_delta(candidate_report: dict[str, Any], seed_report: dict[str, Any]) -> dict[str, Any]:
    rows = {}
    for label in TARGET_SHAPES:
        cand = candidate_report.get('per_shape', {}).get(label, {})
        seed = seed_report.get('per_shape', {}).get(label, {})
        cand_ms = cand.get('kernel_ms')
        seed_ms = seed.get('kernel_ms')
        rows[label] = {'candidate': cand, 'compact_seed_5093': seed, 'candidate_ms': cand_ms, 'compact_seed_5093_ms': seed_ms, 'speedup_vs_compact_seed_5093': seed_ms / cand_ms if cand_ms and seed_ms else None, 'ratio_vs_flashlib': cand.get('ratio_vs_flashlib')}
    return rows

def benchmark_knn_build_rag_microbucket_3505_v6(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    compact_seed_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_compact_seed)
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_3505_v6:benchmark_knn_build_rag_microbucket_3505_v6', 'candidate_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_3505_v6:launch_from_contract_inputs', 'compact_seed_5093_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_5093_v1:launch_from_contract_inputs', 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'K10': f'M64/S{faeb.M64_SPLIT_COUNT}/G{faeb.M64_GROUP_COUNT}', 'K32': f'tailinf-cta1/compactwarp/S{k32_split_count}/G{k32_group_count}/fused'}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, compact_seed_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'compact_seed_5093_summary': compact_seed_report['summary'], 'compact_seed_5093_performance': compact_seed_report['performance'], 'compact_seed_5093_report': compact_seed_report}
