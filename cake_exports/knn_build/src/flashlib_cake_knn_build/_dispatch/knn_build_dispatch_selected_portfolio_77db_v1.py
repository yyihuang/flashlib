"""Opt-in kNN build dispatcher consuming the a194 q4096 K64 split8 seed.
...
row to the validated a194 split8 seed.
...
from . import knn_build_dim_midk_f8c3_q4096k64split_v1 as q4096_k64_split8
from . import knn_build_dispatch_selected_portfolio_f8c3_v1 as base_f8c3
ROUTE_Q4096_K64_SPLIT8 = q4096_k64_split8.ROUTE_Q4096_K64
ROUTE_BASE_F8C3 = 'loom.examples.weave.knn_build_dispatch_selected_portfolio_f8c3_v1:launch_from_contract_inputs'
ROUTE_REFERENCE_E51C = 'loom.examples.weave.knn_build_dispatch_selected_portfolio_e51c_v1:launch_from_contract_inputs'
Q4096_K64_TARGET_SHAPES = ('build_over32_stress_qm4096_k64',)
Q4096_K64_TARGET_SHAPE_SET = set(Q4096_K64_TARGET_SHAPES)
...
PRODUCTION_ROUTE_MODULES = {**base_f8c3.PRODUCTION_ROUTE_MODULES, 'midk_f8c3_q4096k64split8_a194': ROUTE_Q4096_K64_SPLIT8, 'base_f8c3': ROUTE_BASE_F8C3}
CANDIDATE_PORTFOLIOS = ({'id': 'baseline_f8c3_selected_portfolio', 'entrypoint': 'loom.examples.weave.knn_build_dispatch_selected_portfolio_f8c3_v1:benchmark_knn_build_dispatch_selected_portfolio_f8c3_v1', 'consumed_seeds': ('selected_e51c_f552_a330_a4f6_row_level_midk', 'midk_bad5_k64split8_q2048'), 'guard_plan': ('f8c3 selected full55 guard plan',), 'expected_shape_wins': base_f8c3.CONSUMED_SEED_TARGET_SHAPES, 'rejected_reason': 'same-session baseline for 77db q4096 K64 dispatcher-consumption lane'}, {'id': 'selected_77db_f8c3_plus_q4096_k64split8', 'entrypoint': 'loom.examples.weave.knn_build_dispatch_selected_portfolio_77db_v1:benchmark_knn_build_dispatch_selected_portfolio_77db_v1', 'consumed_seeds': ('selected_f8c3_e51c_plus_k64split8', 'midk_f8c3_q4096k64split8_a194'), 'guard_plan': ('exact a194 K64 split8 BF16 build B1 Q=M=4096 D128 K64 guard', 'then f8c3 selected full55 guard plan'), 'expected_shape_wins': CONSUMED_SEED_TARGET_SHAPES, 'rejected_reason': None})
Q4096_K64_ROW_SELECTION = {'build_over32_stress_qm4096_k64': {'selected_seed': 'midk_f8c3_q4096k64split8_a194', 'selected_route': ROUTE_Q4096_K64_SPLIT8, 'targeted_seed_ms': 0.265698, 'targeted_seed_tflops': 16.164846163689603, 'targeted_seed_timing_backend': 'cupti', 'targeted_ratio_vs_flashlib': 2.087192978494381, 'targeted_baseline_f8c3_ms': 0.744356, 'targeted_baseline_f8c3_tflops': 5.77004457007131, 'targeted_speedup_vs_f8c3': 2.8015114904892022, 'reason': 'a194 exact q4096 K64 split8 seed is correct and 2.80x faster than the f8c3 inherited route on same-session CUPTI exact-row timing'}}

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_DISPATCH_SELECTED_77DB_VERIFY_KERNEL')
    if verify_kernel == 'q4096_k64_stage1_tailinf':
        os.environ['LOOM_KNN_DIMMIDK_F8C3_Q4096K64_VERIFY_KERNEL'] = 'stage1_k64_tailinf'
        return q4096_k64_split8._verify_export_ir()
    if verify_kernel == 'q4096_k64_merge_s8':
        os.environ['LOOM_KNN_DIMMIDK_F8C3_Q4096K64_VERIFY_KERNEL'] = 'merge_k64_s8'
        return q4096_k64_split8._verify_export_ir()
    return base_f8c3.ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_selected_portfolio_77db_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))

def _eligible_q4096_k64_split8(inputs: dict[str, Any]) -> bool:
    return q4096_k64_split8._eligible_q4096_k64(inputs)

def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if force_fallback:
        return base_f8c3.route_for_contract_inputs(inputs)
    if _eligible_q4096_k64_split8(inputs):
        return ROUTE_Q4096_K64_SPLIT8
    return base_f8c3.route_for_contract_inputs(inputs)

def _launch_route(inputs: dict[str, Any], route: str) -> None:
    if route == ROUTE_Q4096_K64_SPLIT8:
        q4096_k64_split8._launch_q4096_k64_split(inputs)
        return
    base_f8c3._launch_route(inputs, route)
...
def candidate_base_dispatcher(inputs: dict[str, Any]):
    base_f8c3.launch_from_contract_inputs(inputs)
    return None

def candidate_reference_e51c(inputs: dict[str, Any]):
    base_f8c3.base_e51c.launch_from_contract_inputs(inputs)
    return None
...
def _select_contract_shapes(shape_labels):
    return base_f8c3._select_contract_shapes(shape_labels)
...
def _trace_inputs_from_shape(shape: dict[str, Any]) -> dict[str, Any]:
    return base_f8c3._trace_inputs_from_shape(shape)

def _inputs_for_label(label: str) -> dict[str, Any]:
    return base_f8c3._inputs_for_label(label)
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if force_fallback:
        row = base_f8c3._route_trace_record(inputs)
        row['guard_condition'] = 'forced fallback to f8c3 baseline; 77db q4096 K64 overlay disabled'
        row['coverage'] = 'forced candidate fallback for 77db q4096 K64 overlay'
        row['forced_disabled_seeds'] = ('midk_f8c3_q4096k64split8_a194',)
...
def _timing_backends_for_reports(*reports: dict[str, Any]) -> list[str]:
    return base_f8c3._timing_backends_for_reports(*reports)

def _rows_for_labels(report: dict[str, Any], labels: tuple[str, ...]) -> dict[str, Any]:
    return base_f8c3._rows_for_labels(report, labels)
...
def _frontmatter_seed_delta_matrix(candidate_report: dict[str, Any], baseline_report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in _seed_delta_matrix(candidate_report, baseline_report):
        ...
        rows.append({'shape_key': item['shape_key'], 'baseline_route': item['baseline_route'], 'candidate_deltas': [{'candidate_id': 'selected_77db_f8c3_plus_q4096_k64split8', 'metric_delta': 0.0 if delta is None else float(delta), 'timing_backend': item['timing_backend'] or 'cuda_event'}]})
    return rows

def _below_flashlib_rows(report: dict[str, Any]) -> list[dict[str, Any]]:
    rows = []
    production_routes = set(PRODUCTION_ROUTE_MODULES.values())
    for label, row in sorted(report.get('per_shape', {}).items()):
        ...
        if isinstance(ratio, (float, int)) and ratio < 1.0:
            inputs = _inputs_for_label(label)
            selected_route = route_for_contract_inputs(inputs)
            rows.append({'shape_key': label, 'kernel_ms': row.get('kernel_ms'), 'flashlib_ms': row.get('flashlib_ms'), 'ratio_vs_flashlib': ratio, 'selected_route': selected_route, 'route_kind': 'specialized' if selected_route in production_routes else 'general'})
    return rows

def _reference_payload(report: dict[str, Any], *, shape_labels, route_trace) -> dict[str, Any]:
    return {'measured_entrypoint': ROUTE_REFERENCE_E51C, 'measured_shape_labels': 'all_canonical' if shape_labels is None else tuple(shape_labels), 'tflops': report['summary']['primary_mean'], 'all_correct': report['summary']['all_correct'], 'performance_comparable': report['summary']['performance_comparable'], 'contract_summary': report['summary'], 'contract_performance': report['performance'], 'route_trace': route_trace, 'route_trace_included': True, 'report': report}

def _benchmark_payload(candidate_report: dict[str, Any], baseline_report: dict[str, Any], reference_e51c_report: dict[str, Any], *, use_cupti: bool, shape_labels, measured_function: str) -> dict[str, Any]:
    candidate_metric = candidate_report['summary']['primary_mean'] or 0.0
    baseline_metric = baseline_report['summary']['primary_mean'] or 0.0
    reference_metric = reference_e51c_report['summary']['primary_mean'] or 0.0
...
def benchmark_knn_build_dispatch_selected_portfolio_77db_v1(*, use_cupti: bool=False, shape_labels=None) -> dict[str, Any]:
    ...
    reference_e51c_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_reference_e51c)
    return _benchmark_payload(candidate_report, baseline_report, reference_e51c_report, use_cupti=use_cupti, shape_labels=shape_labels, measured_function='benchmark_knn_build_dispatch_selected_portfolio_77db_v1')

def write_benchmark_artifacts(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=False, shape_labels=None) -> dict[str, str]:
    payload = benchmark_knn_build_dispatch_selected_portfolio_77db_v1(use_cupti=use_cupti, shape_labels=shape_labels)
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    candidate_path = out_dir / 'full55_dispatch_selected_portfolio_77db_v1.json'
    baseline_path = out_dir / 'full55_same_session_baseline_f8c3_for_77db_v1.json'
    reference_path = out_dir / 'full55_same_session_reference_e51c_for_77db_v1.json'
    route_trace_path = out_dir / 'full55_route_trace_selected_portfolio_77db_v1.json'
    forced_trace_path = out_dir / 'full55_forced_fallback_trace_selected_portfolio_77db_v1.json'
    ...
    return {'candidate_payload': str(candidate_path), 'baseline_payload': str(baseline_path), 'reference_e51c_payload': str(reference_path), 'route_trace': str(route_trace_path), 'forced_fallback_trace': str(forced_trace_path)}
