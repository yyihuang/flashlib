...
SHAPE_SPECS = _decode_capture(_json_loads('{"build_dim_sweep_b1_q1024_m1024_d96_k10": {"B": 1, "D": 96, "K": 10, "M": 1024, "Q": 1024, "build": true, "feature_chunks": 1, "split_count": 2}, "build_dim_sweep_b1_q2048_m2048_d192_k10": {"B": 1, "D": 192, "K": 10, "M": 2048, "Q": 2048, "build": true, "feature_chunks": 2, "split_count": 4}, "build_highd_b1_q1024_m1024_d320_k10": {"B": 1, "D": 320, "K": 10, "M": 1024, "Q": 1024, "build": true, "feature_chunks": 3, "split_count": 4}, "rag_microbatch_highd_b1_q16_m50000_d768_k10": {"B": 1, "D": 768, "K": 10, "M": 50000, "Q": 16, "build": false, "feature_chunks": 6, "split_count": 64}, "search_rect_highd_b1_q512_m12000_d320_k10": {"B": 1, "D": 320, "K": 10, "M": 12000, "Q": 512, "build": false, "feature_chunks": 3, "split_count": 16}}'))
...
knn_build_non128_frontier_8199_d384_stage1 = _ir_proxy('loom.examples.weave.knn_build_non128_frontier_8199_widestage_v1:knn_build_non128_frontier_8199_d384_stage1', 256)
stage1_d384_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widestage_v1:stage1_d384_ir"}'))
merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widestage_v1:merge_ir"}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if not force_fallback and label is not None:
        ...
        return f'{ROUTE_PREFIX}:{label}:d{int(spec['D'])}:s{_split_count_for_label(label)}:{_route_kind_for_label(label)}'
...
def route_trace_for_contract_shapes(shape_labels=None, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        rows.append({'shape_key': label, 'selected_route': route_for_contract_inputs(inputs), 'selected_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'selected_seed': 'non128_frontier_8199_widestage_v1', 'expected_seed': 'non128_frontier_8199_widestage_v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': '8199_non128_exact_shape_guard', 'guard_condition': f'exact BF16 B={spec['B']} Q={spec['Q']} M={spec['M']} D={spec['D']} K={spec['K']} build={spec['build']}', 'feature_dim': feature_dim, 'split_count': _split_count_for_label(label), 'producer': _route_kind_for_label(label), 'preprocess_stage': f'd{int(spec['D'])}_weave_pad_to_d{feature_dim}' if int(spec['D']) != feature_dim else None, 'classification': 'seed-consumed'})
    return rows
