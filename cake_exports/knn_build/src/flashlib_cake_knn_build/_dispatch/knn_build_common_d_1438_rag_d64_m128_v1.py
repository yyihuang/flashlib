...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_common_d_1438_rag_d64_m128_v1'
...
knn_build_common_d_1438_rag_d64_m128_stage1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_1438_rag_d64_m128_v1:knn_build_common_d_1438_rag_d64_m128_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 66816, "cta_group": 1, "threads": 512}'))
stage1_d64_m128_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_1438_rag_d64_m128_v1:stage1_d64_m128_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 66816, "cta_group": 1, "threads": 512}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_1438_rag_d64_m128_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 66816, "cta_group": 1, "threads": 512}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return f'{ROUTE_PREFIX}:{label}:d64:q16:m50000:m128:s{_split_count()}:g{_group_count()}'
...
def _launch_d64_m128(inputs: dict[str, Any]) -> None:
    ...
    top_k = int(inputs['K'])
    if dim != D64_D:
        raise ValueError(f'{RAG_D64} expected D={D64_D}, got {dim}')
    ...
    _compiled_stage1_d64_m128().launch(grid=(stage1_grid, 1, 1), block=(D64_THREADS, 1, 1), args=[query, database, inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=D64_STAGE_SMEM_BYTES)
    merge_ir = parent.fused_merge_parent._fused_merge_ir(split_count, group_count)
    _compiled_fused_merge(split_count, group_count).launch(grid=(merge_grid, 1, 1), block=(parent.fused_merge_parent.D768_FUSED_MERGE_THREADS, 1, 1), args=[partial_dists, partial_indices, inputs['out_dists'], inputs['out_indices'], total_queries], shared_mem=merge_ir.computed_smem_bytes)
...
def benchmark_knn_build_common_d_1438_rag_d64_m128_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
    ...
    return {'contract': report['contract'], 'contract_version': report['contract_version'], 'measured_entrypoint': BENCHMARK_ENTRYPOINT, 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': f'D64M128/S{_split_count()}/G{_group_count()}', 'route_trace': route_trace_for_contract_shapes(shape_labels), 'summary': report['summary'], 'performance': report['performance'], 'correctness': report['correctness'], 'report': report}
