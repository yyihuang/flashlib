...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_common_d_5e7f_search_d256_v1'
...
stage1_d256_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_5e7f_search_d256_v1:stage1_d256_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_5e7f_search_d256_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _target_label_for_inputs(inputs) == SEARCH_D256:
        return f'{ROUTE_PREFIX}:{SEARCH_D256}:d256:q1024:m32768:s{_split_count()}:g{_group_count()}:chunked128_fused_merge'
...
def _launch_search_d256(inputs: dict[str, Any]) -> None:
    ...
    if dim != tma_dim:
        raise ValueError(f'{SEARCH_D256} expected D={tma_dim}, got {dim}')
    ...
    _compiled_stage1_d256().launch(grid=(stage1_grid, 1, 1), block=(THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_d256_ir.computed_smem_bytes)
