...
ROUTE_Q16_ROWS4_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
def _warp_merge_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=f'k32s{split_count}r{K32_ROWS4_ROWS_PER_CTA}_abee_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge_k32s148r4_abee_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 148], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))
...
def _rows4_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_k32_q16rows4_abee_v1_q{n_query}_m{n_database}_k32_row16x256b_s{split_count}_r{K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def _launch_rowld1_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    rowld1._compiled_stage1_q16_k32_m64_rowld1().launch(grid=(stage1_grid, 1, 1), block=(rowld1.Q16_ROWLD1_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
