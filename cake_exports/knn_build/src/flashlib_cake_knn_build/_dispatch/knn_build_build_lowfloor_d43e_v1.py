...
ROUTE_Q512_K2_S4 = f'{seed_lowk.ROUTE_PREFIX}:q512_lowk_s{Q512_SPLIT_COUNT}'
ROUTE_Q4096_K8_S4 = seed_c3bf.ROUTE_Q4096_K8_S4
ROUTE_TAIL_K20_S4 = seed_b3ec.ROUTE_TAIL_K20_S4
PRODUCTION_ROUTE_MODULES = {SEED_ID: ROUTE_ENTRYPOINT, SEED_Q512_K2_ID: ROUTE_Q512_K2_S4, SEED_Q4096_K8_ID: ROUTE_Q4096_K8_S4, SEED_MIDK_ID: f'{seed_midk.MODULE}:launch_from_contract_inputs', SEED_TAIL_K20_ID: ROUTE_TAIL_K20_S4, BASELINE_1877_ID: baseline_1877.ROUTE_ENTRYPOINT}
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_build_lowfloor_d43e_v1:ir"}'))
...
def _selected_seed_for_inputs(inputs: dict[str, Any]) -> tuple[str | None, str | None]:
    ...
    if _eligible_midk(inputs):
        return (SEED_MIDK_ID, str(inputs.get('label') or f'q{inputs.get('Q')}_k{inputs.get('K')}'))
...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for label in tuple(shape_labels):
        ...
        rows.append(baseline_1877._normalize_route_row({'shape_key': label, 'selected_route': route, 'selected_entrypoint': route if selected_seed != SEED_MIDK_ID else f'{seed_midk.MODULE}:launch_from_contract_inputs', 'selected_seed': selected_seed, 'expected_seed': selected_seed, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'd43e_build_lowfloor_{selected_seed}', 'guard_condition': guard_conditions[selected_seed], 'matched_label': matched_label, 'baseline_1877_route': baseline_route, 'classification': 'seed-consumed'}))
    return rows
