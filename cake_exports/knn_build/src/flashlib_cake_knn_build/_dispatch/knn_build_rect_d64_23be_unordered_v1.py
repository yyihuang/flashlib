"""D64 rectangular search seed with unordered split-local producer state.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only ``search_rect_b1_q1024_m32768_d64_k10`` through the existing D64
TMA/tcgen05 split producer shape from 73a9 and a rectangular-D64 cached sorted
split merge with 8-thread merge CTAs. The producer keeps unordered worst-slot
top-10 state while scanning each split, then emits each split-local partial row
sorted for the existing cached cursor merge. Guard misses delegate to the
...
from . import knn_build_rect_d64_cf49_v3 as parent_v3
MODULE = 'loom.examples.weave.knn_build_rect_d64_23be_unordered_v1'
SEED_ID = 'rect_d64_23be_unordered_v1'
...
knn_build_rect_d64_23be_unordered_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rect_d64_23be_unordered_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 25856, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
stage1_d64_split_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rect_d64_23be_unordered_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 25856, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
knn_build_rect_d64_23be_s16_cached_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_rect_d64_23be_s16_cached_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 16]], "cta_group": 1, "threads": 8}'))
merge_s16_cached_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rect_d64_23be_s16_cached_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 16]], "cta_group": 1, "threads": 8}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rect_d64_23be_unordered_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 25856, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _rect_split_count() -> int:
    override = os.environ.get('LOOM_KNN_RECT_D64_23BE_SPLITS')
    if override is None:
        return DEFAULT_SPLIT_COUNT
    split_count = int(override)
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(f'LOOM_KNN_RECT_D64_23BE_SPLITS must be one of {SUPPORTED_SPLITS}')
    return split_count

def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible_rect_d64(inputs):
        return f'{ROUTE_RECT_D64}_s{_rect_split_count()}'
...
def _launch_rect_d64(inputs: dict[str, Any]) -> None:
    ...
    stage1_kernel = _compiled_d64_unordered_stage1()
    stage1_kernel.launch(grid=(stage1_grid, 1, 1), block=(THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_d64_split_ir.computed_smem_bytes)
    if split_count == DEFAULT_SPLIT_COUNT:
...
def benchmark_knn_build_rect_d64_23be_unordered_v1(*, use_cupti: bool=True) -> dict[str, Any]:
    """Targeted contract benchmark for the v9 rectangular D64 bucket."""
    candidate_report = _run_with_timing_backend(candidate, use_cupti=use_cupti)
    parent_report = _run_with_timing_backend(parent_v3.candidate, use_cupti=use_cupti)
