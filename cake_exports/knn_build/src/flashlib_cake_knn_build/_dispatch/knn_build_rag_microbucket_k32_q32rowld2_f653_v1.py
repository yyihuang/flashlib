...
ROUTE_Q32_ROWLD2_ENTRYPOINT = ''.join([format(MODULE, ''), ':launch_from_contract_inputs'])
...
def _warp_merge_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=''.join(['k32q32s', format(split_count, ''), 'r', format(K32_ROWS4_ROWS_PER_CTA, ''), '_f653_v1']), TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)
...
def _q32_rowld2_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return ''.join(['rag_microbucket_k32_q32rowld2_f653_v1_q', format(n_query, ''), '_m', format(n_database, ''), '_k32_row16x256b2warp_s', format(split_count, ''), '_r', format(K32_ROWS4_ROWS_PER_CTA, ''), '_warpmerge'])
...
def _launch_q32_rowld2_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(''.join(['k32 q32 rowld2 only supports K=', format(K32_TOP_K_MAX, ''), ', got K=', format(top_k, '')]))
