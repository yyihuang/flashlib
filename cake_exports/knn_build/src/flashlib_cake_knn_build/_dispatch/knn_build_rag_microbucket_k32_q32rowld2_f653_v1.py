"""RAG microbatch K32 Q32 rowld2 exact bucket wrapper.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
targets the exact v11 `rag_microbatch_largek_b1_q32_m100000_d128_k32` floor
row. It reuses the existing ROW_16x256B rowld2 tcgen05/TMA producer with
`ROWS_COVERED=32`, feeds the rows4 warp-row merge, and delegates every guard
miss to the current 2e8e K32 parent route. The production path remains
Weave-only.
"""
...
MODULE = 'loom.examples.weave.knn_build_rag_microbucket_k32_q32rowld2_f653_v1'
Q32_K32_SHAPE = parent.Q32_K32_SHAPE
Q32_ROWLD2_TARGET_SHAPES = (Q32_K32_SHAPE,)
K32_BUCKET_SHAPES = Q32_ROWLD2_TARGET_SHAPES
TARGET_SHAPES = Q32_ROWLD2_TARGET_SHAPES
K32_Q32_SPLIT_COUNT = _decode_capture(_json_loads('145'))
K32_TOP_K_MAX = parent.parent.rows4.K32_TOP_K_MAX
K32_ROWS4_MERGE_THREADS = parent.parent.rows4.K32_ROWS4_MERGE_THREADS
K32_ROWS4_ROWS_PER_CTA = parent.parent.rows4.K32_ROWS4_ROWS_PER_CTA
K32_ROWS4_WARPS = parent.parent.rows4.K32_ROWS4_WARPS
Q32_ROWLD2_STAGE1_THREADS = _decode_capture(_json_loads('128'))
Q32_ROWLD2_ACTIVE_ROWS = 32
ROUTE_PARENT_2E8E = f'{parent.MODULE}:launch_from_contract_inputs'
ROUTE_Q32_ROWLD2_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
SEED_K32_Q32_ROWLD2_F653_V1_ID = 'rag_microbucket_k32_q32rowld2_f653_v1_rowld2_rows4'

def _ir_with_constants(ir_obj: Any, *, suffix: str, **updates: int) -> Any:
    constants = tuple(((name, updates.get(name, value)) for name, value in ir_obj.constants))
    return replace(ir_obj, name=f'{ir_obj.name}_{suffix}', constants=constants)

def _stage1_q32_rowld2_ir() -> Any:
    return _ir_with_constants(rowld2_seed.stage1_q32_k32_m64_rowld2_ir, suffix='q32rowld2_f653_v1', BLOCK_Q=rowld2_seed.Q16_ROWLD1_BLOCK_Q, BLOCK_M=rowld2_seed.Q16_ROWLD1_BLOCK_M, FEAT_D=rowld2_seed.Q16_ROWLD1_FEAT_D, TOP_K_MAX=K32_TOP_K_MAX, ROWS_COVERED=Q32_ROWLD2_ACTIVE_ROWS)
...
def _warp_merge_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=f'k32q32s{split_count}r{K32_ROWS4_ROWS_PER_CTA}_f653_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_K32_Q32ROWLD2_F653_V1_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_K32_Q32ROWLD2_F653_V1_VERIFY_K32_SPLIT', K32_Q32_SPLIT_COUNT))
    if verify_kernel == 'rowld2_stage1':
        return _stage1_q32_rowld2_ir()
    return _warp_merge_ir(split_count)
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge_k32q32s145r4_f653_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 145], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))

def _compiled_stage1_q32_rowld2():
...
def _eligible_q32_rowld2(inputs: dict[str, Any]) -> bool:
    return base._is_bf16_d128_nonbuild(inputs) and int(inputs.get('Q', -1)) == 32 and (int(inputs.get('M', -1)) == 100000) and (int(inputs.get('K', -1)) == 32)

def _q32_rowld2_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_k32_q32rowld2_f653_v1_q{n_query}_m{n_database}_k32_row16x256b2warp_s{split_count}_r{K32_ROWS4_ROWS_PER_CTA}_warpmerge'

def route_for_contract_inputs(inputs: dict[str, Any], *, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> str:
    if _eligible_q32_rowld2(inputs):
        return _q32_rowld2_route_name(inputs, split_count=k32_q32_split_count)
    return parent.route_for_contract_inputs(inputs)

def _launch_q32_rowld2_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    query = inputs['query']
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(f'k32 q32 rowld2 only supports K={K32_TOP_K_MAX}, got K={top_k}')
    ...
    stage1_ir = _stage1_q32_rowld2_ir()
    _compiled_stage1_q32_rowld2().launch(grid=(stage1_grid, 1, 1), block=(Q32_ROWLD2_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> None:
    if _eligible_q32_rowld2(inputs):
        _launch_q32_rowld2_rows4_merge(inputs, split_count=k32_q32_split_count)
        return
...
def benchmark_knn_build_rag_microbucket_k32_q32rowld2_f653_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_q32_split_count: int=K32_Q32_SPLIT_COUNT) -> dict[str, Any]:
