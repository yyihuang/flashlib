...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_common_d_5e7f_rag_d64_d256_v1'
...
def _stage1_ir(feature_chunks: int) -> Any:
    return _ir_with_constants(m64_parent.stage1_m64_ir, suffix=f'd{int(feature_chunks) * K_TILE}_5e7f_rag_d64d256_v1', FEATURE_CHUNKS=int(feature_chunks))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return f'{ROUTE_PREFIX}:{label}:d{int(spec['D'])}:q{int(spec['Q'])}:m{int(spec['M'])}:{('d64m64' if _uses_d64_exact(label) else 'm64n64')}:s{_split_count_for_label(label)}:g{_group_count_for_label(label)}:chunks{_feature_chunks_for_label(label)}'
...
def _launch_d64_exact_rag(inputs: dict[str, Any], label: str) -> None:
    ...
    _compiled_d64_m64_stage1().launch(grid=(stage1_grid, 1, 1), block=(M64_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=d64_m64_stage1_ir.computed_smem_bytes)
