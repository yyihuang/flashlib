...
stage1_q512_lowk_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:stage1_q512_lowk_ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
merge_q512_generic_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:merge_q512_generic_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
stage1_q1024_k16_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:stage1_q1024_k16_ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
merge_q1024_k16_s4_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:merge_q1024_k16_s4_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
merge_q1024_k16_s8_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:merge_q1024_k16_s8_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
merge_q1024_k16_s16_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:merge_q1024_k16_s16_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
...
def _verify_export_ir() -> Any:
    ...
    return stage1_q512_lowk_ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_lowk_f8c3_q512_q1024_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))

def _check_q512_split_count(split_count: int) -> int:
    split_count = int(split_count)
    if split_count not in Q512_SPLIT_CHOICES:
        raise ValueError(f'unsupported q512 low-K split count: {split_count}')
    return split_count

def _check_q1024_k16_split_count(split_count: int) -> int:
    split_count = int(split_count)
    if split_count not in Q1024_K16_SPLIT_CHOICES:
        raise ValueError(f'unsupported q1024 K16 split count: {split_count}')
    return split_count
...
def _route_q512(split_count: int) -> str:
    return f'{ROUTE_PREFIX}:q512_lowk_s{_check_q512_split_count(split_count)}'

def _route_q1024_k16(split_count: int) -> str:
    return f'{ROUTE_PREFIX}:q1024_k16_s{_check_q1024_k16_split_count(split_count)}'
...
def _guard_description(route: str) -> str:
    if route.startswith(f'{ROUTE_PREFIX}:q512_lowk_s'):
        return f'exact BF16 build B1 Q=M=512 D128 K in {{1,2}} low-K split{route.rsplit('s', 1)[-1]} route'
    if route.startswith(f'{ROUTE_PREFIX}:q1024_k16_s'):
        return f'exact BF16 build B1 Q=M=1024 D128 K16 split{route.rsplit('s', 1)[-1]} route'
