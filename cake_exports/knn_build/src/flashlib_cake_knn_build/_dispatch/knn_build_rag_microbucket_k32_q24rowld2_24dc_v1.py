...
ROUTE_PARENT_BDD2 = f'{parent.MODULE}:launch_from_contract_inputs'
ROUTE_Q24_ROWLD2_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
...
def _warp_merge_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(base.k32_warp_row_merge_ir, suffix=f'k32q24s{split_count}r{K32_ROWS4_ROWS_PER_CTA}_24dc_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=base._splits_per_lane(split_count), ROWS_PER_CTA=K32_ROWS4_ROWS_PER_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge_k32q24s144r4_24dc_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 144], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))
...
def _q24_rowld2_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return f'rag_microbucket_k32_q24rowld2_24dc_v1_q{n_query}_m{n_database}_k32_row16x256b2warp_s{split_count}_r{K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def _launch_q24_rowld2_rows4_merge(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(f'k32 q24 rowld2 only supports K={K32_TOP_K_MAX}, got K={top_k}')
