"""RAG frontier bucket seed with sort4/early-stop K32 stage-1 top-k maintenance.
...
top-32 with four 8-slot worst caches, sorts only admitted four-candidate groups,
stops the group visit once the sorted distance no longer beats the live worst,
and emits a sorted stream once per split. Guard misses delegate to the current
exported split72/de1a dispatcher.
"""
...
knn_build_rag_frontier_b6d4_stage1_k32_sort4earlystop = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_frontier_b6d4_stage1_k32_sort4earlystop", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
stage1_k32_sort4earlystop_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_frontier_b6d4_stage1_k32_sort4earlystop", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_FRONTIER_B6D4_V5_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_FRONTIER_B6D4_V5_VERIFY_SPLIT', K32_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_RAG_FRONTIER_B6D4_V5_VERIFY_GROUPS', K32_GROUP_COUNT))
    ...
    return stage1_k32_sort4earlystop_ir
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_frontier_b6d4_stage1_k32_sort4earlystop", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    ...
    if _eligible_k32_rag_frontier(inputs):
        return f'rag_frontier_k32_s{K32_SPLIT_COUNT}_g{K32_GROUP_COUNT}_sort4earlystopstage'
...
def benchmark_knn_build_rag_frontier_b6d4_v5(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
