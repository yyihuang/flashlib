...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible_q1_topology(inputs):
        split_count, group_count = _topology_for_inputs(inputs)
        return f'rag_online_mbucket_99fd_q1_m{int(inputs['M'])}_s{split_count}_g{group_count}'
...
def write_benchmark_artifacts(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=LARGE_M_TARGET_SHAPES) -> dict[str, Any]:
    ...
    for name, payload in payloads.items():
        path = out_dir / f'q1m250m262_s144_{name}_{len(tuple(shape_labels))}row_{suffix}.json'
    ...
    summary_path = out_dir / f'q1m250m262_s144_summary_{len(tuple(shape_labels))}row_{suffix}.json'
