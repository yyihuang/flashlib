...
BENCHMARK_ENTRYPOINT = ''.join([format(MODULE, ''), ':benchmark_knn_build_common_d_5e7f_rag_d64_repair_v1'])
...
def _compiled_stage1():
    return _decode_capture(_json_loads('{"__kernel__": "dispatch_kernel_0229"}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return ''.join([format(ROUTE_PREFIX, ''), ':', format(label, ''), ':d', format(int(spec['D']), ''), ':q', format(int(spec['Q']), ''), ':m', format(int(spec['M']), ''), ':m64n64k64:s', format(_split_count_for_label(label), ''), ':g', format(_group_count_for_label(label), '')])
