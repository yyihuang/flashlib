...
def _select_split_count(n_query: int, *, override: int | None=None) -> int:
    ...
    if num_db_tiles % split_count != 0:
        raise ValueError(f'unsafe exact K96 split_count {split_count}: {num_db_tiles} database tiles would leave a tail')
    return split_count
...
def _exactall_route_name(*, n_query: int, split_count: int) -> str:
    return f'knn_build_over64_k96_exactall_229a_v1_q{n_query}_k96_exactprefill_s{split_count}'
