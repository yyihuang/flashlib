...
CANDIDATE_ENTRYPOINT = f'{MODULE}:benchmark_candidate_17b8_lowmargin_1074_full82_v1'
ROUTE_BASE_17B8_ENTRYPOINT = f'{base99fd.MODULE}:launch_from_contract_inputs'
eval_mod = base99fd.eval_mod
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_17b8_lowmargin_1074_full82_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 91392, "cta_group": 1, "threads": 512}'))
...
def _lowmargin_entrypoint(seed_id: str) -> str:
    ...
    raise ValueError(f'unknown low-margin seed {seed_id!r}')
...
def _lowmargin_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if expected_seed is None:
        raise ValueError(f'shape {label!r} is not eligible for low-margin 1074')
    if force_fallback:
        ...
        row['guard_condition'] = f'forced fallback to selected 17b8/99fd; {expected_seed} disabled'
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, str]:
    ...
    baseline_path = out_dir / f'{denom_label}_same_session_baseline_17b8_99fd_v1.json'
    candidate_path = out_dir / f'{denom_label}_dispatch_17b8_lowmargin_1074_v1.json'
    route_trace_path = out_dir / f'{denom_label}_route_trace_17b8_lowmargin_1074_v1.json'
    forced_trace_path = out_dir / f'{denom_label}_forced_fallback_trace_17b8_lowmargin_1074_v1.json'
    seed_matrix_path = out_dir / f'{denom_label}_seed_delta_matrix_17b8_lowmargin_1074_v1.json'
    consumption_path = out_dir / f'{denom_label}_dispatcher_consumption_17b8_lowmargin_1074_v1.json'
