...
ROUTE_PARENT_BUCKET = f'{parent_bucket.MODULE}:launch_from_contract_inputs'
ROUTE_WARPMERGE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
SEED_K32_WARPMERGE_ID = 'rag_microbucket_k32warpmerge_0077_v1_warp_row_merge'
knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 144], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 1]], "cta_group": 1, "threads": 128}'))
k32_warp_row_merge_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 144], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 1]], "cta_group": 1, "threads": 128}'))
...
def _warp_merge_ir(split_count: int) -> Any:
    if K32_WARP_MERGE_ROWS_PER_CTA <= 0 or K32_WARP_MERGE_ROWS_PER_CTA > K32_WARP_MERGE_WARPS:
        raise ValueError(f'rows_per_cta must be in [1, {K32_WARP_MERGE_WARPS}], got {K32_WARP_MERGE_ROWS_PER_CTA}')
    return _ir_with_constants(k32_warp_row_merge_ir, suffix=f'k32s{split_count}_0077_v1', TOP_K_MAX=K32_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=_splits_per_lane(split_count), ROWS_PER_CTA=K32_WARP_MERGE_ROWS_PER_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_k32warpmerge_0077_v1_warp_row_merge_k32s144_0077_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 32], ["SPLIT_COUNT", 144], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 1]], "cta_group": 1, "threads": 128}'))
...
def _warpmerge_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return f'rag_microbucket_k32warpmerge_0077_v1_q{n_query}_m{n_database}_k32_{producer}_s{split_count}_warpmerge'
...
def _launch_stage1_then_warp_merge(inputs: dict[str, Any], *, split_count: int, stage1_kernel_fn: Callable[[], Any], stage1_ir: Any, stage1_threads: int, block_q: int, block_m: int) -> None:
    ...
    if top_k != K32_TOP_K_MAX:
        raise ValueError(f'k32 warp merge only supports K={K32_TOP_K_MAX}, got K={top_k}')
    ...
    stage1_kernel_fn().launch(grid=(stage1_grid, 1, 1), block=(stage1_threads, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
...
def benchmark_knn_build_rag_microbucket_k32warpmerge_0077_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_k32warpmerge_0077_v1', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'parent_entrypoint': ROUTE_PARENT_BUCKET, 'accelerated_shape_labels': list(K32_BUCKET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q8_Q16_Q32_exact': 'ROW_16x256B/M64N64 stage1 from q32rowld lineage', 'Q16_irregular': 'M64N64 stage1 from 3505_v9 lineage'}, 'merge_topology': {'candidate': f'warp-row split-list merge/{K32_WARP_MERGE_ROWS_PER_CTA} rows per CTA', 'split_count': k32_split_count, 'splits_per_lane': _splits_per_lane(k32_split_count)}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
