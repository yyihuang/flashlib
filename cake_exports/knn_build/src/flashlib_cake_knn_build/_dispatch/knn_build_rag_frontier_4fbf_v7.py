...
def _validate_group_shape(split_count: int, group_count: int) -> None:
    ...
    if group_count > K32_FUSED_MERGE_THREADS:
        raise ValueError(''.join(['group_count=', format(group_count, ''), ' exceeds fused merge threads=', format(K32_FUSED_MERGE_THREADS, '')]))
    if group_count * TOP_K_MAX > K32_FUSED_MERGE_SLOTS:
        raise ValueError(''.join(['group_count=', format(group_count, ''), ' needs ', format(group_count * TOP_K_MAX, ''), ' shared slots, but v7 allocates ', format(K32_FUSED_MERGE_SLOTS, '')]))
...
def _fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(fused_merge_generalized_ir, suffix=''.join(['k32s', format(split_count, ''), 'g', format(group_count, ''), '_4fbf_v7']), TOP_K_MAX=TOP_K_MAX, GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    ...
    if _eligible_k32_rag_frontier(inputs):
        return ''.join(['rag_frontier_k32_s', format(K32_SPLIT_COUNT, ''), '_g', format(K32_GROUP_COUNT, ''), '_tailinf_fused4fbf_v7'])
