"""RAG frontier bucket seed with tail-infinity K32 stage-1 and group-swept fused merge.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
keeps the v6 tail-infinity K32 sort4/early-stop producer and makes the fused
cooperative K32 merge layout group-count tunable. The retained route still
writes exact contract distances and indices for the RAG frontier bucket.
"""
...
K32_GROUP_COUNT = _decode_capture(_json_loads('24'))
...
knn_build_rag_frontier_4fbf_v7_stage1_k32_sort4earlystop_tailinf = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4fbf_v7:knn_build_rag_frontier_4fbf_v7_stage1_k32_sort4earlystop_tailinf", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
stage1_k32_tailinf_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4fbf_v7:stage1_k32_tailinf_ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
knn_build_rag_frontier_4fbf_v7_k32_fused_group_final_merge = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4fbf_v7:knn_build_rag_frontier_4fbf_v7_k32_fused_group_final_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 8192, "cta_group": 1, "threads": 32}'))
fused_merge_generalized_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4fbf_v7:fused_merge_generalized_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 8192, "cta_group": 1, "threads": 32}'))
...
def _validate_group_shape(split_count: int, group_count: int) -> None:
    ...
    if group_count > K32_FUSED_MERGE_THREADS:
        raise ValueError(f'group_count={group_count} exceeds fused merge threads={K32_FUSED_MERGE_THREADS}')
    if group_count * TOP_K_MAX > K32_FUSED_MERGE_SLOTS:
        raise ValueError(f'group_count={group_count} needs {group_count * TOP_K_MAX} shared slots, but v7 allocates {K32_FUSED_MERGE_SLOTS}')
...
def _fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(fused_merge_generalized_ir, suffix=f'k32s{split_count}g{group_count}_4fbf_v7', TOP_K_MAX=TOP_K_MAX, GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_FRONTIER_4FBF_V7_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_FRONTIER_4FBF_V7_VERIFY_SPLIT', K32_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_RAG_FRONTIER_4FBF_V7_VERIFY_GROUPS', K32_GROUP_COUNT))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4fbf_v7:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    ...
    if _eligible_k32_rag_frontier(inputs):
        return f'rag_frontier_k32_s{K32_SPLIT_COUNT}_g{K32_GROUP_COUNT}_tailinf_fused4fbf_v7'
...
def benchmark_knn_build_rag_frontier_4fbf_v7(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    parent_v6_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=parent_v6.candidate_with_k32_topology(parent_v6.K32_SPLIT_COUNT, parent_v6.K32_GROUP_COUNT))
    return _benchmark_payload(candidate_report, baseline_report, parent_v6_report, use_cupti=use_cupti, shape_labels=shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count)
