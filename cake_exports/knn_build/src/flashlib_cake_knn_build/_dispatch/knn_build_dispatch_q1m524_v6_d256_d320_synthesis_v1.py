...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_dispatch_q1m524_v6_d256_d320_synthesis_v1'
...
def _portfolio_name(portfolio_name: str) -> str:
    if portfolio_name not in {'base', 'd256', 'd256_d320'}:
        raise ValueError(f'unknown portfolio {portfolio_name!r}')
...
def write_checkpointed_full_benchmark_artifacts(artifact_dir: str | Path, *, portfolio_name: str='d256_d320', use_cupti: bool=True, shape_labels=None, max_new_rows: int | None=None) -> dict[str, Any]:
    ...
    checkpoint_path = out_dir / f'full{len(labels)}_{name}_progress.json'
    audit = ResumableRowAudit(checkpoint_path, audit_id=f'{CANDIDATE_ID}:{name}:paired-v1', labels=labels, metadata={'contract_version': eval_mod.CONTRACT.contract_version, 'timing_backend': 'cupti' if use_cupti else 'cuda_event', 'candidate_entrypoint': ROUTE_ENTRYPOINT, 'baseline_entrypoint': BASELINE_ENTRYPOINT})
    ...
    denom = f'full{len(labels)}_{name}'
    candidate_path = out_dir / f'{denom}_candidate.json'
    baseline_path = out_dir / f'{denom}_v5_same_session_baseline.json'
    trace_path = out_dir / f'{denom}_route_trace.json'
