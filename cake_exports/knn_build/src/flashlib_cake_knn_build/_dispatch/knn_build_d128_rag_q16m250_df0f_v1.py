...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_d128_rag_q16m250_df0f_v1'
SPEEDUP_FLOOR = 1.2
...
def benchmark_knn_build_d128_rag_q16m250_df0f_v1(*, use_cupti: bool=True, shape_labels=None, baseline_report: dict[str, Any] | None=None, benchmark_correctness: bool=True, time_flashlib: bool=True, speedup_floor: float=SPEEDUP_FLOOR) -> dict[str, Any]:
    ...
    denominator = 'df0f_q16m250_exact1' if labels == TARGET_SHAPES else f'custom_{len(labels)}'
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, benchmark_correctness: bool=True, time_flashlib: bool=True, speedup_floor: float=SPEEDUP_FLOOR) -> dict[str, str]:
    ...
    labels = _shape_labels(shape_labels)
    denom_label = 'df0f_q16m250_exact1' if labels == TARGET_SHAPES else f'custom_{len(labels)}'
    baseline_report = benchmark_parent_v11(use_cupti=use_cupti, shape_labels=labels, benchmark_correctness=benchmark_correctness, time_flashlib=time_flashlib)
    ...
    baseline_path = out_dir / f'{denom_label}_same_session_parent_v11.json'
    payload_path = out_dir / f'{denom_label}_dispatch_df0f_q16m250_s288_v1.json'
    trace_path = out_dir / f'{denom_label}_route_trace_df0f_q16m250_s288_v1.json'
    forced_trace_path = out_dir / f'{denom_label}_forced_fallback_trace_df0f_q16m250_s288_v1.json'
    seed_matrix_path = out_dir / f'{denom_label}_seed_delta_matrix_df0f_q16m250_s288_v1.json'
