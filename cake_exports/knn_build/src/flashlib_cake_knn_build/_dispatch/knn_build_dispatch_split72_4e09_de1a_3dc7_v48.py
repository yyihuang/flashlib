...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_split72_4e09_de1a_3dc7_v48:ir"}'))
...
def _route_trace_record(inputs: dict[str, Any], *, k20_route: str='de1a', force_fallback: bool=False) -> dict[str, Any]:
    ...
    if route in {ROUTE_K20_DE1A, ROUTE_K20_08EC}:
        ...
        return {'shape_key': inputs.get('label'), 'selected_route': route, 'guard_condition': 'exact BF16 D128 non-build K20 large/rectangular label', 'route_kind': 'specialized', 'coverage': f'exact {seed} K20 route', 'consumed_seed': seed, 'fallback': ROUTE_PREVIOUS_MAIN}
    if route == ROUTE_OVER64_K96:
