...
knn_build_v12_d128_q16_k48_dd2b_v1_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_v12_d128_q16_k48_dd2b_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 60672, "constants": [["BLOCK_Q_CONST", 64], ["BLOCK_M_CONST", 64], ["FEAT_D_CONST", 128], ["TOP_K_MAX", 48], ["ROWS_COVERED_CONST", 16]], "cta_group": 1, "threads": 128}'))
stage1_k48_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_v12_d128_q16_k48_dd2b_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 60672, "constants": [["BLOCK_Q_CONST", 64], ["BLOCK_M_CONST", 64], ["FEAT_D_CONST", 128], ["TOP_K_MAX", 48], ["ROWS_COVERED_CONST", 16]], "cta_group": 1, "threads": 128}'))
...
def _warp_merge_ir(split_count: int) -> Any:
    if K48_ROWS_PER_CTA <= 0 or K48_ROWS_PER_CTA > K48_MERGE_WARPS:
        raise ValueError(f'rows_per_cta={K48_ROWS_PER_CTA} exceeds merge warps={K48_MERGE_WARPS}')
    return _ir_with_constants(warpmerge.k32_warp_row_merge_ir, suffix=f'k48s{split_count}r{K48_ROWS_PER_CTA}_dd2b_v1', TOP_K_MAX=K48_TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=warpmerge._splits_per_lane(split_count), ROWS_PER_CTA=K48_ROWS_PER_CTA)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_v12_d128_q16_k48_dd2b_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 60672, "constants": [["BLOCK_Q_CONST", 64], ["BLOCK_M_CONST", 64], ["FEAT_D_CONST", 128], ["TOP_K_MAX", 48], ["ROWS_COVERED_CONST", 16]], "cta_group": 1, "threads": 128}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if _eligible_k48(inputs) and (not force_fallback):
        return f'{ROUTE_PREFIX}:{TARGET_SHAPE}:q16:m100000:d128:k48:row16x256b_2cw:s{K48_SPLIT_COUNT}:r{K48_ROWS_PER_CTA}'
...
def _launch_k48(inputs: dict[str, Any], *, split_count: int=K48_SPLIT_COUNT) -> None:
    ...
    if top_k != K48_TOP_K_MAX:
        raise ValueError(f'dd2b K48 seed supports K={K48_TOP_K_MAX}, got K={top_k}')
    ...
    _compiled_stage1_k48().launch(grid=(stage1_grid, 1, 1), block=(K48_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_k48_ir.computed_smem_bytes)
