...
stage1_k32_tailinf_cta1_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_5093_v1_stage1_k32_tailinf_cta1_compactwarp", "arg_keys": ["tmap_query", "tmap_database", "query_sq", "database_sq", "partial_dists", "partial_indices", "B", "Q", "M", "K", "num_q_tiles", "db_tiles_per_split", "split_count", "total_work"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 96}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> str:
    ...
    if _eligible_q16_k32(inputs):
        return ''.join(['rag_microbucket_5093_v1_q16_k32_tailinf_cta1_cw1_s', format(k32_split_count, ''), '_g', format(k32_group_count, '')])
...
def _target_rows(candidate_report: dict[str, Any], parent_report: dict[str, Any], faeb_report: dict[str, Any], base_report: dict[str, Any], *, k32_split_count: int, k32_group_count: int) -> dict[str, Any]:
    ...
    for label in TARGET_SHAPES:
        ...
        route = ''.join(['rag_microbucket_5093_v1_q16_k32_tailinf_cta1_cw1_s', format(k32_split_count, ''), '_g', format(k32_group_count, '')]) if label == Q16_K32_SHAPE else ROUTE_Q4_K10 if label == Q4_K10_SHAPE else ROUTE_Q64_K10
...
def benchmark_knn_build_rag_microbucket_5093_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    payload['producer_topology']['Q16_K32'] = ''.join(['tailinf-cta1/compactwarp/S', format(k32_split_count, ''), '/G', format(k32_group_count, ''), '/fused'])
