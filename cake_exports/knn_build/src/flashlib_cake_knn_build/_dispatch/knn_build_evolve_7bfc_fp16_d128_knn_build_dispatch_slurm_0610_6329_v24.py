...
knn_build_evolve_7bfc_fp16_d128_base = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_fp16_d128_base", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50176, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))

def _verify_export_ir() -> Any:
    return knn_build_evolve_7bfc_fp16_d128_base
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_fp16_d128_base", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50176, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _create_tensor_map_3d_fp16_oob_zero(data_ptr: int, global_height: int, shared_height: int, width: int, block_width: int):
    ...
    if cached is not None:
        return cached
    err, tmap = driver.cuTensorMapEncodeTiled(driver.CUtensorMapDataType.CU_TENSOR_MAP_DATA_TYPE_FLOAT16, 3, data_ptr, [driver.cuuint64_t(64), driver.cuuint64_t(global_height), driver.cuuint64_t(width // 64)], [driver.cuuint64_t(width * 2), driver.cuuint64_t(128)], [driver.cuuint32_t(64), driver.cuuint32_t(shared_height), driver.cuuint32_t(block_width // 64)], [driver.cuuint32_t(1), driver.cuuint32_t(1), driver.cuuint32_t(1)], driver.CUtensorMapInterleave.CU_TENSOR_MAP_INTERLEAVE_NONE, driver.CUtensorMapSwizzle.CU_TENSOR_MAP_SWIZZLE_128B, driver.CUtensorMapL2promotion.CU_TENSOR_MAP_L2_PROMOTION_NONE, driver.CUtensorMapFloatOOBfill.CU_TENSOR_MAP_FLOAT_OOB_FILL_NAN_REQUEST_ZERO_FMA)
    if err != 0:
        raise RuntimeError(f'cuTensorMapEncodeTiled (3D FP16, OOB zero) failed: CUresult={err}')
...
def _launch_fp16_d128(inputs: dict[str, Any]) -> None:
    ...
    kernel.launch(grid=(grid_dim, 1, 1), block=(THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], inputs['out_dists'], inputs['out_indices'], tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, num_db_tiles, total_tiles], shared_mem=ir.computed_smem_bytes)
