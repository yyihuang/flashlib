...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_rag_stream_k32_q128_s72r2_a162_v1'
...
def _merge_ir(split_count: int=SPLIT_COUNT, rows_per_cta: int=ROWS_PER_CTA) -> Any:
    if rows_per_cta <= 0 or rows_per_cta > merge_base.K32_WARP_MERGE_WARPS:
        raise ValueError(f'rows_per_cta must be in [1, {merge_base.K32_WARP_MERGE_WARPS}], got {rows_per_cta}')
    return merge_base._ir_with_constants(merge_base.k32_warp_row_merge_ir, suffix=f'q128s{split_count}r{rows_per_cta}_a162_v1', TOP_K_MAX=TOP_K_MAX, SPLIT_COUNT=split_count, SPLITS_PER_LANE=merge_base._splits_per_lane(split_count), ROWS_PER_CTA=rows_per_cta)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32rowld_e5db_v1_stage1_q32_k32_m64_q128rowld_60fb_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def _route_name(inputs: dict[str, Any], *, split_count: int, rows_per_cta: int) -> str:
    return f'rag_stream_k32_q128_m{int(inputs['M'])}_a162_s{split_count}_r{rows_per_cta}_rowld_warpmerge'
...
def _launch_q128_stream_s72r2(inputs: dict[str, Any], *, split_count: int, rows_per_cta: int) -> None:
    ...
    top_k = int(inputs['K'])
    if top_k != TOP_K_MAX:
        raise ValueError(f'q128 stream rowld route only supports K={TOP_K_MAX}, got K={top_k}')
    ...
    _compiled_stage1().launch(grid=(stage1_grid, 1, 1), block=(rowld_seed.Q128_ROWLD_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
