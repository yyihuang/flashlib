...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_784a_6bc3_k8_selective_full82_v1:ir"}'))
...
BASE_784A_ENTRYPOINT = f'{MODULE}:benchmark_baseline_784a_005f'
BASE_784A_ROUTE_ENTRYPOINT = f'{base784a.MODULE}:launch_from_contract_inputs'
ROUTE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs'
CANDIDATE_6BC3_K8_ENTRYPOINT = f'{MODULE}:benchmark_candidate_784a_plus_6bc3_k8_selective_full82_v1'
...
if len(FULL82_V9_LABELS) != 82:
    raise RuntimeError(f'expected 82 full82_v9 labels, found {len(FULL82_V9_LABELS)}')
PRODUCTION_ROUTE_MODULES = {**base784a.PRODUCTION_ROUTE_MODULES, **seed6bc3.PRODUCTION_ROUTE_MODULES, BASE_784A_ID: BASE_784A_ROUTE_ENTRYPOINT, seed6bc3.CANDIDATE_ID: f'{seed6bc3.MODULE}:launch_from_contract_inputs'}
...
def _candidate_config(candidate_key: str) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f'unknown 784a/6bc3 K8 selective candidate {candidate_key!r}') from exc
...
def _candidate_kernel_fn(candidate_key: str) -> Callable[[dict[str, Any]], None]:
    ...
    raise ValueError(f'unknown 784a/6bc3 K8 selective candidate {candidate_key!r}')
...
def _route_trace_record(inputs: dict[str, Any], *, candidate_key: str, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if force_fallback or candidate_key == BASE_784A_KEY or expected_seed is None:
        ...
        if force_fallback and expected_seed is not None:
            ...
            row['guard_condition'] = f'forced fallback to 784a baseline; {expected_seed} disabled'
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, str]:
    ...
    baseline_path = out_dir / f'{denom_label}_same_session_baseline_784a_005f_v1.json'
    ...
    payload_path = out_dir / f'{denom_label}_dispatch_784a_plus_6bc3_k8_selective_v1.json'
    trace_path = out_dir / f'{denom_label}_route_trace_784a_plus_6bc3_k8_selective_v1.json'
    forced_trace_path = out_dir / f'{denom_label}_forced_fallback_trace_784a_plus_6bc3_k8_selective_v1.json'
    seed_matrix_path = out_dir / f'{denom_label}_seed_delta_matrix_784a_plus_6bc3_k8_selective_v1.json'
    ...
    artifacts[f'{CANDIDATE_6BC3_K8}_payload'] = str(payload_path)
    artifacts[f'{CANDIDATE_6BC3_K8}_route_trace'] = str(trace_path)
    artifacts[f'{CANDIDATE_6BC3_K8}_forced_fallback_trace'] = str(forced_trace_path)
    artifacts[f'{CANDIDATE_6BC3_K8}_seed_delta_matrix'] = str(seed_matrix_path)
    ...
    summary_path = out_dir / f'{denom_label}_dispatcher_consumption_784a_6bc3_k8_selective_v1.json'
