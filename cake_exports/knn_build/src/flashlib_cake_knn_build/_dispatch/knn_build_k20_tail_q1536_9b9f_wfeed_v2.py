...
merge_k20_tail_s8_warp8_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k20_mergeown_08ec_warp8_select_q1536s8warp8", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 20], ["SPLIT_COUNT", 8]], "cta_group": 1, "threads": 256}'))
merge_k20_tail_s16_warp8_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k20_mergeown_08ec_warp8_select_q1536s16warp8", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 20], ["SPLIT_COUNT", 16]], "cta_group": 1, "threads": 256}'))
...
def _forced_tail_split_count() -> int | None:
    ...
    if split_count not in (2, 4, *WORKFEED_SPLIT_COUNTS):
        raise ValueError(f'unsupported K20 Q1536 tail split count: {split_count}')
    return split_count
...
def _merge_k20_tail_warp8_ir(split_count: int) -> Any:
    ...
    if split_count == 16:
        return merge_k20_tail_s16_warp8_ir
    raise ValueError(f'unsupported K20 Q1536 tail warp8 split count: {split_count}')
