"""RAG microbatch K10 bucket seed with single-CTA producer.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only BF16 non-build ``B=1,Q in {8,16,32},M=100000,D=128,K=10`` rows.
It specializes the inherited tcgen05/TMA K10 split producer for low-Q rows by
using a single-CTA MMA stage with a higher split count, then reuses the fused
cooperative group/final merge from the round-20 seed. Guard misses delegate to
the 4a72 selected dispatcher; no external runtime fallback is used.
"""
...
from . import knn_build_rag_microbatch_4a72_v1 as parent_v1
...
K10_CANDIDATE_SPLITS = (72, 96, 120, 144)
K10_SPLIT_COUNT = _decode_capture(_json_loads('144'))
K10_GROUP_COUNT = _decode_capture(_json_loads('12'))
...
CTA_GROUP = 1
knn_build_rag_microbatch_4a72_v2_stage1_k10_cta1_maxtree = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_4a72_v2:knn_build_rag_microbatch_4a72_v2_stage1_k10_cta1_maxtree", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
stage1_cta1_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_4a72_v2:stage1_cta1_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
knn_build_rag_microbatch_4a72_v2_k10_fused_group_final_merge = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_4a72_v2:knn_build_rag_microbatch_4a72_v2_k10_fused_group_final_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "cta_group": 1, "threads": 32}'))
fused_merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_4a72_v2:fused_merge_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "cta_group": 1, "threads": 32}'))
...
def _fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(fused_merge_ir, suffix=f's{split_count}g{group_count}_4a72_v2', GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)

def _validate_group_shape(split_count: int, group_count: int) -> None:
    if split_count <= 0 or group_count <= 0:
        raise ValueError(f'split_count and group_count must be positive, got {split_count}, {group_count}')
    if split_count % group_count != 0:
        raise ValueError(f'split_count={split_count} must be divisible by group_count={group_count}')
    if group_count > K10_FUSED_MERGE_THREADS:
        raise ValueError(f'group_count={group_count} exceeds fused merge threads={K10_FUSED_MERGE_THREADS}')
    if group_count * TOP_K_MAX > K10_FUSED_MERGE_SLOTS:
        raise ValueError(f'group_count={group_count} needs {group_count * TOP_K_MAX} shared slots, but the K10 fused merge allocates {K10_FUSED_MERGE_SLOTS}')

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBATCH_4A72_V2_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_MICROBATCH_4A72_V2_VERIFY_SPLIT', K10_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_RAG_MICROBATCH_4A72_V2_VERIFY_GROUPS', K10_GROUP_COUNT))
    if verify_kernel == 'stage1':
        return stage1_cta1_ir
    return _fused_merge_ir(split_count, group_count)
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_4a72_v2:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "cta_group": 1, "threads": 32}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, split_count: int=K10_SPLIT_COUNT, group_count: int=K10_GROUP_COUNT) -> str:
    if _eligible_rag_microbatch(inputs):
        return f'rag_microbatch_4a72_v2_cta1_k10_s{split_count}_g{group_count}_fusedmerge'
...
def parent_v1_fused_candidate(inputs: dict[str, Any]):
    if _eligible_rag_microbatch(inputs):
        parent_v1.launch_from_contract_inputs(inputs)
        return None
    current_dispatcher.candidate(inputs)
    return None
...
def benchmark_knn_build_rag_microbatch_4a72_v2(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, split_count: int=K10_SPLIT_COUNT, group_count: int=K10_GROUP_COUNT) -> dict[str, Any]:
    ...
    parent_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=parent_v1_fused_candidate)
    rowbase_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=parent_7399_forced_rowbase_candidate)
    ...
    return _benchmark_payload(candidate_report, parent_report, rowbase_report, current_report, use_cupti=use_cupti, shape_labels=shape_labels, split_count=split_count, group_count=group_count)
...
def benchmark_split_sweep(*, use_cupti: bool=True, split_counts=K10_CANDIDATE_SPLITS, group_count: int=K10_GROUP_COUNT) -> dict[str, Any]:
    ...
    for split_count in split_counts:
        rows[f's{split_count}_g{group_count}'] = benchmark_knn_build_rag_microbatch_4a72_v2(use_cupti=use_cupti, split_count=int(split_count), group_count=group_count)
