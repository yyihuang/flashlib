...
def _fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(fused_merge_ir, suffix=''.join(['s', format(split_count, ''), 'g', format(group_count, ''), '_4a72_v2']), GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, split_count: int=K10_SPLIT_COUNT, group_count: int=K10_GROUP_COUNT) -> str:
    if _eligible_rag_microbatch(inputs):
        return ''.join(['rag_microbatch_4a72_v2_cta1_k10_s', format(split_count, ''), '_g', format(group_count, ''), '_fusedmerge'])
...
def benchmark_split_sweep(*, use_cupti: bool=True, split_counts=K10_CANDIDATE_SPLITS, group_count: int=K10_GROUP_COUNT) -> dict[str, Any]:
    ...
    for split_count in split_counts:
        rows[''.join(['s', format(split_count, ''), '_g', format(group_count, '')])] = benchmark_knn_build_rag_microbatch_4a72_v2(use_cupti=use_cupti, split_count=int(split_count), group_count=group_count)
