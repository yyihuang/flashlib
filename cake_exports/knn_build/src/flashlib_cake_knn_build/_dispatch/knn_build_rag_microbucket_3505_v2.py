"""RAG microbucket Q4/Q64 K10 plus Q16 K32 cta1 tail-infinity seed.
...
tail-infinity K32 tcgen05/TMA producer. The producer keeps the 4fbf top-k
logic but removes the second invalid CTA from the inherited Q128/2-CTA
frontier stage. Guard misses delegate to the current 4247 dispatcher.
"""
...
ROUTE_Q4_K10 = 'rag_microbucket_3505_v2_q4_k10_m64_s128_g8'
ROUTE_Q64_K10 = 'rag_microbucket_3505_v2_q64_k10_m64_s128_g8'
ROUTE_BASE_4247 = parent_3505.ROUTE_BASE_4247
knn_build_rag_microbucket_3505_v2_stage1_k32_tailinf_cta1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_3505_v2_stage1_k32_tailinf_cta1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
stage1_k32_tailinf_cta1_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_3505_v2_stage1_k32_tailinf_cta1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V2_VERIFY_KERNEL')
    split_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V2_VERIFY_K32_SPLIT', K32_SPLIT_COUNT))
    group_count = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_3505_V2_VERIFY_K32_GROUPS', K32_GROUP_COUNT))
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_3505_v2_stage1_k32_tailinf_cta1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def _launch_q16_k32_tailinf_cta1(inputs: dict[str, Any], *, split_count: int=K32_SPLIT_COUNT, group_count: int=K32_GROUP_COUNT) -> None:
    ...
    _compiled_stage1_tailinf_cta1().launch(grid=(stage1_grid, 1, 1), block=(STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_k32_tailinf_cta1_ir.computed_smem_bytes)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> str:
    ...
    if _eligible_q16_k32(inputs):
        return f'rag_microbucket_3505_v2_q16_k32_tailinf_cta1_s{k32_split_count}_g{k32_group_count}'
...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        specialized = str(route).startswith('rag_microbucket_3505_v2')
...
def _target_rows(candidate_report: dict[str, Any], parent_report: dict[str, Any], faeb_report: dict[str, Any], base_report: dict[str, Any], *, k32_split_count: int, k32_group_count: int) -> dict[str, Any]:
    ...
    for label in TARGET_SHAPES:
        ...
        route = f'rag_microbucket_3505_v2_q16_k32_tailinf_cta1_s{k32_split_count}_g{k32_group_count}' if label == Q16_K32_SHAPE else ROUTE_Q4_K10 if label == Q4_K10_SHAPE else ROUTE_Q64_K10
...
def benchmark_knn_build_rag_microbucket_3505_v2(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    payload = parent_3505._benchmark_payload(candidate_report, faeb_report, base_report, use_cupti=use_cupti, shape_labels=shape_labels, measured_function='benchmark_knn_build_rag_microbucket_3505_v2', k32_split_count=k32_split_count, k32_group_count=k32_group_count)
    payload['measured_entrypoint'] = 'loom.examples.weave.knn_build_rag_microbucket_3505_v2:benchmark_knn_build_rag_microbucket_3505_v2'
    ...
    payload['producer_topology']['Q16_K32'] = f'tailinf-cta1/S{k32_split_count}/G{k32_group_count}/fused'
