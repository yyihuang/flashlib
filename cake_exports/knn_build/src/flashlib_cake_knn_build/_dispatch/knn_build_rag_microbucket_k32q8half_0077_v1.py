...
knn_build_rag_microbucket_k32q8half_0077_v1_stage1_q8_k32_m64_halfrow = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32q8half_0077_v1:knn_build_rag_microbucket_k32q8half_0077_v1_stage1_q8_k32_m64_halfrow", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 42240, "cta_group": 1, "threads": 96}'))
stage1_q8_k32_m64_halfrow_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32q8half_0077_v1:stage1_q8_k32_m64_halfrow_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 42240, "cta_group": 1, "threads": 96}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32q8half_0077_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 42240, "cta_group": 1, "threads": 96}'))
...
def _q8_half_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    ...
    return f'rag_microbucket_k32q8half_0077_v1_q{n_query}_m{n_database}_k32_row16x256bhalf_s{split_count}_warpmerge'
...
def benchmark_knn_build_rag_microbucket_k32q8half_0077_v1(*, use_cupti: bool=True, shape_labels=K32_BUCKET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return {'contract': candidate_report['contract'], 'contract_version': candidate_report['contract_version'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_rag_microbucket_k32q8half_0077_v1', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'parent_entrypoint': ROUTE_PARENT_ROWLD1WARP, 'accelerated_shape_labels': list(Q8_HALFWARP_TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'producer_topology': {'Q8_exact': 'ROW_16x256B/M64N64 half-row one-compute-warp stage1', 'Q16_irregular': 'inherited round-102 ROW_16x256B one-compute-warp stage1', 'Q16_exact': 'inherited 0077 ROW_16x256B four-compute-warp stage1', 'Q32_exact': 'inherited 0077 ROW_16x256B four-compute-warp stage1', 'guard_misses': 'delegate to round-102 rowld1warp parent'}, 'merge_topology': {'candidate': f'0077 warp-row split-list merge/{K32_WARP_MERGE_ROWS_PER_CTA} rows per CTA', 'split_count': k32_split_count, 'splits_per_lane': warpmerge_parent._splits_per_lane(k32_split_count)}, 'route_trace': route_trace_for_contract_shapes(shape_labels, k32_split_count=k32_split_count, k32_group_count=k32_group_count), 'target_rows': _per_shape_delta(candidate_report, parent_report), 'summary': candidate_report['summary'], 'performance': candidate_report['performance'], 'correctness': candidate_report['correctness'], 'report': candidate_report, 'parent_summary': parent_report['summary'], 'parent_performance': parent_report['performance'], 'parent_report': parent_report}
