...
merge_k8_s4_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_k32_merge_s4_rowbase_cache_k8split", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 8]], "cta_group": 1, "threads": 32}'))
...
merge_k8_s16_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_k30_merge_s8_rowbase_cache_195e_q1024k8s16", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 8], ["SPLIT_COUNT", 16]], "cta_group": 1, "threads": 32}'))
...
def _verify_export_ir() -> Any:
    ...
    return stage1_k8_ir
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_cg2_u2_stage1_k8split", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 8]], "cta_group": 1, "threads": 192}'))

def _check_split_count(split_count: int) -> int:
    split_count = int(split_count)
    if split_count not in SPLIT_CHOICES:
        raise ValueError(f'unsupported Q1024/K8 split count: {split_count}')
    return split_count
...
def _launch_q1024_k8_split(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    stage1_kernel.launch_cluster(grid=(stage1_grid, 1, 1), block=(fixed_build.STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tile_pairs, db_tiles_per_split, split_count, total_work], cluster_dims=(fixed_build.CTA_GROUP, 1, 1), shared_mem=stage1_k8_ir.computed_smem_bytes)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, split_count: int=DEFAULT_SPLIT_COUNT) -> str:
    if _eligible_q1024_k8(inputs):
        return f'{ROUTE_PREFIX}:q1024_k8_s{_check_split_count(split_count)}'
...
def benchmark_knn_build_q1024_k8_195e_v1(*, use_cupti: bool=True, split_count: int=DEFAULT_SPLIT_COUNT, run_baseline: bool=True, scan_splits: bool=False) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {'tflops': candidate_report['summary']['primary_mean'] or 0.0, 'all_correct': candidate_report['summary']['all_correct'], 'performance_comparable': candidate_report['summary']['performance_comparable'], 'invalid_performance_reason': candidate_report['summary']['invalid_performance_reason'], 'measured_entrypoint': f'{MODULE}:benchmark_knn_build_q1024_k8_195e_v1', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'measured_shape_labels': TARGET_SHAPES, 'route_trace': route_trace_for_shapes(split_count=split_count), 'contract_summary': candidate_report['summary'], 'contract_performance': candidate_report['performance'], 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'use_cupti': use_cupti, 'split_count': split_count, 'split_scan': _scan_split_counts(use_cupti=use_cupti) if scan_splits else {}, 'shape_dispatch_registry': {'available_shape_kernels': [{'shape_key': TARGET_SHAPES[0], 'guard': 'BF16 build B=1 Q=M=1024 D=128 K=8', 'candidate_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'kernel_ref': CANDIDATE_ID, 'correctness': 'pass' if candidate_report['summary']['all_correct'] else 'fail', 'timing_backend': next((row.get('timing_backend') for row in candidate_report.get('per_shape', {}).values() if row.get('timing_backend')), None), 'benchmark_evidence': f'{MODULE}:benchmark_knn_build_q1024_k8_195e_v1'}]}, 'report': candidate_report}
