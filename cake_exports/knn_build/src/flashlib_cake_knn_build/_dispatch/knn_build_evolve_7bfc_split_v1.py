...
knn_build_evolve_7bfc_split_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
knn_build_evolve_7bfc_split_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10]], "cta_group": 1, "threads": 256}'))
stage1_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _partial_buffers(*, split_count: int, bsz: int, n_query: int, top_k: int, device) -> tuple[Any, Any]:
    ...
    key = (device.type, int(index or 0), int(split_count), int(bsz), int(n_query), int(top_k))
...
def launch_from_contract_inputs(inputs: dict[str, Any]) -> None:
    ...
    stage1_kernel.launch(grid=(stage1_grid, 1, 1), block=(STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
