...
BASELINE_ENTRYPOINT = f'{base_6998.MODULE}:benchmark_candidate_residual_19b3_overlay_6998_v1'
PRODUCTION_ROUTE_MODULES = {**base_6998.PRODUCTION_ROUTE_MODULES, SEED_DIRECT_RAG_K10_ID: ROUTE_DIRECT_RAG_K10_ENTRYPOINT, BASELINE_ID: f'{base_6998.MODULE}:launch_from_contract_inputs'}
CANDIDATE_DISPATCHERS = (*base_6998.CANDIDATE_DISPATCHERS, {'id': 'candidate_6998_ragk10_direct_split72_v1', 'entrypoint': f'{MODULE}:benchmark_candidate_6998_ragk10_direct_split72_v1', 'consumed_seeds': (SEED_DIRECT_RAG_K10_ID,), 'guard_plan': ('exact residual BF16 RAG stream K10 guard', 'direct split72 Weave seed launcher for matching row', 'fall through to 6998 residual 19b3 overlay for every other shape'), 'expected_shape_wins': TARGET_SHAPES, 'fallback': f'{base_6998.MODULE}:launch_from_contract_inputs', 'rejected_reason': None})
...
def _verify_export_ir() -> Any:
    ...
    return base_6998.ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_6998_ragk10_direct_split72_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
