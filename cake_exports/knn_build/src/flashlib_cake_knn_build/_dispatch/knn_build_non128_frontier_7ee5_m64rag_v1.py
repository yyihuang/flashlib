...
SPLIT_DEFAULTS = _decode_capture(_json_loads('{"build_dim_sweep_b1_q1024_m1024_d96_k10": 8, "build_dim_sweep_b1_q2048_m2048_d192_k10": 8, "build_highd_b1_q1024_m1024_d320_k10": 8, "rag_microbatch_highd_b1_q16_m50000_d768_k10": 72, "search_rect_highd_b1_q512_m12000_d320_k10": 32}'))
...
knn_build_non128_frontier_7ee5_m64rag_stage1 = _ir_proxy('loom.examples.weave.knn_build_non128_frontier_7ee5_m64rag_v1:knn_build_non128_frontier_7ee5_m64rag_stage1', 256)
stage1_m64_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_7ee5_m64rag_v1:stage1_m64_ir"}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_7ee5_m64rag_v1:ir"}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if _uses_m64_d768(label):
        ...
        return f'{ROUTE_PREFIX}:{label}:d{int(spec['D'])}:m64n64:s{_split_count_for_label(label)}:chunks{M64_FEATURE_CHUNKS}'
...
def _launch_m64_d768(inputs: dict[str, Any], label: str) -> None:
    ...
    if dim != M64_FEATURE_CHUNKS * K_TILE:
        raise ValueError(f'M64 D768 route expected D={M64_FEATURE_CHUNKS * K_TILE}, got {dim}')
    ...
    _compiled_stage1_m64().launch(grid=(stage1_grid, 1, 1), block=(M64_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_m64_ir.computed_smem_bytes)
...
def route_trace_for_contract_shapes(shape_labels=None, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        rows.append({'shape_key': label, 'selected_route': route, 'selected_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'selected_seed': 'non128_frontier_7ee5_m64rag_v1' if uses_m64 else 'non128_frontier_8199_splitretune_v1', 'expected_seed': 'non128_frontier_7ee5_m64rag_v1' if uses_m64 else 'non128_frontier_8199_splitretune_v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': '7ee5_m64_d768_exact_shape_guard' if uses_m64 else '8199_splitretune_non128_exact_shape_guard', 'guard_condition': f'exact BF16 B={spec['B']} Q={spec['Q']} M={spec['M']} D={spec['D']} K={spec['K']} build={spec['build']}', 'feature_chunks': M64_FEATURE_CHUNKS if uses_m64 else spec['feature_chunks'], 'split_count': _split_count_for_label(label), 'producer_topology': 'M64_N64_row_owned' if uses_m64 else 'parent_8199', 'preprocess_stage': f'd{int(spec['D'])}_weave_pad_to_d{int(spec['feature_chunks']) * K_TILE}' if not uses_m64 and int(spec['D']) != int(spec['feature_chunks']) * K_TILE else None, 'classification': 'm64-d768' if uses_m64 else 'parent-8199'})
    return rows
