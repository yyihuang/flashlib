...
stage1_k64_tailinf_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k64_stage1_tailinf_k64over32tailinfsplitgrid", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 64]], "cta_group": 1, "threads": 192}'))
merge_k64_s8_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k64_merge_s8_unordered_warp_select_k64over32s8warpselect", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 64], ["SPLIT_COUNT", 8]], "cta_group": 1, "threads": 128}'))
merge_k64_s12_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k64_merge_sN_unordered_chunkprefill_k64over32s12chunkprefill", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 64], ["SPLIT_COUNT", 12]], "cta_group": 1, "threads": 32}'))
merge_k64_s16_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k64_merge_sN_unordered_chunkprefill_k64over32s16chunkprefill", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 64], ["SPLIT_COUNT", 16]], "cta_group": 1, "threads": 32}'))
...
def _verify_export_ir() -> Any:
    ...
    return stage1_k64_tailinf_ir
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_k64_stage1_tailinf_k64over32tailinfsplitgrid", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 64]], "cta_group": 1, "threads": 192}'))

def _check_split_count(split_count: int) -> int:
    split_count = int(split_count)
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(f'unsupported q4096 K64 split count: {split_count}')
    return split_count
...
def _split_route_name(split_count: int) -> str:
    return ROUTE_Q4096_K64.replace('split8', f'split{_check_split_count(split_count)}')
...
def _launch_q4096_k64_split(inputs: dict[str, Any], *, split_count: int=DEFAULT_Q4096_K64_SPLITS) -> None:
    ...
    stage1_kernel.launch_cluster(grid=(stage1_grid, 1, 1), block=(k64_seed.STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tile_pairs, db_tiles_per_split, split_count, total_work], cluster_dims=(k64_seed.CTA_GROUP, 1, 1), shared_mem=stage1_ir_obj.computed_smem_bytes)
...
def _guard_description(route: str) -> str:
    if route.startswith(ROUTE_Q4096_K64.rsplit(':', 1)[0]):
        ...
        return f'exact BF16 build B1 Q=M=4096 D128 K64 tail-infinity split{split} route'
