...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_dispatch_v11_common_d_seed_portfolio_a4ec_v1'
BASE_ENTRYPOINT = base_current.ROUTE_ENTRYPOINT
A4EC_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_a4ec_baseline'
FA04_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_fa04_baseline'
F328_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_f328_baseline'
MIXED_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_mixed_baseline'
D64_REPAIR_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_d64_repair_baseline'
D64_M128_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_d64_m128_baseline'
D64_Q4096_C271_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_c271_baseline'
C271_7DC5_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_c271_7dc5_baseline'
PRE_Q128_K10_WARPMERGE_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_q128_k10_warpmerge_baseline'
BASE_2498_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_2498_baseline'
PRE_Q1_M524_N128_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_q1_m524_n128_baseline'
PRE_Q32_EXACT_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_q32exact_baseline'
PRE_Q32TAIL_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_q32tail_baseline'
PRE_EXPANDED_K32_Q48_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_expanded_k32_q48_baseline'
PRE_LOWK_C3D2_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_lowk_c3d2_baseline'
D665_LOWK_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_d665_lowk_baseline'
PRE_D64_TAIL_017A_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_d64_tail_017a_baseline'
PRE_D256_Q4_E2DF_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_d256_q4_e2df_baseline'
PRE_D256_Q128_59FE_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_d256_q128_59fe_baseline'
PRE_D256_K32_59FE_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_d256_k32_59fe_baseline'
PRE_HIGHD_RAG_22E9_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_highd_rag_22e9_baseline'
PRE_HIGHD_SEARCH_BE66_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_highd_search_be66_baseline'
PRE_D128_K48_DD2B_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_d128_k48_dd2b_baseline'
PRE_RECT_D128_K20_S12WARP4_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_rect_d128_k20_s12warp4_baseline'
PRE_Q128_K10_ROWLD_1BED_BASE_ENTRYPOINT = f'{MODULE}:launch_from_contract_inputs_pre_q128_k10_rowld_1bed_baseline'
...
K13_K48_WRAPPER_ENTRYPOINT = f'{K13_K48_WRAPPER_MODULE}:launch_from_contract_inputs'
...
Q128_K10_ROWLD_1BED_ENTRYPOINT = f'{Q128_K10_ROWLD_1BED_MODULE}:launch_from_contract_inputs'
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_v11_common_d_seed_portfolio_a4ec_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def _base_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if label in HIGHD_SEARCH_BE66_CONSUMED_SHAPES and (not force_fallback):
        ...
        row['failure_message'] = f'knn_build_evolve_7bfc_v1 expects D=128, got {int(inputs.get('D', -1))}'
...
def _specialized_trace_record(inputs: dict[str, Any], *, portfolio_id: str) -> dict[str, Any] | None:
    ...
    if portfolio_id in PORTFOLIOS_WITH_SEARCH_D256 and rag_d64d256_label is not None:
        ...
        return _normalize_route_row({'shape_key': rag_d64d256_label, 'selected_route': rag_d64d256.route_for_contract_inputs(inputs), 'selected_entrypoint': RAG_D64D256_ENTRYPOINT, 'selected_seed': seed_id, 'expected_seed': seed_id, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': 'ba22_common_d_5e7f_rag_d64_d256_exact_guard', 'guard_condition': f'exact BF16 non-build B={spec['B']} Q={spec['Q']} M={spec['M']} D={spec['D']} K={spec['K']}', 'classification': 'kernel-slow' if float(timing['ratio_vs_flashlib']) < SPEEDUP_FLOOR else 'seed-consumed', 'feature_chunks': spec['feature_chunks'], 'split_count': rag_d64d256._split_count_for_label(rag_d64d256_label), 'group_count': rag_d64d256._group_count_for_label(rag_d64d256_label), 'producer_topology': 'D64_M64_tcgen05_tma' if rag_d64d256._uses_d64_exact(rag_d64d256_label) else 'M64_N64_tcgen05_tma', 'preprocess_stage': None if rag_d64d256._uses_d64_exact(rag_d64d256_label) else f'd{int(spec['D'])}_weave_pad_to_d{tma_dim}' if int(spec['D']) != tma_dim else None, 'merge_topology': 'fused_group_split_merge', 'replaced_route': replaced_route, 'base_dispatcher_route': base_row.get('selected_route'), 'shape_specific_kernel_ms': timing['kernel_ms'], 'speedup_vs_external_baseline': timing['ratio_vs_flashlib'], 'external_baseline_ms': timing['flashlib_ms'], 'external_baseline_ref': 'historical_same_shape'})
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False, portfolio_id: str=CANDIDATE_DEFAULT) -> dict[str, Any]:
    if force_fallback:
        ...
        if label in CONSUMED_SEED_SHAPES:
            ...
            row['guard_id'] = f'forced_fallback_a4ec_{label}'
...
def write_trace_artifacts(artifact_dir: str | Path, *, shape_labels=None) -> dict[str, str]:
    ...
    route_trace_path = out_dir / f'{denom}_route_trace_v11_common_d_seed_portfolio_a4ec_v1.json'
    forced_trace_path = out_dir / f'{denom}_forced_fallback_trace_v11_common_d_seed_portfolio_a4ec_v1.json'
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    denom = _denominator_label(shape_labels)
    candidate_path = out_dir / f'{denom}_dispatch_v11_common_d_seed_portfolio_a4ec_v1.json'
    baseline_path = out_dir / f'{denom}_same_session_baseline_common_d_v11_for_seed_portfolio_a4ec_v1.json'
    route_trace_path = out_dir / f'{denom}_route_trace_v11_common_d_seed_portfolio_a4ec_v1.json'
    forced_trace_path = out_dir / f'{denom}_forced_fallback_trace_v11_common_d_seed_portfolio_a4ec_v1.json'
    seed_matrix_path = out_dir / f'{denom}_seed_delta_matrix_v11_common_d_seed_portfolio_a4ec_v1.json'
