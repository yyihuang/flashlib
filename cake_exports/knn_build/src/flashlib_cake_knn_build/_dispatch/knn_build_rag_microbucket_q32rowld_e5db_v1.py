...
knn_build_rag_microbucket_q32rowld_e5db_v1_stage1_q32_k32_m64 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32rowld_e5db_v1_stage1_q32_k32_m64", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
stage1_q32_k32_m64_rowld_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32rowld_e5db_v1_stage1_q32_k32_m64", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def _q32_k32_m64_rowld_route_name(*, split_count: int, group_count: int) -> str:
    return f'rag_microbucket_q32rowld_e5db_v1_q32_m100000_k32_m64n64_row16x256b_s{split_count}_g{group_count}'

def _compact_route_name(*, split_count: int, group_count: int, inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_q32rowld_e5db_v1_inherit_v7_q{n_query}_m{n_database}_k32_tailinf_cta1_cw1_s{split_count}_g{group_count}'

def _m64_route_name(inputs: dict[str, Any]) -> str:
    n_query = int(inputs.get('Q', -1))
    return f'rag_microbucket_q32rowld_e5db_v1_inherit_v7_q{n_query}_k10_m64_s{faeb.M64_SPLIT_COUNT}_g{faeb.M64_GROUP_COUNT}'
...
def _launch_q32_k32_m64_rowld(inputs: dict[str, Any], *, split_count: int=K32_SPLIT_COUNT, group_count: int=K32_GROUP_COUNT) -> None:
    ...
    _compiled_stage1_q32_k32_m64_rowld().launch(grid=(stage1_grid, 1, 1), block=(Q32_M64_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_q32_k32_m64_rowld_ir.computed_smem_bytes)
