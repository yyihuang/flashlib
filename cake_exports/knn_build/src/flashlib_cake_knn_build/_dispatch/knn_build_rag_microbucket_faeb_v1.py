"""Exact RAG microbucket Q4/Q64 K10 and Q16 K32 seed.
...
tcgen05/TMA producer families. Guard misses delegate to the current 2cfd
...
ROUTE_Q16_K32 = f'rag_microbucket_faeb_q16_k32_s{K32_SPLIT_COUNT}_g{K32_GROUP_COUNT}_fused'
ROUTE_BASE_2CFD = 'loom.examples.weave.knn_build_dispatch_e3de_8712_bcb3_2cfd_v1:launch_from_contract_inputs'
PRODUCTION_ROUTE_MODULES = {'q4_q64_k10_m64': 'loom.examples.weave.knn_build_rag_microbatch_m64_d4f7_v1:_launch_rag_microbatch_m64', 'q16_k32_fused': 'loom.examples.weave.knn_build_rag_microbucket_faeb_v1:_launch_q16_k32_fused', 'base_2cfd': ROUTE_BASE_2CFD}
...
def _k32_fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(k32_frontier.fused_merge_ir, suffix=f's{split_count}g{group_count}_faeb', GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RAG_MICROBUCKET_FAEB_VERIFY_KERNEL')
    k32_split = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_FAEB_VERIFY_K32_SPLIT', K32_SPLIT_COUNT))
    k32_groups = int(os.environ.get('LOOM_KNN_RAG_MICROBUCKET_FAEB_VERIFY_K32_GROUPS', K32_GROUP_COUNT))
    ...
    return rag_m64.stage1_m64_ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_faeb_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 91392, "cta_group": 1, "threads": 512}'))
...
def _launch_q16_k32_fused(inputs: dict[str, Any], *, split_count: int=K32_SPLIT_COUNT, group_count: int=K32_GROUP_COUNT) -> None:
    ...
    stage1_kernel.launch_cluster(grid=(stage1_grid, 1, 1), block=(k32_frontier.STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tile_pairs, db_tiles_per_split, split_count, total_work], cluster_dims=(k32_frontier.CTA_GROUP, 1, 1), shared_mem=k32_frontier.v5.stage1_k32_sort4earlystop_ir.computed_smem_bytes)
...
def _benchmark_payload(candidate_report: dict[str, Any], baseline_report: dict[str, Any], *, use_cupti: bool, shape_labels, measured_function: str, k32_split_count: int, k32_group_count: int) -> dict[str, Any]:
    return {'tflops': candidate_report['summary']['primary_mean'] or 0.0, 'all_correct': candidate_report['summary']['all_correct'], 'baseline_all_correct': baseline_report['summary']['all_correct'], 'performance_comparable': candidate_report['summary']['performance_comparable'], 'baseline_performance_comparable': baseline_report['summary']['performance_comparable'], 'invalid_performance_reason': candidate_report['summary']['invalid_performance_reason'], 'measured_entrypoint': f'loom.examples.weave.knn_build_rag_microbucket_faeb_v1:{measured_function}', 'baseline_entrypoint': 'loom.examples.weave.knn_build_dispatch_e3de_8712_bcb3_2cfd_v1:launch_from_contract_inputs', 'accelerated_shape_labels': list(TARGET_SHAPES), 'measured_shape_labels': list(shape_labels), 'producer_topology': {'Q4_K10': f'M64/S{M64_SPLIT_COUNT}/G{M64_GROUP_COUNT}', 'Q64_K10': f'M64/S{M64_SPLIT_COUNT}/G{M64_GROUP_COUNT}', 'Q16_K32': f'sort4earlystop/S{k32_split_count}/G{k32_group_count}/fused'}, 'timing_backends': _timing_backends_for_reports(candidate_report, baseline_report), 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'route_trace': route_trace_for_contract_shapes(shape_labels), 'route_trace_included': True, 'target_rows': _target_rows(candidate_report, baseline_report), 'contract_summary': candidate_report['summary'], 'baseline_contract_summary': baseline_report['summary'], 'contract_performance': candidate_report['performance'], 'baseline_contract_performance': baseline_report['performance'], 'report': candidate_report, 'baseline_report': baseline_report}

def benchmark_knn_build_rag_microbucket_faeb_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_split_count: int=K32_SPLIT_COUNT, k32_group_count: int=K32_GROUP_COUNT) -> dict[str, Any]:
    ...
    return _benchmark_payload(candidate_report, baseline_report, use_cupti=use_cupti, shape_labels=shape_labels, measured_function='benchmark_knn_build_rag_microbucket_faeb_v1', k32_split_count=k32_split_count, k32_group_count=k32_group_count)
...
def benchmark_q4_s144_ab(*, use_cupti: bool=True) -> dict[str, Any]:
    ...
    return {'measured_entrypoint': 'loom.examples.weave.knn_build_rag_microbucket_faeb_v1:benchmark_q4_s144_ab', 'timing_backend_requested': 'cupti' if use_cupti else 'cuda_event', 'candidate_s144': candidate_report, 'baseline_m64': baseline_report}
