...
ROUTE_Q16_K32 = ''.join(['rag_microbucket_faeb_q16_k32_s', format(K32_SPLIT_COUNT, ''), '_g', format(K32_GROUP_COUNT, ''), '_fused'])
...
def _k32_fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(k32_frontier.fused_merge_ir, suffix=''.join(['s', format(split_count, ''), 'g', format(group_count, ''), '_faeb']), GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
