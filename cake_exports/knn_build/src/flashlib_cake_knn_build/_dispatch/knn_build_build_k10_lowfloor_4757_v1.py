"""Build K10 low-floor exact wrapper for the 4757 continuation.

Minimum target architecture: sm_100a. This additive bucket-kernel wrapper keeps
the current Q24/Q128 full90 seed portfolio as fallback and routes exact BF16
build K10 low-floor rows through the existing fixed-build K10 Weave route. The
route remains Weave-only; FlashLib is used only by the contract harness.
"""
...
MODULE = 'loom.examples.weave.knn_build_build_k10_lowfloor_4757_v1'
...
BUILD_Q6144_K10 = 'build_large_b1_q6144_m6144_d128_k10'
TARGET_SHAPE_PARAMS = {BUILD_Q512_K10: (1, 512), BUILD_Q1024_K10: (1, 1024), BUILD_B2_Q1024_K10: (2, 1024), BUILD_TAIL_Q1536_K10: (1, 1536), BUILD_Q6144_K10: (1, 6144)}
TARGET_SHAPES = _decode_capture(_json_loads('{"__tuple__": ["build_k_sweep_qm512_k10", "build_qm1024_d128_k10", "build_batch_b2_q1024_m1024_d128_k10", "build_tail_b1_q1536_m1536_d128_k10", "build_large_b1_q6144_m6144_d128_k10"]}'))
TARGET_SHAPE_SET = set(TARGET_SHAPES)
SEED_K10_ID = '4757_fixedbuild_k10_v2_q512_q1024_b2_q1536_q6144'
PARENT_PORTFOLIO_ID = parent.CANDIDATE_CONFIGS[parent.DEFAULT_CANDIDATE_KEY]['candidate_id']
CANDIDATE_ID = 'build_k10_lowfloor_4757_v1'
...
def _matched_target_label(inputs: dict[str, Any]) -> str | None:
    bsz = int(inputs.get('B', -1))
    q = int(inputs.get('Q', -1))
    for label, (target_b, target_q) in TARGET_SHAPE_PARAMS.items():
        if bsz == target_b and q == target_q and _label_can_hit(inputs, label):
            return label
    return None
...
def _eligible_k10_lowfloor(inputs: dict[str, Any]) -> bool:
    if not (bool(inputs.get('build', False)) and int(inputs.get('Q', -1)) == int(inputs.get('M', -2)) and (int(inputs.get('D', -1)) == fixedbuild_k10.FEAT_D) and (int(inputs.get('K', -1)) == fixedbuild_k10.TOP_K_MAX) and (_dtype_name(inputs, 'query') == 'bfloat16') and (_dtype_name(inputs, 'database') in ('', 'bfloat16'))):
        return False
    return _matched_target_label(inputs) is not None
...
def candidate_build_k10_lowfloor_4757_v1(inputs: dict[str, Any]) -> None:
    launch_from_contract_inputs(inputs)

def candidate(inputs: dict[str, Any]) -> None:
    candidate_build_k10_lowfloor_4757_v1(inputs)
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if expected_seed is None or force_fallback:
        ...
        if force_fallback and expected_seed is not None:
            parent_row['guard_id'] = f'forced_fallback_{expected_seed}_disabled'
            parent_row['guard_condition'] = f'forced fallback to parent full90 portfolio; {expected_seed} disabled'
            parent_row['classification'] = 'guard-miss'
        return parent._normalize_route_row(parent_row)
    bsz = int(inputs.get('B', -1))
    q = int(inputs.get('Q', -1))
    split_count = fixedbuild_k10._fixed_build_k10_split_count(inputs)
    return parent._normalize_route_row({'shape_key': label, 'selected_route': route, 'selected_entrypoint': _selected_entrypoint(route), 'selected_seed': expected_seed, 'expected_seed': expected_seed, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'4757_fixedbuild_k10_b{bsz}_q{q}_exact_guard', 'guard_condition': f'exact BF16 build B={bsz} Q=M={q} D=128 K=10', 'coverage': '4757 build K10 low-floor overlay before Q24/Q128 full90 portfolio fallback', 'consumed_seed': expected_seed, 'replaced_route': parent_route, 'parent_portfolio_route': parent_route, 'baseline_dispatcher_route': parent_row.get('baseline_dispatcher_route') or parent_row.get('baseline_d5f8_route'), 'split_count': split_count, 'classification': 'seed-consumed'})
...
def benchmark_candidate_build_k10_lowfloor_4757_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, run_baseline: bool=True, time_flashlib: bool=True) -> dict[str, Any]:
...
def write_benchmark_artifact(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, run_baseline: bool=True, time_flashlib: bool=True) -> dict[str, str]:
    payload = benchmark_candidate_build_k10_lowfloor_4757_v1(use_cupti=use_cupti, shape_labels=shape_labels, run_baseline=run_baseline, time_flashlib=time_flashlib)
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    path = out_dir / 'build_k10_lowfloor_4757_v1.json'
