...
knn_build_evolve_7bfc_v1 = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50176, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50176, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _ir_for_top_k_max(top_k_max: int):
    ...
    return replace(ir, name=f'{ir.name}_k{top_k_max}_fallback', constants=constants)
...
def _compiled_kernel(top_k_max: int=TOP_K_MAX):
    return _decode_capture(_json_loads('{"__kernel__": "dispatch_kernel_0079"}'))
...
def launch_from_contract_inputs(inputs: dict[str, Any]) -> None:
    ...
    top_k = int(inputs['K'])
    if dim != FEAT_D:
        raise ValueError(f'knn_build_evolve_7bfc_v1 expects D={FEAT_D}, got {dim}')
    if top_k > TOP_K_FALLBACK_MAX:
        raise ValueError(f'knn_build_evolve_7bfc_v1 supports K <= {TOP_K_FALLBACK_MAX}, got {top_k}')
    ...
    args = [inputs['query_sq'], inputs['database_sq'], inputs['out_dists'], inputs['out_indices'], tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, num_db_tiles, total_tiles]
