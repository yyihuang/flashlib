...
BASE_9DB7_ENTRYPOINT = f'{base9db7.MODULE}:launch_from_contract_inputs'
...
K12_4F30_ENTRYPOINT = f'{k12_4f30.MODULE}:launch_from_contract_inputs'
...
def _selected_seed_for_inputs(inputs: dict[str, Any], *, enable_over32_v25: bool=True) -> tuple[str | None, str | None]:
    ...
    if _eligible_v20_k12(inputs):
        return (SEED_V20_K12_ID, str(inputs.get('label') or f'q{inputs.get('Q')}_k12'))
    if _eligible_v12_midbuild(inputs):
        return (SEED_V12_MIDBUILD_ID, str(inputs.get('label') or f'q{inputs.get('Q')}_k{inputs.get('K')}'))
    if enable_over32_v25 and _eligible_over32_k48(inputs):
        return (SEED_OVER32_V25_ID, str(inputs.get('label') or f'q{inputs.get('Q')}_k48'))
    return (None, None)
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False, enable_over32_v25: bool=True) -> dict[str, Any]:
    ...
    return {'shape_key': inputs.get('label'), 'selected_route': selected_route, 'selected_entrypoint': PRODUCTION_ROUTE_MODULES[selected_seed], 'selected_seed': selected_seed, 'expected_seed': selected_seed, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'dbd7_lowfloor_{selected_seed}', 'guard_condition': guard_conditions[selected_seed], 'matched_label': matched_label, 'base_9db7_route': base_route, 'classification': 'seed-consumed'}
...
def write_benchmark_artifact(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, run_baseline: bool=True, enable_over32_v25: bool=True) -> dict[str, str]:
    ...
    path = out_dir / f'buildbucket_dbd7_lowfloor_v1_{suffix}.json'
