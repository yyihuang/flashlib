...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_dispatch_q1m524_v5_k1_s2_consumption_v1'
BASELINE_ENTRYPOINT = prior.ROUTE_ENTRYPOINT
SEED_ID = 'k1_q512_group2_573e'
SEED_ENTRYPOINT = f'{seed_s2.ROUTE_PREFIX}:launch_from_contract_inputs'
...
def _contract_metric_schema(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for label, row in rows.items():
        if not isinstance(row, dict):
            raise ValueError(f'full112 payload row {label!r} must be a mapping')
        ...
        if params is None or not isinstance(kernel_ms, (int, float)) or kernel_ms <= 0:
            raise ValueError(f'full112 payload row {label!r} lacks contract params or kernel_ms')
    ...
    if len(tflops_values) != len(params_by_label):
        raise ValueError(f'full112 payload has {len(tflops_values)} timed rows, expected {len(params_by_label)} contract rows')
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    denom = f'full{len(payload['measured_shape_labels'])}'
    candidate_path = out_dir / f'{denom}_k1_s2_candidate.json'
    baseline_path = out_dir / f'{denom}_prior_same_session_baseline.json'
    trace_path = out_dir / f'{denom}_k1_s2_route_trace.json'
...
def write_checkpointed_full_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    denom = f'full{len(labels)}'
    progress_path = out_dir / f'{denom}_k1_s2_progress.json'
    candidate_path = out_dir / f'{denom}_k1_s2_candidate.json'
    baseline_path = out_dir / f'{denom}_prior_same_session_baseline.json'
    trace_path = out_dir / f'{denom}_k1_s2_route_trace.json'
