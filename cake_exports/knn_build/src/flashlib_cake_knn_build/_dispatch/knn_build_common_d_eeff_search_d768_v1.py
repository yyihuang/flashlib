"""kNN common-D D768 rectangular search seed for round eeff.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only ``search_rect_common_d768_b1_q512_m8192_k10`` through the existing
chunked 128-row D768 tcgen05/TMA producer and a fused split merge. Guard misses
delegate to the current v11 common-D fallback dispatcher.
"""
...
MODULE = 'loom.examples.weave.knn_build_common_d_eeff_search_d768_v1'
ROUTE_PREFIX = 'knn_build_common_d_eeff_search_d768_v1'
SEARCH_D768 = 'search_rect_common_d768_b1_q512_m8192_k10'
TARGET_SHAPES = (SEARCH_D768,)
...
stage1_d768_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_eeff_search_d768_v1:stage1_d768_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_eeff_search_d768_v1:merge_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 1024, "cta_group": 1, "threads": 32}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_common_d_eeff_search_d768_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def _matches_target(inputs: dict[str, Any]) -> bool:
    ...
    if label is not None and str(label) != SEARCH_D768:
        return False
...
def _target_label_for_inputs(inputs: dict[str, Any]) -> str | None:
    return SEARCH_D768 if _matches_target(inputs) else None

def _split_count() -> int:
    return int(os.environ.get('LOOM_KNN_COMMON_D_EEFF_SEARCH_D768_SPLITS', SEARCH_SPLIT_COUNT))

def _group_count() -> int:
    return int(os.environ.get('LOOM_KNN_COMMON_D_EEFF_SEARCH_D768_GROUPS', SEARCH_GROUP_COUNT))

def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _target_label_for_inputs(inputs) == SEARCH_D768:
        return f'{ROUTE_PREFIX}:{SEARCH_D768}:d768:q512:m8192:s{_split_count()}:g{_group_count()}:chunked128_fused_merge'
    return default_dispatcher.route_for_contract_inputs(inputs, force_fallback=force_fallback)

def _launch_search_d768(inputs: dict[str, Any]) -> None:
    query = inputs['query']
    database = inputs['database']
    if str(query.dtype) != 'torch.bfloat16' or str(database.dtype) != 'torch.bfloat16':
        raise ValueError('knn_build_common_d_eeff_search_d768_v1 supports bfloat16 inputs only')
    bsz = int(inputs['B'])
    ...
    if dim != tma_dim:
        raise ValueError(f'{SEARCH_D768} expected D={tma_dim}, got {dim}')
    ...
    _compiled_stage1_d768().launch(grid=(stage1_grid, 1, 1), block=(THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_d768_ir.computed_smem_bytes)
...
def benchmark_knn_build_common_d_eeff_search_d768_v1(*, use_cupti: bool | None=None, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
...
def route_trace_for_contract_shapes(shape_labels=None, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        if force_fallback or _target_label_for_inputs(inputs) is None:
            rows.append({'shape_key': str(shape['label']), 'selected_route': default_dispatcher.route_for_contract_inputs(inputs, force_fallback=force_fallback), 'selected_entrypoint': default_dispatcher.ROUTE_ENTRYPOINT, 'selected_seed': None, 'expected_seed': 'common_d_eeff_search_d768_v1' if str(shape['label']) in TARGET_SHAPE_SET else None, 'route_kind': 'forced_fallback' if force_fallback else 'parent_delegate', 'route_source': 'common-d-v11-fallback-dispatcher', 'guard_id': 'forced_fallback' if force_fallback else 'guard_miss', 'classification': 'forced_fallback' if force_fallback else 'delegated'})
            continue
        rows.append({'shape_key': SEARCH_D768, 'selected_route': route_for_contract_inputs(inputs), 'selected_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'selected_seed': 'common_d_eeff_search_d768_v1', 'expected_seed': 'common_d_eeff_search_d768_v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': 'eeff_common_d_search_d768_exact_guard', 'guard_condition': 'exact BF16 search B=1 Q=512 M=8192 D=768 K=10', 'feature_chunks': FEATURE_CHUNKS, 'split_count': _split_count(), 'group_count': _group_count(), 'producer': 'chunked128_d768_tcgen05_tma', 'merge': 'fused_group_k10_split_merge', 'classification': 'common-d-search-d768-seed'})
    return rows
