...
def _compiled_stage1_d768():
    return _decode_capture(_json_loads('{"__kernel__": "dispatch_kernel_0074"}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _target_label_for_inputs(inputs) == SEARCH_D768:
        return ''.join([format(ROUTE_PREFIX, ''), ':', format(SEARCH_D768, ''), ':d768:q512:m8192:s', format(_split_count(), ''), ':g', format(_group_count(), ''), ':chunked128_fused_merge'])
...
def _launch_search_d768(inputs: dict[str, Any]) -> None:
    ...
    if dim != tma_dim:
        raise ValueError(''.join([format(SEARCH_D768, ''), ' expected D=', format(tma_dim, ''), ', got ', format(dim, '')]))
