...
knn_build_rag_microbatch_m64_d4f7_stage1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_m64_d4f7_v1:knn_build_rag_microbatch_m64_d4f7_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 91392, "cta_group": 1, "threads": 512}'))
stage1_m64_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_m64_d4f7_v1:stage1_m64_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 91392, "cta_group": 1, "threads": 512}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbatch_m64_d4f7_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 91392, "cta_group": 1, "threads": 512}'))
...
def _compiled_stage1_m64():
    return _decode_capture(_json_loads('{"__kernel__": "dispatch_kernel_0107"}'))
...
def _launch_rag_microbatch_m64(inputs: dict[str, Any], *, split_count: int=MICRO_SPLIT_COUNT, group_count: int=MICRO_GROUP_COUNT) -> None:
    ...
    _compiled_stage1_m64().launch(grid=(stage1_grid, 1, 1), block=(MICRO_THREADS, 1, 1), args=[query, database, inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=MICRO_STAGE_SMEM_BYTES)
    ...
    merge_kernel.launch(grid=(merge_grid, 1, 1), block=(parent_micro.K10_FUSED_MERGE_THREADS, 1, 1), args=[partial_dists, partial_indices, inputs['out_dists'], inputs['out_indices'], total_queries], shared_mem=merge_ir.computed_smem_bytes)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, split_count: int=MICRO_SPLIT_COUNT, group_count: int=MICRO_GROUP_COUNT) -> str:
    if _eligible_rag_microbatch(inputs):
        return f'rag_microbatch_m64_d4f7_q64m128_s{split_count}_g{group_count}'
...
def _shape_payload(candidate_report: dict[str, Any], parent_report: dict[str, Any], *, split_count: int, group_count: int) -> dict[str, Any]:
    ...
    for label in TARGET_SHAPES:
        ...
        rows[label] = {'candidate': cand, 'parent_4a72': parent, 'candidate_route': f'rag_microbatch_m64_d4f7_q64m128_s{split_count}_g{group_count}', 'candidate_ms': cand_ms, 'parent_4a72_ms': parent_ms, 'flashlib_ms': flashlib_ms, 'speedup_vs_parent_4a72': parent_ms / cand_ms if cand_ms and parent_ms else None, 'ratio_vs_flashlib': flashlib_ms / cand_ms if cand_ms and flashlib_ms else None}
    return rows
