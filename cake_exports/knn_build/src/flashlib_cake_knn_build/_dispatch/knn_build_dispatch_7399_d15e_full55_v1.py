"""Opt-in kNN build full55 dispatcher consuming 7399 RAG and d15e routes.
...
keeps the existing a989 large-square and 6c1e K96 exact routes, replaces the
four exact RAG frontier rows with the 7399 RAG seed, adds the exact d15e
rectangular ``search_rect_b1_q1024_m8192_d128_k10`` seed, and delegates every
other row to the same Weave-only baseline policy used by 7c3a/fd02.
...
RAG_K32_TARGET_SHAPES = rag_7399.K32_TARGET_SHAPES
...
def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_DISPATCH_7399_D15E_VERIFY_KERNEL')
    ...
    if verify_kernel == 'rag_7399_k32_stage1':
        os.environ['LOOM_KNN_RAG_FRONTIER_7399_V1_VERIFY_KERNEL'] = 'stage1'
        return rag_7399._verify_export_ir()
    if verify_kernel == 'rag_7399_k32_fused_merge':
        os.environ['LOOM_KNN_RAG_FRONTIER_7399_V1_VERIFY_KERNEL'] = 'fused_merge'
        return rag_7399._verify_export_ir()
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_7399_d15e_full55_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def _eligible_7399_rag_k32(inputs: dict[str, Any]) -> bool:
    return _label_can_hit(inputs, RAG_K32_TARGET_SHAPE_SET) and rag_7399._eligible_k32_rag_frontier(inputs)
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if _eligible_7399_rag_k32(inputs):
        return ROUTE_RAG_7399_K32
...
def _launch_route(inputs: dict[str, Any], route: str) -> None:
    if route == ROUTE_RAG_7399_K10:
        ...
        return
    if route == ROUTE_RAG_7399_K32:
        rag_7399._launch_k32_rag_frontier_fused_merge(inputs)
        return
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if route == ROUTE_RAG_7399_K32:
        return {'shape_key': inputs.get('label'), 'selected_route': route, 'guard_condition': 'exact 7399 RAG frontier BF16 B1 Q128 M100000 D128 K32 non-build label', 'route_kind': 'specialized', 'coverage': 'exact 7399 RAG K32 S72/G8 fused cooperative group/final merge seed', 'consumed_seed': 'rag_frontier_7399_v1', 'replaced_route': base_route, 'baseline_7c3a_route': base_route, 'baseline_route_kind': _route_kind_for_base(base_route), 'inherited_route': inherited_route, 'parity_status': 'fail', 'parity_reason': '7399 K32 CUPTI ratio_vs_flashlib is 0.9436378734204341 in the source seed payload'}
    if route == ROUTE_RECT_D15E:
...
def benchmark_knn_build_dispatch_7399_d15e_full55_v1(*, use_cupti: bool=False, shape_labels=None) -> dict[str, Any]:
    ...
    return _benchmark_payload(candidate_report, baseline_report, use_cupti=use_cupti, shape_labels=shape_labels, measured_function='benchmark_knn_build_dispatch_7399_d15e_full55_v1')

def write_benchmark_artifacts(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=False, shape_labels=None) -> dict[str, str]:
    payload = benchmark_knn_build_dispatch_7399_d15e_full55_v1(use_cupti=use_cupti, shape_labels=shape_labels)
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    candidate_path = out_dir / 'full55_dispatch_7399_d15e_full55_v1.json'
    baseline_path = out_dir / 'full55_same_session_baseline_7c3a_for_7399_d15e_v1.json'
    route_trace_path = out_dir / 'full55_route_trace_7399_d15e_full55_v1.json'
