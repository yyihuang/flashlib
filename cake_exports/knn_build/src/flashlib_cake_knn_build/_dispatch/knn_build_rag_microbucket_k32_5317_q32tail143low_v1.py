...
def _q32tail143low_route_name(inputs: dict[str, Any]) -> str:
    return f'rag_microbucket_k32_5317_q32tail143low_v1_q{int(inputs.get('Q', -1))}_m{int(inputs.get('M', -1))}_k32_row16x256b2cw_exact_s{K32_Q32TAIL143_LOW_SPLIT_COUNT}_r{q32exact.rows4.K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def write_benchmark_artifact(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, str]:
    ...
    path = out_dir / f'5317_q32tail143low_{len(tuple(shape_labels))}row_{suffix}.json'
