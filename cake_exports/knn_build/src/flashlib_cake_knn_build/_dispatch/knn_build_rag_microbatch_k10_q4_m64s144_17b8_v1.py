"""Q4-only M64 split144 repair for the kNN build RAG microbatch lane.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
routes only ``rag_microbatch_b1_q4_m100000_d128_k10`` through the existing
M64 tcgen05 producer with a split144/group12 topology. Q64 and all guard
misses delegate to the previously consumed Q4/Q64 M64 wrapper, so this branch
can be evaluated as a Q4-specific seed without widening the Q4/Q64 guard.
"""
...
MODULE = 'loom.examples.weave.knn_build_rag_microbatch_k10_q4_m64s144_17b8_v1'
...
SPLIT_COUNT = 144
GROUP_COUNT = 12
SEED_ID = 'rag_microbatch_k10_q4_m64_s144_g12_17b8_v1'
BASELINE_SEED_ID = base_q4q64.SEED_ID
ROUTE_Q4_M64S144 = 'rag_microbatch_k10_q4_m64_s144_g12_17b8_v1'
...
def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_17B8_Q4_M64S144_VERIFY_KERNEL')
...
def _eligible_q4_m64s144(inputs: dict[str, Any]) -> bool:
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible_q4_m64s144(inputs):
        return ROUTE_Q4_M64S144
...
def candidate_q4_m64s144_17b8_v1(inputs: dict[str, Any]) -> None:
...
def _trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if force_fallback and _eligible_q4_m64s144(inputs):
        row['expected_seed'] = SEED_ID
        row['guard_id'] = 'forced_fallback_17b8_q4_m64_s144_disabled'
        row['guard_condition'] = 'forced fallback to q4q64 baseline; Q4 M64 split144 seed disabled'
...
def benchmark_baseline_q4q64(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, Any]:
    ...
    report['measured_entrypoint'] = f'{base_q4q64.MODULE}:benchmark_candidate_rag_microbatch_k10_q4q64_m64_3505_d555_v1'
    report['route_trace'] = base_q4q64.route_trace_for_contract_shapes(shape_labels)
    report['route_trace_included'] = True
    return report

def benchmark_candidate_q4_m64s144_17b8_v1(*, use_cupti: bool=True, shape_labels=TARGET_SHAPES, baseline_report: dict[str, Any] | None=None, benchmark_correctness: bool=True, time_flashlib: bool=True) -> dict[str, Any]:
    ...
    candidate_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_q4_m64s144_17b8_v1, correctness=benchmark_correctness, time_flashlib=time_flashlib)
