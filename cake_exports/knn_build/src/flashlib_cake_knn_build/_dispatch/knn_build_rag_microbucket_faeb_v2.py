...
ROUTE_Q16_K32 = f'rag_microbucket_faeb_v2_q16_k32_s{K32_SPLIT_COUNT}_g{K32_GROUP_COUNT}_widefused'
...
knn_build_rag_microbucket_faeb_v2_k32_wide_fused_group_final_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_faeb_v2_k32_wide_fused_group_final_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 4096, "constants": [["TOP_K_MAX", 32], ["GROUP_COUNT", 9], ["GROUP_SPLITS", 8]], "cta_group": 1, "threads": 32}'))
wide_fused_merge_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_faeb_v2_k32_wide_fused_group_final_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 4096, "constants": [["TOP_K_MAX", 32], ["GROUP_COUNT", 9], ["GROUP_SPLITS", 8]], "cta_group": 1, "threads": 32}'))

def _k32_fused_merge_ir(split_count: int, group_count: int) -> Any:
    _validate_k32_group_shape(split_count, group_count)
    return _ir_with_constants(wide_fused_merge_ir, suffix=f's{split_count}g{group_count}_faeb', GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
def _validate_k32_group_shape(split_count: int, group_count: int) -> None:
    ...
    if group_count > K32_GROUP_CAPACITY:
        raise ValueError(f'group_count={group_count} exceeds faeb v2 fused-merge capacity {K32_GROUP_CAPACITY}')
