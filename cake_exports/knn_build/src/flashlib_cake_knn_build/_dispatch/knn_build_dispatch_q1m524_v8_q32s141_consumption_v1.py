...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_dispatch_q1m524_v8_q32s141_consumption_v1'
...
def write_checkpointed_full_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, max_new_rows: int | None=None) -> dict[str, Any]:
    ...
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = out_dir / f'full{len(labels)}_v8_q32s141_paired_progress.json'
