...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_microbucket_k32_0cb5_q31tail_v1:ir"}'))
...
def _q31tail_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    n_query = int(inputs.get('Q', -1))
    n_database = int(inputs.get('M', -1))
    return f'rag_microbucket_k32_0cb5_q31tail_v1_q{n_query}_m{n_database}_k32_row16x256b2warp_uneven_s{split_count}_r{uneven.K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_q31tail_split_count: int=K32_Q31TAIL_SPLIT_COUNT) -> dict[str, str]:
    ...
    path = out_dir / f'0cb5_q31tail_{len(tuple(shape_labels))}row_s{k32_q31tail_split_count}_{suffix}.json'
