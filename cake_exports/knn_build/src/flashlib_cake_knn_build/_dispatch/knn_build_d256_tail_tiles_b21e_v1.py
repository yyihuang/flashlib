...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_d256_tail_tiles_b21e_v1'
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible(inputs):
        ...
        return f'd256_tail_tiles_b21e:exact_q128_m100000_d256_k32:s{SPLIT_COUNT}:m64n64k256:tail{tail_tiles}'
...
def _select_contract_shapes(shape_labels=TARGET_SHAPES):
    ...
    if {str(shape['label']) for shape in selected} != wanted:
        raise ValueError(f'unknown shape labels: {sorted(wanted)}')
    return selected
