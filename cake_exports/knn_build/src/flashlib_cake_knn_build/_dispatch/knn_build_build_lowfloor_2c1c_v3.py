...
ROUTE_Q512_S4 = f'{seed_lowk.ROUTE_PREFIX}:q512_lowk_s{Q512_SPLIT_COUNT}'
ROUTE_Q1024_K16 = f'{seed_lowk.ROUTE_PREFIX}:q1024_k16_s{Q1024_K16_SPLIT_COUNT}'
ROUTE_MIDK = f'{seed_midk.MODULE}:launch_from_contract_inputs'
ROUTE_Q4096_K13_UNORDERED = f'{MODULE}:q4096_k13_unordered_s{Q4096_K13_SPLIT_COUNT}'
ROUTE_D64_Q4096 = seed_d64.ROUTE_SPLIT4_UNORDERED
ROUTE_PARENT_SELECTED = selected_parent.CANDIDATE_9A17_ONLY_ENTRYPOINT
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_candidate_build_lowfloor_2c1c_v3'
...
stage1_q4096_k13_unordered_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_cg2_u2_stage1_k32_unordered_2c1ck13unordered", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 13]], "cta_group": 1, "threads": 192}'))
merge_q4096_k13_unordered_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_k32_merge_s4_unordered_2c1ck13unordered", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 13], ["SPLIT_COUNT", 4]], "cta_group": 1, "threads": 32}'))
...
def _compiled_merge_q4096_k13_unordered():
    return _decode_capture(_json_loads('{"__kernel__": "dispatch_kernel_0050"}'))
...
def _selected_seed_for_inputs(inputs: dict[str, Any]) -> tuple[str | None, str | None]:
    if _eligible_q512(inputs):
        return (SEED_Q512_ID, str(inputs.get('label') or f'q512_k{inputs.get('K')}'))
    if _eligible_q1024_k16(inputs):
        return (SEED_Q1024_ID, TARGET_Q1024_K16)
    if _eligible_midk_q2048(inputs):
        return (SEED_MIDK_Q2048_ID, str(inputs.get('label') or f'q2048_k{inputs.get('K')}'))
...
def q4096_k13_unordered_s4(inputs: dict[str, Any]) -> None:
    ...
    _compiled_stage1_q4096_k13_unordered().launch_cluster(grid=(stage1_grid, 1, 1), block=(seed_midk.v9.STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tile_pairs, db_tiles_per_split, split_count, total_work], cluster_dims=(seed_midk.v9.CTA_GROUP, 1, 1), shared_mem=stage1_q4096_k13_unordered_ir.computed_smem_bytes)
...
def route_trace_for_contract_shapes(shape_labels=TARGET_SHAPES, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for label in tuple(shape_labels):
        ...
        rows.append(selected_parent._normalize_route_row({'shape_key': label, 'selected_route': route, 'selected_entrypoint': selected_entrypoints[selected_seed], 'selected_seed': selected_seed, 'expected_seed': selected_seed, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': f'2c1c_build_lowfloor_{selected_seed}', 'guard_condition': guard_conditions[selected_seed], 'matched_label': matched_label, 'split_count': split_counts[selected_seed], 'parent_selected_route': parent_route, 'baseline_dispatcher_route': parent_row.get('selected_route'), 'classification': 'seed-consumed'}))
    return rows
