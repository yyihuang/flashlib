"""kNN non-D128 frontier wide-combined seed for round 101 v1.

Minimum target architecture: sm_100a. This additive seed preserves the
round-8199 split-retuned non-D128 route and selectively routes D192/D320 rows
through the existing wide tcgen05/TMA producers. D96 and D768 stay on the
validated split-retuned parent path. The contract-visible path remains
Weave-only: optional BF16 padding, tcgen05 dot-product stage, split-local K10
candidates, and Weave split merge.
"""
...
from . import knn_build_non128_frontier_8199_splitretune_v1 as splitretune
from . import knn_build_non128_frontier_8199_widestage_v1 as widestage
BLOCK_Q = splitretune.BLOCK_Q
BLOCK_M = splitretune.BLOCK_M
K_TILE = splitretune.K_TILE
TOP_K_MAX = splitretune.TOP_K_MAX
THREADS = splitretune.THREADS
MERGE_THREADS = splitretune.MERGE_THREADS
GRID_DIM_DEFAULT = splitretune.GRID_DIM_DEFAULT
MODULE = 'loom.examples.weave.knn_build_non128_frontier_8199_widecombine_v1'
ROUTE_PREFIX = 'knn_build_non128_frontier_8199_widecombine_v1'
...
stage1_d256_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widecombine_v1:stage1_d256_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "cta_group": 1, "threads": 192}'))
stage1_d384_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widecombine_v1:stage1_d384_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 148736, "cta_group": 1, "threads": 192}'))
pad_bf16_rows_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widecombine_v1:pad_bf16_rows_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))
merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widecombine_v1:merge_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 256}'))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_NON128_FRONTIER_8199_WIDECOMBINE_VERIFY_KERNEL')
    ...
    if verify_kernel == 'stage1_chunks1':
        return splitretune.stage1_d128_ir
    if verify_kernel == 'stage1_chunks6':
        return splitretune.stage1_d768_ir
    if verify_kernel == 'pad_d96':
        return splitretune.parent._pad_ir(128)
    if verify_kernel == 'pad_d192':
        return splitretune.parent._pad_ir(256)
    if verify_kernel == 'pad_d320':
        return splitretune.parent._pad_ir(384)
    if verify_kernel == 'merge':
        return merge_ir
    return stage1_d384_ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_non128_frontier_8199_widecombine_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 148736, "cta_group": 1, "threads": 192}'))
...
def _split_count_for_label(label: str) -> int:
    env_key = f'LOOM_KNN_NON128_FRONTIER_8199_WIDECOMBINE_SPLITS_{label.upper().replace('-', '_')}'
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return splitretune.route_for_contract_inputs(inputs, force_fallback=force_fallback)

def _pad_if_needed(tensor, *, rows: int, src_cols: int, dst_cols: int):
    if src_cols == dst_cols:
        return tensor
    return splitretune.parent._pad_bf16_rows(tensor, rows=rows, src_cols=src_cols, dst_cols=dst_cols)
...
def _launch_wide_stage(inputs: dict[str, Any], label: str, *, feature_dim: int, kernel, stage1_ir) -> None:
    ...
    if str(query.dtype) != 'torch.bfloat16' or str(database.dtype) != 'torch.bfloat16':
        raise ValueError('knn_build_non128_frontier_8199_widecombine_v1 supports bfloat16 inputs only')
    bsz = int(inputs['B'])
    ...
    kernel.launch(grid=(stage1_grid, 1, 1), block=(THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> None:
    ...
    if force_fallback or label is None:
        splitretune.launch_from_contract_inputs(inputs, force_fallback=force_fallback)
        return
    if label in WIDE_D256_SHAPES:
        _launch_wide_stage(inputs, label, feature_dim=256, kernel=wide_d256._compiled_d256_stage1(), stage1_ir=stage1_d256_ir)
        return
    if label in WIDE_D384_SHAPES:
        _launch_wide_stage(inputs, label, feature_dim=384, kernel=widestage._compiled_d384_stage1(), stage1_ir=stage1_d384_ir)
        return
    splitretune.launch_from_contract_inputs(inputs, force_fallback=False)
...
def benchmark_knn_build_non128_frontier_8199_widecombine_v1(*, use_cupti: bool | None=None, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
...
def route_trace_for_contract_shapes(shape_labels=None, *, force_fallback: bool=False) -> list[dict[str, Any]]:
    ...
    for shape in selected:
        ...
        if label is None or force_fallback:
            rows.append({'shape_key': inputs['label'], 'selected_route': splitretune.ROUTE_FALLBACK, 'selected_entrypoint': splitretune.ROUTE_FALLBACK, 'selected_seed': None, 'expected_seed': 'non128_frontier_8199_widecombine_v1' if inputs['label'] in TARGET_SHAPE_SET else None, 'route_kind': 'fallback', 'route_source': 'splitretune-parent-dispatcher', 'guard_id': 'forced_fallback' if force_fallback else 'splitretune_parent_guard', 'classification': 'forced_fallback' if force_fallback else 'delegated'})
            continue
        spec = SHAPE_SPECS[label]
        feature_dim = _feature_dim_for_label(label)
        rows.append({'shape_key': label, 'selected_route': route_for_contract_inputs(inputs), 'selected_entrypoint': f'{MODULE}:launch_from_contract_inputs', 'selected_seed': 'non128_frontier_8199_widecombine_v1', 'expected_seed': 'non128_frontier_8199_widecombine_v1', 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'guard_id': '8199_non128_widecombine_exact_shape_guard', 'guard_condition': f'exact BF16 B={spec['B']} Q={spec['Q']} M={spec['M']} D={spec['D']} K={spec['K']} build={spec['build']}', 'feature_dim': feature_dim, 'split_count': _split_count_for_label(label), 'producer': _producer_for_label(label), 'preprocess_stage': f'd{int(spec['D'])}_weave_pad_to_d{feature_dim}' if int(spec['D']) != feature_dim else None, 'classification': 'seed-consumed'})
    return rows
