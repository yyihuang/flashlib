...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    candidate_path = out_dir / ''.join([format(denom, ''), '_dispatch_e3de_9138_bcb3_4247_v1.json'])
    baseline_2cfd_path = out_dir / ''.join([format(denom, ''), '_same_session_baseline_2cfd_for_4247_v1.json'])
    baseline_8700_path = out_dir / ''.join([format(denom, ''), '_same_session_baseline_8700_for_4247_v1.json'])
    route_trace_path = out_dir / ''.join([format(denom, ''), '_route_trace_e3de_9138_bcb3_4247_v1.json'])
    forced_trace_path = out_dir / ''.join([format(denom, ''), '_forced_fallback_trace_e3de_9138_bcb3_4247_v1.json'])
    seed_matrix_path = out_dir / ''.join([format(denom, ''), '_seed_delta_matrix_e3de_9138_bcb3_4247_v1.json'])
