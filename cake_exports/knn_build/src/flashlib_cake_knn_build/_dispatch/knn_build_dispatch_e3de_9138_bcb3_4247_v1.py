"""4247 kNN build dispatcher A/B candidate from 2cfd and 9138.

Minimum target architecture: sm_100a. This dispatcher-consumption wrapper starts
from the 2cfd full67 dispatcher and changes only the exact rectangular D64
search guard from the 8712 v2 seed to the 9138 v3 seed. The inherited e3de D64
build route and bcb3 Q1 online M-bucket routes remain unchanged. Guard misses
delegate through the same e3de/8700 Weave stack, so every production route stays
Weave-only.
"""
...
from . import knn_build_dispatch_e3de_8712_bcb3_2cfd_v1 as base_2cfd
from . import knn_build_dispatch_d64_fdd7_e3de_v1 as base_e3de
from . import knn_build_over64_k96_a2f8_v1 as k96_a2f8
from . import knn_build_ragonline_mbucket_aa88_q1m_v3 as q1_bcb3
from . import knn_build_rect_d64_cf49_v3 as rect_9138
...
ROUTE_RECT_9138 = 'loom.examples.weave.knn_build_rect_d64_cf49_v3:rect_d64_split_cached_s16'
...
ROUTE_K96_A2F8 = 'loom.examples.weave.knn_build_over64_k96_a2f8_v1:generated_v8_k96'
DEFAULT_PORTFOLIO_ID = base_e3de.DEFAULT_PORTFOLIO_ID
D64_TARGET_SHAPES = base_e3de.D64_TARGET_SHAPES
RECT_TARGET_SHAPES = rect_9138.TARGET_SHAPES
Q1_TARGET_SHAPES = q1_bcb3.TARGET_SHAPES
K96_COVERAGE_TARGET_SHAPES = ('build_over64_stress_qm1024_k96', 'build_over64_stress_qm2048_k96', 'build_over64_stress_qm4096_k96')
K96_GENERATED_VARIANT_SHAPES = ('build_over64_stress_qm1024_k96', 'build_over64_stress_qm4096_k96')
K96_COVERAGE_TARGET_SHAPE_SET = set(K96_COVERAGE_TARGET_SHAPES)
K96_GENERATED_VARIANT_SHAPE_SET = set(K96_GENERATED_VARIANT_SHAPES)
COVERAGE_REPAIR_TARGET_SHAPES = K96_GENERATED_VARIANT_SHAPES
...
def _eligible_k96_a2f8_v8(inputs: dict[str, Any]) -> bool:
    return _label_can_hit(inputs, K96_GENERATED_VARIANT_SHAPE_SET) and bool(inputs.get('build', False)) and (_dtype_name(inputs) == 'bfloat16') and (int(inputs['B']) == 1) and (int(inputs['D']) == k96_a2f8.FEAT_D) and (int(inputs['K']) == k96_a2f8.OVER64_TOP_K) and (int(inputs['Q']) == int(inputs['M'])) and (int(inputs['Q']) in (1024, 4096))

def route_for_contract_inputs(inputs: dict[str, Any], *, portfolio_id: str=DEFAULT_PORTFOLIO_ID, force_fallback: bool=False, enable_d64_fdd7: bool=True, enable_rect_d64: bool=True, enable_q1_mbucket: bool=True, enable_k96_a2f8: bool=True, enable_rag_seed_portfolio: bool=True, enable_q512_k456: bool=True) -> str:
    if not force_fallback and enable_k96_a2f8 and _eligible_k96_a2f8_v8(inputs):
        return ROUTE_K96_A2F8
    ...
    if not force_fallback and enable_rect_d64 and rect_9138._eligible_rect_d64(inputs):
        return ROUTE_RECT_9138
...
def _launch_route(inputs: dict[str, Any], route: str) -> None:
    if route == ROUTE_K96_A2F8 and _eligible_k96_a2f8_v8(inputs):
        k96_a2f8._launch_over64_k96_split_path(inputs)
        return
    if route == ROUTE_RECT_9138 and rect_9138._eligible_rect_d64(inputs):
        rect_9138.launch_from_contract_inputs(inputs)
        return
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, portfolio_id: str=DEFAULT_PORTFOLIO_ID, force_fallback: bool=False, enable_d64_fdd7: bool=True, enable_rect_d64: bool=True, enable_q1_mbucket: bool=True, enable_k96_a2f8: bool=True, enable_rag_seed_portfolio: bool=True, enable_q512_k456: bool=True) -> None:
    route = route_for_contract_inputs(inputs, portfolio_id=portfolio_id, force_fallback=force_fallback, enable_d64_fdd7=enable_d64_fdd7, enable_rect_d64=enable_rect_d64, enable_q1_mbucket=enable_q1_mbucket, enable_k96_a2f8=enable_k96_a2f8, enable_rag_seed_portfolio=enable_rag_seed_portfolio, enable_q512_k456=enable_q512_k456)
...
def candidate_base_2cfd(inputs: dict[str, Any]) -> None:
    base_2cfd.launch_from_contract_inputs(inputs)
...
def _base_route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    row['base_2cfd_route'] = base_2cfd.route_for_contract_inputs(inputs)
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    base_2cfd_route = base_2cfd.route_for_contract_inputs(inputs)
    base_8700_route = base_e3de.base_8700.route_for_contract_inputs(inputs, portfolio_id=DEFAULT_PORTFOLIO_ID)
    if force_fallback and (base_e3de._eligible_d64_fdd7(inputs) or rect_9138._eligible_rect_d64(inputs) or q1_bcb3._eligible_rag_online_mbucket(inputs) or _eligible_k96_a2f8_v8(inputs)):
        ...
        row['guard_id'] = 'forced_fallback_4247_overlays_disabled'
        row['guard_condition'] = 'forced fallback to inherited e3de/8700 path; 4247 overlays disabled'
        row['forced_disabled_seeds'] = ('d64_fdd7_aa88_v2', 'rect_d64_cf49_v3_9138', 'q1_mbucket_aa88_q1m_v3_bcb3', 'over64_k96_a2f8_v1')
        row['candidate_guard_status'] = 'forced_fallback'
        if _eligible_k96_a2f8_v8(inputs):
            row['guard_id'] = 'forced_fallback_k96_a2f8_disabled'
            if str(row['selected_route']) == ROUTE_K96_A2F8:
                row['guard_condition'] = 'forced fallback requested, but K96 A2F8 route remained selected'
                row['classification'] = 'route-ok'
            elif 'knn_build_over64_k96' in str(row['selected_route']):
                row['guard_condition'] = 'forced fallback disables c142 K96 A2F8 coverage; inherited exact K96 route covers this row'
                row['classification'] = 'route-ok'
            else:
                row['guard_condition'] = 'forced fallback disables c142 K96 A2F8 coverage; inherited e3de route is known to reject K=96'
                row['classification'] = 'guard-miss'
        else:
            row['classification'] = 'route-ok'
        return row
    route = route_for_contract_inputs(inputs, force_fallback=force_fallback)
    if label in K96_COVERAGE_TARGET_SHAPE_SET and route == ROUTE_K96_A2F8:
        targeted = dict(TARGETED_SEED_ROWS[label])
        generated = label in K96_GENERATED_VARIANT_SHAPES
        return {'shape_key': inputs.get('label'), 'selected_route': route, 'selected_entrypoint': ROUTE_ENTRYPOINTS[route], 'selected_seed': ROUTE_SEED_ID[route], 'expected_seed': 'over64_k96_a2f8_v1', 'route_kind': 'specialized', 'route_source': 'generated-variant' if generated else 'shape-specific-seed', 'guard_id': 'c142_v8_k96_a2f8_exact_qm_guard', 'guard_condition': f'exact BF16 build B=1 Q=M={int(inputs.get('Q'))} D=128 K=96 using A2F8 K96 schedule', 'coverage': 'generated Q1024/Q4096 variant of the A2F8 K96 schedule' if generated else 'exact A2F8 K96 seed selected before inherited K<=32 fallback', 'consumed_seed': ROUTE_SEED_ID[route], 'replaced_route': base_e3de_route, 'base_e3de_route': base_e3de_route, 'base_2cfd_route': base_2cfd_route, 'base_8700_route': base_8700_route, 'row_selection': targeted, 'targeted_seed_timing_backend': 'cupti' if not generated else None, 'targeted_seed_kernel_ms': targeted['kernel_ms'], 'targeted_seed_ratio_vs_flashlib': targeted['ratio_vs_flashlib'], 'classification': targeted['classification'], 'dispatcher_kernel_ms': None, 'shape_specific_kernel_ms': targeted['kernel_ms'], 'relative_speedup_vs_baseline': None}
    if label in RECT_TARGET_SHAPES and route == ROUTE_RECT_9138:
        targeted = dict(TARGETED_SEED_ROWS[label])
...
def _frontmatter_seed_delta_matrix(candidate_report: dict[str, Any], baseline_report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in _seed_delta_matrix(candidate_report, baseline_report):
        rows.append({'shape_key': item['shape_key'], 'baseline_route': item['baseline_route'], 'candidate_deltas': [{'candidate_id': 'e3de_rect9138_q1bcb3_4247_v1', 'selected_seed': item['selected_seed'], 'metric_delta': item['metric_delta_ms'], 'ratio_vs_flashlib': item['ratio_vs_flashlib'], 'timing_backend': item['timing_backend'] or 'cupti'}]})
    return rows
...
def _per_consumed_row_delta(candidate_report: dict[str, Any], baseline_report: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in _seed_delta_matrix(candidate_report, baseline_report):
        ...
        deltas[label] = {'candidate_ms': item['candidate_ms'], 'baseline_2cfd_ms': item['baseline_ms'], 'flashlib_ms': item['flashlib_ms'], 'speedup_vs_baseline_2cfd': item['speedup_vs_baseline_2cfd'], 'ratio_vs_flashlib': item['ratio_vs_flashlib'], 'candidate_route': item['candidate_route'], 'baseline_2cfd_route': item['baseline_route'], 'selected_seed': item['selected_seed'], 'targeted_seed_kernel_ms': item['targeted_seed_kernel_ms'], 'targeted_seed_ratio_vs_flashlib': item['targeted_seed_ratio_vs_flashlib'], 'candidate_passed': candidate_report.get('per_shape', {}).get(label, {}).get('passed'), 'baseline_passed': baseline_report.get('per_shape', {}).get(label, {}).get('passed')}
    return deltas
...
def _annotate_route_trace(route_trace: list[dict[str, Any]], candidate_report: dict[str, Any], baseline_report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in route_trace:
        ...
        out['route_changed_vs_base_2cfd'] = out.get('selected_route') != out.get('base_2cfd_route')
        if label in K96_COVERAGE_TARGET_SHAPE_SET and out['route_changed_vs_base_2cfd']:
            ratio = candidate_row.get('ratio_vs_flashlib')
            if isinstance(ratio, (float, int)) and ratio < 1.0:
                out['classification'] = 'kernel-slow'
            elif label in K96_GENERATED_VARIANT_SHAPES:
                out['classification'] = 'route-ok'
            else:
                out['classification'] = 'seed-consumed'
        elif label in RECT_TARGET_SHAPES and out['route_changed_vs_base_2cfd']:
...
def _hot_bucket_parity(report: dict[str, Any]) -> dict[str, str]:
    buckets = {'d64_build_q1024_q2048_q4096_k10': D64_TARGET_SHAPES, 'rectangular_search_q1024_m32768_d64_k10': RECT_TARGET_SHAPES, 'rag_online_q1_m100000_m131071_m250000_k10': Q1_TARGET_SHAPES, 'build_over64_k96_v8_q1024_q2048_q4096': K96_COVERAGE_TARGET_SHAPES}
    out = {}
...
def _benchmark_candidate(*, use_cupti: bool=True, shape_labels=None, kernel_fn: Callable[[dict[str, Any]], Any]=candidate, measured_function: str, candidate_id: str, baseline_2cfd_report: dict[str, Any] | None=None, baseline_8700_report: dict[str, Any] | None=None) -> dict[str, Any]:
    candidate_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=kernel_fn)
    if baseline_2cfd_report is None:
        try:
            baseline_2cfd_report = _run_with_timing_backend(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_base_2cfd)
        except Exception as exc:
            baseline_2cfd_report = _failed_baseline_report(exc, shape_labels=shape_labels, baseline_id='baseline_2cfd')
    if baseline_8700_report is None:
        try:
            ...
            baseline_8700_report = _failed_baseline_report(exc, shape_labels=shape_labels, baseline_id='baseline_8700')
    return _benchmark_payload(candidate_report, baseline_2cfd_report, baseline_8700_report, use_cupti=use_cupti, shape_labels=shape_labels, measured_function=measured_function, candidate_id=candidate_id)

def benchmark_knn_build_dispatch_e3de_9138_bcb3_4247_v1(*, use_cupti: bool=True, shape_labels=None, baseline_2cfd_report: dict[str, Any] | None=None, baseline_8700_report: dict[str, Any] | None=None) -> dict[str, Any]:
    return _benchmark_candidate(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate, measured_function='benchmark_knn_build_dispatch_e3de_9138_bcb3_4247_v1', candidate_id='e3de_rect9138_q1bcb3_4247_v1', baseline_2cfd_report=baseline_2cfd_report, baseline_8700_report=baseline_8700_report)

def benchmark_rect_only_4247_v1(*, use_cupti: bool=True, shape_labels=None, baseline_2cfd_report: dict[str, Any] | None=None, baseline_8700_report: dict[str, Any] | None=None) -> dict[str, Any]:
    return _benchmark_candidate(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_rect_only, measured_function='benchmark_rect_only_4247_v1', candidate_id='rect_only_4247', baseline_2cfd_report=baseline_2cfd_report, baseline_8700_report=baseline_8700_report)

def benchmark_q1_only_4247_v1(*, use_cupti: bool=True, shape_labels=None, baseline_2cfd_report: dict[str, Any] | None=None, baseline_8700_report: dict[str, Any] | None=None) -> dict[str, Any]:
    return _benchmark_candidate(use_cupti=use_cupti, shape_labels=shape_labels, kernel_fn=candidate_q1_only, measured_function='benchmark_q1_only_4247_v1', candidate_id='q1_only_4247', baseline_2cfd_report=baseline_2cfd_report, baseline_8700_report=baseline_8700_report)
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    payload = benchmark_knn_build_dispatch_e3de_9138_bcb3_4247_v1(use_cupti=use_cupti, shape_labels=shape_labels)
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    denom = _denominator_label(shape_labels)
    candidate_path = out_dir / f'{denom}_dispatch_e3de_9138_bcb3_4247_v1.json'
    baseline_2cfd_path = out_dir / f'{denom}_same_session_baseline_2cfd_for_4247_v1.json'
    baseline_8700_path = out_dir / f'{denom}_same_session_baseline_8700_for_4247_v1.json'
    route_trace_path = out_dir / f'{denom}_route_trace_e3de_9138_bcb3_4247_v1.json'
    forced_trace_path = out_dir / f'{denom}_forced_fallback_trace_e3de_9138_bcb3_4247_v1.json'
    seed_matrix_path = out_dir / f'{denom}_seed_delta_matrix_e3de_9138_bcb3_4247_v1.json'
    ...
    return {'candidate_payload': str(candidate_path), 'same_session_baseline_payload': str(baseline_2cfd_path), 'same_session_8700_payload': str(baseline_8700_path), 'route_trace': str(route_trace_path), 'forced_fallback_trace': str(forced_trace_path), 'seed_delta_matrix': str(seed_matrix_path)}
