...
knn_build_rag_frontier_4b5c_k32_group_merge = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4b5c_v3:knn_build_rag_frontier_4b5c_k32_group_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
...
def _group_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    return _ir_with_constants(knn_build_rag_frontier_4b5c_k32_group_merge, suffix=f'k32s{split_count}g{group_count}_4b5c_v3', TOP_K_MAX=TOP_K_MAX, GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4b5c_v3:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
...
def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    ...
    if _eligible_k32_rag_frontier(inputs):
        return f'rag_frontier_k32_s{K32_SPLIT_COUNT}_g{K32_GROUP_COUNT}_2stage'
...
def benchmark_k32_group_sweep(*, use_cupti: bool=True, group_counts=(4, 8, 9, 12)) -> dict[str, Any]:
    ...
    for group_count in group_counts:
        rows[f'g{group_count}'] = benchmark_knn_build_rag_frontier_4b5c_v3(use_cupti=use_cupti, shape_labels=K32_TARGET_SHAPES, k32_split_count=K32_SPLIT_COUNT, k32_group_count=int(group_count))
