...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_rag_microbucket_q32_lowk_c3d2_v1'
_insert_sorted_pair = _ir_proxy('loom.examples.weave.knn_build_rag_microbucket_q32_lowk_c3d2_v1:_insert_sorted_pair', 256)
knn_build_rag_microbucket_q32_lowk_c3d2_v1_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_lowk_c3d2_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 97536, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 20]], "cta_group": 1, "threads": 192}'))
stage1_ir_base = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_lowk_c3d2_v1_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 97536, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 20]], "cta_group": 1, "threads": 192}'))
knn_build_rag_microbucket_q32_lowk_c3d2_v1_merge = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_lowk_c3d2_v1_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["PARTIAL_TOP_K", 20], ["OUT_TOP_K", 20], ["SPLIT_COUNT", 152], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))
merge_ir_base = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_lowk_c3d2_v1_merge", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["PARTIAL_TOP_K", 20], ["OUT_TOP_K", 20], ["SPLIT_COUNT", 152], ["SPLITS_PER_LANE", 5], ["ROWS_PER_CTA", 4]], "cta_group": 1, "threads": 128}'))
...
def _merge_ir(top_k: int, split_count: int) -> Any:
    if top_k not in SUPPORTED_TOP_K:
        raise ValueError(f'unsupported low-K top_k={top_k}; expected one of {SUPPORTED_TOP_K}')
    return _ir_with_constants(merge_ir_base, suffix=f'k{top_k}s{split_count}_c3d2_v1', PARTIAL_TOP_K=top_k, OUT_TOP_K=top_k, SPLIT_COUNT=split_count, SPLITS_PER_LANE=_splits_per_lane(split_count), ROWS_PER_CTA=Q32_LOWK_ROWS_PER_MERGE_CTA)

def _stage1_ir(top_k: int) -> Any:
    if top_k not in SUPPORTED_TOP_K:
        raise ValueError(f'unsupported low-K top_k={top_k}; expected one of {SUPPORTED_TOP_K}')
    return _ir_with_constants(stage1_ir_base, suffix=f'k{top_k}s{Q32_LOWK_SPLIT_COUNT}_c3d2_v1', BLOCK_Q=Q32_LOWK_BLOCK_Q, BLOCK_M=Q32_LOWK_BLOCK_M, FEAT_D=Q32_LOWK_FEAT_D, TOP_K_MAX=top_k)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_microbucket_q32_lowk_c3d2_v1_stage1_k20s152_c3d2_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 97536, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 20]], "cta_group": 1, "threads": 192}'))
...
def _route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    return f'rag_microbucket_q32_lowk_c3d2_v1_q32_m100000_k{int(inputs.get('K', -1))}_e5db_lowk_s{split_count}_r{Q32_LOWK_ROWS_PER_MERGE_CTA}_lowkmerge'
...
def _launch_q32_lowk(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    top_k = int(inputs['K'])
    if top_k not in SUPPORTED_TOP_K:
        raise ValueError(f'q32 low-K seed only supports K in {SUPPORTED_TOP_K}, got {top_k}')
    ...
    _compiled_stage1_q32_lowk(top_k).launch(grid=(stage1_grid, 1, 1), block=(Q32_LOWK_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_ir.computed_smem_bytes)
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, split_count: int=Q32_LOWK_SPLIT_COUNT) -> dict[str, str]:
    ...
    path = out_dir / f'q32_lowk_c3d2_{len(tuple(shape_labels))}row_s{split_count}_{suffix}.json'
    path.write_text(json.dumps(payload, indent=2, sort_keys=True) + '\n')
    route_path = out_dir / f'q32_lowk_c3d2_{len(tuple(shape_labels))}row_route_trace.json'
