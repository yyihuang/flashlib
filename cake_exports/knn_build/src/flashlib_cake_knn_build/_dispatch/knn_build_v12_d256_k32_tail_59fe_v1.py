...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_v12_d256_k32_tail_59fe_v1'
...
knn_build_v12_d256_k32_tail_59fe_v1_stage1_rowld = _decode_capture(_json_loads('{"__ir__": "knn_build_v12_d256_k32_tail_59fe_v1_stage1_rowld", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["K_TILE", 128], ["FEATURE_CHUNKS", 2], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
stage1_d256_k32_rowld_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_v12_d256_k32_tail_59fe_v1_stage1_rowld", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 99584, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["K_TILE", 128], ["FEATURE_CHUNKS", 2], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if not force_fallback and label is not None:
        return f'v12_d256_k32_tail_59fe:{label}:q{int(SHAPE_SPECS[label]['Q'])}:m{int(SHAPE_SPECS[label]['M'])}:d256:k32:rowld64x64:s{_split_count_for_label(label)}:warpmerge_r{D256_WARP_MERGE_ROWS_PER_CTA}'
...
def _launch_d256_k32_rowld_warpmerge(inputs: dict[str, Any], label: str, *, split_count: int) -> None:
    ...
    if top_k != D256_TOP_K_MAX:
        raise ValueError(f'{label} expected K={D256_TOP_K_MAX}, got {top_k}')
    ...
    _compiled_stage1_d256_k32_rowld().launch(grid=(stage1_grid, 1, 1), block=(D256_STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_d256_k32_rowld_ir.computed_smem_bytes)
