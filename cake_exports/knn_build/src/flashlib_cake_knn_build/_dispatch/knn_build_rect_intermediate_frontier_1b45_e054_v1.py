"""kNN search rectangular Q2048/M32768 K10 exact seed.

Minimum target architecture: sm_100a. This additive bucket-kernel candidate
targets only ``search_rect_b1_q2048_m32768_d128_k10``. It routes the row through
the existing K10 tcgen05/TMA stage-1 producer with an explicit M-axis split
fanout and a matching cached K10 merge. Guard misses delegate to the current
exported Weave dispatcher; the benchmark hook reports only the target bucket
row.
"""
...
merge_k10_s16_cache_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_k10_merge_s7_rowbase_cache_recte054_s16", "arg_keys": ["partial_dists", "partial_indices", "out_dists", "out_indices", "total_queries"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 16]], "cta_group": 1, "threads": 32}'))
merge_k10_s32_cache_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_k10_merge_s7_rowbase_cache_recte054_s32", "arg_keys": ["partial_dists", "partial_indices", "out_dists", "out_indices", "total_queries"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 32]], "cta_group": 1, "threads": 32}'))
merge_k10_s64_cache_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_k10_merge_s7_rowbase_cache_recte054_s64", "arg_keys": ["partial_dists", "partial_indices", "out_dists", "out_indices", "total_queries"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 10], ["SPLIT_COUNT", 64]], "cta_group": 1, "threads": 32}'))

def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_RECT_INTERMEDIATE_E054_VERIFY_KERNEL')
    if verify_kernel == 'merge_s16':
        return merge_k10_s16_cache_ir
    if verify_kernel == 'merge_s32':
        return merge_k10_s32_cache_ir
    if verify_kernel == 'merge_s64':
...
def _merge_ir_for_split(split_count: int) -> Any:
    ...
    if split_count == 64:
        return merge_k10_s64_cache_ir
    raise ValueError(f'unsupported rect intermediate split count: {split_count}')
...
def _rect_split_count() -> int:
    split_text = os.environ.get('LOOM_KNN_RECT_INTERMEDIATE_E054_SPLIT_COUNT')
    if not split_text:
        return SPLIT_COUNT_DEFAULT
    split_count = int(split_text)
    if split_count not in SUPPORTED_SPLITS:
        raise ValueError(f'LOOM_KNN_RECT_INTERMEDIATE_E054_SPLIT_COUNT must be one of {SUPPORTED_SPLITS}')
    return split_count
...
def benchmark_knn_build_rect_intermediate_frontier_1b45_e054_v1(*, use_cupti: bool=False) -> dict[str, Any]:
