...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_k20_search_rect_3cef_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def _split_count() -> int:
    ...
    if split_count not in (4, 8):
        raise ValueError(f'unsupported K20 search split count: {split_count}')
    return split_count
