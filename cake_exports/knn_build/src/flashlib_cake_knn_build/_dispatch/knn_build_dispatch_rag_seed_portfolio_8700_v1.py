...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_rag_seed_portfolio_8700_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))

def _portfolio_route_map(portfolio_id: str) -> dict[int, str]:
    try:
        return PORTFOLIO_ROUTE_BY_Q[portfolio_id]
    except KeyError as exc:
        raise ValueError(f'unknown RAG portfolio id {portfolio_id!r}; expected one of {PORTFOLIO_IDS}') from exc
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None, portfolio_ids: tuple[str, ...]=PORTFOLIO_IDS) -> dict[str, str]:
    ...
    paths: dict[str, str] = {}
    summary_path = out_dir / f'{denom}_synthesis_summary_rag_seed_portfolio_8700_v1.json'
    baseline_path = out_dir / f'{denom}_same_session_baseline_397b_for_8700_v1.json'
    seed_matrix_path = out_dir / f'{denom}_historical_seed_delta_matrix_8700_v1.json'
    ...
    for portfolio_id, payload in summary['payloads'].items():
        candidate_path = out_dir / f'{denom}_dispatch_rag_seed_portfolio_8700_v1_{portfolio_id}.json'
        route_trace_path = out_dir / f'{denom}_route_trace_rag_seed_portfolio_8700_v1_{portfolio_id}.json'
        forced_trace_path = out_dir / f'{denom}_forced_fallback_trace_rag_seed_portfolio_8700_v1_{portfolio_id}.json'
        ...
        paths[f'candidate_{portfolio_id}_payload'] = str(candidate_path)
        paths[f'candidate_{portfolio_id}_route_trace'] = str(route_trace_path)
        paths[f'candidate_{portfolio_id}_forced_fallback_trace'] = str(forced_trace_path)
    return paths
...
def _gpu_identity() -> dict[str, Any]:
    try:
        ...
        return {'cuda_available': True, 'device_index': device, 'device_name': torch.cuda.get_device_name(device), 'capability': f'sm_{capability[0]}{capability[1]}', 'multi_processor_count': props.multi_processor_count, 'total_memory_bytes': props.total_memory}
...
def write_variance_audit_artifacts(artifact_dir: str | Path, *, pair_count: int=3, use_cupti: bool=True, shape_labels=None, portfolio_id: str=DEFAULT_PORTFOLIO_ID) -> dict[str, str]:
    ...
    route_trace_paths = {'candidate': _write_json(out_dir / f'{denom}_route_trace_rag_seed_portfolio_8700_v1_{portfolio_id}.json', candidate_route_trace), 'forced_fallback': _write_json(out_dir / f'{denom}_forced_fallback_trace_rag_seed_portfolio_8700_v1_{portfolio_id}.json', forced_fallback_trace), 'baseline': _write_json(out_dir / f'{denom}_route_trace_selected_portfolio_397b_v1.json', baseline_route_trace)}
    ...
    for pair_index in range(1, pair_count + 1):
        ...
        candidate_payload['audit_candidate_id'] = f'rag_seed_portfolio_8700_v1_{portfolio_id}'
        ...
        candidate_path = _write_json(out_dir / f'pair{pair_index}_rag_seed_portfolio_8700_v1_{portfolio_id}_{denom}.json', candidate_payload)
        baseline_path = _write_json(out_dir / f'pair{pair_index}_baseline_397b_{denom}.json', baseline_payload)
        candidate_record = _report_record(candidate_id=f'rag_seed_portfolio_8700_v1_{portfolio_id}', pair_index=pair_index, path=candidate_path, report=candidate_report, route_trace_count=len(candidate_route_trace))
