"""Opt-in kNN build full55 dispatcher for the a961 selected K64 portfolio.

Minimum target architecture: sm_100a. This dispatcher-consumption candidate is
wrapper-only. It starts from the e51c selected full55 portfolio and consumes one
additional exact Weave seed: the q2048 K64 split8 route from
...
from . import knn_build_dim_midk_bad5_k64split8_v1 as k64_seed
from . import knn_build_dispatch_selected_portfolio_e51c_v1 as base_e51c
ROUTE_K64_A961 = k64_seed.ROUTE_K64_Q2048
...
ADJACENT_GUARD_MISS_SHAPES = ('build_k_sweep_qm1024_k16', 'build_over32_stress_qm4096_k64')
...
PRODUCTION_ROUTE_MODULES = {**base_e51c.PRODUCTION_ROUTE_MODULES, 'midk_k64_q2048_split8': ROUTE_K64_A961, 'base_e51c': ROUTE_BASE_E51C}
...
def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_DISPATCH_SELECTED_A961_VERIFY_KERNEL')
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if _eligible_k64_q2048(inputs):
        return ROUTE_K64_A961
    return base_e51c.route_for_contract_inputs(inputs)

def _launch_route(inputs: dict[str, Any], route: str) -> None:
    if route == ROUTE_K64_A961:
        k64_seed._launch_k64_q2048_split8(inputs)
        return
...
def _frontmatter_seed_delta_matrix(candidate_report: dict[str, Any], baseline_report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for item in _seed_delta_matrix(candidate_report, baseline_report):
        ...
        rows.append({'shape_key': item['shape_key'], 'baseline_route': item['baseline_route'], 'candidate_deltas': [{'candidate_id': 'selected_a961_e51c_plus_k64_split8', 'metric_delta': 0.0 if delta is None else float(delta), 'timing_backend': item['timing_backend'] or 'cuda_event'}]})
    return rows
...
def benchmark_knn_build_dispatch_selected_portfolio_a961_v1(*, use_cupti: bool=False, shape_labels=None) -> dict[str, Any]:
    ...
    return _benchmark_payload(candidate_report, baseline_report, use_cupti=use_cupti, shape_labels=shape_labels, measured_function='benchmark_knn_build_dispatch_selected_portfolio_a961_v1')

def write_benchmark_artifacts(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=False, shape_labels=None) -> dict[str, str]:
    payload = benchmark_knn_build_dispatch_selected_portfolio_a961_v1(use_cupti=use_cupti, shape_labels=shape_labels)
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    candidate_path = out_dir / 'full55_dispatch_selected_portfolio_a961_v1.json'
    baseline_path = out_dir / 'full55_same_session_baseline_e51c_for_a961_v1.json'
    route_trace_path = out_dir / 'full55_route_trace_selected_portfolio_a961_v1.json'
    forced_trace_path = out_dir / 'full55_forced_fallback_trace_selected_portfolio_a961_v1.json'
