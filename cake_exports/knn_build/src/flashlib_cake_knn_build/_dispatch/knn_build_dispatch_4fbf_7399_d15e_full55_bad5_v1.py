"""Opt-in kNN build full55 dispatcher consuming the 4fbf RAG K32 route.

Minimum target architecture: sm_100a. This dispatcher-synthesis candidate is a
wrapper-only portfolio. It starts from the repeated b0e0/fd02 guard scaffold,
keeps the existing a989 large-square, 6c1e K96, 7399 RAG K10, and d15e exact
routes, replaces only the exact RAG K32 frontier row with the 4fbf v7 seed, and
delegates every other row to the same Weave-only baseline policy used by
7c3a/fd02.
...
from . import knn_build_dispatch_b6d4_d15e_fd02_v1 as fd02
from . import knn_build_rag_frontier_4fbf_v7 as rag_4fbf
from . import knn_build_rag_frontier_7399_v1 as rag_7399
large_square = fd02.large_square
over64_k96 = fd02.over64_k96
rect_d15e = fd02.rect_d15e
baseline_3dc7 = fd02.baseline_3dc7
ROUTE_LARGE_SQUARE_K20K32 = fd02.ROUTE_LARGE_SQUARE_K20K32
ROUTE_OVER64_K96 = fd02.ROUTE_OVER64_K96
ROUTE_RAG_7C3A_K10 = fd02.ROUTE_RAG_7C3A_K10
ROUTE_RAG_7399_K10 = 'loom.examples.weave.knn_build_rag_frontier_7399_v1:k10_s72'
ROUTE_RAG_7399_K32 = 'loom.examples.weave.knn_build_rag_frontier_7399_v1:k32_s72_g8_fusedmerge'
ROUTE_RAG_4FBF_K32 = 'loom.examples.weave.knn_build_rag_frontier_4fbf_v7:k32_s72_g24_tailinf_fused'
ROUTE_RECT_D15E = fd02.ROUTE_RECT_D15E
ROUTE_BASELINE_3DC7 = fd02.ROUTE_BASELINE_3DC7
ROUTE_BASELINE_7C3A_POLICY = fd02.ROUTE_BASELINE_7C3A_POLICY
LARGE_SQUARE_TARGET_SHAPES = fd02.LARGE_SQUARE_TARGET_SHAPES
K96_TARGET_SHAPES = fd02.K96_TARGET_SHAPES
RAG_7C3A_K10_TARGET_SHAPES = fd02.RAG_7C3A_K10_TARGET_SHAPES
RAG_K10_TARGET_SHAPES = rag_7399.K10_TARGET_SHAPES
RAG_K32_TARGET_SHAPES = rag_4fbf.K32_TARGET_SHAPES
RAG_TARGET_SHAPES = rag_7399.TARGET_SHAPES
RECT_D15E_TARGET_SHAPES = fd02.RECT_D15E_TARGET_SHAPES
...
CONSUMED_SEED_TARGET_SHAPES = (*RAG_K32_TARGET_SHAPES,)
...
def _verify_export_ir() -> Any:
    verify_kernel = os.environ.get('LOOM_KNN_DISPATCH_4FBF_7399_D15E_VERIFY_KERNEL')
    ...
    if verify_kernel == 'rag_4fbf_k32_stage1':
        os.environ['LOOM_KNN_RAG_FRONTIER_4FBF_V7_VERIFY_KERNEL'] = 'k32_stage1'
        return rag_4fbf._verify_export_ir()
    if verify_kernel == 'rag_4fbf_k32_fused_merge':
        os.environ['LOOM_KNN_RAG_FRONTIER_4FBF_V7_VERIFY_KERNEL'] = 'k32_fused_merge'
        return rag_4fbf._verify_export_ir()
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_4fbf_7399_d15e_full55_bad5_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def _eligible_7c3a_rag_k10(inputs: dict[str, Any]) -> bool:
    return _label_can_hit(inputs, RAG_7C3A_K10_TARGET_SHAPE_SET) and fd02.rag_7c3a._eligible_k10_rag_frontier(inputs)

def _eligible_7399_rag_k10(inputs: dict[str, Any]) -> bool:
    return _label_can_hit(inputs, RAG_K10_TARGET_SHAPE_SET) and rag_7399._eligible_k10_rag_frontier(inputs)

def _eligible_4fbf_rag_k32(inputs: dict[str, Any]) -> bool:
    return _label_can_hit(inputs, RAG_K32_TARGET_SHAPE_SET) and rag_4fbf._eligible_k32_rag_frontier(inputs)
...
def _base_7399_route_for_contract_inputs(inputs: dict[str, Any]) -> str:
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    if _eligible_over64_k96(inputs):
        return ROUTE_OVER64_K96
    if _eligible_7399_rag_k10(inputs):
        return ROUTE_RAG_7399_K10
    if _eligible_4fbf_rag_k32(inputs):
...
def _launch_base_7c3a_route(inputs: dict[str, Any], route: str) -> None:
    fd02._launch_base_7c3a_route(inputs, route)

def _launch_route(inputs: dict[str, Any], route: str) -> None:
    if route == ROUTE_RAG_7399_K10:
        rag_7399._launch_k10_rag_frontier_s72(inputs)
        return
    if route == ROUTE_RAG_4FBF_K32:
        rag_4fbf._launch_k32_rag_frontier_sort4earlystop_stage(inputs)
        return
...
def candidate_without_d15e(inputs: dict[str, Any]):
    route = route_for_contract_inputs(inputs)
    if route == ROUTE_RECT_D15E:
        route = _base_7c3a_route_for_contract_inputs(inputs)
...
def candidate_baseline_7399_d15e(inputs: dict[str, Any]):
...
def _route_trace_record(inputs: dict[str, Any], *, force_fallback: bool=False) -> dict[str, Any]:
    ...
    if route == ROUTE_RAG_7399_K10:
        return {'shape_key': inputs.get('label'), 'selected_route': route, 'guard_condition': 'exact 7399 RAG frontier BF16 D128 non-build K10 label', 'route_kind': 'specialized', 'coverage': 'exact 7399 inherited b6d4/v3 RAG K10 split-72 seed', 'consumed_seed': 'rag_frontier_7399_v1', 'replaced_route': base_route, 'baseline_7c3a_route': base_route, 'baseline_route_kind': _route_kind_for_base(base_route), 'inherited_route': inherited_route, 'parity_status': 'pass', 'parity_reason': '7399 K10 CUPTI ratio_vs_flashlib range is 1.0443 to 1.2952 except microbatch 1.0630'}
    if route == ROUTE_RAG_4FBF_K32:
        return {'shape_key': inputs.get('label'), 'selected_route': route, 'guard_condition': 'exact 4fbf v7 RAG frontier BF16 B1 Q128 M100000 D128 K32 non-build label', 'route_kind': 'specialized', 'coverage': 'exact 4fbf v7 RAG K32 S72/G24 tail-sentinel fused cooperative merge seed', 'consumed_seed': 'rag_frontier_4fbf_v7', 'replaced_route': ROUTE_RAG_7399_K32, 'baseline_7c3a_route': base_route, 'baseline_7399_route': _base_7399_route_for_contract_inputs(inputs), 'baseline_route_kind': _route_kind_for_base(base_route), 'inherited_route': inherited_route, 'parity_status': 'pass', 'parity_reason': '4fbf v7 K32 CUPTI ratio_vs_flashlib is 1.0955379235836016 in the source seed payload'}
    if route == ROUTE_RECT_D15E:
...
def _timing_backends_for_reports(*reports: dict[str, Any]) -> list[str]:
    return fd02._timing_backends_for_reports(*reports)

def _rows_for_labels(report: dict[str, Any], labels: tuple[str, ...]) -> dict[str, Any]:
    return fd02._rows_for_labels(report, labels)

def _params_for_label(label: str) -> dict[str, Any]:
    return fd02._params_for_label(label)
