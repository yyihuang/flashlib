...
def _merge_k32_cache_ir(split_count: int) -> Any:
    ...
    return _ir_with_constants(parent_k32.merge_k30_s8_ir, suffix=f'k32s{split_count}_4b5c_v2', TOP_K_MAX=32, SPLIT_COUNT=split_count)
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_rag_frontier_4b5c_v2:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
...
def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    ...
    if _eligible_k32_rag_frontier(inputs):
        return f'rag_frontier_k32_s{K32_SPLIT_COUNT}'
...
def benchmark_k32_split_sweep(*, use_cupti: bool=True, split_counts=K32_CANDIDATE_SPLITS) -> dict[str, Any]:
    ...
    for split_count in split_counts:
        rows[f's{split_count}'] = benchmark_knn_build_rag_frontier_4b5c_v2(use_cupti=use_cupti, shape_labels=K32_TARGET_SHAPES, k32_split_count=int(split_count))
