...
knn_build_q4096_k8_fd9b_stage1_unordered_exact_prefill = _decode_capture(_json_loads('{"__ir__": "knn_build_q4096_k8_fd9b_stage1_unordered_exact_prefill", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 8]], "cta_group": 1, "threads": 192}'))
knn_build_q4096_k8_fd9b_merge_s4_unordered_warp_select = _decode_capture(_json_loads('{"__ir__": "knn_build_q4096_k8_fd9b_merge_s4_unordered_warp_select", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 8], ["SPLIT_COUNT", 4]], "cta_group": 1, "threads": 128}'))
stage1_k8_unordered_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_evolve_7bfc_split_cg2_u2_stage1_k32_unordered_fd9b_k8unordered", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 8]], "cta_group": 1, "threads": 192}'))
stage1_k8_exact_prefill_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_q4096_k8_fd9b_stage1_unordered_exact_prefill", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 8]], "cta_group": 1, "threads": 192}'))
merge_k8_warp_select_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_q4096_k8_fd9b_merge_s4_unordered_warp_select", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 8], ["SPLIT_COUNT", 4]], "cta_group": 1, "threads": 128}'))
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_q4096_k8_fd9b_merge_s4_unordered_warp_select", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["TOP_K_MAX", 8], ["SPLIT_COUNT", 4]], "cta_group": 1, "threads": 128}'))
...
def _launch_q4096_k8_unordered(inputs: dict[str, Any], *, exact_prefill: bool) -> None:
    ...
    stage1_kernel.launch_cluster(grid=(stage1_grid, 1, 1), block=(v20.STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tile_pairs, db_tiles_per_split, split_count, total_work], cluster_dims=(v20.CTA_GROUP, 1, 1), shared_mem=stage1_ir_obj.computed_smem_bytes)
...
def launch_from_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False, variant: str='exact_prefill') -> None:
    if not force_fallback and _eligible_q4096_k8(inputs):
        ...
        if variant == 'exact_prefill':
            ...
            return
        raise ValueError(f'unknown fd9b q4096/k8 variant {variant!r}')
