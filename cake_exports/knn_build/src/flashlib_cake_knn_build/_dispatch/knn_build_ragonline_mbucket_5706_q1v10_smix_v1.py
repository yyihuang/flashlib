...
SPLIT_BY_M = _decode_capture(_json_loads('{"100000": 144, "131071": 144, "250000": 144, "262143": 144, "524287": 144, "65536": 144}'))
GROUP_BY_M = {}
...
def _verify_export_ir() -> Any:
    ...
    return base_f30c.ir
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_ragonline_mbucket_5706_q1v10_smix_v1:ir"}'))
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    if not force_fallback and _eligible_q1_mix(inputs):
        split_count, group_count = _topology_for_inputs(inputs)
        return f'rag_online_mbucket_5706_q1v10_m{int(inputs['M'])}_s{split_count}_g{group_count}'
...
def write_benchmark_artifacts(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES) -> dict[str, Any]:
    ...
    for name, payload in payloads.items():
        path = out_dir / f'q1v10_5706_{name}_{len(tuple(shape_labels))}row_{suffix}.json'
    ...
    summary_path = out_dir / f'q1v10_5706_summary_{len(tuple(shape_labels))}row_{suffix}.json'
