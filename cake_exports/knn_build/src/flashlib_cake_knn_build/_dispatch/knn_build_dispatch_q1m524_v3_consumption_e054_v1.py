...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_dispatch_q1m524_v3_consumption_e054_v1'
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_q1m524_v3_consumption_e054_v1:ir"}'))
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    candidate_path = out_dir / f'{denom}_q1m524_v3_candidate.json'
    baseline_path = out_dir / f'{denom}_a4ec_same_session_baseline.json'
    trace_path = out_dir / f'{denom}_q1m524_v3_route_trace.json'
...
def write_checkpointed_full_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    out_dir = Path(artifact_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    progress_path = out_dir / f'full{len(labels)}_q1m524_v3_progress.json'
    candidate_path = out_dir / f'full{len(labels)}_q1m524_v3_candidate.json'
    baseline_path = out_dir / f'full{len(labels)}_a4ec_same_session_baseline.json'
    trace_path = out_dir / f'full{len(labels)}_q1m524_v3_route_trace.json'
