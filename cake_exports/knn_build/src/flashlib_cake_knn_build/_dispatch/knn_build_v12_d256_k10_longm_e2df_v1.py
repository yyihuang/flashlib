...
BENCHMARK_ENTRYPOINT = ''.join([format(MODULE, ''), ':benchmark_knn_build_v12_d256_k10_longm_e2df_v1'])
...
def route_for_contract_inputs(inputs: dict[str, Any], *, force_fallback: bool=False) -> str:
    ...
    return ''.join(['v12_d256_k10_e2df:', format(label, ''), ':q', format(int(spec['Q']), ''), ':m', format(int(spec['M']), ''), ':d256:m64n64:s', format(_split_count_for_label(label), ''), ':g', format(_group_count_for_label(label), '')])
...
def _launch_d256_k10_longm(inputs: dict[str, Any], label: str) -> None:
    ...
    if top_k != d256_parent.TOP_K_MAX:
        raise ValueError(''.join([format(label, ''), ' expected K=', format(d256_parent.TOP_K_MAX, ''), ', got ', format(top_k, '')]))
