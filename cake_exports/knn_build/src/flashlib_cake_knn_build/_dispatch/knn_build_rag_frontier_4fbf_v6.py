...
knn_build_rag_frontier_4fbf_stage1_k32_sort4earlystop_tailinf = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_frontier_4fbf_stage1_k32_sort4earlystop_tailinf", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
stage1_k32_tailinf_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_frontier_4fbf_stage1_k32_sort4earlystop_tailinf", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def _fused_merge_ir(split_count: int, group_count: int) -> Any:
    ...
    if group_count != 8:
        raise ValueError(f'4fbf fused merge uses fixed 8-group shared layout, got group_count={group_count}')
    return _ir_with_constants(fused_parent.fused_merge_ir, suffix=f'k32s{split_count}g{group_count}_4fbf_v6', TOP_K_MAX=TOP_K_MAX, GROUP_COUNT=group_count, GROUP_SPLITS=split_count // group_count)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_frontier_4fbf_stage1_k32_sort4earlystop_tailinf", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "constants": [["BLOCK_Q", 128], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 32]], "cta_group": 1, "threads": 192}'))
...
def route_for_contract_inputs(inputs: dict[str, Any]) -> str:
    ...
    if _eligible_k32_rag_frontier(inputs):
        return f'rag_frontier_k32_s{K32_SPLIT_COUNT}_g{K32_GROUP_COUNT}_tailinf_fused4fbf'
