...
BENCHMARK_ENTRYPOINT = f'{MODULE}:benchmark_knn_build_rag_stream_k10_q128_1bed_rowld_v1'
...
knn_build_rag_stream_k10_q128_1bed_rowld_stage1 = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_stream_k10_q128_1bed_rowld_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 54528, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
stage1_rowld_k10_ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_stream_k10_q128_1bed_rowld_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 54528, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _seed_id(split_count: int) -> str:
    return f'{SEED_ID_PREFIX}_s{split_count}'

def _route_name(split_count: int) -> str:
    return f'{MODULE}:q128_m100000_k10_rowld_s{split_count}_warpmerge_r{ROWS_PER_MERGE_CTA}'

def _validate_split_count(split_count: int) -> int:
    if split_count not in SUPPORTED_SPLIT_COUNTS:
        raise ValueError(f'unsupported split count for {MODULE}: {split_count}; expected one of {SUPPORTED_SPLIT_COUNTS}')
    return split_count
...
def _merge_ir(split_count: int) -> Any:
    ...
    return df0f._ir_with_constants(df0f.merge_k10_s74_warp_ir, suffix=f'rowld_s{split_count}_1bed_v1', SPLIT_COUNT=split_count)
...
ir = _decode_capture(_json_loads('{"__ir__": "knn_build_rag_stream_k10_q128_1bed_rowld_stage1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 54528, "constants": [["BLOCK_Q", 64], ["BLOCK_M", 64], ["FEAT_D", 128], ["TOP_K_MAX", 10]], "cta_group": 1, "threads": 192}'))
...
def _launch_q128_m100000_k10_rowld(inputs: dict[str, Any], *, split_count: int) -> None:
    ...
    _compiled_stage1().launch(grid=(stage1_grid, 1, 1), block=(STAGE1_THREADS, 1, 1), args=[inputs['query_sq'], inputs['database_sq'], partial_dists, partial_indices, tmap_query, tmap_database, bsz, n_query, n_database, top_k, num_q_tiles, db_tiles_per_split, split_count, total_work], shared_mem=stage1_rowld_k10_ir.computed_smem_bytes)
...
def _write_artifact(payload: dict[str, Any], artifact_dir: Path) -> Path:
    ...
    path = artifact_dir / f'rag_stream_k10_q128_s{split_count}_rowld_1bed_v1.json'
