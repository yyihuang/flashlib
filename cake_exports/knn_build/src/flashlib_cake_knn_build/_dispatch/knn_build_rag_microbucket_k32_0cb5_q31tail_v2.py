...
def _q31tail_v2_route_name(inputs: dict[str, Any], *, split_count: int) -> str:
    return f'rag_microbucket_k32_0cb5_q31tail_v2_q{int(inputs.get('Q', -1))}_m{int(inputs.get('M', -1))}_k32_row16x256b2cw_q31exact_s{split_count}_r{q32exact.rows4.K32_ROWS4_ROWS_PER_CTA}_warpmerge'
...
def write_benchmark_artifact(artifact_dir: str | os.PathLike[str], *, use_cupti: bool=True, shape_labels=TARGET_SHAPES, k32_q31_split_count: int=K32_Q31_EXACT_SPLIT_COUNT) -> dict[str, str]:
    ...
    path = out_dir / f'0cb5_q31tail_v2_{len(tuple(shape_labels))}row_s{k32_q31_split_count}_{suffix}.json'
