...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_build_dispatch_selected_portfolio_4a72_v1:ir", "cluster_dims": [2, 1, 1], "computed_smem_bytes": 50432, "cta_group": 1, "threads": 192}'))
...
def _baseline_exception_report(exc: BaseException) -> dict[str, Any]:
    ...
    return {'contract': eval_mod.CONTRACT.kernel, 'contract_version': eval_mod.CONTRACT.contract_version, 'summary': {'all_correct': False, 'correctness_failure_count': 1, 'correctness_shapes': 0, 'failed_correctness_shapes': 1, 'first_correctness_failure': {'failure_kind': 'baseline_exception', 'message': message}, 'invalid_performance_reason': f'baseline_exception: {message}', 'performance_comparable': False, 'primary_mean': None, 'primary_metric': 'tflops'}, 'correctness': {'all_correct': False, 'checked_shape_count': 0, 'failed_shape_count': 1, 'failures': [{'failure_kind': 'baseline_exception', 'message': message}], 'first_failure': {'failure_kind': 'baseline_exception', 'message': message}}, 'performance': {'comparable': False, 'debug_measurements_present': False, 'invalid_reason': f'baseline_exception: {message}', 'primary_mean': None, 'primary_metric': 'tflops', 'valid_measurement_count': 0}, 'per_shape': {}, 'baseline_exception': message}
...
def write_benchmark_artifacts(artifact_dir: str | Path, *, use_cupti: bool=True, shape_labels=None) -> dict[str, str]:
    ...
    candidate_path = out_dir / f'{denom}_dispatch_selected_portfolio_4a72_v1.json'
    baseline_path = out_dir / f'{denom}_same_session_baseline_f16b_for_4a72_v1.json'
    route_trace_path = out_dir / f'{denom}_route_trace_selected_portfolio_4a72_v1.json'
    forced_trace_path = out_dir / f'{denom}_forced_fallback_trace_selected_portfolio_4a72_v1.json'
