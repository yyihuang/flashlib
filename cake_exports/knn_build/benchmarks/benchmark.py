...
MEASUREMENT_ORDER_SEED = "flashlib-knn-build-export-paired-v1"
...
def _correctness_query_indices(shape: dict[str, Any], query):
    ...
    return (
        torch.linspace(0, n_query - 1, sample_count, device=query.device)
        .round()
        .to(torch.int64)
        .unique()
    )
    ...
    *,
    ...
    if benchmark:
        ...
        if public_route_info["selected_route"] != route_info["selected_route"]:
            raise RuntimeError(
                "cold public and prepared KNN-build calls selected different routes: "
    ...
    if correctness or benchmark:
        query_indices = _correctness_query_indices(shape, query)
        _, ref_indices = _reference_topk(query, database, k, query_indices)
        torch.cuda.synchronize()
    ...
    if benchmark:
        ...
        result.update(
            {f"baseline_{key}": value for key, value in baseline_correctness.items()}
        )
        ...
        def run_public():
            public_output_holder[0] = knn_build(
                query,
                database,
                k,
                build=build,
