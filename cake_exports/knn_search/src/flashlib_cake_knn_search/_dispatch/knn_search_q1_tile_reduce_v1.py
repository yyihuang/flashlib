...
knn_search_q1_tile_reduce_merge_v1 = _decode_capture(_json_loads('{"__ir__": "knn_search_q1_tile_reduce_merge_v1", "arg_keys": ["partial_distances", "partial_indices", "out_distances", "out_indices", "B", "Q", "K", "num_m_tiles", "num_groups", "tiles_per_group"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 640, "constants": [["K_MAX_", 10]], "cta_group": 1, "threads": 256}'))
ir = _decode_capture(_json_loads('{"__ir__": "knn_search_q1_tile_reduce_partial_v1", "arg_keys": ["queries", "database", "partial_distances", "partial_indices", "B", "Q", "M", "K", "num_m_tiles"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 5120, "constants": [["D_", 128], ["K_MAX_", 10], ["BLOCK_M_", 256], ["NUM_WARPS_", 8], ["NUM_ROW_WORKERS_", 64], ["SUBWARP_WIDTH_", 4], ["SUBWARPS_PER_WARP_", 8], ["LOCAL_LIST_CAP_", 4]], "cta_group": 1, "threads": 256}'))

def _compile_kernels() -> dict[str, Any]:
    return _decode_capture(_json_loads('{"merge": {"__kernel__": "dispatch_kernel_0002"}, "partial": {"__kernel__": "dispatch_kernel_0001"}}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    import torch
    if int(inputs['Q']) != 1:
        raise ValueError(''.join(['knn_search_q1_tile_reduce_v1 supports Q=1, got Q=', format(inputs['Q'], '')]))
    if int(inputs['D']) != D_STATIC:
        raise ValueError(''.join(['knn_search_q1_tile_reduce_v1 supports D=', format(D_STATIC, ''), ', got D=', format(inputs['D'], '')]))
    if int(inputs['K']) > K_MAX:
        raise ValueError(''.join(['knn_search_q1_tile_reduce_v1 supports K <= ', format(K_MAX, ''), ', got K=', format(inputs['K'], '')]))
    if not _KERNELS:
