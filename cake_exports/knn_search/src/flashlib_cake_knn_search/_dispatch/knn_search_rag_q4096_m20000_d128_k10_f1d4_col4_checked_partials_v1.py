...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_rag_q4096_m20000_d128_k10_f1d4_col4_checked_partials_v1:ir"}'))
...
def _validate_inputs(inputs: dict[str, Any]) -> None:
    ...
    if missing:
        raise ValueError(f'checked col4 capture missing tensor fields: {missing}')
...
def capture_col4_partials(inputs: dict[str, Any]) -> tuple[Any, Any]:
    ...
    if split_m != 18 or split_m * mma.MMA_POST_MMA_COL_COHORTS != PARTIAL_LIST_COUNT:
        raise RuntimeError(f'unexpected checked producer ABI: split_m={split_m}')
...
def check_col4_partials(inputs: dict[str, Any], rows: Iterable[int]=DEFAULT_ROWS) -> dict[str, Any]:
    ...
    torch.cuda.synchronize(inputs['queries'].device)
