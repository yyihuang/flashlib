"""Round-99 padded-D tcgen05 route for blind non-D128 kNN labels.

Minimum target architecture: sm_100a for the tcgen05 path. This additive
candidate owns ``B=1,Q=128,M=65536,K=10,D in {64,96,192,320}`` by padding each
128-wide tensor-core chunk in shared memory and reusing the source-clean Q128
split-M merge consumer. Guard misses delegate to the scalar-capacity parent.
"""
...
partial_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:partial_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 120576, "cta_group": 1, "threads": 640}'))
merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:merge_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 120576, "cta_group": 1, "threads": 640}'))
...
_knn_accumulate_q_norm_padded_d = _ir_proxy('loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:_knn_accumulate_q_norm_padded_d', 256)
_knn_stage_q_pass_padded_d = _ir_proxy('loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:_knn_stage_q_pass_padded_d', 256)
...
knn_search_blind_lowd_non_d128_tcgen05_partial_0615_r99_v1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:knn_search_blind_lowd_non_d128_tcgen05_partial_0615_r99_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 120576, "cta_group": 1, "threads": 640}'))
partial_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:partial_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 120576, "cta_group": 1, "threads": 640}'))
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_blind_lowd_non_d128_tcgen05_dispatch0610_r99_ec7c_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 120576, "cta_group": 1, "threads": 640}'))
...
def _compile_non_d128_mma_kernels(dim: int) -> dict[str, Any]:
    ...
    partial_source = generate_kernel(knn_search_blind_lowd_non_d128_tcgen05_partial_0615_r99_v1, validate=False, smem_bytes=MMA_SMEM_BYTES, D_TOTAL_=int(dim), NUM_D_PASSES_=int(_num_d_passes(dim)), Q_NORM_PARTS_=int(_q_norm_parts(dim)))
    ...
    return {'partial': CUDAKernel(partial_cubin, f'kernel_{knn_search_blind_lowd_non_d128_tcgen05_partial_0615_r99_v1.name}'), 'merge': CUDAKernel(merge_cubin, f'kernel_{mma.knn_search_mma_split_merge_q128_const148_v1.name}')}

def _non_d128_mma_scratch(inputs: dict[str, Any], split_m: int, num_q_tiles: int) -> tuple[Any, Any]:
    import torch
    ...
    cached = _NON_D128_MMA_SCRATCH.get(key)
    if cached is None:
        shape = (int(inputs['B']), int(num_q_tiles), int(split_m), BLOCK_Q, K_MAX)
        cached = (torch.empty(shape, dtype=torch.float32, device=inputs['queries'].device), torch.empty(shape, dtype=torch.int32, device=inputs['queries'].device))
        _NON_D128_MMA_SCRATCH[key] = cached
    return cached

def _use_blind_lowd_non_d128_mma(inputs: dict[str, Any]) -> bool:
...
def selected_route(inputs: dict[str, Any]) -> str:
    if _use_blind_lowd_non_d128_mma(inputs):
        return 'round99_blind_lowd_non_d128_padded_tcgen05'
    return 'scalar_capacity_parent'

def _launch_blind_lowd_non_d128_mma(inputs: dict[str, Any]) -> dict[str, Any]:
    import torch
    dim = int(inputs['D'])
    kernels = _NON_D128_MMA_KERNELS.get(dim)
    if kernels is None:
        kernels = _compile_non_d128_mma_kernels(dim)
        _NON_D128_MMA_KERNELS[dim] = kernels
    bsz = int(inputs['B'])
    q_rows = int(inputs['Q'])
    m_rows = int(inputs['M'])
    k = int(inputs['K'])
    ...
    partial_dist, partial_idx = _non_d128_mma_scratch(inputs, split_m, num_q_tiles)
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    if _use_blind_lowd_non_d128_mma(inputs):
        return _launch_blind_lowd_non_d128_mma(inputs)
    return scalar_capacity.launch_for_eval(inputs)

def knn_search_compile_and_launch_blind_lowd_non_d128_tcgen05(*, benchmark: bool=True, shapes: list[dict[str, Any]] | None=None) -> dict[str, Any]:
    from .._dispatch_runtime import evaluate
    result = evaluate(launch_for_eval, shapes=BLIND_LOWD_NON_D128_SHAPES if shapes is None else shapes, benchmark=benchmark)
