...
ir = _decode_capture(_json_loads('{"__ir__": "knn_search_lowq_tile_reduce_partial_0615_196e_blockm640_tailguard_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 5120, "constants": [["D_", 128], ["K_MAX_", 10], ["BLOCK_M_", 640], ["NUM_ROW_WORKERS_", 64], ["SUBWARP_WIDTH_", 4], ["SUBWARPS_PER_WARP_", 8], ["LOCAL_LIST_CAP_", 10]], "cta_group": 1, "threads": 256}'))
partial_ir = _decode_capture(_json_loads('{"__ir__": "knn_search_lowq_tile_reduce_partial_0615_196e_blockm640_tailguard_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 5120, "constants": [["D_", 128], ["K_MAX_", 10], ["BLOCK_M_", 640], ["NUM_ROW_WORKERS_", 64], ["SUBWARP_WIDTH_", 4], ["SUBWARPS_PER_WARP_", 8], ["LOCAL_LIST_CAP_", 10]], "cta_group": 1, "threads": 256}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if q_rows < 2 or q_rows > 64:
        raise ValueError(f'knn_search_lowq_q2q4_blockm640_tailguard_196e supports 2 <= Q <= 64, got Q={q_rows}')
    if int(inputs['D']) != D_STATIC:
        raise ValueError(f'knn_search_lowq_q2q4_blockm640_tailguard_196e supports D={D_STATIC}, got D={inputs['D']}')
    if int(inputs['K']) > K_MAX:
        raise ValueError(f'knn_search_lowq_q2q4_blockm640_tailguard_196e supports K <= {K_MAX}, got K={inputs['K']}')
    if not _KERNELS:
