"""Q128 split-policy candidate for exact BF16 squared-L2 kNN.

Minimum target architecture: sm_100a for the inherited MMA path. This wrapper
keeps the round-3 large-M Q128 path at ``split_m=148`` and uses a lower
``split_m=128`` cap for Q128 small/mid-M shapes, where the generic merge is
already required and the producer probe favored fewer split lists.
"""
...
from . import knn_search_q128_small_mid_split_v2 as _base
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_q128_split_policy_f295_v1:ir"}'))
...
def _select_split_m(q_rows: int, m_rows: int) -> int:
    if q_rows == BLOCK_Q and m_rows >= 8192:
        ...
        split_cap = Q128_LARGE_M_SPLIT_CAP if m_rows >= Q128_LARGE_M_THRESHOLD else Q128_SMALL_M_SPLIT_CAP
...
def _launch_mma_with_q128_policy(inputs: dict[str, Any]) -> dict[str, Any]:
    import torch
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    if int(inputs['K']) > K_MAX:
        raise ValueError(f'knn_search_q128_split_policy_f295_v1 supports K <= {K_MAX}, got {inputs['K']}')
    if _use_q128_split_dispatch(inputs):
        return _launch_mma_with_q128_policy(inputs)
    return _base.launch_for_eval(inputs)

def knn_search_compile_and_launch_q128_split_policy(*, benchmark: bool=True, shapes: list[dict[str, Any]] | None=None) -> dict[str, Any]:
