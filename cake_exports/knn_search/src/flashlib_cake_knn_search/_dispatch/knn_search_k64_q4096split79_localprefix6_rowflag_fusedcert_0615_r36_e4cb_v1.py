...
knn_search_k64_q4096split79_localprefix6_rowflag_fusedcert_merge_0615_r36_e4cb_v1 = _decode_capture(_json_loads('{"__ir__": "knn_search_k64_q4096split79_localprefix6_rowflag_fusedcert_merge_0615_r36_e4cb_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["K_MAX_", 64], ["K_PREFIX_READ_", 6], ["K_STRIDE_", 7]], "cta_group": 1, "threads": 32}'))
...
merge_ir = _decode_capture(_json_loads('{"__ir__": "knn_search_k64_q4096split79_localprefix6_rowflag_fusedcert_merge_0615_r36_e4cb_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["K_MAX_", 64], ["K_PREFIX_READ_", 6], ["K_STRIDE_", 7]], "cta_group": 1, "threads": 32}'))
...
def _compile_k64_q4096_rowflag_fusedcert_kernels() -> dict[str, Any]:
    return _decode_capture(_json_loads('{"merge": {"__kernel__": "dispatch_kernel_0306"}, "partial": {"__kernel__": "dispatch_kernel_0305"}}'))
...
def _launch_q4096_k64_rowflag_fusedcert(inputs: dict[str, Any]) -> dict[str, Any]:
    if not _KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_KERNELS:
        _KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_KERNELS.update(_compile_k64_q4096_rowflag_fusedcert_kernels())
    bsz = int(inputs['B'])
    q_rows = int(inputs['Q'])
    m_rows = int(inputs['M'])
    k = int(inputs['K'])
    ...
    partial_dist, partial_idx = _scratch_rowflag_fusedcert(inputs, partial_list_count, num_q_tiles)
    overflow_rows = _row_flags(inputs, bsz * q_rows)
    _KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_KERNELS['partial'].launch(grid=(bsz * num_q_tiles * split_m, 1, 1), block=(THREADS, 1, 1), args=[inputs['queries'], inputs['database'], partial_dist, partial_idx, bsz, q_rows, m_rows, split_m, num_q_tiles, total_m_tiles, tiles_per_split], shared_mem=MMA_SMEM_BYTES)
    _KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_KERNELS['merge'].launch(grid=(bsz * q_rows, 1, 1), block=(MERGE_THREADS, 1, 1), args=[partial_dist, partial_idx, inputs['out_distances'], inputs['out_indices'], overflow_rows, bsz, q_rows, k, partial_list_count, num_q_tiles], shared_mem=MERGE_SMEM_BYTES)
    ...
    _KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_STATS['last_overflow'] = overflow
    if overflow:
    ...
    _KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_STATS['certified_count'] = int(_KNN_SEARCH_K64_Q4096_ROWFLAG_FUSEDCERT_E4CB_STATS['certified_count'] or 0) + 1
