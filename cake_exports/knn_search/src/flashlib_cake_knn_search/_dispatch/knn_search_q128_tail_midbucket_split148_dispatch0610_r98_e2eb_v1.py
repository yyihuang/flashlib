...
def selected_route(inputs: dict[str, Any]) -> str:
    if _use_q128_split_dispatch(inputs):
        ...
        return ''.join(['q128_tail_midbucket_split', format(split_m, ''), '_mma'])
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    if int(inputs['K']) > K_MAX:
        raise ValueError(''.join(['knn_search_q128_tail_midbucket_split148_dispatch0610_r98_e2eb_v1 supports K <= ', format(K_MAX, ''), ', got ', format(inputs['K'], '')]))
    if _use_q128_split_dispatch(inputs):
