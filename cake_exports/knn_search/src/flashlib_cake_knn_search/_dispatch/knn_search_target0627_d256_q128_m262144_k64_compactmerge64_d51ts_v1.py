"""D256 exact compact-merge tile-64 A/B candidate.

Minimum target architecture: sm_100a.  The full tcgen05 producer writes all
512 sorted partial lists; one two-warp CTA consumes each 64-list compact group
before the inherited rows8 final merge writes exact top-64 outputs.
"""
...
from . import knn_search_target0627_d256_q128_m262144_k64_final8compact_1056_v1 as base
THREADS = base.THREADS
MERGE_THREADS = base.MERGE_THREADS
COMPACT_MERGE_THREADS = 64
...
LISTS_PER_GROUP = base.base.LISTS_PER_GROUP
WARP_LANES = 32
...
final_merge_ir = _decode_capture(_json_loads('{"__ir__": "knn_search_d256_split256_rows8_finalmerge8compact_1056_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["K_MAX_", 64]], "cta_group": 1, "threads": 256}'))
...
knn_search_d256_split256_groupmerge64_tile64_d51ts_v1 = _decode_capture(_json_loads('{"__ir__": "knn_search_d256_split256_groupmerge64_tile64_d51ts_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["K_MAX_", 64]], "cta_group": 1, "threads": 64}'))
group_merge_ir = _decode_capture(_json_loads('{"__ir__": "knn_search_d256_split256_groupmerge64_tile64_d51ts_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "constants": [["K_MAX_", 64]], "cta_group": 1, "threads": 64}'))

def _compile_kernels() -> dict[str, Any]:
    return _decode_capture(_json_loads('{"final": {"__kernel__": "dispatch_kernel_0371"}, "group": {"__kernel__": "dispatch_kernel_0370"}, "partial": {"__kernel__": "dispatch_kernel_0369"}}'))
...
def _launch(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    partial_dist, partial_idx = base.base.base._partial_scratch(inputs, num_q_tiles)
    group_dist, group_idx = base.base.base._group_scratch(inputs)
    ...
    kernels['group'].launch(grid=(bsz * q_rows * HIERARCHICAL_GROUPS, 1, 1), block=(COMPACT_MERGE_THREADS, 1, 1), args=[partial_dist, partial_idx, group_dist, group_idx, bsz, q_rows, k, num_q_tiles], shared_mem=0)
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    return _launch(inputs) if base.base.base._use_target(inputs) else base.launch_for_eval(inputs)

def selected_route(inputs: dict[str, Any]) -> str:
    return ROUTE if base.base.base._use_target(inputs) else base.selected_route(inputs)

def route_info(inputs: dict[str, Any]) -> dict[str, Any]:
    if not base.base.base._use_target(inputs):
        return base.route_info(inputs)
    return {'route': ROUTE, 'selected_route': ROUTE, 'selected_entrypoint': ENTRYPOINT, 'route_kind': 'specialized', 'route_source': 'shape-specific-seed', 'production_policy': 'weave_only', 'arch_requirement': 'sm_100a', 'split_m': SPLIT_M, 'partial_list_count': PARTIAL_LISTS, 'compact_merge_tile': 64, 'hierarchical_groups': HIERARCHICAL_GROUPS, 'producer_to_group_abi': '[B,q_tile,split*2+cohort,q_local,K64] f32/i32 top64', 'group_to_rows8_abi': '[B,q_global,group,K64] f32/i32 sorted top64', 'producer_layout': 'split-major/cohort-minor TMEM top64', 'consumer_layout': '64-thread CTA with one warp owning the 64-list compact merge, then eight warp-owned Q rows/CTA', 'padding_tag': 'none', 'uses_materialized_padding': False, 'uses_kernel_padding': False, 'padding_overhead_timed': False, 'workspace_reuse': 'cached exact-shape scratch'}
