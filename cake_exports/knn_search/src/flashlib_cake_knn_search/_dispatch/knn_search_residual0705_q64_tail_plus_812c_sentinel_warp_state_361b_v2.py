"""Warp-distributed 812c sentinel path for ``residual_q64_m_tail_plus``.

Minimum target architecture: sm_100a.  This structural reset retains the
812c proof boundary for M=262145: the split128 quotient partition visits all
4097 M64 tiles exactly once; the final tile's column 0 is valid, while the
database loader zero-fills columns 1..63 and assigns them INF norms before
tcgen05.  Strict insertion therefore cannot emit an invalid index.

The tcgen05, cohort-local ROW_16x256B.x1 drain, retired-B SMEM plane, two
convergent CTA handoff barriers, 256-list ABI, exact group/final merges, and
caller-owned outputs remain unchanged.  Instead of 812c's per-tile full-K64
SMEM rehydration, each logical query/cohort retains a sorted exact K64 as the
proven 04b4 XOR-2 K32+K32 register pair.  The 63 INF candidates traverse the
same strict boundary exchange and remain non-winning.

There is no materialized padding, sidecar, separate tail producer, or
reference fallback on the exact target route.
"""
...
ROUTE = 'residual0705_q64_tail_plus_812c_sentinel_warp_state_361b_tcgen05'
ENTRYPOINT = 'loom.examples.weave.knn_search_residual0705_q64_tail_plus_812c_sentinel_warp_state_361b_v2:launch_for_eval'
...
_insert_warp_pair_sorted_k64 = _ir_proxy('loom.examples.weave.knn_search_residual0705_q64_tail_plus_812c_sentinel_warp_state_361b_v2:_insert_warp_pair_sorted_k64', 256)
knn_search_q64_tail_plus_812c_sentinel_warp_state_361b_v2 = _decode_capture(_json_loads('{"__ir__": "knn_search_q64_tail_plus_812c_sentinel_warp_state_361b_v2", "arg_keys": ["queries", "database", "partial_distances", "partial_indices", "B", "Q", "M", "split_m", "num_q_tiles", "total_m_tiles", "tiles_per_split"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 106240, "constants": [["K_MAX_", 64]], "cta_group": 1, "threads": 256}'))
partial_ir = _decode_capture(_json_loads('{"__ir__": "knn_search_q64_tail_plus_812c_sentinel_warp_state_361b_v2", "arg_keys": ["queries", "database", "partial_distances", "partial_indices", "B", "Q", "M", "split_m", "num_q_tiles", "total_m_tiles", "tiles_per_split"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 106240, "constants": [["K_MAX_", 64]], "cta_group": 1, "threads": 256}'))
ir = _decode_capture(_json_loads('{"__ir__": "knn_search_q64_tail_plus_812c_sentinel_warp_state_361b_v2", "arg_keys": ["queries", "database", "partial_distances", "partial_indices", "B", "Q", "M", "split_m", "num_q_tiles", "total_m_tiles", "tiles_per_split"], "cluster_dims": [1, 1, 1], "computed_smem_bytes": 106240, "constants": [["K_MAX_", 64]], "cta_group": 1, "threads": 256}'))
_KERNELS: dict[str, Any] = {}

def _compile_kernels() -> dict[str, Any]:
    return _decode_capture(_json_loads('{"final": {"__kernel__": "dispatch_kernel_0151"}, "group": {"__kernel__": "dispatch_kernel_0154"}, "partial": {"__kernel__": "dispatch_kernel_0153"}}'))

def _use_target(inputs: dict[str, Any]) -> bool:
    return not bool(inputs.get('force_fallback', False)) and int(inputs['B']) == TARGET_PARAMS['B'] and (int(inputs['Q']) == TARGET_PARAMS['Q']) and (int(inputs['M']) == TARGET_PARAMS['M']) and (int(inputs['D']) == TARGET_PARAMS['D']) and (int(inputs['K']) == TARGET_PARAMS['K']) and (str(inputs['queries'].dtype) == 'torch.bfloat16') and (not bool(inputs.get('self_search', False))) and (detect_gpu_arch() in {'sm_100a', 'sm_103a'})
