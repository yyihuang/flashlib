...
def _active_profile() -> str:
    ...
    if profile not in PROFILE_CHOICES:
        raise ValueError(f'unknown {PROFILE_ENV}={profile!r}; expected one of {sorted(PROFILE_CHOICES)}')
    return profile
...
def launch_for_profile(inputs: dict[str, Any], profile: str) -> dict[str, Any]:
    if int(inputs['K']) > K_MAX:
        raise ValueError(f'knn_search_q128_m8192_split_probe_0626_ca7b_v1 supports K <= {K_MAX}')
    if profile not in PROFILE_CHOICES:
