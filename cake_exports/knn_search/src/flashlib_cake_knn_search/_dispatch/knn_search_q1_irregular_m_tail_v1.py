...
def _compile_kernels() -> dict[str, Any]:
    return _decode_capture(_json_loads('{"merge": {"__kernel__": "dispatch_kernel_0188"}, "partial": {"__kernel__": "dispatch_kernel_0187"}}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    import torch
    if int(inputs['Q']) != 1:
        raise ValueError(f'knn_search_q1_irregular_m_tail_v1 supports Q=1, got Q={inputs['Q']}')
    if int(inputs['D']) != D_STATIC:
        raise ValueError(f'knn_search_q1_irregular_m_tail_v1 supports D={D_STATIC}, got D={inputs['D']}')
    if int(inputs['K']) > K_MAX:
        raise ValueError(f'knn_search_q1_irregular_m_tail_v1 supports K <= {K_MAX}, got K={inputs['K']}')
    if not _KERNELS:
