...
knn_search_q1_flashdecode_merge128_0614_r92_v1 = _decode_capture(_json_loads('{"__ir__": "knn_search_q1_flashdecode_merge128_0614_r92_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 640, "constants": [["K_MAX_", 10]], "cta_group": 1, "threads": 256}'))

def _compile_kernels() -> dict[str, Any]:
    return _decode_capture(_json_loads('{"merge": {"__kernel__": "dispatch_kernel_0343"}, "partial": {"__kernel__": "dispatch_kernel_0342"}}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    if not supports_shape(inputs):
        raise ValueError(f'knn_search_q1_flashdecode_0614_r92_4b34_v1 supports B=1, Q=1, {MIN_M}<=M<={MAX_M}, D={D_STATIC}, K<={K_MAX}; got B={inputs.get('B', 1)}, Q={inputs['Q']}, M={inputs['M']}, D={inputs['D']}, K={inputs['K']}')
    if not _KERNELS:
    ...
    if tiles_per_group > 128:
        raise ValueError(f'q1 flashdecode merge128 supports at most 128 tiles per group, got {tiles_per_group}')
