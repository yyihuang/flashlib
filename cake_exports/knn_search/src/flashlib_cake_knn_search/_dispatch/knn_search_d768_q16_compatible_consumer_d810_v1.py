...
def _compile_direct_mma_kernels(original_d: int) -> dict[str, Any]:
    ...
    return {'partial': CUDAKernel(partial_cubin, ''.join(['kernel_', format(knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1.symbol, '')])), 'merge': CUDAKernel(merge_cubin, ''.join(['kernel_', format(lowd.merge_ir.symbol, '')])), 'shared_mem': HIGH_MMA_SMEM_BYTES}
