"""Exact D768/Q16/M65536/K10 tcgen05 kNN with a two-stripe TMEM top-k.

Minimum target architecture: sm_100a. This additive exact-bucket candidate
keeps the 148-CTA direct-stride tcgen05 producer and constant-148 merge.  The
BLOCK_M=256 accumulator is reduced as two 128-column TMEM stripes before its
top-10 lists enter the existing fixed148 partial ABI.  The four TMEM cohorts
cover query rows 0..127; this Q16 route initializes and drains rows 0..15.
"""
...
TARGET_M = 65536
TARGET_SPLIT_M = 148
TARGET_LABEL = 'target0627_d768_q16_m65536_k10'
TARGET_ROUTE = 'd768_q16_n256_row16x256b_twelve_stage_d810'
...
_knn_accumulate_q_norm_highd_direct_d = _ir_proxy('loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:_knn_accumulate_q_norm_highd_direct_d', 256)
_knn_stage_q_pass_highd_direct_d = _ir_proxy('loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:_knn_stage_q_pass_highd_direct_d', 256)
_knn_stage_database_pass_highd_direct_d = _ir_proxy('loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:_knn_stage_database_pass_highd_direct_d', 256)
_knn_accumulate_db_norm_blockm256 = _ir_proxy('loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:_knn_accumulate_db_norm_blockm256', 256)
_knn_mma_issue_blockm256 = _ir_proxy('loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:_knn_mma_issue_blockm256', 256)
knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1 = _ir_proxy('loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1', 256)
partial_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:partial_ir"}'))
merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:merge_ir"}'))
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_d768_q16_compatible_consumer_d810_v1:ir"}'))
ROUTE_HIGH_DYNAMIC_D_TCGEN05 = 'q8_blockm256_two_stripe_tmem_fixed148_fragmentmap_v2'
...
HIGH_DYNAMIC_D_SHAPES = _decode_capture(_json_loads('[{"label": "blind_ext_dyn_d768_q32_m32768_k10", "params": {"B": 1, "D": 768, "K": 10, "M": 32768, "Q": 32, "dtype": "bfloat16", "min_recall": 0.999, "seed": 610923, "self_search": false}}, {"label": "blind_ext_dyn_d1024_q16_m32768_k10", "params": {"B": 1, "D": 1024, "K": 10, "M": 32768, "Q": 16, "dtype": "bfloat16", "min_recall": 0.999, "seed": 610924, "self_search": false}}]'))
...
def _compile_direct_mma_kernels(original_d: int) -> dict[str, Any]:
    ...
    partial_source = generate_kernel(knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1, validate=False, smem_bytes=HIGH_MMA_SMEM_BYTES, D_ORIG_=int(original_d), NUM_D_PASSES_=int(math.ceil(original_d / D_STAGE)), Q_NORM_PARTS_=int(math.ceil(original_d / lowd.MMA_STAGE_VEC_ELEMS)))
    ...
    return {'partial': CUDAKernel(partial_cubin, f'kernel_{knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1.name}'), 'merge': CUDAKernel(merge_cubin, f'kernel_{lowd.merge_ir.name}'), 'shared_mem': HIGH_MMA_SMEM_BYTES}
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    if not (int(inputs['B']) == 1 and int(inputs['Q']) == TARGET_Q and (int(inputs['M']) == TARGET_M) and (int(inputs['D']) == HIGH_D_MAX) and (int(inputs['K']) == K_MAX) and (not bool(inputs.get('self_search', False))) and mma._tcgen05_capable_arch()):
        raise ValueError('supports only B=1,Q=16,M=65536,D=768,K=10, non-self search')
...
def knn_search_compile_and_launch_d768_q16_compatible_consumer(*, benchmark: bool=True) -> dict[str, Any]:
    from .._dispatch_runtime import evaluate
    target_shapes = [{'label': 'target0627_d768_q16_m65536_k10', 'params': {'B': 1, 'Q': TARGET_Q, 'M': TARGET_M, 'D': HIGH_D_MAX, 'K': K_MAX, 'dtype': 'bfloat16', 'seed': 612110, 'self_search': False, 'min_recall': 0.999}}]
