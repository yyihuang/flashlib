...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if q_rows < 2 or q_rows > 64:
        raise ValueError(''.join(['knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1 supports 2 <= Q <= 64, got Q=', format(q_rows, '')]))
    if int(inputs['D']) != D_STATIC:
        raise ValueError(''.join(['knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1 supports D=', format(D_STATIC, ''), ', got D=', format(inputs['D'], '')]))
    if int(inputs['K']) > K_MAX:
        raise ValueError(''.join(['knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1 supports K <= ', format(K_MAX, ''), ', got K=', format(inputs['K'], '')]))
    if not _KERNELS:
