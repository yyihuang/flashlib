...
``distances`` and ``indices`` outputs. Round 8 uses a wider ``BLOCK_M=512``
tile to reduce low-Q partial-list and merge overhead.
"""
...
LOCAL_LIST_CAP = _decode_capture(_json_loads('8'))
...
knn_search_lowq_tile_reduce_partial_dispatch0610_r8_blockm512_v1 = _ir_proxy('loom.examples.weave.knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1:knn_search_lowq_tile_reduce_partial_dispatch0610_r8_blockm512_v1', 256)
knn_search_lowq_tile_reduce_merge_dispatch0610_r8_blockm512_v1 = _ir_proxy('loom.examples.weave.knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1:knn_search_lowq_tile_reduce_merge_dispatch0610_r8_blockm512_v1', 256)
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1:ir"}'))
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if q_rows < 2 or q_rows > 64:
        raise ValueError(f'knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1 supports 2 <= Q <= 64, got Q={q_rows}')
    if int(inputs['D']) != D_STATIC:
        raise ValueError(f'knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1 supports D={D_STATIC}, got D={inputs['D']}')
    if int(inputs['K']) > K_MAX:
        raise ValueError(f'knn_search_lowq_tile_reduce_dispatch0610_r8_blockm512_v1 supports K <= {K_MAX}, got K={inputs['K']}')
    if not _KERNELS:
