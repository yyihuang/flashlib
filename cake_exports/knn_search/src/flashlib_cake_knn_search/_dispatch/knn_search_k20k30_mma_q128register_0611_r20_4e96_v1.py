"""Round-20 registered K20/K30 Q128 guarded-merge route for kNN search.

Minimum target architecture: sm_100a for the K13-K30 tcgen05 producer path.
This wrapper preserves the registered round-17 dispatcher for all other
shapes, and routes only ``B=1,Q=128,M>=131072,D=128,K=13..30`` through the
round-18 producer-side cohort repair plus a Q128/split-148 merge that guards
runtime-K stores.
"""
...
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_k20k30_mma_q128register_0611_r20_4e96_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 165120, "cta_group": 1, "threads": 512}'))
_KNN_SEARCH_BUCKET_KERNELS: dict[int, dict[str, Any]] = {}
_KNN_SEARCH_BUCKET_SCRATCH: dict[tuple[int, int, int, int, int, int, int, str], tuple[Any, Any]] = {}
knn_search_k20k30_q128_split148_guarded_merge_v1 = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_k20k30_mma_q128register_0611_r20_4e96_v1:knn_search_k20k30_q128_split148_guarded_merge_v1", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
...
def _compile_bucket_kernels(k_max: int) -> dict[str, Any]:
    ...
    merge_source = generate_kernel(knn_search_k20k30_q128_split148_guarded_merge_v1, validate=False, smem_bytes=MERGE_SMEM_BYTES, K_MAX_=int(k_max))
    ...
    return {'partial': CUDAKernel(partial_cubin, f'kernel_{base.knn_search_mma_split_partial_v1.name}'), 'merge': CUDAKernel(merge_cubin, f'kernel_{knn_search_k20k30_q128_split148_guarded_merge_v1.name}')}
...
def _use_bucket_mma(inputs: dict[str, Any]) -> bool:
    return _bucket_for_k(int(inputs['K'])) is not None and int(inputs['Q']) == BLOCK_Q and (int(inputs['M']) >= 131072) and (int(inputs['D']) == D_STATIC) and base._tcgen05_capable_arch()
