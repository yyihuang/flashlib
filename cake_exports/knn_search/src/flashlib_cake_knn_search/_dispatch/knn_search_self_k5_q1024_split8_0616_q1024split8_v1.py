"""Self-search K5 Q1024 split-M8 seed for exact BF16 squared-L2 kNN.

Minimum target architecture: sm_100a for the Q1024 tcgen05/TMEM route. This
additive seed preserves the 0616 Q256/Q512 direct CUDA-core K5 route and adds
an exact ``B=1,Q=M=1024,D=128,K=5,self_search=True`` guard that launches the
incumbent MMA partial producer with ``split_m=8`` and stream merge. Guard
misses delegate to the prior self-K5 direct wrapper unchanged.
"""
...
from . import knn_search_self_k5_direct_0616_selfk5direct_v1 as parent
THREADS = mma.THREADS
MERGE_THREADS = mma.MERGE_THREADS
BLOCK_Q = mma.BLOCK_Q
BLOCK_M = mma.BLOCK_M
D_STATIC = mma.D_STATIC
SELF_K5_K = parent.K_MAX
Q1024_SELF_ROWS = 1024
Q1024_SELF_SPLIT_M = 8
ROUTE_SELF_K5_Q1024_SPLIT8 = 'round_q1024split8_self_k5_q1024_m1024_split8_partial'
ROUTE_PARENT_SELF_K5_DIRECT = parent.ROUTE_SELF_K5_DIRECT
ROUTE_PARENT_ED52 = parent.ROUTE_PARENT_ED52
ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_self_k5_q1024_split8_0616_q1024split8_v1:ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 108800, "cta_group": 1, "threads": 640}'))
partial_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_self_k5_q1024_split8_0616_q1024split8_v1:partial_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 108800, "cta_group": 1, "threads": 640}'))
merge_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_self_k5_q1024_split8_0616_q1024split8_v1:merge_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 0, "cta_group": 1, "threads": 32}'))
parent_ir = _decode_capture(_json_loads('{"__ir__": "loom.examples.weave.knn_search_self_k5_q1024_split8_0616_q1024split8_v1:parent_ir", "cluster_dims": [1, 1, 1], "computed_smem_bytes": 384, "cta_group": 1, "threads": 256}'))
...
_Q1024_SPLIT8_ENTRY: dict[str, str] = {'shape_key': 'dispatch0616_q1024split8_self_k5_q1024_m1024_split8_partial', 'guard': 'B == 1 and Q == M == 1024 and D == 128 and K == 5 and self_search and tcgen05_capable_arch', 'route': ROUTE_SELF_K5_Q1024_SPLIT8, 'entrypoint': 'loom.examples.weave.knn_search_self_k5_q1024_split8_0616_q1024split8_v1:launch_for_eval'}
SHAPE_DISPATCH_REGISTRY: tuple[dict[str, str], ...] = (_Q1024_SPLIT8_ENTRY, *parent.SHAPE_DISPATCH_REGISTRY)

def _use_q1024_self_split8(inputs: dict[str, Any]) -> bool:
    q_rows = int(inputs['Q'])
...
def selected_route(inputs: dict[str, Any]) -> str:
    if _use_q1024_self_split8(inputs):
        return ROUTE_SELF_K5_Q1024_SPLIT8
    return parent.selected_route(inputs)

def selected_route_name(inputs: dict[str, Any]) -> str:
    return selected_route(inputs)

def _launch_q1024_self_split8(inputs: dict[str, Any]) -> dict[str, Any]:
    import torch
...
def route_info(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if route == ROUTE_SELF_K5_Q1024_SPLIT8:
        parent_info = dict(parent.route_info(inputs))
        parent_route = parent_info.get('route') or parent_info.get('selected_route')
        return {'route': route, 'selected_route': route, 'selected_entrypoint': _Q1024_SPLIT8_ENTRY['entrypoint'], 'parent_route': parent_route, 'replaced_route': parent_route, 'route_kind': 'specialized', 'coverage_class': 'performance_route_self_k5_q1024_split8_partial', 'coverage_only': False, 'production_policy': 'weave_only', 'external_fallback': None, 'guard_order': [entry['shape_key'] for entry in SHAPE_DISPATCH_REGISTRY], 'forced_fallback': False, 'selected_guard': _Q1024_SPLIT8_ENTRY['guard'], 'fallback': ROUTE_PARENT_ED52, 'split_m': Q1024_SELF_SPLIT_M, 'partial_key': 'partial', 'merge_key': 'merge_stream'}
...
def launch_for_eval(inputs: dict[str, Any]) -> dict[str, Any]:
    if _use_q1024_self_split8(inputs):
        return _launch_q1024_self_split8(inputs)
    return parent.launch_for_eval(inputs)

def knn_search_compile_and_launch_self_k5_q1024_split8(*, benchmark: bool=True, shapes: list[dict[str, Any]] | None=None) -> dict[str, Any]:
