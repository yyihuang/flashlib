...
kernel_knn_search_dynamic_lowd_direct_0624_3676_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int M, int K, int D)
{
    ...
    int* warp_idx = reinterpret_cast<int*>(smem_raw + 20512);
    const int warp_idx_addr = smem + 20512;
    int* warp_thread = reinterpret_cast<int*>(smem_raw + 20544);
    const int warp_thread_addr = smem + 20544;
            ...
            if (m_row < M) {
                ...
                if (D > 2) {
                    float _vec_load_7[1];
                    {
                        ...
                        _vec_load_7[0] = __bfloat162float(_bf16_7);
                    }
                    float diff2 = q2 - _vec_load_7[0];
                    dist += diff2 * diff2;
                }
                if (D > 3) {
                    float _vec_load_8[1];
                    {
                        ...
                        _vec_load_8[0] = __bfloat162float(_bf16_8);
                    }
                    float diff3 = q3 - _vec_load_8[0];
                    dist += diff3 * diff3;
                }
                if (D > 4) {
                    float _vec_load_9[1];
                    {
                        ...
                        _vec_load_9[0] = __bfloat162float(_bf16_9);
                    }
                    float diff4 = q4 - _vec_load_9[0];
                    dist += diff4 * diff4;
                }
