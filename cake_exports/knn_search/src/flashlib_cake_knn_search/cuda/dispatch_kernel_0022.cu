...
kernel_knn_search_floor13_k64_prefix8_merge_0622_f3ce_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int partial_list_count, int num_q_tiles)
{
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        #pragma unroll
        for (int slot_1 = 1; slot_1 < 64; slot_1++) {
        ...
        if (lane == winner_lane) {
            #pragma unroll
            for (int slot_2 = 0; slot_2 < 64; slot_2++) {
