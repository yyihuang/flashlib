...
kernel_knn_search_mma_split_partial_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_db_norm_part_next = smem + 141312;
    const int smem_smem_db_norm = smem + 142336;
    const int smem_smem_db_norm_next = smem + 142592;
    ...
    float* smem_db_norm_part_next = (float*)(smem_raw + 141312);
    #define smem_db_norm_part_next_addr (smem + 141312)
    float* smem_db_norm = (float*)(smem_raw + 142336);
    #define smem_db_norm_addr (smem + 142336)
    float* smem_db_norm_next = (float*)(smem_raw + 142592);
    #define smem_db_norm_next_addr (smem + 142592)
    ...
    for (int e_vec = tid; e_vec < 2048; e_vec += 256) {
        ...
        if (batch_id < B) {
            if (q_abs < Q) {
                {
                    const uint4* _vptr_0 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 256 + d_col));
                    uint4 _vld_0[2];
                    #pragma unroll
    ...
    for (int vv = 0; vv < 4; vv++) {
        ...
        if (batch_id < B) {
            if (m_abs_part < M) {
                {
                    const uint4* _vptr_1 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 256 + d_col));
                    uint4 _vld_1[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (batch_id < B) {
            if (m_abs < M) {
                ...
                for (int part = 0; part < 4; part++) {
                    db_norm += smem_db_norm_part[tid + part * 64];
                }
            }
        }
            ...
            if (db_stage_phase == 0) {
                ...
                for (int vv = 0; vv < 4; vv++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 256 + d_col));
                                uint4 _vld_2[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr = (smem_b_next_addr + (d_col / 64 * 8192 + norm_row_0 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 8192 + norm_row_0 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr), "r"(db_pack[0]), "r"(db_pack[1]), "r"(db_pack[2]), "r"(db_pack[3]) : "memory");
                    int b_store_addr_hi = (smem_b_next_addr + ((d_col + 8) / 64 * 8192 + norm_row_0 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 8192 + norm_row_0 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi), "r"(db_pack[4]), "r"(db_pack[5]), "r"(db_pack[6]), "r"(db_pack[7]) : "memory");
                    #pragma unroll
                    for (int vi = 0; vi < 16; vi++) {
                ...
                if (tid < 64) {
                    ...
                    if (batch_id < B) {
                        if (m_abs < M) {
                            ...
                            #pragma unroll
                            for (int part = 0; part < 4; part++) {
                                db_norm += smem_db_norm_part_next[tid + part * 64];
                            }
                        }
                    }
                ...
                for (int vv = 0; vv < 4; vv++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            {
                                const uint4* _vptr_3 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 256 + d_col));
                                uint4 _vld_3[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
            ...
            if (q_local < 128) {
                ...
                if (q_global < Q) {
                    ...
                    for (int j_rel = 0; j_rel < 32; j_rel += 8) {
                        ...
                        if (cand00_d < best_d[K_MAX_ - 1]) {
                            float old_second_tail = best_d[K_MAX_ - 2];
                            best_d[K_MAX_ - 1] = cand00_d;
                            best_i[K_MAX_ - 1] = cand00_i;
                            if (cand00_d < old_second_tail) {
                                #pragma unroll
                                ...
                                if (cand01_d < old_second_tail) {
                                    best_d[K_MAX_ - 1] = cand01_d;
                                    best_i[K_MAX_ - 1] = cand01_i;
                                    if (cand01_d < best_d[K_MAX_ - 2]) {
                                        #pragma unroll
                                        for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                        ...
                        if (cand10_d < best_d[K_MAX_ - 1]) {
                            float old_second_tail = best_d[K_MAX_ - 2];
                            best_d[K_MAX_ - 1] = cand10_d;
                            best_i[K_MAX_ - 1] = cand10_i;
                            if (cand10_d < old_second_tail) {
                                #pragma unroll
                                for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                                    float lower0_d = best_d[kk + 1];
                                    int lower0_i = best_i[kk + 1];
                                ...
                                if (cand11_d < old_second_tail) {
                                    best_d[K_MAX_ - 1] = cand11_d;
                                    best_i[K_MAX_ - 1] = cand11_i;
                                    if (cand11_d < best_d[K_MAX_ - 2]) {
                                        #pragma unroll
                                        for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                                            float lower1_d = best_d[kk + 1];
                                            int lower1_i = best_i[kk + 1];
                        ...
                        if (cand20_d < best_d[K_MAX_ - 1]) {
                            float old_second_tail = best_d[K_MAX_ - 2];
                            best_d[K_MAX_ - 1] = cand20_d;
                            best_i[K_MAX_ - 1] = cand20_i;
                            if (cand20_d < old_second_tail) {
                                #pragma unroll
                                for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                                    float lower0_d = best_d[kk + 1];
                                    int lower0_i = best_i[kk + 1];
                                ...
                                if (cand21_d < old_second_tail) {
                                    best_d[K_MAX_ - 1] = cand21_d;
                                    best_i[K_MAX_ - 1] = cand21_i;
                                    if (cand21_d < best_d[K_MAX_ - 2]) {
                                        #pragma unroll
                                        for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                                            float lower1_d = best_d[kk + 1];
                                            int lower1_i = best_i[kk + 1];
                        ...
                        if (cand30_d < best_d[K_MAX_ - 1]) {
                            float old_second_tail = best_d[K_MAX_ - 2];
                            best_d[K_MAX_ - 1] = cand30_d;
                            best_i[K_MAX_ - 1] = cand30_i;
                            if (cand30_d < old_second_tail) {
                                #pragma unroll
                                for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                                    float lower0_d = best_d[kk + 1];
                                    int lower0_i = best_i[kk + 1];
                                ...
                                if (cand31_d < old_second_tail) {
                                    best_d[K_MAX_ - 1] = cand31_d;
                                    best_i[K_MAX_ - 1] = cand31_i;
                                    if (cand31_d < best_d[K_MAX_ - 2]) {
                                        #pragma unroll
                                        for (int kk = K_MAX_ - 2; kk >= 0; kk--) {
                                            float lower1_d = best_d[kk + 1];
                                            int lower1_i = best_i[kk + 1];
