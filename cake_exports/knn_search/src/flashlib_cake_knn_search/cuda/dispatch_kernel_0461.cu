...
kernel_knn_search_d3_dbscan_q4096_k32_direct_9d5c_r117_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int M, int K)
{
    ...
    int* local_idx = reinterpret_cast<int*>(smem_raw + 16384);
    const int local_idx_addr = smem + 16384;
    float* warp_dist = reinterpret_cast<float*>(smem_raw + 32768);
    const int warp_dist_addr = smem + 32768;
    int* warp_idx = reinterpret_cast<int*>(smem_raw + 32800);
    const int warp_idx_addr = smem + 32800;
    int* warp_thread = reinterpret_cast<int*>(smem_raw + 32832);
    const int warp_thread_addr = smem + 32832;
