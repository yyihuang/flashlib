...
#define K_MAX_ 10
#define D_ORIG_ 768
#define NUM_D_PASSES_ 6
#define Q_NORM_PARTS_ 48
...
kernel_knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int active_split_m)
{
    ...
    float* smem_db_norm_part = reinterpret_cast<float*>(smem_raw + 123904);
    const int smem_db_norm_part_addr = smem + 123904;
    float* smem_db_norm = reinterpret_cast<float*>(smem_raw + 128000);
    const int smem_db_norm_addr = smem + 128000;
    float* smem_cohort_topk_d = reinterpret_cast<float*>(smem_raw + 129024);
    const int smem_cohort_topk_d_addr = smem + 129024;
    int* smem_cohort_topk_i = reinterpret_cast<int*>(smem_raw + 149504);
    const int smem_cohort_topk_i_addr = smem + 149504;
    ...
    if (warp == 0) {
        ...
        // mma_done8: 1 barriers, init_count=1
        mbarrier_init_pred(smem + 64, 1, leader);
        // mma_done9: 1 barriers, init_count=1
        mbarrier_init_pred(smem + 72, 1, leader);
        // mma_done10: 1 barriers, init_count=1
        mbarrier_init_pred(smem + 80, 1, leader);
        // mma_done11: 1 barriers, init_count=1
    ...
    #define mma_done8_addr (mbar_base + 64)
    #define mma_done9_addr (mbar_base + 72)
    #define mma_done10_addr (mbar_base + 80)
    #define mma_done11_addr (mbar_base + 88)
    ...
    #pragma unroll 1
    for (int e_vec = tid; e_vec < 16 * Q_NORM_PARTS_; e_vec += 640) {
    ...
    unsigned int _phase_mma_done8_0 = 0;
    unsigned int _phase_mma_done9_0 = 0;
    unsigned int _phase_mma_done10_0 = 0;
    unsigned int _phase_mma_done11_0 = 0;
    #pragma unroll 1
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        int m_start = m_tile * 256;
        #pragma unroll 1
        for (int e_vec_1 = tid; e_vec_1 < 128; e_vec_1 += 640) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_2 = tid; e_vec_2 < 128; e_vec_2 += 640) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_3 = tid; e_vec_3 < 128; e_vec_3 += 640) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_4 = tid; e_vec_4 < 128; e_vec_4 += 640) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_5 = tid; e_vec_5 < 128; e_vec_5 += 640) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_6 = tid; e_vec_6 < 128; e_vec_6 += 640) {
                    ...
                    if (q_global < Q) {
                        ...
                        for (int j_rel = 0; j_rel < 32; j_rel += 8) {
                            ...
                            for (int _lf = 0; _lf < 1; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair0)[_lf], _fma_b2_13, _fma_c2_14);
                            ...
                            #pragma unroll
                            for (int _lf = 0; _lf < 1; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair1)[_lf], _fma_b2_15, _fma_c2_16);
                            ...
                            #pragma unroll
                            for (int _lf = 0; _lf < 1; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair2)[_lf], _fma_b2_17, _fma_c2_18);
                            ...
                            #pragma unroll
                            for (int _lf = 0; _lf < 1; _lf++)
                                fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair3)[_lf], _fma_b2_19, _fma_c2_20);
