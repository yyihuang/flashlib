...
#define K_MAX_ 10
#define D_ORIG_ 511
#define NUM_D_PASSES_ 4
#define Q_NORM_PARTS_ 32
                ...
                if (batch_id < B) {
                    if (q_abs_4 < Q) {
                        ...
                        for (int vi_19 = 0; vi_19 < 16; vi_19++) {
                            int read_d_7 = global_d_6 + vi_19;
                            if (read_d_7 < D_ORIG_) {
                                q_vals_4[vi_19] = queries[(unsigned long long)((batch_id * Q + q_abs_4) * D_ORIG_ + read_d_7)];
                            }
                        }
                    }
                }
            ...
            if (tid < 512) {
                ...
                for (int vv_3 = 0; vv_3 < 2; vv_3++) {
                    ...
                    for (int vi_20 = 0; vi_20 < 16; vi_20++) {
                        db_vals_3[vi_20] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_3_2 < M) {
                            #pragma unroll
                            for (int vi_21 = 0; vi_21 < 16; vi_21++) {
                                int read_d_8 = global_d_7 + vi_21;
                                if (read_d_8 < D_ORIG_) {
                                    db_vals_3[vi_21] = database[(unsigned long long)((batch_id * M + m_abs_part_3_2) * D_ORIG_ + read_d_8)];
                                }
                            }
                        }
                    }
                    ...
                    for (int vi_22 = 0; vi_22 < 16; vi_22++) {
                        acc_part_4_2 += db_vals_3[vi_22] * db_vals_3[vi_22];
                    }
                }
