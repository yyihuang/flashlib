...
kernel_knn_search_k64_q4096split79_localprefix_merge_0614_r36_edd7_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
            ...
            for (int slot_2 = 0; slot_2 < 10; slot_2++) {
                if (local_best_slot == slot_2) {
                    ...
                    if (next_head < K_PREFIX_) {
                        ...
                        if (valid_slot_1 != 0) {
                            unsigned long long partial_base_1 = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 316 + split_id_1) * 128 + q_local) * K_PREFIX_ + next_head);
