...
kernel_knn_search_lowq_tile_reduce_partial_0615_196e_blockm640_tailguard_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int K, int num_m_tiles)
{
    ...
    if (batch_id < B) {
        ...
        for (int row_iter = 0; row_iter < LOCAL_LIST_CAP_; row_iter++) {
            ...
            dist += _shfl_xor_1;
            dist = ((row_valid != 0) ? dist : LOOM_INF);
            if (sub_lane == 0) {
                if (row_valid != 0) {
                    float carry_d = dist;
                    int carry_i = m_row;
