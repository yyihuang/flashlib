...
kernel_knn_search_target0630_d4096_q4_m32768_k10_merge_independent_warp_q4tail237_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
        ...
        if (lane == winner_lane) {
            ...
            for (int slot = 0; slot < 4; slot++) {
                if (local_best_slot == slot) {
                    ...
                    head_d[slot] = LOOM_INF;
                    head_i[slot] = -1;
                    if (split_id < split_m && next_head < K_MAX_) {
                        unsigned long long partial_base = (unsigned long long)((split_id * 128 + q_local) * K_MAX_ + next_head);
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
