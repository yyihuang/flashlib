...
kernel_knn_search_residual_q128_groupmerge76_fanin8_a8f5_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ group_distances, int* __restrict__ group_indices, int B, int Q, int K, int num_q_tiles)
{
    ...
    int list_group_base = group_id * 76;
    int list_group_end = list_group_base + 76;
    float head_d[3];
    int head_i[3];
    int head_k[3];
    #pragma unroll
    for (int slot = 0; slot < 3; slot++) {
        ...
        head_i[slot] = -1;
        if (partial_id < list_group_end) {
            unsigned long long partial_base = (((batch_id * num_q_tiles + q_tile) * 608 + partial_id) * 128 + q_local) * K_MAX_;
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        #pragma unroll
        for (int slot_1 = 1; slot_1 < 3; slot_1++) {
            float candidate_d = head_d[slot_1];
            int take = ((candidate_d < local_best_d) ? 1 : 0);
            local_best_d = ((take != 0) ? candidate_d : local_best_d);
        ...
        if (lane == winner_lane) {
            #pragma unroll
            for (int slot_2 = 0; slot_2 < 3; slot_2++) {
                if (local_best_slot == slot_2) {
                    ...
                    head_i[slot_2] = -1;
                    if (partial_id_1 < list_group_end) {
                        if (next_head < K_MAX_) {
                            unsigned long long partial_base_1 = (((batch_id * num_q_tiles + q_tile) * 608 + partial_id_1) * 128 + q_local) * K_MAX_ + next_head;
