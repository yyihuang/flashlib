...
kernel_knn_search_q64_tail_groupmerge76_split152_3dee_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ group_distances, int* __restrict__ group_indices, int B, int Q, int K, int num_q_tiles)
{
    ...
    int list_base = group_id * 76;
    int group_end = list_base + 76;
    float head_d[3];
    int head_i[3];
    int head_k[3];
    #pragma unroll
        ...
        if (lane == winner_lane) {
            ...
            for (int slot_2 = 0; slot_2 < 3; slot_2++) {
                if (best_slot == slot_2) {
                    ...
                    head_i[slot_2] = -1;
                    if (partial_id_1 < group_end) {
                        if (next_head < K_MAX_) {
                            unsigned long long partial_base_1 = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 304 + partial_id_1) * 64 + q_local) * K_MAX_ + next_head);
