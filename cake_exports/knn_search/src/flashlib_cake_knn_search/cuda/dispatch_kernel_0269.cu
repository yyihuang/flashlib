...
kernel_knn_search_blind_d384_tcgen05_partial_dispatch0610_r2_f94e_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_db_norm_part = smem + 78848;
    const int smem_smem_db_norm = smem + 80896;
    const int smem_smem_cohort_topk_d = smem + 81408;
    const int smem_smem_cohort_topk_i = smem + 101888;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 78848);
    #define smem_db_norm_part_addr (smem + 78848)
    float* smem_db_norm = (float*)(smem_raw + 80896);
    #define smem_db_norm_addr (smem + 80896)
    float* smem_cohort_topk_d = (float*)(smem_raw + 81408);
    #define smem_cohort_topk_d_addr (smem + 81408)
    int* smem_cohort_topk_i = (int*)(smem_raw + 101888);
    #define smem_cohort_topk_i_addr (smem + 101888)
    ...
    for (int e_vec = tid; e_vec < 3072; e_vec += 640) {
        ...
        float q_vals[16];
        {
            const uint4* _vptr_0 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(q_row * 384 + d_col));
            uint4 _vld_0[2];
            #pragma unroll
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
            ...
            {
                const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(q_row * 384 + global_d));
                uint4 _vld_1[2];
                #pragma unroll
                for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                {
                    const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)(m_abs_part * 384 + global_d));
                    uint4 _vld_2[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
            ...
            unsigned int q_pack[8];
            {
                const uint4* _vptr_3 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(q_row * 384 + global_d));
                uint4 _vld_3[2];
                #pragma unroll
                for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                unsigned int db_pack[8];
                {
                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)(m_abs_part_3 * 384 + global_d));
                    uint4 _vld_4[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
            ...
            unsigned int q_pack[8];
            {
                const uint4* _vptr_5 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(q_row * 384 + global_d));
                uint4 _vld_5[2];
                #pragma unroll
                for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                unsigned int db_pack[8];
                {
                    const uint4* _vptr_6 = reinterpret_cast<const uint4*>(database + (unsigned long long)(m_abs_part_8 * 384 + global_d));
                    uint4 _vld_6[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
