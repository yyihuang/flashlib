...
kernel_knn_search_k1_top1_margin_0614_r93_merge8_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
    ...
    if (q_global < Q) {
        if (split_lane < split_m) {
            unsigned long long partial_base = (unsigned long long)(((batch_id * num_q_tiles + q_tile) * split_m + split_lane) * 128 + q_local);
