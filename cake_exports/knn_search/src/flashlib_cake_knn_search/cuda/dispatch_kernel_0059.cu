...
kernel_knn_search_blind_lowd_non_d128_tcgen05_partial_0615_r99_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_db_norm_part = smem + 76800;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 76800);
    #define smem_db_norm_part_addr (smem + 76800)
    ...
    for (int e_vec = tid; e_vec < 128 * Q_NORM_PARTS_; e_vec += 640) {
        ...
        if (batch_id < B) {
            if (q_abs < Q) {
                {
                    const uint4* _vptr_0 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * D_TOTAL_ + d_col));
                    uint4 _vld_0[2];
                    #pragma unroll
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    if (global_d < D_TOTAL_) {
                        {
                            const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * D_TOTAL_ + global_d));
                            uint4 _vld_1[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part < M) {
                        if (global_d < D_TOTAL_) {
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * D_TOTAL_ + global_d));
                                uint4 _vld_2[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
            ...
            for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
                ...
                if (batch_id < B) {
                    if (q_abs < Q) {
                        if (global_d < D_TOTAL_) {
                            {
                                const uint4* _vptr_3 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * D_TOTAL_ + global_d));
                                uint4 _vld_3[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
                ...
                for (int vv = 0; vv < 2; vv++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            if (global_d < D_TOTAL_) {
                                {
                                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * D_TOTAL_ + global_d));
                                    uint4 _vld_4[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
            ...
            for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
                ...
                if (batch_id < B) {
                    if (q_abs < Q) {
                        if (global_d < D_TOTAL_) {
                            {
                                const uint4* _vptr_5 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * D_TOTAL_ + global_d));
                                uint4 _vld_5[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
                ...
                for (int vv = 0; vv < 2; vv++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            if (global_d < D_TOTAL_) {
                                {
                                    const uint4* _vptr_6 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * D_TOTAL_ + global_d));
                                    uint4 _vld_6[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
