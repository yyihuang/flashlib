...
kernel_knn_search_k64_q4096m20000_prefixcert_fused_merge_0615_576b_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int32_t* __restrict__ overflow_flag, int B, int Q, int K, int partial_list_count, int num_q_tiles)
{
            ...
            for (int slot = 0; slot < 10; slot++) {
                if (local_best_slot == slot) {
                    ...
                    if (next_head < K_PREFIX_READ_) {
                        if (split_id < partial_list_count) {
                            unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 316 + split_id) * 128 + q_local) * K_STRIDE_ + next_head);
                            head_d[slot] = partial_distances[partial_base];
                            head_i[slot] = partial_indices[partial_base];
                        }
                    }
