...
kernel_knn_search_d256_split256_rows8_finalmerge8compact_1056_v1(float* __restrict__ group_distances, int32_t* __restrict__ group_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K)
{
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        if (lane == 0) {
            if (out_k < K) {
                out_distances[out_base + out_k] = global_winner_d;
                out_indices[out_base + out_k] = global_winner_i;
            }
        }
        if (lane == global_winner_lane) {
            ...
            if (lane < 8) {
                if (next_head < K_MAX_) {
                    unsigned long long group_base = (unsigned long long)(((batch_id * Q + q_global) * 8 + lane) * K_MAX_ + next_head);
                    head_d = group_distances[group_base];
                    head_i = group_indices[group_base];
                }
            }
