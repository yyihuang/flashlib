...
kernel_knn_search_d256_mma_split_partial_0612_r34_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    float* smem_db_norm_part0 = reinterpret_cast<float*>(smem_raw + 74752);
    const int smem_db_norm_part0_addr = smem + 74752;
    float* smem_db_norm_part1 = reinterpret_cast<float*>(smem_raw + 76800);
    const int smem_db_norm_part1_addr = smem + 76800;
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec_1 = tid; e_vec_1 < 1024; e_vec_1 += 640) {
            ...
            if (batch_id < B) {
                if (q_abs_1 < Q) {
                    {
                        const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_1) * 256 + global_d) + 0);
                        uint4 _vld_1[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part < M) {
                        {
                            const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 256 + global_d_1) + 0);
                            uint4 _vld_2[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int e_vec_2 = tid; e_vec_2 < 1024; e_vec_2 += 640) {
            ...
            if (batch_id < B) {
                if (q_abs_2 < Q) {
                    {
                        const uint4* _vptr_3 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_2) * 256 + global_d_2) + 0);
                        uint4 _vld_3[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv_1 = 0; vv_1 < 2; vv_1++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_3 < M) {
                        {
                            const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 256 + global_d_3) + 0);
                            uint4 _vld_4[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
