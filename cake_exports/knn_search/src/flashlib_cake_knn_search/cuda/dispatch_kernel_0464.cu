...
#define SMEM_SMEM_Q_CACHE_OFF 54656
#define SMEM_SMEM_Q_CACHE_STAGE_BYTES 8192
#define SMEM_SMEM_Q_CACHE_STRIDE 8192
#define SMEM_TOTAL 62848
#define THREADS 512
#define K_MAX_ 10
...
kernel_knn_search_d4096_q1_m65536_k10_partial_qcache_69ea_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    const int smem_smem_db_norm_part = smem + 51200;
    const int smem_smem_db_norm = smem + 53248;
    const int smem_smem_local_d = smem + 53760;
    const int smem_smem_local_i = smem + 54080;
    const int smem_smem_q_cache = smem + 54656;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 51200);
    #define smem_db_norm_part_addr (smem + 51200)
    float* smem_db_norm = (float*)(smem_raw + 53248);
    #define smem_db_norm_addr (smem + 53248)
    float* smem_local_d = (float*)(smem_raw + 53760);
    #define smem_local_d_addr (smem + 53760)
    int* smem_local_i = (int*)(smem_raw + 54080);
    #define smem_local_i_addr (smem + 54080)
    __nv_bfloat16* smem_q_cache = (__nv_bfloat16*)(smem_raw + 54656);
    #define smem_q_cache_addr (smem + 54656)
    ...
    float best_d[10];
    int best_i[10];
    int q_cache_addr = smem_q_cache_addr + 0;
    #pragma unroll 1
    for (int q_part = tid; q_part < 256; q_part += 512) {
        ...
        if (batch_id < B) {
            {
                const uint4* _vptr_0 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(batch_id * 4096 + d_col));
                uint4 _vld_0[2];
                #pragma unroll
        ...
        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_cache_addr + d_col * 2), "r"(q_pack[0]), "r"(q_pack[1]), "r"(q_pack[2]), "r"(q_pack[3]) : "memory");
        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_cache_addr + d_col * 2 + 16), "r"(q_pack[4]), "r"(q_pack[5]), "r"(q_pack[6]), "r"(q_pack[7]) : "memory");
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        int q_cache_addr_0 = smem_q_cache_addr + 0;
        #pragma unroll 1
        for (int e_vec = tid; e_vec < 8; e_vec += 512) {
            int d_col = e_vec * 16;
            unsigned int q_pack[8];
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&q_pack[0])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[3]))
                : "r"(q_cache_addr_0 + (0 + d_col) * 2));
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&q_pack[4])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 3]))
                : "r"(q_cache_addr_0 + (0 + d_col) * 2 + 16));
            int q_store_addr = (smem_a_addr + (d_col / 64 * 8192 + 0 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 8192 + 0 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr), "r"(q_pack[0]), "r"(q_pack[1]), "r"(q_pack[2]), "r"(q_pack[3]) : "memory");
            int q_store_addr_hi = (smem_a_addr + ((d_col + 8) / 64 * 8192 + 0 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 8192 + 0 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part < M) {
                        {
                            const uint4* _vptr_1 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 4096 + global_d));
                            uint4 _vld_1[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int d_pass = 1; d_pass < 32; d_pass++) {
            ...
            int q_cache_addr_1 = smem_q_cache_addr + 0;
            #pragma unroll 1
            for (int e_vec = tid; e_vec < 8; e_vec += 512) {
                int d_col = e_vec * 16;
                unsigned int q_pack[8];
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&q_pack[0])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[3]))
                    : "r"(q_cache_addr_1 + (d_offset + d_col) * 2));
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&q_pack[4])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 3]))
                    : "r"(q_cache_addr_1 + (d_offset + d_col) * 2 + 16));
                int q_store_addr = (smem_a_addr + (d_col / 64 * 8192 + 0 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 8192 + 0 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr), "r"(q_pack[0]), "r"(q_pack[1]), "r"(q_pack[2]), "r"(q_pack[3]) : "memory");
                int q_store_addr_hi = (smem_a_addr + ((d_col + 8) / 64 * 8192 + 0 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 8192 + 0 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi), "r"(q_pack[4]), "r"(q_pack[5]), "r"(q_pack[6]), "r"(q_pack[7]) : "memory");
            ...
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            int norm_row_2 = tid % 128;
            int norm_part_3 = tid / 128;
            int d_base_4 = norm_part_3 * 32;
            int m_abs_part_5 = m_start + norm_row_2;
            float acc_part_6 = 0.0f;
            if (tid < 512) {
                #pragma unroll 1
                for (int vv = 0; vv < 2; vv++) {
                    int d_col = d_base_4 + vv * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_5 < M) {
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_5) * 4096 + global_d));
                                uint4 _vld_2[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr = (smem_b_addr + (d_col / 64 * 16384 + norm_row_2 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 16384 + norm_row_2 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr), "r"(db_pack[0]), "r"(db_pack[1]), "r"(db_pack[2]), "r"(db_pack[3]) : "memory");
                    int b_store_addr_hi = (smem_b_addr + ((d_col + 8) / 64 * 16384 + norm_row_2 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 16384 + norm_row_2 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi), "r"(db_pack[4]), "r"(db_pack[5]), "r"(db_pack[6]), "r"(db_pack[7]) : "memory");
                    #pragma unroll
                    for (int vi = 0; vi < 16; vi++) {
                        acc_part_6 += db_vals[vi] * db_vals[vi];
                    }
                }
                smem_db_norm_part[tid] = acc_part_6;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 128) {
        ...
        if (warp == 0 || warp == 4) {
            ...
            if (lane < 4) {
                ...
                for (int repeat = 0; repeat < 8; repeat++) {
                    ...
                    float _max_0 = max_noftz(q_norm + smem_db_norm[col_base] - 2.0f * dots[reg_base], 0.0f);
                    float d0 = _max_0;
                    float _max_1 = max_noftz(q_norm + smem_db_norm[col_base + 1] - 2.0f * dots[reg_base + 1], 0.0f);
                    float d1 = _max_1;
                    int take1 = ((d1 < d0) ? 1 : 0);
                    if (((take1 != 0) ? d1 : d0) < best_d[9]) {
    ...
    if (tid == 0) {
        ...
        for (int out_k = 0; out_k < K_MAX_; out_k++) {
            ...
            for (int slot = 1; slot < 8; slot++) {
                int take = ((head_d[slot] < winner_d) ? 1 : 0);
                winner_d = ((take != 0) ? head_d[slot] : winner_d);
                winner_i = ((take != 0) ? head_i[slot] : winner_i);
                winner_slot = ((take != 0) ? slot : winner_slot);
            }
            ...
            for (int slot = 0; slot < 8; slot++) {
                if (winner_slot == slot) {
                    ...
                    head_i[slot] = -1;
                    if (next_head < K_MAX_) {
                        head_d[slot] = smem_local_d[slot * K_MAX_ + next_head];
                        head_i[slot] = smem_local_i[slot * K_MAX_ + next_head];
                    }
                }
            }
        }
