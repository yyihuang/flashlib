...
kernel_knn_search_d256_localcta_rows8_merge_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K, int partial_list_count, int num_q_tiles)
{
        ...
        if (lane == winner_lane) {
            ...
            for (int slot = 0; slot < 8; slot++) {
                if (local_best_slot == slot) {
                    ...
                    if (partial_id < partial_list_count) {
                        if (next_head < K_MAX_) {
                            unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * partial_list_count + partial_id) * 128 + q_local) * K_MAX_ + next_head);
                            head_d[slot] = partial_distances[partial_base];
                            head_i[slot] = partial_indices[partial_base];
                        }
                    }
