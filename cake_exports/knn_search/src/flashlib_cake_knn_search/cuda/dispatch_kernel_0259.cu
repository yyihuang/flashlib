    ...
    if (linear < total) {
        if (linear < total_q) {
            ...
            *((__nv_bfloat16*)(padded_queries + linear)) = __float2bfloat16_rn(value);
            ...
            int d_col = db_linear - db_linear / D_PAD_ * D_PAD_;
            int row = db_linear / D_PAD_;
            float value = 0.0f;
            if (d_col < D_ORIG_) {
                value = database[row * D_ORIG_ + d_col];
            }
            *((__nv_bfloat16*)(padded_database + db_linear)) = __float2bfloat16_rn(value);
        }
    }
}
