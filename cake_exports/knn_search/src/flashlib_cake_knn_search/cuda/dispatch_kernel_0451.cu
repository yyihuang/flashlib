...
kernel_knn_search_dynamic_d3_self_q2048_r123_7d2a_direct_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int M, int K)
{
    ...
    const int smem_local_idx = smem + 5120;
    const int smem_warp_dist = smem + 10240;
    const int smem_warp_idx = smem + 10256;
    const int smem_warp_thread = smem + 10272;
    ...
    int* local_idx = (int*)(smem_raw + 5120);
    #define local_idx_addr (smem + 5120)
    float* warp_dist = (float*)(smem_raw + 10240);
    #define warp_dist_addr (smem + 10240)
    int* warp_idx = (int*)(smem_raw + 10256);
    #define warp_idx_addr (smem + 10256)
    int* warp_thread = (int*)(smem_raw + 10272);
    #define warp_thread_addr (smem + 10272)
