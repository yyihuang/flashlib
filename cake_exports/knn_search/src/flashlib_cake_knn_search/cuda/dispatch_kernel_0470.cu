...
kernel_knn_search_target0627_d4096_q4_m32768_k10_merge148_r217_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
    ...
    for (int slot = 0; slot < 5; slot++) {
        ...
        if (split_id < split_m) {
            ...
            partial_base = partial_base * K_MAX_;
            ...
            for (int slot = 0; slot < 5; slot++) {
                if (local_best_slot == slot) {
                    ...
                    if (split_id < split_m && next_head < K_MAX_) {
                        unsigned long long partial_base = (unsigned long long)(((batch_id * num_q_tiles + q_tile) * split_m + split_id) * 128 + q_local);
                        partial_base = partial_base * K_MAX_ + next_head;
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
