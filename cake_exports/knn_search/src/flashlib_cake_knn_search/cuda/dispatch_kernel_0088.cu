...
kernel_knn_search_d256_mma_split_partial_0612_r34_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_db_norm_part0 = smem + 74752;
    const int smem_smem_db_norm_part1 = smem + 76800;
    const int smem_smem_db_norm = smem + 78848;
    const int smem_smem_cohort_topk_d = smem + 79360;
    const int smem_smem_cohort_topk_i = smem + 99840;
    ...
    float* smem_db_norm_part0 = (float*)(smem_raw + 74752);
    #define smem_db_norm_part0_addr (smem + 74752)
    float* smem_db_norm_part1 = (float*)(smem_raw + 76800);
    #define smem_db_norm_part1_addr (smem + 76800)
    float* smem_db_norm = (float*)(smem_raw + 78848);
    #define smem_db_norm_addr (smem + 78848)
    float* smem_cohort_topk_d = (float*)(smem_raw + 79360);
    #define smem_cohort_topk_d_addr (smem + 79360)
    int* smem_cohort_topk_i = (int*)(smem_raw + 99840);
    #define smem_cohort_topk_i_addr (smem + 99840)
    ...
    #pragma unroll 1
    for (int e_vec = tid; e_vec < 2048; e_vec += 640) {
        int q_elem = e_vec * 16;
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 256 + global_d));
                        uint4 _vld_1[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part < M) {
                        {
                            const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 256 + global_d));
                            uint4 _vld_2[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_3 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 256 + global_d));
                        uint4 _vld_3[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_3 < M) {
                        {
                            const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 256 + global_d));
                            uint4 _vld_4[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
