...
kernel_knn_search_target_d4096_q8_m16384_k10_tcgen05_partial_0623_5ff7_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    int loop_begin = tile_begin;
    int loop_end = tile_end;
    if (loop_end <= loop_begin) {
        loop_begin = total_m_tiles;
        loop_end = total_m_tiles + 1;
    }
    unsigned int _phase_mma_done_0 = 0;
    #pragma unroll 1
    for (int m_tile = loop_begin; m_tile < loop_end; m_tile++) {
        int m_start = m_tile * 128;
        #pragma unroll 1
        for (int d_pass = 0; d_pass < NUM_D_PASSES_; d_pass++) {
            int d_offset = d_pass * 128;
            int init_flag = ((d_pass == 0) ? 1 : 0);
            #pragma unroll 1
            for (int e_vec_1 = tid; e_vec_1 < 512; e_vec_1 += 512) {
                int q_elem = e_vec_1 * 16;
                int q_row_1 = q_elem / 128;
                int d_col_1 = q_elem - q_row_1 * 128;
                int global_d = d_col_1 + d_offset;
            ...
            if (tid < 512) {
                ...
                for (int vv = 0; vv < 2; vv++) {
                    ...
                    int global_d_1 = d_col_2 + d_offset;
                    float db_vals[16];
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n"
            :: "r"(_mma_a_lo_0), "r"(_mma_b_lo_0), "r"(tmem_acc), "r"(((init_flag) ? 0 : 1)));
