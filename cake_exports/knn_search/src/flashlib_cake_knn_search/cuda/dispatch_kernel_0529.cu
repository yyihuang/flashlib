...
kernel_knn_search_d4096_q1_m65536_k10_partial_qcache_pipe_69ea_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int d_pass = 1; d_pass < 32; d_pass++) {
            ...
            if (tid < 8) {
                ...
                int q_cache_addr_0_1 = smem_q_cache_addr;
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    : "r"(q_cache_addr_0_1 + (d_offset + d_col_3) * 2));
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    ...
                    : "r"(q_cache_addr_0_1 + (d_offset + d_col_3) * 2 + 16));
            ...
            if (tid < 512) {
                ...
                for (int vv_1 = 0; vv_1 < 2; vv_1++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 4096 + global_d_1) + 0);
                                uint4 _vld_2[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
