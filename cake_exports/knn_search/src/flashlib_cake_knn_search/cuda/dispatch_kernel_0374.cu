...
kernel_knn_search_ext_k64_highq_prefix8_merge_0618_c2e0_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices)
{
            ...
            for (int slot_2 = 0; slot_2 < 24; slot_2++) {
                if (local_best_slot == slot_2) {
                    ...
                    if (next_head < K_PREFIX_) {
                        unsigned long long partial_base_1 = (unsigned long long)(((q_tile * 768 + split_id_1) * 128 + q_local) * K_PREFIX_ + next_head);
