...
kernel_knn_search_k64_q4096split79_localprefix_merge_0614_r36_edd7_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
    ...
    for (int slot = 0; slot < 10; slot++) {
        ...
        if (valid_slot != 0) {
            unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 316 + split_id) * 128 + q_local) * K_PREFIX_);
            ...
            for (int slot = 0; slot < 10; slot++) {
                if (local_best_slot == slot) {
                    ...
                    if (next_head < K_PREFIX_) {
                        ...
                        if (valid_slot != 0) {
                            unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 316 + split_id) * 128 + q_local) * K_PREFIX_ + next_head);
                            head_d[slot] = partial_distances[partial_base];
                            head_i[slot] = partial_indices[partial_base];
                        }
                    }
