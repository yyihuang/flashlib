...
kernel_knn_search_ext_k64_highq_prefix8_merge_0618_c2e0_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices)
{
    ...
    for (int slot = 0; slot < 24; slot++) {
        ...
        head_k[slot] = 0;
        unsigned long long partial_base = (unsigned long long)(((q_tile * 768 + split_id) * 128 + q_local) * K_PREFIX_);
            ...
            for (int slot = 0; slot < 24; slot++) {
                if (local_best_slot == slot) {
                    ...
                    if (next_head < K_PREFIX_) {
                        unsigned long long partial_base = (unsigned long long)(((q_tile * 768 + split_id) * 128 + q_local) * K_PREFIX_ + next_head);
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
