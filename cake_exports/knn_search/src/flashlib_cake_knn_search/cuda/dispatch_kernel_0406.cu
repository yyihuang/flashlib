...
kernel_knn_search_blind_k64_q4096_m32768_prefix8_merge_5132_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices)
{
            ...
            for (int slot = 0; slot < 16; slot++) {
                if (local_best_slot == slot) {
                    ...
                    if (next_head < K_PREFIX_) {
                        unsigned long long partial_base = (unsigned long long)(((q_tile * 512 + split_id) * 128 + q_local) * K_PREFIX_ + next_head);
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
