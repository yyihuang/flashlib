...
#define SMEM_SMEM_Q_NORM_PART_STAGE_BYTES 32768
#define SMEM_SMEM_Q_NORM_PART_STRIDE 32768
#define SMEM_SMEM_DB_NORM_PART_OFF 132096
#define SMEM_SMEM_DB_NORM_PART_STAGE_BYTES 4096
#define SMEM_SMEM_DB_NORM_PART_STRIDE 4096
#define SMEM_SMEM_DB_NORM_OFF 136192
#define SMEM_SMEM_DB_NORM_STAGE_BYTES 1024
#define SMEM_SMEM_DB_NORM_STRIDE 1024
#define SMEM_SMEM_COHORT_TOPK_D_OFF 137216
#define SMEM_SMEM_COHORT_TOPK_D_STAGE_BYTES 20480
#define SMEM_SMEM_COHORT_TOPK_D_STRIDE 20480
#define SMEM_SMEM_COHORT_TOPK_I_OFF 157696
...
#define K_MAX_ 10
#define D_ORIG_ 1024
#define NUM_D_PASSES_ 8
#define Q_NORM_PARTS_ 64
...
kernel_knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int active_split_m)
{
    ...
    float* smem_db_norm = reinterpret_cast<float*>(smem_raw + 136192);
    const int smem_db_norm_addr = smem + 136192;
    float* smem_cohort_topk_d = reinterpret_cast<float*>(smem_raw + 137216);
    const int smem_cohort_topk_d_addr = smem + 137216;
    int* smem_cohort_topk_i = reinterpret_cast<int*>(smem_raw + 157696);
    const int smem_cohort_topk_i_addr = smem + 157696;
    ...
    if (warp == 0) {
        ...
        // mma_done4: 1 barriers, init_count=1
        mbarrier_init_pred(smem + 32, 1, leader);
        // mma_done5: 1 barriers, init_count=1
        mbarrier_init_pred(smem + 40, 1, leader);
        // mma_done6: 1 barriers, init_count=1
        mbarrier_init_pred(smem + 48, 1, leader);
        // mma_done7: 1 barriers, init_count=1
    ...
    #define mma_done4_addr (mbar_base + 32)
    #define mma_done5_addr (mbar_base + 40)
    #define mma_done6_addr (mbar_base + 48)
    #define mma_done7_addr (mbar_base + 56)
    ...
    #pragma unroll 1
    for (int e_vec = tid; e_vec < 8 * Q_NORM_PARTS_; e_vec += 640) {
            ...
            if (q_local < 128) {
                if (q_global < Q) {
                    ...
                    for (int part = 0; part < Q_NORM_PARTS_; part++) {
                        q_norm += smem_q_norm_part[q_local * 64 + part];
                    }
                }
            }
        }
    ...
    int tile_begin = 0;
    int tile_end = 0;
    if (split_id < active_split_m) {
        tile_begin = split_id * total_m_tiles / active_split_m;
        int next_split = split_id + 1;
        tile_end = next_split * total_m_tiles / active_split_m;
    }
    ...
    unsigned int _phase_mma_done4_0 = 0;
    unsigned int _phase_mma_done5_0 = 0;
    unsigned int _phase_mma_done6_0 = 0;
    unsigned int _phase_mma_done7_0 = 0;
    #pragma unroll 1
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        int m_start = m_tile * 256;
        #pragma unroll 1
        for (int e_vec_1 = tid; e_vec_1 < 64; e_vec_1 += 640) {
        ...
        __syncthreads();
        #pragma unroll 1
        for (int norm_linear = tid; norm_linear < 1024; norm_linear += 640) {
            int norm_row = norm_linear % 256;
            int norm_part = norm_linear / 256;
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                int b_store_addr = (smem_b_addr + (unsigned int)(d_col_2 / 64 * 32768 + norm_row * 128 + d_col_2 % 64 * 2 ^ (d_col_2 / 64 * 32768 + norm_row * 128 + d_col_2 % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr), "r"(db_pack[0]), "r"(db_pack[1]), "r"(db_pack[2]), "r"(db_pack[3]) : "memory");
                int b_store_addr_hi = (smem_b_addr + (unsigned int)((d_col_2 + 8) / 64 * 32768 + norm_row * 128 + (d_col_2 + 8) % 64 * 2 ^ ((d_col_2 + 8) / 64 * 32768 + norm_row * 128 + (d_col_2 + 8) % 64 * 2 >> 7 & 7) << 4));
            ...
            smem_db_norm_part[norm_linear] = acc_part;
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        __syncthreads();
        if (tid < 256) {
            ...
            if (m_abs < M) {
                ...
                for (int part_1 = 0; part_1 < 4; part_1++) {
                    pass_norm += smem_db_norm_part[tid + part_1 * 256];
                }
            }
            {
        ...
        {
            #pragma unroll 1
            for (int e_vec_2 = tid; e_vec_2 < 64; e_vec_2 += 640) {
            ...
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_1 = tid; norm_linear_1 < 1024; norm_linear_1 += 640) {
                int norm_row_1 = norm_linear_1 % 256;
                int norm_part_1 = norm_linear_1 / 256;
                int d_base_1 = norm_part_1 * 32;
                int m_abs_part_1 = m_start + norm_row_1;
                float acc_part_1 = 0.0f;
                #pragma unroll 1
                for (int vv_1 = 0; vv_1 < 2; vv_1++) {
                    int d_col_4 = d_base_1 + vv_1 * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_1 < M) {
                            if (global_d_3 < D_ORIG_) {
                                {
                                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_1) * D_ORIG_ + global_d_3) + 0);
                                    uint4 _vld_4[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_1 = (smem_b_addr + (unsigned int)(d_col_4 / 64 * 32768 + norm_row_1 * 128 + d_col_4 % 64 * 2 ^ (d_col_4 / 64 * 32768 + norm_row_1 * 128 + d_col_4 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_1), "r"(db_pack_1[0]), "r"(db_pack_1[1]), "r"(db_pack_1[2]), "r"(db_pack_1[3]) : "memory");
                    int b_store_addr_hi_1 = (smem_b_addr + (unsigned int)((d_col_4 + 8) / 64 * 32768 + norm_row_1 * 128 + (d_col_4 + 8) % 64 * 2 ^ ((d_col_4 + 8) / 64 * 32768 + norm_row_1 * 128 + (d_col_4 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_5 = 0; vi_5 < 16; vi_5++) {
                        acc_part_1 += db_vals_1[vi_5] * db_vals_1[vi_5];
                    }
                }
                smem_db_norm_part[norm_linear_1] = acc_part_1;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                ...
                if (m_abs_1 < M) {
                    ...
                    for (int part_2 = 0; part_2 < 4; part_2++) {
                        pass_norm_1 += smem_db_norm_part[tid + part_2 * 256];
                    }
                }
                {
                    if (m_abs_1 < M) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_3 = tid; e_vec_3 < 64; e_vec_3 += 640) {
                ...
                {
                    const uint4* _vptr_5 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_3) * D_ORIG_ + global_d_4) + 0);
                    uint4 _vld_5[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
            ...
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_2 = tid; norm_linear_2 < 1024; norm_linear_2 += 640) {
                int norm_row_2 = norm_linear_2 % 256;
                int norm_part_2 = norm_linear_2 / 256;
                int d_base_2 = norm_part_2 * 32;
                int m_abs_part_2 = m_start + norm_row_2;
                float acc_part_2 = 0.0f;
                #pragma unroll 1
                for (int vv_2 = 0; vv_2 < 2; vv_2++) {
                    int d_col_6 = d_base_2 + vv_2 * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_2 < M) {
                            if (global_d_5 < D_ORIG_) {
                                {
                                    const uint4* _vptr_6 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_2) * D_ORIG_ + global_d_5) + 0);
                                    uint4 _vld_6[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_2 = (smem_b_addr + (unsigned int)(d_col_6 / 64 * 32768 + norm_row_2 * 128 + d_col_6 % 64 * 2 ^ (d_col_6 / 64 * 32768 + norm_row_2 * 128 + d_col_6 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_2), "r"(db_pack_2[0]), "r"(db_pack_2[1]), "r"(db_pack_2[2]), "r"(db_pack_2[3]) : "memory");
                    int b_store_addr_hi_2 = (smem_b_addr + (unsigned int)((d_col_6 + 8) / 64 * 32768 + norm_row_2 * 128 + (d_col_6 + 8) % 64 * 2 ^ ((d_col_6 + 8) / 64 * 32768 + norm_row_2 * 128 + (d_col_6 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_7 = 0; vi_7 < 16; vi_7++) {
                        acc_part_2 += db_vals_2[vi_7] * db_vals_2[vi_7];
                    }
                }
                smem_db_norm_part[norm_linear_2] = acc_part_2;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                ...
                if (m_abs_2 < M) {
                    ...
                    for (int part_3 = 0; part_3 < 4; part_3++) {
                        pass_norm_2 += smem_db_norm_part[tid + part_3 * 256];
                    }
                }
                {
                    if (m_abs_2 < M) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_4 = tid; e_vec_4 < 64; e_vec_4 += 640) {
                ...
                {
                    const uint4* _vptr_7 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_4) * D_ORIG_ + global_d_6) + 0);
                    uint4 _vld_7[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
            ...
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_3 = tid; norm_linear_3 < 1024; norm_linear_3 += 640) {
                int norm_row_3 = norm_linear_3 % 256;
                int norm_part_3 = norm_linear_3 / 256;
                int d_base_3 = norm_part_3 * 32;
                int m_abs_part_3 = m_start + norm_row_3;
                float acc_part_3 = 0.0f;
                #pragma unroll 1
                for (int vv_3 = 0; vv_3 < 2; vv_3++) {
                    int d_col_8 = d_base_3 + vv_3 * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            if (global_d_7 < D_ORIG_) {
                                {
                                    const uint4* _vptr_8 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * D_ORIG_ + global_d_7) + 0);
                                    uint4 _vld_8[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_3 = (smem_b_addr + (unsigned int)(d_col_8 / 64 * 32768 + norm_row_3 * 128 + d_col_8 % 64 * 2 ^ (d_col_8 / 64 * 32768 + norm_row_3 * 128 + d_col_8 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_3), "r"(db_pack_3[0]), "r"(db_pack_3[1]), "r"(db_pack_3[2]), "r"(db_pack_3[3]) : "memory");
                    int b_store_addr_hi_3 = (smem_b_addr + (unsigned int)((d_col_8 + 8) / 64 * 32768 + norm_row_3 * 128 + (d_col_8 + 8) % 64 * 2 ^ ((d_col_8 + 8) / 64 * 32768 + norm_row_3 * 128 + (d_col_8 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_9 = 0; vi_9 < 16; vi_9++) {
                        acc_part_3 += db_vals_3[vi_9] * db_vals_3[vi_9];
                    }
                }
                smem_db_norm_part[norm_linear_3] = acc_part_3;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                ...
                if (m_abs_3 < M) {
                    ...
                    for (int part_4 = 0; part_4 < 4; part_4++) {
                        pass_norm_3 += smem_db_norm_part[tid + part_4 * 256];
                    }
                }
                {
                    if (m_abs_3 < M) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_5 = tid; e_vec_5 < 64; e_vec_5 += 640) {
                int q_elem_4 = e_vec_5 * 16;
                int q_row_5 = q_elem_4 / 128;
                int d_col_9 = q_elem_4 - q_row_5 * 128;
                int global_d_8 = d_col_9 + 512;
                int q_abs_5 = q_start + q_row_5;
                float q_vals_5[16];
                unsigned int q_pack_4[8];
                {
                    const uint4* _vptr_9 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_5) * D_ORIG_ + global_d_8) + 0);
                    uint4 _vld_9[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
                        _vld_9[_blk] = _vptr_9[_blk];
                        __nv_bfloat16* _velems_9 = reinterpret_cast<__nv_bfloat16*>(&_vld_9[_blk]);
                        #pragma unroll
                        for (int _j = 0; _j < 8; _j++)
                            q_vals_5[0 + _blk * 8 + _j] = __bfloat162float(_velems_9[_j]);
                    }
                }
                #pragma unroll
                for (int _lp = 0; _lp < 8; _lp++) {
            ...
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_4 = tid; norm_linear_4 < 1024; norm_linear_4 += 640) {
                int norm_row_4 = norm_linear_4 % 256;
                int norm_part_4 = norm_linear_4 / 256;
                int d_base_4 = norm_part_4 * 32;
                int m_abs_part_4 = m_start + norm_row_4;
                float acc_part_4 = 0.0f;
                #pragma unroll 1
                for (int vv_4 = 0; vv_4 < 2; vv_4++) {
                    int d_col_10 = d_base_4 + vv_4 * 16;
                    int global_d_9 = d_col_10 + 512;
                    float db_vals_4[16];
                    unsigned int db_pack_4[8];
                    #pragma unroll
                    for (int vi_10 = 0; vi_10 < 16; vi_10++) {
                        db_vals_4[vi_10] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_4 < M) {
                            if (global_d_9 < D_ORIG_) {
                                {
                                    const uint4* _vptr_10 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_4) * D_ORIG_ + global_d_9) + 0);
                                    uint4 _vld_10[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                                        _vld_10[_blk] = _vptr_10[_blk];
                                        __nv_bfloat16* _velems_10 = reinterpret_cast<__nv_bfloat16*>(&_vld_10[_blk]);
                                        #pragma unroll
                                        for (int _j = 0; _j < 8; _j++)
                                            db_vals_4[0 + _blk * 8 + _j] = __bfloat162float(_velems_10[_j]);
                                    }
                                }
                            }
                        }
                    ...
                    int b_store_addr_4 = (smem_b_addr + (unsigned int)(d_col_10 / 64 * 32768 + norm_row_4 * 128 + d_col_10 % 64 * 2 ^ (d_col_10 / 64 * 32768 + norm_row_4 * 128 + d_col_10 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_4), "r"(db_pack_4[0]), "r"(db_pack_4[1]), "r"(db_pack_4[2]), "r"(db_pack_4[3]) : "memory");
                    int b_store_addr_hi_4 = (smem_b_addr + (unsigned int)((d_col_10 + 8) / 64 * 32768 + norm_row_4 * 128 + (d_col_10 + 8) % 64 * 2 ^ ((d_col_10 + 8) / 64 * 32768 + norm_row_4 * 128 + (d_col_10 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_11 = 0; vi_11 < 16; vi_11++) {
                        acc_part_4 += db_vals_4[vi_11] * db_vals_4[vi_11];
                    }
                }
                smem_db_norm_part[norm_linear_4] = acc_part_4;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                int m_abs_4 = m_start + tid;
                float pass_norm_4 = LOOM_INF;
                if (m_abs_4 < M) {
                    pass_norm_4 = 0.0f;
                    #pragma unroll
                    for (int part_5 = 0; part_5 < 4; part_5++) {
                        pass_norm_4 += smem_db_norm_part[tid + part_5 * 256];
                    }
                }
                {
                    if (m_abs_4 < M) {
                        smem_db_norm[tid] = smem_db_norm[tid] + pass_norm_4;
                    } else {
                        smem_db_norm[tid] = LOOM_INF;
                    }
                }
            ...
            _phase_mma_done4_0 ^= 1;
        }
        {
            #pragma unroll 1
            for (int e_vec_6 = tid; e_vec_6 < 64; e_vec_6 += 640) {
                int q_elem_5 = e_vec_6 * 16;
                int q_row_6 = q_elem_5 / 128;
                int d_col_11 = q_elem_5 - q_row_6 * 128;
                int global_d_10 = d_col_11 + 640;
                int q_abs_6 = q_start + q_row_6;
                float q_vals_6[16];
                unsigned int q_pack_5[8];
                {
                    const uint4* _vptr_11 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_6) * D_ORIG_ + global_d_10) + 0);
                    uint4 _vld_11[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
                        _vld_11[_blk] = _vptr_11[_blk];
                        __nv_bfloat16* _velems_11 = reinterpret_cast<__nv_bfloat16*>(&_vld_11[_blk]);
                        #pragma unroll
                        for (int _j = 0; _j < 8; _j++)
                            q_vals_6[0 + _blk * 8 + _j] = __bfloat162float(_velems_11[_j]);
                    }
                }
                #pragma unroll
                for (int _lp = 0; _lp < 8; _lp++) {
            ...
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_5 = tid; norm_linear_5 < 1024; norm_linear_5 += 640) {
                int norm_row_5 = norm_linear_5 % 256;
                int norm_part_5 = norm_linear_5 / 256;
                int d_base_5 = norm_part_5 * 32;
                int m_abs_part_5 = m_start + norm_row_5;
                float acc_part_5 = 0.0f;
                #pragma unroll 1
                for (int vv_5 = 0; vv_5 < 2; vv_5++) {
                    int d_col_12 = d_base_5 + vv_5 * 16;
                    int global_d_11 = d_col_12 + 640;
                    float db_vals_5[16];
                    unsigned int db_pack_5[8];
                    #pragma unroll
                    for (int vi_12 = 0; vi_12 < 16; vi_12++) {
                        db_vals_5[vi_12] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_5 < M) {
                            if (global_d_11 < D_ORIG_) {
                                {
                                    const uint4* _vptr_12 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_5) * D_ORIG_ + global_d_11) + 0);
                                    uint4 _vld_12[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                                        _vld_12[_blk] = _vptr_12[_blk];
                                        __nv_bfloat16* _velems_12 = reinterpret_cast<__nv_bfloat16*>(&_vld_12[_blk]);
                                        #pragma unroll
                                        for (int _j = 0; _j < 8; _j++)
                                            db_vals_5[0 + _blk * 8 + _j] = __bfloat162float(_velems_12[_j]);
                                    }
                                }
                            }
                        }
                    ...
                    int b_store_addr_5 = (smem_b_addr + (unsigned int)(d_col_12 / 64 * 32768 + norm_row_5 * 128 + d_col_12 % 64 * 2 ^ (d_col_12 / 64 * 32768 + norm_row_5 * 128 + d_col_12 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_5), "r"(db_pack_5[0]), "r"(db_pack_5[1]), "r"(db_pack_5[2]), "r"(db_pack_5[3]) : "memory");
                    int b_store_addr_hi_5 = (smem_b_addr + (unsigned int)((d_col_12 + 8) / 64 * 32768 + norm_row_5 * 128 + (d_col_12 + 8) % 64 * 2 ^ ((d_col_12 + 8) / 64 * 32768 + norm_row_5 * 128 + (d_col_12 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_13 = 0; vi_13 < 16; vi_13++) {
                        acc_part_5 += db_vals_5[vi_13] * db_vals_5[vi_13];
                    }
                }
                smem_db_norm_part[norm_linear_5] = acc_part_5;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                int m_abs_5 = m_start + tid;
                float pass_norm_5 = LOOM_INF;
                if (m_abs_5 < M) {
                    pass_norm_5 = 0.0f;
                    #pragma unroll
                    for (int part_6 = 0; part_6 < 4; part_6++) {
                        pass_norm_5 += smem_db_norm_part[tid + part_6 * 256];
                    }
                }
                {
                    if (m_abs_5 < M) {
                        smem_db_norm[tid] = smem_db_norm[tid] + pass_norm_5;
                    } else {
                        smem_db_norm[tid] = LOOM_INF;
                    }
                }
            ...
            _phase_mma_done5_0 ^= 1;
        }
        {
            #pragma unroll 1
            for (int e_vec_7 = tid; e_vec_7 < 64; e_vec_7 += 640) {
                int q_elem_6 = e_vec_7 * 16;
                int q_row_7 = q_elem_6 / 128;
                int d_col_13 = q_elem_6 - q_row_7 * 128;
                int global_d_12 = d_col_13 + 768;
                int q_abs_7 = q_start + q_row_7;
                float q_vals_7[16];
                unsigned int q_pack_6[8];
                {
                    const uint4* _vptr_13 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_7) * D_ORIG_ + global_d_12) + 0);
                    uint4 _vld_13[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
                        ...
                        #pragma unroll
                        for (int _j = 0; _j < 8; _j++)
                            q_vals_7[0 + _blk * 8 + _j] = __bfloat162float(_velems_13[_j]);
                    }
                }
                #pragma unroll
                for (int _lp = 0; _lp < 8; _lp++) {
                    __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(q_vals_7[_lp*2 + 0], q_vals_7[_lp*2+1 + 0]));
                    q_pack_6[_lp] = *(uint32_t*)&_bf2;
                }
                int q_store_addr_6 = (smem_a_addr + (unsigned int)(d_col_13 / 64 * 16384 + q_row_7 * 128 + d_col_13 % 64 * 2 ^ (d_col_13 / 64 * 16384 + q_row_7 * 128 + d_col_13 % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_6), "r"(q_pack_6[0]), "r"(q_pack_6[1]), "r"(q_pack_6[2]), "r"(q_pack_6[3]) : "memory");
                int q_store_addr_hi_6 = (smem_a_addr + (unsigned int)((d_col_13 + 8) / 64 * 16384 + q_row_7 * 128 + (d_col_13 + 8) % 64 * 2 ^ ((d_col_13 + 8) / 64 * 16384 + q_row_7 * 128 + (d_col_13 + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi_6), "r"(q_pack_6[4]), "r"(q_pack_6[5]), "r"(q_pack_6[6]), "r"(q_pack_6[7]) : "memory");
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_6 = tid; norm_linear_6 < 1024; norm_linear_6 += 640) {
                int norm_row_6 = norm_linear_6 % 256;
                int norm_part_6 = norm_linear_6 / 256;
                int d_base_6 = norm_part_6 * 32;
                int m_abs_part_6 = m_start + norm_row_6;
                float acc_part_6 = 0.0f;
                #pragma unroll 1
                for (int vv_6 = 0; vv_6 < 2; vv_6++) {
                    int d_col_14 = d_base_6 + vv_6 * 16;
                    int global_d_13 = d_col_14 + 768;
                    float db_vals_6[16];
                    unsigned int db_pack_6[8];
                    #pragma unroll
                    for (int vi_14 = 0; vi_14 < 16; vi_14++) {
                        db_vals_6[vi_14] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_6 < M) {
                            if (global_d_13 < D_ORIG_) {
                                {
                                    const uint4* _vptr_14 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_6) * D_ORIG_ + global_d_13) + 0);
                                    uint4 _vld_14[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                                        ...
                                        #pragma unroll
                                        for (int _j = 0; _j < 8; _j++)
                                            db_vals_6[0 + _blk * 8 + _j] = __bfloat162float(_velems_14[_j]);
                                    }
                                }
                            }
                        }
                    ...
                    #pragma unroll
                    for (int _lp = 0; _lp < 8; _lp++) {
                        __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(db_vals_6[_lp*2 + 0], db_vals_6[_lp*2+1 + 0]));
                        db_pack_6[_lp] = *(uint32_t*)&_bf2;
                    }
                    int b_store_addr_6 = (smem_b_addr + (unsigned int)(d_col_14 / 64 * 32768 + norm_row_6 * 128 + d_col_14 % 64 * 2 ^ (d_col_14 / 64 * 32768 + norm_row_6 * 128 + d_col_14 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_6), "r"(db_pack_6[0]), "r"(db_pack_6[1]), "r"(db_pack_6[2]), "r"(db_pack_6[3]) : "memory");
                    int b_store_addr_hi_6 = (smem_b_addr + (unsigned int)((d_col_14 + 8) / 64 * 32768 + norm_row_6 * 128 + (d_col_14 + 8) % 64 * 2 ^ ((d_col_14 + 8) / 64 * 32768 + norm_row_6 * 128 + (d_col_14 + 8) % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi_6), "r"(db_pack_6[4]), "r"(db_pack_6[5]), "r"(db_pack_6[6]), "r"(db_pack_6[7]) : "memory");
                    #pragma unroll
                    for (int vi_15 = 0; vi_15 < 16; vi_15++) {
                        acc_part_6 += db_vals_6[vi_15] * db_vals_6[vi_15];
                    }
                }
                smem_db_norm_part[norm_linear_6] = acc_part_6;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                int m_abs_6 = m_start + tid;
                float pass_norm_6 = LOOM_INF;
                if (m_abs_6 < M) {
                    pass_norm_6 = 0.0f;
                    #pragma unroll
                    for (int part_7 = 0; part_7 < 4; part_7++) {
                        pass_norm_6 += smem_db_norm_part[tid + part_7 * 256];
                    }
                }
                {
                    if (m_abs_6 < M) {
                        smem_db_norm[tid] = smem_db_norm[tid] + pass_norm_6;
                    } else {
                        smem_db_norm[tid] = LOOM_INF;
                    }
                }
            ...
            __syncthreads();
            if (warp == 0) {
                int _mma_a_lo_6 = make_warp_uniform((smem_a_addr >> 4) & 0x3FFF);
                int _mma_b_lo_6 = make_warp_uniform((smem_b_addr >> 4) & 0x3FFF);
                asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n"
            :: "r"(_mma_a_lo_6), "r"(_mma_b_lo_6), "r"(tmem_acc), "r"(1));
                elect_commit(mma_done6_addr);
            }
            mbarrier_wait(mma_done6_addr, _phase_mma_done6_0);
            _phase_mma_done6_0 ^= 1;
        }
        {
            #pragma unroll 1
            for (int e_vec_8 = tid; e_vec_8 < 64; e_vec_8 += 640) {
                int q_elem_7 = e_vec_8 * 16;
                int q_row_8 = q_elem_7 / 128;
                int d_col_15 = q_elem_7 - q_row_8 * 128;
                int global_d_14 = d_col_15 + 896;
                int q_abs_8 = q_start + q_row_8;
                float q_vals_8[16];
                unsigned int q_pack_7[8];
                {
                    const uint4* _vptr_15 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_8) * D_ORIG_ + global_d_14) + 0);
                    uint4 _vld_15[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
                        ...
                        #pragma unroll
                        for (int _j = 0; _j < 8; _j++)
                            q_vals_8[0 + _blk * 8 + _j] = __bfloat162float(_velems_15[_j]);
                    }
                }
                #pragma unroll
                for (int _lp = 0; _lp < 8; _lp++) {
                    __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(q_vals_8[_lp*2 + 0], q_vals_8[_lp*2+1 + 0]));
                    q_pack_7[_lp] = *(uint32_t*)&_bf2;
                }
                int q_store_addr_7 = (smem_a_addr + (unsigned int)(d_col_15 / 64 * 16384 + q_row_8 * 128 + d_col_15 % 64 * 2 ^ (d_col_15 / 64 * 16384 + q_row_8 * 128 + d_col_15 % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_7), "r"(q_pack_7[0]), "r"(q_pack_7[1]), "r"(q_pack_7[2]), "r"(q_pack_7[3]) : "memory");
                int q_store_addr_hi_7 = (smem_a_addr + (unsigned int)((d_col_15 + 8) / 64 * 16384 + q_row_8 * 128 + (d_col_15 + 8) % 64 * 2 ^ ((d_col_15 + 8) / 64 * 16384 + q_row_8 * 128 + (d_col_15 + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi_7), "r"(q_pack_7[4]), "r"(q_pack_7[5]), "r"(q_pack_7[6]), "r"(q_pack_7[7]) : "memory");
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            #pragma unroll 1
            for (int norm_linear_7 = tid; norm_linear_7 < 1024; norm_linear_7 += 640) {
                int norm_row_7 = norm_linear_7 % 256;
                int norm_part_7 = norm_linear_7 / 256;
                int d_base_7 = norm_part_7 * 32;
                int m_abs_part_7 = m_start + norm_row_7;
                float acc_part_7 = 0.0f;
                #pragma unroll 1
                for (int vv_7 = 0; vv_7 < 2; vv_7++) {
                    int d_col_16 = d_base_7 + vv_7 * 16;
                    int global_d_15 = d_col_16 + 896;
                    float db_vals_7[16];
                    unsigned int db_pack_7[8];
                    #pragma unroll
                    for (int vi_16 = 0; vi_16 < 16; vi_16++) {
                        db_vals_7[vi_16] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_7 < M) {
                            if (global_d_15 < D_ORIG_) {
                                {
                                    const uint4* _vptr_16 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_7) * D_ORIG_ + global_d_15) + 0);
                                    uint4 _vld_16[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                                        ...
                                        #pragma unroll
                                        for (int _j = 0; _j < 8; _j++)
                                            db_vals_7[0 + _blk * 8 + _j] = __bfloat162float(_velems_16[_j]);
                                    }
                                }
                            }
                        }
                    ...
                    #pragma unroll
                    for (int _lp = 0; _lp < 8; _lp++) {
                        __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(db_vals_7[_lp*2 + 0], db_vals_7[_lp*2+1 + 0]));
                        db_pack_7[_lp] = *(uint32_t*)&_bf2;
                    }
                    int b_store_addr_7 = (smem_b_addr + (unsigned int)(d_col_16 / 64 * 32768 + norm_row_7 * 128 + d_col_16 % 64 * 2 ^ (d_col_16 / 64 * 32768 + norm_row_7 * 128 + d_col_16 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_7), "r"(db_pack_7[0]), "r"(db_pack_7[1]), "r"(db_pack_7[2]), "r"(db_pack_7[3]) : "memory");
                    int b_store_addr_hi_7 = (smem_b_addr + (unsigned int)((d_col_16 + 8) / 64 * 32768 + norm_row_7 * 128 + (d_col_16 + 8) % 64 * 2 ^ ((d_col_16 + 8) / 64 * 32768 + norm_row_7 * 128 + (d_col_16 + 8) % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi_7), "r"(db_pack_7[4]), "r"(db_pack_7[5]), "r"(db_pack_7[6]), "r"(db_pack_7[7]) : "memory");
                    #pragma unroll
                    for (int vi_17 = 0; vi_17 < 16; vi_17++) {
                        acc_part_7 += db_vals_7[vi_17] * db_vals_7[vi_17];
                    }
                }
                smem_db_norm_part[norm_linear_7] = acc_part_7;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 256) {
                int m_abs_7 = m_start + tid;
                float pass_norm_7 = LOOM_INF;
                if (m_abs_7 < M) {
                    pass_norm_7 = 0.0f;
                    #pragma unroll
                    for (int part_8 = 0; part_8 < 4; part_8++) {
                        pass_norm_7 += smem_db_norm_part[tid + part_8 * 256];
                    }
                }
                {
                    if (m_abs_7 < M) {
                        smem_db_norm[tid] = smem_db_norm[tid] + pass_norm_7;
                    } else {
                        smem_db_norm[tid] = LOOM_INF;
                    }
                }
            ...
            __syncthreads();
            if (warp == 0) {
                int _mma_a_lo_7 = make_warp_uniform((smem_a_addr >> 4) & 0x3FFF);
                int _mma_b_lo_7 = make_warp_uniform((smem_b_addr >> 4) & 0x3FFF);
                asm volatile(
            "{\n\t"
            ".reg .pred leader, p0, p1;\n\t"
            ".reg .b32 adhi, bdhi, alo, blo, id;\n\t"
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n"
            :: "r"(_mma_a_lo_7), "r"(_mma_b_lo_7), "r"(tmem_acc), "r"(1));
                elect_commit(mma_done7_addr);
            }
            mbarrier_wait(mma_done7_addr, _phase_mma_done7_0);
            _phase_mma_done7_0 ^= 1;
        }
        if (col_chunk < 4) {
            if (q_local < 128) {
                #pragma unroll
                for (int stripe = 0; stripe < 2; stripe++) {
                    const int col_base = stripe * 128 + col_chunk * 32;
