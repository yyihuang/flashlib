...
kernel_knn_search_dynamic_tinyd_tile_reduce_merge_0618_c8b9_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int num_m_tiles, int num_groups, int tiles_per_group)
{
    ...
    int* group_idx = reinterpret_cast<int*>(smem_raw + 160);
    const int group_idx_addr = smem + 160;
    ...
    if (batch_id < B) {
        ...
        for (int out_k = 0; out_k < K_MAX_; out_k++) {
            ...
            group_head0 += ((winner_src == lane) ? 1 : 0);
            group_head1 += ((winner_src == lane + 32) ? 1 : 0);
        }
        __syncthreads();
        if (warp == 0) {
            int final_head = 0;
            ...
            for (int out_k_1 = 0; out_k_1 < K_MAX_; out_k_1++) {
                ...
                float _shfl_xor_15 = __shfl_xor_sync(0xFFFFFFFF, winner_d_1, 2);
                float peer_d_1 = _shfl_xor_15;
                int _shfl_xor_16 = __shfl_xor_sync(0xFFFFFFFF, winner_i_1, 2);
                ...
                float _shfl_xor_17 = __shfl_xor_sync(0xFFFFFFFF, winner_d_1, 1);
                float peer_d_0_1 = _shfl_xor_17;
                int _shfl_xor_18 = __shfl_xor_sync(0xFFFFFFFF, winner_i_1, 1);
                ...
                final_head += ((head_i == winner_i_1) ? 1 : 0);
            }
        }
    }
}
