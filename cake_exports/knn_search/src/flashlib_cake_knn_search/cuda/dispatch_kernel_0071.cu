...
kernel_knn_search_target0628_d128_q4096_m20000_k3_e8f1_k3partial_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_cohort_topk_i = smem + 39936;
    ...
    int* smem_cohort_topk_i = (int*)(smem_raw + 39936);
    #define smem_cohort_topk_i_addr (smem + 39936)
                    ...
                    for (int j_rel = 0; j_rel < 32; j_rel += 8) {
                        ...
                        if (cand00_d < best_d[2]) {
                            ...
                            for (int kk = 2; kk >= 1; kk--) {
                                float lower_d = best_d[kk];
                                int lower_i = best_i[kk];
                                float upper_d = best_d[kk - 1];
                                int upper_i = best_i[kk - 1];
                                int swap_up = ((lower_d < upper_d) ? 1 : 0);
                                best_d[kk - 1] = ((swap_up != 0) ? lower_d : upper_d);
                                best_i[kk - 1] = ((swap_up != 0) ? lower_i : upper_i);
                                best_d[kk] = ((swap_up != 0) ? upper_d : lower_d);
                                best_i[kk] = ((swap_up != 0) ? upper_i : lower_i);
                            }
                        }
