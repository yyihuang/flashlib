...
kernel_knn_search_d512_q32_k64_q64tile_merge_6bea_r119_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int partial_list_count, int num_q_tiles)
{
            ...
            for (int slot_2 = 0; slot_2 < 16; slot_2++) {
                if (local_best_slot == slot_2) {
                    ...
                    if (split_id_1 < partial_list_count) {
                        if (next_head < K_MAX_) {
                            unsigned long long partial_base_1 = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * partial_list_count + split_id_1) * 64 + q_local) * K_MAX_ + next_head);
