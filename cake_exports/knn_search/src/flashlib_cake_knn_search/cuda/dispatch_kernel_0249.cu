...
kernel_knn_search_lowq_row16_mma_partial_dispatch0610_r18_d14a_vec16_bguard_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    float* smem_db_norm_part = reinterpret_cast<float*>(smem_raw + 52224);
    const int smem_db_norm_part_addr = smem + 52224;
    float* smem_db_norm = reinterpret_cast<float*>(smem_raw + 54272);
    const int smem_db_norm_addr = smem + 54272;
    float* smem_local_d = reinterpret_cast<float*>(smem_raw + 54784);
    const int smem_local_d_addr = smem + 54784;
    int* smem_local_i = reinterpret_cast<int*>(smem_raw + 75264);
    const int smem_local_i_addr = smem + 75264;
