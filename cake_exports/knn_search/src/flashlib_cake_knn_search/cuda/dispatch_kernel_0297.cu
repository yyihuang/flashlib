...
kernel_knn_search_self_k5_direct_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int M, int K)
{
    ...
    const int smem_smem_idx = smem + 160;
    ...
    int* smem_idx = (int*)(smem_raw + 160);
    #define smem_idx_addr (smem + 160)
