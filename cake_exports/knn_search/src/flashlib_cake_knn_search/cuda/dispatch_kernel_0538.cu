...
kernel_knn_search_target0627_d4096_q4_m32768_k10_merge148_r217_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
    ...
    for (int slot = 0; slot < 5; slot++) {
        ...
        if (split_id < split_m) {
            ...
            partial_base = partial_base * (unsigned long long)K_MAX_;
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        #pragma unroll
        for (int slot_1 = 1; slot_1 < 5; slot_1++) {
            ...
            for (int slot_2 = 0; slot_2 < 5; slot_2++) {
                if (local_best_slot == slot_2) {
                    ...
                    if (split_id_1 < split_m && next_head < K_MAX_) {
                        unsigned long long partial_base_1 = (unsigned long long)(((batch_id * num_q_tiles + q_tile) * split_m + split_id_1) * 128 + q_local);
                        partial_base_1 = partial_base_1 * (unsigned long long)K_MAX_ + (unsigned long long)next_head;
