...
kernel_knn_search_target0628_d4096_q4_m8192_k64_group16_merge_2ced_v1(float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ group_distances, int* __restrict__ group_indices, int B, int Q, int partial_list_count, int num_q_tiles)
{
    ...
    int linear = bid;
    int group_id = linear - linear / 16 * 16;
    int q_linear = linear / 16;
    int batch_id = q_linear / Q;
    int q_global = q_linear - batch_id * Q;
    int q_tile = q_global / 64;
    int q_local = q_global - q_tile * 64;
    int group_begin = group_id * partial_list_count / 16;
    int group_end = (group_id + 1) * partial_list_count / 16;
    ...
    unsigned long long out_base = (unsigned long long)(((batch_id * Q + q_global) * 16 + group_id) * K_MAX_);
    #pragma unroll
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        winner_i = ((take != 0) ? peer_i : winner_i);
        winner_lane = ((take != 0) ? peer_lane : winner_lane);
        ...
        winner_i = ((take_3 != 0) ? peer_i_1 : winner_i);
        winner_lane = ((take_3 != 0) ? peer_lane_2 : winner_lane);
        ...
        winner_i = ((take_7 != 0) ? peer_i_5 : winner_i);
        winner_lane = ((take_7 != 0) ? peer_lane_6 : winner_lane);
        ...
        winner_i = ((take_11 != 0) ? peer_i_9 : winner_i);
        winner_lane = ((take_11 != 0) ? peer_lane_10 : winner_lane);
        ...
        if (lane == 0) {
            group_distances[out_base + (unsigned long long)out_k] = winner_d;
            group_indices[out_base + (unsigned long long)out_k] = winner_i;
        }
        if (lane == winner_lane) {
            head_k += 1;
            head_d = LOOM_INF;
