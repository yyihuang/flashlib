...
kernel_knn_search_target0628_d64_q256_m131072_k10_finalmerge_885d_hmerge8_v1(float* __restrict__ group_distances, int32_t* __restrict__ group_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K)
{
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        int head_i = -1;
        if (lane < 8) {
            unsigned long long group_base = (unsigned long long)(((batch_id * Q + q_global) * 8 + lane) * K_MAX_ + local_head);
