...
kernel_knn_search_mma_split_merge_q4096_lowk_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
    ...
    for (int out_k = 0; out_k < K_OUT_MAX_; out_k++) {
        ...
        if (split_lane < split_m) {
            unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * split_m + split_lane) * 128 + q_local) * 10 + local_head);
