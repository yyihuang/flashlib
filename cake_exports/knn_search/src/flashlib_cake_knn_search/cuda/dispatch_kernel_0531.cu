...
kernel_knn_search_d4096_q1_m65536_k10_partial_qcache_69ea_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    __nv_bfloat16* smem_q_cache = reinterpret_cast<__nv_bfloat16*>(smem_raw + 54656);
    const int smem_q_cache_addr = smem + 54656;
    ...
    float best_d[10];
    int best_i[10];
    int q_cache_addr = smem_q_cache_addr;
    #pragma unroll 1
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        int q_cache_addr_0 = smem_q_cache_addr;
        #pragma unroll 1
        for (int e_vec = tid; e_vec < 8; e_vec += 512) {
            int d_col_1 = e_vec * 16;
            unsigned int q_pack_1[8];
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[0])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[(0) + 3]))
                : "r"(q_cache_addr_0 + d_col_1 * 2));
            asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                : "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[4])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_1[(4) + 3]))
                : "r"(q_cache_addr_0 + d_col_1 * 2 + 16));
            int q_store_addr = (smem_a_addr + (unsigned int)(d_col_1 / 64 * 8192 + d_col_1 % 64 * 2 ^ (d_col_1 / 64 * 8192 + d_col_1 % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr), "r"(q_pack_1[0]), "r"(q_pack_1[1]), "r"(q_pack_1[2]), "r"(q_pack_1[3]) : "memory");
            int q_store_addr_hi = (smem_a_addr + (unsigned int)((d_col_1 + 8) / 64 * 8192 + (d_col_1 + 8) % 64 * 2 ^ ((d_col_1 + 8) / 64 * 8192 + (d_col_1 + 8) % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi), "r"(q_pack_1[4]), "r"(q_pack_1[5]), "r"(q_pack_1[6]), "r"(q_pack_1[7]) : "memory");
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        __syncthreads();
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part < M) {
                        {
                            const uint4* _vptr_1 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 4096 + global_d) + 0);
                            uint4 _vld_1[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int d_pass = 1; d_pass < 32; d_pass++) {
            ...
            int q_cache_addr_1 = smem_q_cache_addr;
            #pragma unroll 1
            for (int e_vec_1 = tid; e_vec_1 < 8; e_vec_1 += 512) {
                int d_col_3 = e_vec_1 * 16;
                unsigned int q_pack_2[8];
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[0])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[(0) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[(0) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[(0) + 3]))
                    : "r"(q_cache_addr_1 + (d_offset + d_col_3) * 2));
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[4])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack_2[(4) + 3]))
                    : "r"(q_cache_addr_1 + (d_offset + d_col_3) * 2 + 16));
                int q_store_addr_1 = (smem_a_addr + (unsigned int)(d_col_3 / 64 * 8192 + d_col_3 % 64 * 2 ^ (d_col_3 / 64 * 8192 + d_col_3 % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_1), "r"(q_pack_2[0]), "r"(q_pack_2[1]), "r"(q_pack_2[2]), "r"(q_pack_2[3]) : "memory");
                int q_store_addr_hi_1 = (smem_a_addr + (unsigned int)((d_col_3 + 8) / 64 * 8192 + (d_col_3 + 8) % 64 * 2 ^ ((d_col_3 + 8) / 64 * 8192 + (d_col_3 + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi_1), "r"(q_pack_2[4]), "r"(q_pack_2[5]), "r"(q_pack_2[6]), "r"(q_pack_2[7]) : "memory");
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            ...
            if (tid < 512) {
                ...
                for (int vv_1 = 0; vv_1 < 2; vv_1++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_5 < M) {
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_5) * 4096 + global_d_1) + 0);
                                uint4 _vld_2[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_1 = (smem_b_addr + (unsigned int)(d_col_4 / 64 * 16384 + norm_row_2 * 128 + d_col_4 % 64 * 2 ^ (d_col_4 / 64 * 16384 + norm_row_2 * 128 + d_col_4 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_1), "r"(db_pack_1[0]), "r"(db_pack_1[1]), "r"(db_pack_1[2]), "r"(db_pack_1[3]) : "memory");
                    int b_store_addr_hi_1 = (smem_b_addr + (unsigned int)((d_col_4 + 8) / 64 * 16384 + norm_row_2 * 128 + (d_col_4 + 8) % 64 * 2 ^ ((d_col_4 + 8) / 64 * 16384 + norm_row_2 * 128 + (d_col_4 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_5 = 0; vi_5 < 16; vi_5++) {
                        acc_part_6 += db_vals_1[vi_5] * db_vals_1[vi_5];
                    }
                }
