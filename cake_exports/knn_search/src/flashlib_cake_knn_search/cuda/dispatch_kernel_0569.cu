...
#define SMEM_SMEM_COHORT_TOPK_D_STAGE_BYTES 6144
#define SMEM_SMEM_COHORT_TOPK_D_STRIDE 6144
#define SMEM_SMEM_COHORT_TOPK_I_OFF 39936
#define SMEM_SMEM_COHORT_TOPK_I_STAGE_BYTES 6144
#define SMEM_SMEM_COHORT_TOPK_I_STRIDE 6144
...
kernel_knn_search_target0628_d128_q4096_m20000_k3_e8f1_k3partial_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    int* smem_cohort_topk_i = reinterpret_cast<int*>(smem_raw + 39936);
    const int smem_cohort_topk_i_addr = smem + 39936;
            ...
            if (q_local < 128) {
                ...
                if (q_global < Q) {
                    ...
                    for (int j_rel = 0; j_rel < 32; j_rel += 8) {
                        ...
                        if (cand00_d < best_d[2]) {
                            best_d[2] = cand00_d;
                            best_i[2] = cand00_i;
                            #pragma unroll
                            for (int kk_1 = 2; kk_1 >= 1; kk_1--) {
                        ...
                        if (cand01_d < best_d[2]) {
                            best_d[2] = cand01_d;
                            best_i[2] = cand01_i;
                            #pragma unroll
                            for (int kk_2 = 2; kk_2 >= 1; kk_2--) {
                        ...
                        if (cand10_d < best_d[2]) {
                            best_d[2] = cand10_d;
                            best_i[2] = cand10_i;
                            #pragma unroll
                            for (int kk_3 = 2; kk_3 >= 1; kk_3--) {
                        ...
                        if (cand11_d < best_d[2]) {
                            best_d[2] = cand11_d;
                            best_i[2] = cand11_i;
                            #pragma unroll
                            for (int kk_4 = 2; kk_4 >= 1; kk_4--) {
                        ...
                        if (cand20_d < best_d[2]) {
                            best_d[2] = cand20_d;
                            best_i[2] = cand20_i;
                            #pragma unroll
                            for (int kk_5 = 2; kk_5 >= 1; kk_5--) {
                        ...
                        if (cand21_d < best_d[2]) {
                            best_d[2] = cand21_d;
                            best_i[2] = cand21_i;
                            #pragma unroll
                            for (int kk_6 = 2; kk_6 >= 1; kk_6--) {
                        ...
                        if (cand30_d < best_d[2]) {
                            best_d[2] = cand30_d;
                            best_i[2] = cand30_i;
                            #pragma unroll
                            for (int kk_7 = 2; kk_7 >= 1; kk_7--) {
                        ...
                        if (cand31_d < best_d[2]) {
                            best_d[2] = cand31_d;
                            best_i[2] = cand31_i;
                            #pragma unroll
                            for (int kk_8 = 2; kk_8 >= 1; kk_8--) {
    ...
    int scratch_base = q_local * 3;
    if (col_chunk < 4) {
        if (q_local < 128) {
            const int cohort_scratch_base = col_chunk * 128 * 3;
            #pragma unroll
            for (int kk_9 = 0; kk_9 < 3; kk_9++) {
    ...
    __syncthreads();
    unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * split_m + split_id) * 128 + q_local) * 3);
    int pair_scratch_base = q_local * 3;
    if (col_chunk == 0) {
        if (q_local < 128) {
            if (q_global < Q) {
                const int cohort0_base = 0;
                const int cohort1_base = 384;
                int head0_k = 0;
                int head1_k = 0;
    ...
    if (col_chunk == 2) {
        if (q_local < 128) {
            if (q_global < Q) {
                const int cohort2_base = 768;
                const int cohort3_base = 1152;
                int head0_k_1 = 0;
                int head1_k_1 = 0;
                ...
                #pragma unroll
                for (int out_k_1 = 0; out_k_1 < 3; out_k_1++) {
        ...
        if (q_local < 128) {
            if (q_local < 64) {
                if (q_global < Q) {
                    ...
                    #pragma unroll
                    for (int out_k_2 = 0; out_k_2 < 3; out_k_2++) {
        ...
        if (q_local < 128) {
            if (q_local >= 64) {
                if (q_global < Q) {
                    ...
                    #pragma unroll
                    for (int out_k_3 = 0; out_k_3 < 3; out_k_3++) {
