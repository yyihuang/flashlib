...
kernel_knn_search_blind_k64_q128m262144_groupmerge64_0614_r19_6389_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ group_distances, int32_t* __restrict__ group_indices, int B, int Q, int K, int num_q_tiles)
{
    ...
    int list_group_base = group_id * 128;
    float head_d[4];
    int head_i[4];
    int head_k[4];
    #pragma unroll
    for (int slot = 0; slot < 4; slot++) {
        ...
        head_k[slot] = 0;
        unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 4096 + split_id) * 128 + q_local) * K_MAX_);
        ...
        if (lane == winner_lane) {
            ...
            for (int slot = 0; slot < 4; slot++) {
                if (local_best_slot == slot) {
                    ...
                    head_i[slot] = -1;
                    if (next_head < K_MAX_) {
                        unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile) * 4096 + split_id) * 128 + q_local) * K_MAX_ + next_head);
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
