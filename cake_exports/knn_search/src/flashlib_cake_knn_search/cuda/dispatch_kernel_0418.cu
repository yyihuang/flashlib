...
kernel_knn_search_d768_q64_m65536_k10_partial_0623_e35f_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    float* smem_db_norm_part = reinterpret_cast<float*>(smem_raw + 62464);
    const int smem_db_norm_part_addr = smem + 62464;
    float* smem_db_norm = reinterpret_cast<float*>(smem_raw + 64512);
    const int smem_db_norm_addr = smem + 64512;
    float* smem_local_d = reinterpret_cast<float*>(smem_raw + 65024);
    const int smem_local_d_addr = smem + 65024;
    int* smem_local_i = reinterpret_cast<int*>(smem_raw + 85504);
    const int smem_local_i_addr = smem + 85504;
    ...
    int best_bot_i[10];
    #pragma unroll 1
    for (int e_vec = tid; e_vec < 3072; e_vec += 512) {
        int q_row = e_vec / 48;
    ...
    if (warp < 8) {
        ...
        for (int part = 0; part < 48; part++) {
            q_norm_top += smem_q_norm_part[row_top * 48 + part];
            q_norm_bot += smem_q_norm_part[row_bot * 48 + part];
        }
    }
    #pragma unroll
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec_4 = tid; e_vec_4 < 512; e_vec_4 += 512) {
            ...
            if (batch_id < B) {
                if (q_abs_4 < Q) {
                    {
                        const uint4* _vptr_7 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_4) * 768 + global_d_6) + 0);
                        uint4 _vld_7[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv_3 = 0; vv_3 < 2; vv_3++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_13 < M) {
                        {
                            const uint4* _vptr_8 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_13) * 768 + global_d_7) + 0);
                            uint4 _vld_8[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        #pragma unroll 1
        for (int e_vec_5 = tid; e_vec_5 < 512; e_vec_5 += 512) {
            ...
            if (batch_id < B) {
                if (q_abs_5 < Q) {
                    {
                        const uint4* _vptr_9 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_5) * 768 + global_d_8) + 0);
                        uint4 _vld_9[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
            ...
            int q_store_addr_4 = (smem_a_addr + (unsigned int)(d_col_9 / 64 * 8192 + q_row_5 * 128 + d_col_9 % 64 * 2 ^ (d_col_9 / 64 * 8192 + q_row_5 * 128 + d_col_9 % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_4), "r"(q_pack_4[0]), "r"(q_pack_4[1]), "r"(q_pack_4[2]), "r"(q_pack_4[3]) : "memory");
            int q_store_addr_hi_4 = (smem_a_addr + (unsigned int)((d_col_9 + 8) / 64 * 8192 + q_row_5 * 128 + (d_col_9 + 8) % 64 * 2 ^ ((d_col_9 + 8) / 64 * 8192 + q_row_5 * 128 + (d_col_9 + 8) % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi_4), "r"(q_pack_4[4]), "r"(q_pack_4[5]), "r"(q_pack_4[6]), "r"(q_pack_4[7]) : "memory");
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        __syncthreads();
        int norm_row_15 = tid % 128;
        int norm_part_16 = tid / 128;
        int d_base_17 = norm_part_16 * 32;
        int m_abs_part_18 = m_start + norm_row_15;
        ...
        if (tid < 512) {
            ...
            for (int vv_4 = 0; vv_4 < 2; vv_4++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_18 < M) {
                        {
                            const uint4* _vptr_10 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_18) * 768 + global_d_9) + 0);
                            uint4 _vld_10[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
                ...
                int b_store_addr_4 = (smem_b_addr + (unsigned int)(d_col_10 / 64 * 16384 + norm_row_15 * 128 + d_col_10 % 64 * 2 ^ (d_col_10 / 64 * 16384 + norm_row_15 * 128 + d_col_10 % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_4), "r"(db_pack_4[0]), "r"(db_pack_4[1]), "r"(db_pack_4[2]), "r"(db_pack_4[3]) : "memory");
                int b_store_addr_hi_4 = (smem_b_addr + (unsigned int)((d_col_10 + 8) / 64 * 16384 + norm_row_15 * 128 + (d_col_10 + 8) % 64 * 2 ^ ((d_col_10 + 8) / 64 * 16384 + norm_row_15 * 128 + (d_col_10 + 8) % 64 * 2 >> 7 & 7) << 4));
            ...
            smem_db_norm_part[tid] = acc_part_19;
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        __syncthreads();
        if (tid < 128) {
        ...
        #pragma unroll 1
        for (int e_vec_6 = tid; e_vec_6 < 512; e_vec_6 += 512) {
            ...
            if (batch_id < B) {
                if (q_abs_6 < Q) {
                    {
                        const uint4* _vptr_11 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_6) * 768 + global_d_10) + 0);
                        uint4 _vld_11[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
            ...
            int q_store_addr_5 = (smem_a_addr + (unsigned int)(d_col_11 / 64 * 8192 + q_row_6 * 128 + d_col_11 % 64 * 2 ^ (d_col_11 / 64 * 8192 + q_row_6 * 128 + d_col_11 % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_5), "r"(q_pack_5[0]), "r"(q_pack_5[1]), "r"(q_pack_5[2]), "r"(q_pack_5[3]) : "memory");
            int q_store_addr_hi_5 = (smem_a_addr + (unsigned int)((d_col_11 + 8) / 64 * 8192 + q_row_6 * 128 + (d_col_11 + 8) % 64 * 2 ^ ((d_col_11 + 8) / 64 * 8192 + q_row_6 * 128 + (d_col_11 + 8) % 64 * 2 >> 7 & 7) << 4));
            asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi_5), "r"(q_pack_5[4]), "r"(q_pack_5[5]), "r"(q_pack_5[6]), "r"(q_pack_5[7]) : "memory");
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        __syncthreads();
        int norm_row_20 = tid % 128;
        int norm_part_21 = tid / 128;
        int d_base_22 = norm_part_21 * 32;
        int m_abs_part_23 = m_start + norm_row_20;
        ...
        if (tid < 512) {
            ...
            for (int vv_5 = 0; vv_5 < 2; vv_5++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_23 < M) {
                        {
                            const uint4* _vptr_12 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_23) * 768 + global_d_11) + 0);
                            uint4 _vld_12[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
                ...
                int b_store_addr_5 = (smem_b_addr + (unsigned int)(d_col_12 / 64 * 16384 + norm_row_20 * 128 + d_col_12 % 64 * 2 ^ (d_col_12 / 64 * 16384 + norm_row_20 * 128 + d_col_12 % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_5), "r"(db_pack_5[0]), "r"(db_pack_5[1]), "r"(db_pack_5[2]), "r"(db_pack_5[3]) : "memory");
                int b_store_addr_hi_5 = (smem_b_addr + (unsigned int)((d_col_12 + 8) / 64 * 16384 + norm_row_20 * 128 + (d_col_12 + 8) % 64 * 2 ^ ((d_col_12 + 8) / 64 * 16384 + norm_row_20 * 128 + (d_col_12 + 8) % 64 * 2 >> 7 & 7) << 4));
            ...
            smem_db_norm_part[tid] = acc_part_24;
        }
        asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
        __syncthreads();
        if (tid < 128) {
