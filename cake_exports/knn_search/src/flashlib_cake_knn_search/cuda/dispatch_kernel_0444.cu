...
kernel_knn_search_d2048_q8_m16384_k10_partial_0623_6185_d51b_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    const int smem_smem_db_norm_part = smem + 82944;
    const int smem_smem_db_norm = smem + 84992;
    const int smem_smem_local_d = smem + 85504;
    const int smem_smem_local_i = smem + 105984;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 82944);
    #define smem_db_norm_part_addr (smem + 82944)
    float* smem_db_norm = (float*)(smem_raw + 84992);
    #define smem_db_norm_addr (smem + 84992)
    float* smem_local_d = (float*)(smem_raw + 85504);
    #define smem_local_d_addr (smem + 85504)
    int* smem_local_i = (int*)(smem_raw + 105984);
    #define smem_local_i_addr (smem + 105984)
    ...
    int best_bot_i[10];
    #pragma unroll 1
    for (int e_vec = tid; e_vec < 8192; e_vec += 512) {
        int q_row = e_vec / 128;
        int q_part = e_vec - q_row * 128;
        int d_col = q_part * 16;
        ...
        if (batch_id < B) {
            if (q_abs < Q) {
                {
                    const uint4* _vptr_0 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 2048 + d_col));
                    uint4 _vld_0[2];
                    #pragma unroll
        ...
        smem_q_norm_part[q_row * 128 + q_part] = q_norm_part;
    }
    __syncthreads();
    if (warp < 8) {
        ...
        for (int part = 0; part < 128; part++) {
            q_norm_top += smem_q_norm_part[row_top * 128 + part];
            q_norm_bot += smem_q_norm_part[row_bot * 128 + part];
        }
    }
    #pragma unroll
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec = tid; e_vec < 512; e_vec += 512) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 2048 + global_d));
                        uint4 _vld_1[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part < M) {
                        {
                            const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 2048 + global_d));
                            uint4 _vld_2[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        _phase_mma_done_0 ^= 1;
        #pragma unroll
        for (int d_pass = 1; d_pass < 16; d_pass++) {
            ...
            for (int e_vec = tid; e_vec < 512; e_vec += 512) {
                ...
                if (batch_id < B) {
                    if (q_abs < Q) {
                        {
                            const uint4* _vptr_3 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 2048 + global_d));
                            uint4 _vld_3[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
            ...
            if (tid < 512) {
                ...
                for (int vv = 0; vv < 2; vv++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            {
                                const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 2048 + global_d));
                                uint4 _vld_4[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
