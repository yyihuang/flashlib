...
kernel_knn_search_d130_q64_k64_directpad_partial_0625_4b95_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
        ...
        if (batch_id < B) {
            if (q_abs < Q) {
                ...
                for (int vi_1 = 0; vi_1 < 16; vi_1++) {
                    int d_abs = d_col + vi_1;
                    if (d_abs < 130) {
                        q_vals[vi_1] = queries[(unsigned long long)((batch_id * Q + q_abs) * 130 + d_abs)];
                    }
                }
            }
        }
        ...
        if (batch_id < B) {
            if (m_abs_part < M) {
                ...
                for (int vi_4 = 0; vi_4 < 16; vi_4++) {
                    int d_abs_1 = d_col_1 + vi_4;
                    if (d_abs_1 < 130) {
                        db_vals[vi_4] = database[(unsigned long long)((batch_id * M + m_abs_part) * 130 + d_abs_1)];
                    }
                }
            }
        }
            ...
            if (db_stage_phase == 0) {
                ...
                for (int vv_1 = 0; vv_1 < 4; vv_1++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            ...
                            for (int vi_7 = 0; vi_7 < 16; vi_7++) {
                                int d_abs_2 = d_col_2 + vi_7;
                                if (d_abs_2 < 130) {
                                    db_vals_1[vi_7] = database[(unsigned long long)((batch_id * M + m_abs_part_3) * 130 + d_abs_2)];
                                }
                            }
                        }
                    }
                    ...
                    for (int vi_8 = 0; vi_8 < 16; vi_8++) {
                        acc_part_4 += db_vals_1[vi_8] * db_vals_1[vi_8];
                    }
                }
                ...
                for (int vv_2 = 0; vv_2 < 4; vv_2++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3_1 < M) {
                            ...
                            for (int vi_10 = 0; vi_10 < 16; vi_10++) {
                                int d_abs_3 = d_col_3 + vi_10;
                                if (d_abs_3 < 130) {
                                    db_vals_2[vi_10] = database[(unsigned long long)((batch_id * M + m_abs_part_3_1) * 130 + d_abs_3)];
                                }
                            }
                        }
                    }
                    ...
                    for (int vi_11 = 0; vi_11 < 16; vi_11++) {
                        acc_part_4_1 += db_vals_2[vi_11] * db_vals_2[vi_11];
                    }
                }
