...
kernel_knn_search_dynamic_d512_q64_tcgen05_partial_0618_9286_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    const int smem_smem_db_norm_part = smem + 58368;
    const int smem_smem_db_norm = smem + 60416;
    const int smem_smem_local_d = smem + 60928;
    const int smem_smem_local_i = smem + 81408;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 58368);
    #define smem_db_norm_part_addr (smem + 58368)
    float* smem_db_norm = (float*)(smem_raw + 60416);
    #define smem_db_norm_addr (smem + 60416)
    float* smem_local_d = (float*)(smem_raw + 60928);
    #define smem_local_d_addr (smem + 60928)
    int* smem_local_i = (int*)(smem_raw + 81408);
    #define smem_local_i_addr (smem + 81408)
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec = tid; e_vec < 512; e_vec += 512) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_5 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 512 + global_d));
                        uint4 _vld_5[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_8 < M) {
                        {
                            const uint4* _vptr_6 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_8) * 512 + global_d));
                            uint4 _vld_6[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int e_vec = tid; e_vec < 512; e_vec += 512) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_7 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 512 + global_d));
                        uint4 _vld_7[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_13 < M) {
                        {
                            const uint4* _vptr_8 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_13) * 512 + global_d));
                            uint4 _vld_8[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
