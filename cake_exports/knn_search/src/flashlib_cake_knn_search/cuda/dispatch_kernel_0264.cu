...
#define SMEM_LOCAL_DIST_STAGE_BYTES 8192
#define SMEM_LOCAL_DIST_STRIDE 8192
#define SMEM_LOCAL_IDX_OFF 8192
#define SMEM_LOCAL_IDX_STAGE_BYTES 8192
#define SMEM_LOCAL_IDX_STRIDE 8192
#define SMEM_WARP_DIST_OFF 16384
#define SMEM_WARP_DIST_STAGE_BYTES 32
#define SMEM_WARP_DIST_STRIDE 32
#define SMEM_WARP_IDX_OFF 16416
#define SMEM_WARP_IDX_STAGE_BYTES 32
#define SMEM_WARP_IDX_STRIDE 32
#define SMEM_WARP_THREAD_OFF 16448
...
kernel_knn_search_lowd_dbscan_direct_dispatch0610_r1_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int M, int K)
{
    ...
    const int smem_local_idx = smem + 8192;
    const int smem_warp_dist = smem + 16384;
    const int smem_warp_idx = smem + 16416;
    const int smem_warp_thread = smem + 16448;
    ...
    int* local_idx = (int*)(smem_raw + 8192);
    #define local_idx_addr (smem + 8192)
    float* warp_dist = (float*)(smem_raw + 16384);
    #define warp_dist_addr (smem + 16384)
    int* warp_idx = (int*)(smem_raw + 16416);
    #define warp_idx_addr (smem + 16416)
    int* warp_thread = (int*)(smem_raw + 16448);
    #define warp_thread_addr (smem + 16448)
    ...
    if (batch_id < B) {
        int q_group = q_row / 4 * 4;
        int q_group_lane = q_row - q_group;
        int q_group_offset = q_group_lane * D_;
        unsigned long long q_base = (unsigned long long)((batch_id * Q + q_group) * D_);
        float q_vec[8];
        {
            const uint4* _vptr_0 = reinterpret_cast<const uint4*>(queries + q_base);
            uint4 _vld_0[1];
            #pragma unroll
        ...
        float q0 = q_vec[q_group_offset];
        float q1 = q_vec[q_group_offset + 1];
        #pragma unroll
        for (int local_slot = 0; local_slot < LOCAL_LIST_CAP_; local_slot++) {
            int m_row = tid + local_slot * 256;
            if (m_row < M) {
                int m_group = m_row / 4 * 4;
                int m_group_lane = m_row - m_group;
                int m_group_offset = m_group_lane * D_;
                unsigned long long db_base = (unsigned long long)((batch_id * M + m_group) * D_);
                float db_vec[8];
                {
                    const uint4* _vptr_1 = reinterpret_cast<const uint4*>(database + db_base);
                    uint4 _vld_1[1];
                    #pragma unroll
                    for (int _blk = 0; _blk < 1; _blk++) {
                ...
                float diff0 = q0 - db_vec[m_group_offset];
                float diff1 = q1 - db_vec[m_group_offset + 1];
