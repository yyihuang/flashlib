...
kernel_knn_search_q8_blockm256_two_stripe_tmem_8d4fe4ead6cd_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int active_split_m)
{
    ...
    const int smem_smem_db_norm_part = smem + 123904;
    const int smem_smem_db_norm = smem + 128000;
    const int smem_smem_cohort_topk_d = smem + 129024;
    const int smem_smem_cohort_topk_i = smem + 149504;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 123904);
    #define smem_db_norm_part_addr (smem + 123904)
    float* smem_db_norm = (float*)(smem_raw + 128000);
    #define smem_db_norm_addr (smem + 128000)
    float* smem_cohort_topk_d = (float*)(smem_raw + 129024);
    #define smem_cohort_topk_d_addr (smem + 129024)
    int* smem_cohort_topk_i = (int*)(smem_raw + 149504);
    #define smem_cohort_topk_i_addr (smem + 149504)
    ...
    uint32_t _phase_mma_done8_0 = 0;
    uint32_t _phase_mma_done9_0 = 0;
    uint32_t _phase_mma_done10_0 = 0;
    uint32_t _phase_mma_done11_0 = 0;
    #pragma unroll 1
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        int m_start = m_tile * 256;
        #pragma unroll 1
        for (int e_vec = tid; e_vec < 128; e_vec += 640) {
            int q_elem = e_vec * 16;
