...
kernel_knn_search_high_d_low_q_d768_q64_norm_merge_0268_v1(__nv_bfloat16* __restrict__ queries, float* __restrict__ partial_distances, int* __restrict__ partial_indices, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int K, int split_m, int num_q_tiles)
{
    ...
    float q_norm = 0.0f;
    unsigned long long q_base = (unsigned long long)((batch_id * Q + q_global) * 768);
    #pragma unroll 1
    for (int d_vec = lane * 8; d_vec < 768; d_vec += 256) {
        float _vec_load_0[8];
        {
        ...
        for (int vi = 0; vi < 8; vi++) {
            q_norm += _vec_load_0[vi] * _vec_load_0[vi];
        }
    }
    float _warp_reduce_0 = q_norm;
    #pragma unroll
    for (int offset = 16; offset > 0; offset >>= 1)
        _warp_reduce_0 += __shfl_xor_sync(0xFFFFFFFF, _warp_reduce_0, offset);
    q_norm = _warp_reduce_0;
    float head_d[5];
    int head_i[5];
    int head_k[5];
    #pragma unroll
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        if (lane == 0) {
            out_distances[out_base + (unsigned long long)out_k] = winner_d + q_norm;
