...
kernel_knn_search_d130_q64_k64_directpad_partial_0625_4b95_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
        ...
        if (batch_id < B) {
            if (q_abs < Q) {
                ...
                for (int vi = 0; vi < 16; vi++) {
                    int d_abs = d_col + vi;
                    if (d_abs < 130) {
                        q_vals[vi] = queries[(unsigned long long)((batch_id * Q + q_abs) * 130 + d_abs)];
                    }
                }
            }
        }
        ...
        if (batch_id < B) {
            if (m_abs_part < M) {
                ...
                for (int vi = 0; vi < 16; vi++) {
                    int d_abs = d_col + vi;
                    if (d_abs < 130) {
                        db_vals[vi] = database[(unsigned long long)((batch_id * M + m_abs_part) * 130 + d_abs)];
                    }
                }
            }
        }
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            ...
                            for (int vi = 0; vi < 16; vi++) {
                                int d_abs = d_col + vi;
                                if (d_abs < 130) {
                                    db_vals[vi] = database[(unsigned long long)((batch_id * M + m_abs_part_3) * 130 + d_abs)];
                                }
                            }
                        }
                    }
