...
kernel_knn_search_d768_q64_m65536_k10_partial_0623_e35f_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    const int smem_smem_db_norm_part = smem + 62464;
    const int smem_smem_db_norm = smem + 64512;
    const int smem_smem_local_d = smem + 65024;
    const int smem_smem_local_i = smem + 85504;
    ...
    float* smem_db_norm_part = (float*)(smem_raw + 62464);
    #define smem_db_norm_part_addr (smem + 62464)
    float* smem_db_norm = (float*)(smem_raw + 64512);
    #define smem_db_norm_addr (smem + 64512)
    float* smem_local_d = (float*)(smem_raw + 65024);
    #define smem_local_d_addr (smem + 65024)
    int* smem_local_i = (int*)(smem_raw + 85504);
    #define smem_local_i_addr (smem + 85504)
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec = tid; e_vec < 512; e_vec += 512) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_7 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 768 + global_d));
                        uint4 _vld_7[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_13 < M) {
                        {
                            const uint4* _vptr_8 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_13) * 768 + global_d));
                            uint4 _vld_8[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
                ...
                int b_store_addr = (smem_b_addr + (d_col / 64 * 16384 + norm_row_10 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 16384 + norm_row_10 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr), "r"(db_pack[0]), "r"(db_pack[1]), "r"(db_pack[2]), "r"(db_pack[3]) : "memory");
                int b_store_addr_hi = (smem_b_addr + ((d_col + 8) / 64 * 16384 + norm_row_10 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 16384 + norm_row_10 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi), "r"(db_pack[4]), "r"(db_pack[5]), "r"(db_pack[6]), "r"(db_pack[7]) : "memory");
                #pragma unroll
                for (int vi = 0; vi < 16; vi++) {
        ...
        for (int e_vec = tid; e_vec < 512; e_vec += 512) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_9 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 768 + global_d));
                        uint4 _vld_9[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_18 < M) {
                        {
                            const uint4* _vptr_10 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_18) * 768 + global_d));
                            uint4 _vld_10[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
                ...
                int b_store_addr = (smem_b_addr + (d_col / 64 * 16384 + norm_row_15 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 16384 + norm_row_15 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr), "r"(db_pack[0]), "r"(db_pack[1]), "r"(db_pack[2]), "r"(db_pack[3]) : "memory");
                int b_store_addr_hi = (smem_b_addr + ((d_col + 8) / 64 * 16384 + norm_row_15 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 16384 + norm_row_15 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi), "r"(db_pack[4]), "r"(db_pack[5]), "r"(db_pack[6]), "r"(db_pack[7]) : "memory");
                #pragma unroll
                for (int vi = 0; vi < 16; vi++) {
        ...
        for (int e_vec = tid; e_vec < 512; e_vec += 512) {
            ...
            if (batch_id < B) {
                if (q_abs < Q) {
                    {
                        const uint4* _vptr_11 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 768 + global_d));
                        uint4 _vld_11[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (tid < 512) {
            ...
            for (int vv = 0; vv < 2; vv++) {
                ...
                if (batch_id < B) {
                    if (m_abs_part_23 < M) {
                        {
                            const uint4* _vptr_12 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_23) * 768 + global_d));
                            uint4 _vld_12[2];
                            #pragma unroll
                            for (int _blk = 0; _blk < 2; _blk++) {
                ...
                int b_store_addr = (smem_b_addr + (d_col / 64 * 16384 + norm_row_20 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 16384 + norm_row_20 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr), "r"(db_pack[0]), "r"(db_pack[1]), "r"(db_pack[2]), "r"(db_pack[3]) : "memory");
                int b_store_addr_hi = (smem_b_addr + ((d_col + 8) / 64 * 16384 + norm_row_20 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 16384 + norm_row_20 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_hi), "r"(db_pack[4]), "r"(db_pack[5]), "r"(db_pack[6]), "r"(db_pack[7]) : "memory");
                #pragma unroll
                for (int vi = 0; vi < 16; vi++) {
