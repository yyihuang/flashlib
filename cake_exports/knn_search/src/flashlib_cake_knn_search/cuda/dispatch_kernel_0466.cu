...
kernel_knn_search_d4096_q1_m65536_k10_partial_compactsmem_69ea_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int e_vec = tid; e_vec < 8; e_vec += 512) {
            ...
            if (batch_id < B) {
                {
                    const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(batch_id * 4096 + 0 + d_col));
                    uint4 _vld_1[2];
                    #pragma unroll
                    for (int _blk = 0; _blk < 2; _blk++) {
        ...
        for (int d_pass = 1; d_pass < 32; d_pass++) {
            ...
            for (int e_vec = tid; e_vec < 8; e_vec += 512) {
                ...
                if (batch_id < B) {
                    {
                        const uint4* _vptr_3 = reinterpret_cast<const uint4*>(queries + (unsigned long long)(batch_id * 4096 + d_offset + d_col));
                        uint4 _vld_3[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
