...
kernel_knn_search_k1_top1_pairq_partial_0622_598a_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    int _desc_lo_0 = make_warp_uniform((smem_a0_addr >> 4) & 0x3FFF);
    int _desc_lo_1 = make_warp_uniform((smem_a1_addr >> 4) & 0x3FFF);
    int _desc_lo_2 = make_warp_uniform((smem_b_addr >> 4) & 0x3FFF);
    int _desc_lo_3 = make_warp_uniform((smem_b_next_addr >> 4) & 0x3FFF);
    int work_id = bid;
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        if (db_stage_phase == 0) {
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n"
            :: "r"(_desc_lo_0), "r"(_desc_lo_3), "r"(taddr), "r"(0));
            elect_commit(mma_done_addr);
        }
        ...
        if (db_stage_phase == 0) {
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n"
            :: "r"(_desc_lo_1), "r"(_desc_lo_2), "r"(taddr), "r"(0));
                elect_commit(mma_done_addr);
            }
        } else if (warp == 0) {
            asm volatile(
            ...
            "@leader tcgen05.mma.cta_group::1.kind::f16 [%2], da, db, id, p1;\n\t"
            "}\n"
            :: "r"(_desc_lo_1), "r"(_desc_lo_3), "r"(taddr), "r"(0));
            elect_commit(mma_done_addr);
        }
        mbarrier_wait(mma_done_addr, _phase_mma_done_0);
        _phase_mma_done_0 ^= 1;
