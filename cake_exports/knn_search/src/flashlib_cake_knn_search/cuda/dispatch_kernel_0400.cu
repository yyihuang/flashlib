...
kernel_knn_search_k1_top1_pairq_partial_0622_598a_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_a0 = smem + 1024;
    const int smem_smem_a1 = smem + 33792;
    const int smem_smem_b = smem + 66560;
    const int smem_smem_b_next = smem + 99328;
    const int smem_smem_q_norm_part0 = smem + 132096;
    const int smem_smem_q_norm_part1 = smem + 136192;
    const int smem_smem_db_norm_part = smem + 140288;
    const int smem_smem_db_norm_part_next = smem + 142336;
    const int smem_smem_db_norm = smem + 144384;
    const int smem_smem_db_norm_next = smem + 144896;
    const int smem_smem_cohort_topk_d = smem + 145408;
    const int smem_smem_cohort_topk_i = smem + 147456;
    ...
    __nv_bfloat16* smem_a0 = (__nv_bfloat16*)(smem_raw + 1024);
    #define smem_a0_addr (smem + 1024)
    __nv_bfloat16* smem_a1 = (__nv_bfloat16*)(smem_raw + 33792);
    #define smem_a1_addr (smem + 33792)
    ...
    float* smem_q_norm_part0 = (float*)(smem_raw + 132096);
    #define smem_q_norm_part0_addr (smem + 132096)
    float* smem_q_norm_part1 = (float*)(smem_raw + 136192);
    #define smem_q_norm_part1_addr (smem + 136192)
    float* smem_db_norm_part = (float*)(smem_raw + 140288);
    #define smem_db_norm_part_addr (smem + 140288)
    float* smem_db_norm_part_next = (float*)(smem_raw + 142336);
    #define smem_db_norm_part_next_addr (smem + 142336)
    float* smem_db_norm = (float*)(smem_raw + 144384);
    #define smem_db_norm_addr (smem + 144384)
    float* smem_db_norm_next = (float*)(smem_raw + 144896);
    #define smem_db_norm_next_addr (smem + 144896)
    float* smem_cohort_topk_d = (float*)(smem_raw + 145408);
    #define smem_cohort_topk_d_addr (smem + 145408)
    int* smem_cohort_topk_i = (int*)(smem_raw + 147456);
    #define smem_cohort_topk_i_addr (smem + 147456)
    ...
    for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
        ...
        int q_store_addr = (smem_a0_addr + (d_col / 64 * 16384 + q_row * 128 + d_col % 64 * 2 ^ (d_col / 64 * 16384 + q_row * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr), "r"(q_pack[0]), "r"(q_pack[1]), "r"(q_pack[2]), "r"(q_pack[3]) : "memory");
        int q_store_addr_hi = (smem_a0_addr + ((d_col + 8) / 64 * 16384 + q_row * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 16384 + q_row * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
    ...
    for (int e_vec = tid; e_vec < 1024; e_vec += 640) {
        ...
        if (q_abs < Q) {
            {
                const uint4* _vptr_1 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs) * 128 + d_col));
                uint4 _vld_1[2];
                #pragma unroll
                for (int _blk = 0; _blk < 2; _blk++) {
        ...
        int q_norm_part_col = d_col / 16;
        smem_q_norm_part1[q_row * 8 + q_norm_part_col] = q_norm_part;
        #pragma unroll
        for (int _lp = 0; _lp < 8; _lp++) {
            __nv_bfloat162 _bf2 = __float22bfloat162_rn(make_float2(q_vals[_lp*2 + 0], q_vals[_lp*2+1 + 0]));
            q_pack[_lp] = *(uint32_t*)&_bf2;
        }
        int q_store_addr = (smem_a1_addr + (d_col / 64 * 16384 + q_row * 128 + d_col % 64 * 2 ^ (d_col / 64 * 16384 + q_row * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr), "r"(q_pack[0]), "r"(q_pack[1]), "r"(q_pack[2]), "r"(q_pack[3]) : "memory");
        int q_store_addr_hi = (smem_a1_addr + ((d_col + 8) / 64 * 16384 + q_row * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 16384 + q_row * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
        asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi), "r"(q_pack[4]), "r"(q_pack[5]), "r"(q_pack[6]), "r"(q_pack[7]) : "memory");
    }
    asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
    __syncthreads();
    if (col_chunk < 4) {
        if (q_local < 128) {
            ...
            if (q_global1 < Q) {
                #pragma unroll
                for (int part = 0; part < 8; part++) {
                    q_norm1 += smem_q_norm_part1[q_local * 8 + part];
                }
            }
        }
    }
    ...
    if (tid < 512) {
        ...
        for (int vv = 0; vv < 2; vv++) {
            ...
            unsigned int db_pack[8];
            {
                ...
                if (m_abs_part < M) {
                    {
                        const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part) * 128 + d_col));
                        uint4 _vld_2[2];
                        #pragma unroll
                        for (int _blk = 0; _blk < 2; _blk++) {
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
                ...
                if (tid < 512) {
                    ...
                    for (int vv = 0; vv < 2; vv++) {
                        ...
                        unsigned int db_pack[8];
                        {
                            ...
                            if (m_abs_part_3 < M) {
                                {
                                    const uint4* _vptr_4 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 128 + d_col));
                                    uint4 _vld_4[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
        ...
        if (col_chunk < 4) {
            if (q_local < 128) {
                ...
                float dots[32];
                tmem_ld_x32(&dots[0], taddr + (row_base_tmem << 16) + col_base1);
                asm volatile("tcgen05.wait::ld.sync.aligned;");
                if (q_global1 < Q) {
                    #pragma unroll 4
                    for (int j_rel = 0; j_rel < 32; j_rel += 8) {
                        ...
                        float cand30_d = ((take31 != 0) ? dist31 : dist30);
                        int cand30_i = ((take31 != 0) ? m_abs31 : m_abs30);
                        if (cand00_d < best1_d[0]) {
                            best1_d[0] = cand00_d;
                            best1_i[0] = cand00_i;
                        }
                        if (cand10_d < best1_d[0]) {
                            best1_d[0] = cand10_d;
                            best1_i[0] = cand10_i;
                        }
                        if (cand20_d < best1_d[0]) {
                            best1_d[0] = cand20_d;
                            best1_i[0] = cand20_i;
                        }
                        if (cand30_d < best1_d[0]) {
                            best1_d[0] = cand30_d;
                            best1_i[0] = cand30_i;
                        }
                    }
    ...
    __syncthreads();
    unsigned long long partial_base = (unsigned long long)((((batch_id * num_q_tiles + q_tile0) * split_m + split_id) * 128 + q_local) * 1);
    if (col_chunk == 0) {
        if (q_local < 128) {
            if (q_global0 < Q) {
    ...
    __syncthreads();
    int scratch_base_0 = q_local * 1;
    if (col_chunk < 4) {
        if (q_local < 128) {
            const int cohort_scratch_base = col_chunk * 128 * 1;
            smem_cohort_topk_d[cohort_scratch_base + scratch_base_0] = best1_d[0];
            smem_cohort_topk_i[cohort_scratch_base + scratch_base_0] = best1_i[0];
        }
    }
    __syncthreads();
    unsigned long long partial_base_1 = (unsigned long long)((((batch_id * num_q_tiles + q_tile1) * split_m + split_id) * 128 + q_local) * 1);
    if (col_chunk == 0) {
        if (q_local < 128) {
            if (q_global1 < Q) {
                const int cohort1_base = 128;
                const int cohort2_base = 256;
                const int cohort3_base = 384;
                float cand1_d = smem_cohort_topk_d[cohort1_base + scratch_base_0];
                int cand1_i = smem_cohort_topk_i[cohort1_base + scratch_base_0];
                float cand2_d = smem_cohort_topk_d[cohort2_base + scratch_base_0];
                int cand2_i = smem_cohort_topk_i[cohort2_base + scratch_base_0];
                float cand3_d = smem_cohort_topk_d[cohort3_base + scratch_base_0];
                int cand3_i = smem_cohort_topk_i[cohort3_base + scratch_base_0];
                if (cand1_d < best1_d[0]) {
                    best1_d[0] = cand1_d;
                    best1_i[0] = cand1_i;
                }
                if (cand2_d < best1_d[0]) {
                    best1_d[0] = cand2_d;
                    best1_i[0] = cand2_i;
                }
                if (cand3_d < best1_d[0]) {
                    best1_d[0] = cand3_d;
                    best1_i[0] = cand3_i;
                }
