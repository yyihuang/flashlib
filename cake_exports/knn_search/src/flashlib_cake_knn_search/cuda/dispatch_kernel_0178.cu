...
kernel_knn_search_target0628_d1024_q32_m65536_k64_final_merge16_9571_v2(float* __restrict__ group_distances, int32_t* __restrict__ group_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices)
{
    ...
    for (int out_k = 0; out_k < K_MAX_; out_k++) {
        ...
        int head_i = -1;
        if (lane < 16) {
            unsigned long long group_base = (unsigned long long)((q_global * 16 + lane) * K_MAX_ + local_head);
