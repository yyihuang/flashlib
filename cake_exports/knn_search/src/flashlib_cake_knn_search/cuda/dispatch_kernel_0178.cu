...
kernel_knn_search_d4096_q1_m65536_k10_partial_qcache_pipe_69ea_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles)
{
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        for (int d_pass = 1; d_pass < 32; d_pass++) {
            ...
            if (tid < 8) {
                ...
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&q_pack[0])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[3]))
                    : "r"(q_cache_addr_0 + (d_offset + d_col) * 2));
                asm volatile("ld.shared.v4.b32 {%0,%1,%2,%3}, [%4];"
                    : "=r"(*reinterpret_cast<uint32_t*>(&q_pack[4])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 1])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 2])), "=r"(*reinterpret_cast<uint32_t*>(&q_pack[(4) + 3]))
                    : "r"(q_cache_addr_0 + (d_offset + d_col) * 2 + 16));
                int q_store_addr = (smem_a_addr + (d_col / 64 * 8192 + 0 * 128 + d_col % 64 * 2 ^ (d_col / 64 * 8192 + 0 * 128 + d_col % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr), "r"(q_pack[0]), "r"(q_pack[1]), "r"(q_pack[2]), "r"(q_pack[3]) : "memory");
                int q_store_addr_hi = (smem_a_addr + ((d_col + 8) / 64 * 8192 + 0 * 128 + (d_col + 8) % 64 * 2 ^ ((d_col + 8) / 64 * 8192 + 0 * 128 + (d_col + 8) % 64 * 2 >> 7 & 7) << 4));
                asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(q_store_addr_hi), "r"(q_pack[4]), "r"(q_pack[5]), "r"(q_pack[6]), "r"(q_pack[7]) : "memory");
            ...
            if (tid < 512) {
                ...
                for (int vv = 0; vv < 2; vv++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3 < M) {
                            {
                                const uint4* _vptr_2 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 4096 + global_d));
                                uint4 _vld_2[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
