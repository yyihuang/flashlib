...
kernel_knn_search_ext_k_capacity_q4096_m49152_merge24_0618_28ec_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices)
{
        ...
        if (lane == winner_lane) {
            ...
            for (int slot = 0; slot < 24; slot++) {
                if (local_best_slot == slot) {
                    ...
                    head_i[slot] = -1;
                    if (next_head < K_MAX_) {
                        unsigned long long partial_base = (unsigned long long)(((q_tile * 768 + split_id) * 128 + q_local) * K_MAX_ + next_head);
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
