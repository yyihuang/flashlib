...
kernel_knn_search_k64_q4096split79_localprefix_cert_0615_r32_edd7_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int32_t* __restrict__ overflow_flag, int B, int Q, int K, int partial_list_count, int num_q_tiles)
{
    ...
    float threshold_d = out_distances[out_base + K_MAX_ - 1];
    int threshold_i = out_indices[out_base + K_MAX_ - 1];
    int any_fail = 0;
    #pragma unroll
