...
kernel_knn_search_target0628_d1024_q32_m65536_k64_group_merge16_9571_v2(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ group_distances, int32_t* __restrict__ group_indices)
{
    ...
    int q_global = q_group_linear / 16;
    ...
    unsigned long long group_base = (unsigned long long)((q_global * 16 + group_id) * K_MAX_);
    #pragma unroll
        ...
        if (lane == winner_lane) {
            ...
            for (int slot = 0; slot < 2; slot++) {
                if (local_best_slot == slot) {
                    ...
                    head_i[slot] = -1;
                    if (next_head < K_MAX_) {
                        unsigned long long partial_base = (unsigned long long)((split_id * 32 + q_local) * K_MAX_ + next_head);
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
