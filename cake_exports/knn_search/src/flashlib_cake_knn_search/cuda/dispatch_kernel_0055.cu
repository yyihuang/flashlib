    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
        ...
        }
        {
            ...
            for (int e_vec_3 = tid; e_vec_3 < 1024; e_vec_3 += 640) {
                ...
                if (batch_id < B) {
                    if (q_abs_3 < Q) {
                        if (global_d_4 < D_TOTAL_) {
                            {
                                const uint4* _vptr_5 = reinterpret_cast<const uint4*>(queries + (unsigned long long)((batch_id * Q + q_abs_3) * D_TOTAL_ + global_d_4) + 0);
                                uint4 _vld_5[2];
                                #pragma unroll
                                for (int _blk = 0; _blk < 2; _blk++) {
            ...
            if (tid < 512) {
                ...
                for (int vv_2 = 0; vv_2 < 2; vv_2++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3_1 < M) {
                            if (global_d_5 < D_TOTAL_) {
                                {
                                    const uint4* _vptr_6 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_1) * D_TOTAL_ + global_d_5) + 0);
                                    uint4 _vld_6[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
            ...
            if (q_local < 128) {
                ...
                if (q_global < Q) {
                    ...
                    for (int j_rel_1 = 0; j_rel_1 < 32; j_rel_1 += 8) {
                        ...
                        #pragma unroll
                        for (int _lf = 0; _lf < 1; _lf++)
                            fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair0_1)[_lf], _fma_b2_15, _fma_c2_16);
                        ...
                        #pragma unroll
                        for (int _lf = 0; _lf < 1; _lf++)
                            fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair1_1)[_lf], _fma_b2_17, _fma_c2_18);
                        ...
                        #pragma unroll
                        for (int _lf = 0; _lf < 1; _lf++)
                            fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair2_1)[_lf], _fma_b2_19, _fma_c2_20);
                        ...
                        #pragma unroll
                        for (int _lf = 0; _lf < 1; _lf++)
                            fma_f32x2_inplace(&reinterpret_cast<float2*>(dist_pair3_1)[_lf], _fma_b2_21, _fma_c2_22);
