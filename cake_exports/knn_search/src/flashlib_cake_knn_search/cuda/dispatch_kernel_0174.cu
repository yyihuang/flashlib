...
kernel_knn_search_dynamic_d768d1024_q32q16_tcgen05_partial_0618_9286_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    float* smem_db_norm_part = reinterpret_cast<float*>(smem_raw + 99328);
    const int smem_db_norm_part_addr = smem + 99328;
    float* smem_db_norm = reinterpret_cast<float*>(smem_raw + 101376);
    const int smem_db_norm_addr = smem + 101376;
    float* smem_cohort_topk_d = reinterpret_cast<float*>(smem_raw + 101888);
    const int smem_cohort_topk_d_addr = smem + 101888;
    int* smem_cohort_topk_i = reinterpret_cast<int*>(smem_raw + 122368);
    const int smem_cohort_topk_i_addr = smem + 122368;
    ...
    for (int m_tile = tile_begin; m_tile < tile_end; m_tile++) {
            ...
            if (tid < 512) {
                ...
                for (int vv_2 = 0; vv_2 < 2; vv_2++) {
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3_1 < M) {
                            if (global_d_5 < D_ORIG_) {
                                {
                                    const uint4* _vptr_6 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_1) * D_ORIG_ + global_d_5) + 0);
                                    uint4 _vld_6[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    for (int vi_10 = 0; vi_10 < 16; vi_10++) {
                        acc_part_4_1 += db_vals_2[vi_10] * db_vals_2[vi_10];
                    }
                }
        ...
        {
            #pragma unroll 1
            for (int e_vec_4 = tid; e_vec_4 < 1024; e_vec_4 += 640) {
            ...
            int m_abs_part_3_2 = m_start + norm_row_0_2;
            float acc_part_4_2 = 0.0f;
            if (tid < 512) {
                #pragma unroll 1
                for (int vv_3 = 0; vv_3 < 2; vv_3++) {
                    int d_col_8 = d_base_2_2 + vv_3 * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3_2 < M) {
                            if (global_d_7 < D_ORIG_) {
                                {
                                    const uint4* _vptr_8 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_2) * D_ORIG_ + global_d_7) + 0);
                                    uint4 _vld_8[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_3 = (smem_b_addr + (unsigned int)(d_col_8 / 64 * 16384 + norm_row_0_2 * 128 + d_col_8 % 64 * 2 ^ (d_col_8 / 64 * 16384 + norm_row_0_2 * 128 + d_col_8 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_3), "r"(db_pack_3[0]), "r"(db_pack_3[1]), "r"(db_pack_3[2]), "r"(db_pack_3[3]) : "memory");
                    int b_store_addr_hi_3 = (smem_b_addr + (unsigned int)((d_col_8 + 8) / 64 * 16384 + norm_row_0_2 * 128 + (d_col_8 + 8) % 64 * 2 ^ ((d_col_8 + 8) / 64 * 16384 + norm_row_0_2 * 128 + (d_col_8 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_13 = 0; vi_13 < 16; vi_13++) {
                        acc_part_4_2 += db_vals_3[vi_13] * db_vals_3[vi_13];
                    }
                }
        ...
        {
            #pragma unroll 1
            for (int e_vec_5 = tid; e_vec_5 < 1024; e_vec_5 += 640) {
            ...
            int m_abs_part_3_3 = m_start + norm_row_0_3;
            float acc_part_4_3 = 0.0f;
            if (tid < 512) {
                #pragma unroll 1
                for (int vv_4 = 0; vv_4 < 2; vv_4++) {
                    int d_col_10 = d_base_2_3 + vv_4 * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3_3 < M) {
                            if (global_d_9 < D_ORIG_) {
                                {
                                    const uint4* _vptr_10 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_3) * D_ORIG_ + global_d_9) + 0);
                                    uint4 _vld_10[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_4 = (smem_b_addr + (unsigned int)(d_col_10 / 64 * 16384 + norm_row_0_3 * 128 + d_col_10 % 64 * 2 ^ (d_col_10 / 64 * 16384 + norm_row_0_3 * 128 + d_col_10 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_4), "r"(db_pack_4[0]), "r"(db_pack_4[1]), "r"(db_pack_4[2]), "r"(db_pack_4[3]) : "memory");
                    int b_store_addr_hi_4 = (smem_b_addr + (unsigned int)((d_col_10 + 8) / 64 * 16384 + norm_row_0_3 * 128 + (d_col_10 + 8) % 64 * 2 ^ ((d_col_10 + 8) / 64 * 16384 + norm_row_0_3 * 128 + (d_col_10 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_16 = 0; vi_16 < 16; vi_16++) {
                        acc_part_4_3 += db_vals_4[vi_16] * db_vals_4[vi_16];
                    }
                }
            ...
            if (tid < 128) {
                ...
                if (m_abs_4 < M) {
                    ...
                    for (int part_5 = 0; part_5 < 4; part_5++) {
                        pass_norm_4 += smem_db_norm_part[tid + part_5 * 128];
                    }
                }
                {
                    if (m_abs_4 < M) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_6 = tid; e_vec_6 < 1024; e_vec_6 += 640) {
            ...
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            int norm_row_0_4 = tid % 128;
            int norm_part_1_4 = tid / 128;
            int d_base_2_4 = norm_part_1_4 * 32;
            int m_abs_part_3_4 = m_start + norm_row_0_4;
            float acc_part_4_4 = 0.0f;
            if (tid < 512) {
                #pragma unroll 1
                for (int vv_5 = 0; vv_5 < 2; vv_5++) {
                    int d_col_12 = d_base_2_4 + vv_5 * 16;
                    ...
                    if (batch_id < B) {
                        if (m_abs_part_3_4 < M) {
                            if (global_d_11 < D_ORIG_) {
                                {
                                    const uint4* _vptr_12 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_4) * D_ORIG_ + global_d_11) + 0);
                                    uint4 _vld_12[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_5 = (smem_b_addr + (unsigned int)(d_col_12 / 64 * 16384 + norm_row_0_4 * 128 + d_col_12 % 64 * 2 ^ (d_col_12 / 64 * 16384 + norm_row_0_4 * 128 + d_col_12 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_5), "r"(db_pack_5[0]), "r"(db_pack_5[1]), "r"(db_pack_5[2]), "r"(db_pack_5[3]) : "memory");
                    int b_store_addr_hi_5 = (smem_b_addr + (unsigned int)((d_col_12 + 8) / 64 * 16384 + norm_row_0_4 * 128 + (d_col_12 + 8) % 64 * 2 ^ ((d_col_12 + 8) / 64 * 16384 + norm_row_0_4 * 128 + (d_col_12 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    for (int vi_19 = 0; vi_19 < 16; vi_19++) {
                        acc_part_4_4 += db_vals_5[vi_19] * db_vals_5[vi_19];
                    }
                }
                smem_db_norm_part[tid] = acc_part_4_4;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 128) {
                ...
                if (m_abs_5 < M) {
                    ...
                    for (int part_6 = 0; part_6 < 4; part_6++) {
                        pass_norm_5 += smem_db_norm_part[tid + part_6 * 128];
                    }
                }
                {
                    if (m_abs_5 < M) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_7 = tid; e_vec_7 < 1024; e_vec_7 += 640) {
                ...
                #pragma unroll
                for (int vi_20 = 0; vi_20 < 16; vi_20++) {
                    q_vals_7[vi_20] = 0.0f;
                }
                if (batch_id < B) {
                    if (q_abs_7 < Q) {
                        if (global_d_12 < D_ORIG_) {
                            {
            ...
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            int norm_row_0_5 = tid % 128;
            int norm_part_1_5 = tid / 128;
            int d_base_2_5 = norm_part_1_5 * 32;
            int m_abs_part_3_5 = m_start + norm_row_0_5;
            float acc_part_4_5 = 0.0f;
            if (tid < 512) {
                #pragma unroll 1
                for (int vv_6 = 0; vv_6 < 2; vv_6++) {
                    int d_col_14 = d_base_2_5 + vv_6 * 16;
                    int global_d_13 = d_col_14 + 768;
                    float db_vals_6[16];
                    unsigned int db_pack_6[8];
                    #pragma unroll
                    for (int vi_21 = 0; vi_21 < 16; vi_21++) {
                        db_vals_6[vi_21] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_3_5 < M) {
                            if (global_d_13 < D_ORIG_) {
                                {
                                    const uint4* _vptr_14 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_5) * D_ORIG_ + global_d_13) + 0);
                                    uint4 _vld_14[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_6 = (smem_b_addr + (unsigned int)(d_col_14 / 64 * 16384 + norm_row_0_5 * 128 + d_col_14 % 64 * 2 ^ (d_col_14 / 64 * 16384 + norm_row_0_5 * 128 + d_col_14 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_6), "r"(db_pack_6[0]), "r"(db_pack_6[1]), "r"(db_pack_6[2]), "r"(db_pack_6[3]) : "memory");
                    int b_store_addr_hi_6 = (smem_b_addr + (unsigned int)((d_col_14 + 8) / 64 * 16384 + norm_row_0_5 * 128 + (d_col_14 + 8) % 64 * 2 ^ ((d_col_14 + 8) / 64 * 16384 + norm_row_0_5 * 128 + (d_col_14 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    #pragma unroll
                    for (int vi_22 = 0; vi_22 < 16; vi_22++) {
                        acc_part_4_5 += db_vals_6[vi_22] * db_vals_6[vi_22];
                    }
                }
                smem_db_norm_part[tid] = acc_part_4_5;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 128) {
                ...
                if (m_abs_6 < M) {
                    ...
                    for (int part_7 = 0; part_7 < 4; part_7++) {
                        pass_norm_6 += smem_db_norm_part[tid + part_7 * 128];
                    }
                }
                {
                    if (m_abs_6 < M) {
        ...
        {
            #pragma unroll 1
            for (int e_vec_8 = tid; e_vec_8 < 1024; e_vec_8 += 640) {
                ...
                #pragma unroll
                for (int vi_23 = 0; vi_23 < 16; vi_23++) {
                    q_vals_8[vi_23] = 0.0f;
                }
                if (batch_id < B) {
                    if (q_abs_8 < Q) {
                        if (global_d_14 < D_ORIG_) {
                            {
            ...
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            int norm_row_0_6 = tid % 128;
            int norm_part_1_6 = tid / 128;
            int d_base_2_6 = norm_part_1_6 * 32;
            int m_abs_part_3_6 = m_start + norm_row_0_6;
            float acc_part_4_6 = 0.0f;
            if (tid < 512) {
                #pragma unroll 1
                for (int vv_7 = 0; vv_7 < 2; vv_7++) {
                    int d_col_16 = d_base_2_6 + vv_7 * 16;
                    int global_d_15 = d_col_16 + 896;
                    float db_vals_7[16];
                    unsigned int db_pack_7[8];
                    #pragma unroll
                    for (int vi_24 = 0; vi_24 < 16; vi_24++) {
                        db_vals_7[vi_24] = 0.0f;
                    }
                    if (batch_id < B) {
                        if (m_abs_part_3_6 < M) {
                            if (global_d_15 < D_ORIG_) {
                                {
                                    const uint4* _vptr_16 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3_6) * D_ORIG_ + global_d_15) + 0);
                                    uint4 _vld_16[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
                    ...
                    int b_store_addr_7 = (smem_b_addr + (unsigned int)(d_col_16 / 64 * 16384 + norm_row_0_6 * 128 + d_col_16 % 64 * 2 ^ (d_col_16 / 64 * 16384 + norm_row_0_6 * 128 + d_col_16 % 64 * 2 >> 7 & 7) << 4));
                    asm volatile("st.shared.v4.b32 [%0], {%1,%2,%3,%4};" :: "r"(b_store_addr_7), "r"(db_pack_7[0]), "r"(db_pack_7[1]), "r"(db_pack_7[2]), "r"(db_pack_7[3]) : "memory");
                    int b_store_addr_hi_7 = (smem_b_addr + (unsigned int)((d_col_16 + 8) / 64 * 16384 + norm_row_0_6 * 128 + (d_col_16 + 8) % 64 * 2 ^ ((d_col_16 + 8) / 64 * 16384 + norm_row_0_6 * 128 + (d_col_16 + 8) % 64 * 2 >> 7 & 7) << 4));
                    ...
                    #pragma unroll
                    for (int vi_25 = 0; vi_25 < 16; vi_25++) {
                        acc_part_4_6 += db_vals_7[vi_25] * db_vals_7[vi_25];
                    }
                }
                smem_db_norm_part[tid] = acc_part_4_6;
            }
            asm volatile("fence.proxy.async.shared::cta;" ::: "memory");
            __syncthreads();
            if (tid < 128) {
                ...
                if (m_abs_7 < M) {
                    ...
                    for (int part_8 = 0; part_8 < 4; part_8++) {
                        pass_norm_7 += smem_db_norm_part[tid + part_8 * 128];
                    }
                }
                {
                    if (m_abs_7 < M) {
