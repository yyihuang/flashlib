...
kernel_knn_search_q4096_lowk_k2partial_0613_r45_48e9_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, int B, int Q, int M, int split_m, int num_q_tiles, int total_m_tiles, int tiles_per_split)
{
    ...
    const int smem_smem_cohort_topk_i = smem + 37888;
    ...
    int* smem_cohort_topk_i = (int*)(smem_raw + 37888);
    #define smem_cohort_topk_i_addr (smem + 37888)
                ...
                if (tid < 512) {
                    ...
                    for (int vv = 0; vv < 2; vv++) {
                        ...
                        unsigned int db_pack[8];
                        {
                            ...
                            if (m_abs_part_3 < M) {
                                {
                                    const uint4* _vptr_5 = reinterpret_cast<const uint4*>(database + (unsigned long long)((batch_id * M + m_abs_part_3) * 128 + d_col));
                                    uint4 _vld_5[2];
                                    #pragma unroll
                                    for (int _blk = 0; _blk < 2; _blk++) {
