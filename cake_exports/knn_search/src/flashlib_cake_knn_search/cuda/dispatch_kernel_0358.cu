...
kernel_knn_search_d256_groupmerge64_fanin8cta_blockm64_891a_v1(float* __restrict__ partial_distances, int32_t* __restrict__ partial_indices, float* __restrict__ group_distances, int32_t* __restrict__ group_indices, int B, int Q, int K, int num_q_tiles)
{
        ...
        if (lane == winner_lane) {
            ...
            for (int slot = 0; slot < 2; slot++) {
                if (local_best_slot == slot) {
                    ...
                    head_i[slot] = -1;
                    if (next_head < K_MAX_) {
                        unsigned long long partial_base = (((batch_id * num_q_tiles + q_tile) * 512 + partial_id) * 128 + q_local) * K_MAX_ + next_head;
                        head_d[slot] = partial_distances[partial_base];
                        head_i[slot] = partial_indices[partial_base];
                    }
                }
