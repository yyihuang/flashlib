...
kernel_knn_search_lowd_dbscan_direct_dispatch0610_r1_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int* __restrict__ out_indices, int B, int Q, int M, int K)
{
    ...
    int* local_idx = reinterpret_cast<int*>(smem_raw + 8192);
    const int local_idx_addr = smem + 8192;
    float* warp_dist = reinterpret_cast<float*>(smem_raw + 16384);
    const int warp_dist_addr = smem + 16384;
    int* warp_idx = reinterpret_cast<int*>(smem_raw + 16416);
    const int warp_idx_addr = smem + 16416;
    int* warp_thread = reinterpret_cast<int*>(smem_raw + 16448);
    const int warp_thread_addr = smem + 16448;
    ...
    if (batch_id < B) {
        ...
        float q0 = _vec_load_0[q_group_offset];
        float q1 = _vec_load_0[q_group_offset + 1];
        #pragma unroll
        for (int local_slot = 0; local_slot < LOCAL_LIST_CAP_; local_slot++) {
            ...
            if (m_row < M) {
                ...
                float diff0 = q0 - _vec_load_1[m_group_offset];
                float diff1 = q1 - _vec_load_1[m_group_offset + 1];
