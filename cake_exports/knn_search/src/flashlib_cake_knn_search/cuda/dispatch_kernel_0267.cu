...
kernel_knn_search_lowd_dbscan_d2_direct_0611_r22_6e85_v1(__nv_bfloat16* __restrict__ queries, __nv_bfloat16* __restrict__ database, float* __restrict__ out_distances, int32_t* __restrict__ out_indices, int B, int Q, int M, int K)
{
    ...
    const int smem_smem_idx = smem + 8192;
    ...
    int* smem_idx = (int*)(smem_raw + 8192);
    #define smem_idx_addr (smem + 8192)
    ...
    if (batch_id < B) {
        ...
        #pragma unroll
        for (int kk = 0; kk < LOCAL_CAP_; kk++) {
            int smem_off = tid * LOCAL_CAP_ + kk;
        ...
        if (tid == 0) {
            ...
            for (int src_thread = 0; src_thread < 128; src_thread++) {
                ...
                for (int src_k = 0; src_k < LOCAL_CAP_; src_k++) {
                    int smem_off = src_thread * LOCAL_CAP_ + src_k;
